...
namespace VMAP
{
    ...
    public:
        ...
        {
#ifdef VMAP_DEBUG
            FC_LOG_DEBUG("maps", "AreaInfoCallback: trying to intersect '%s'", prims[entry].name.c_str());
#endif
    ...
    public:
        ...
        {
#ifdef VMAP_DEBUG
            FC_LOG_DEBUG("maps", "LocationInfoCallback: trying to intersect '%s'", prims[entry].name.c_str());
#endif
    ...
    */
    ...
    {
        FC_LOG_DEBUG("maps", "StaticMapTree::InitMap() : initializing StaticMapTree '%s'", fname.c_str());
        bool success = false;
    ...
    bool StaticMapTree::LoadMapTile(uint32 tileX, uint32 tileY, VMapManager2* vm)
    {
        if (!iTreeValues)
        {
            FC_LOG_ERROR("misc", "StaticMapTree::LoadMapTile() : tree has not been initialized [%u, %u]", tileX, tileY);
            return false;
        }
        bool result = true;
        ...
        if (fileResult.File)
        {
            ...
            for (uint32 i = 0; i < numSpawns && result; ++i)
            {
                ...
                if (result)
                {
                    ...
                    if (!model)
                        FC_LOG_ERROR("misc", "StaticMapTree::LoadMapTile() : could not acquire WorldModel pointer [%u, %u]", tileX, tileY);
                    ...
                    if (spawnIndex != iSpawnIndices.end())
                    {
                        ...
                        if (!iLoadedSpawns.count(referencedVal))
                        {
                            ...
                            {
                                FC_LOG_ERROR("maps", "StaticMapTree::LoadMapTile() : invalid tree element (%u/%u) referenced in tile %s", referencedVal, iNTreeValues, fileResult.Name.c_str());
                                continue;
                            }
                        ...
                        else
                        {
                            ...
                            if (iTreeValues[referencedVal].ID != spawn.ID)
                                FC_LOG_DEBUG("maps", "StaticMapTree::LoadMapTile() : trying to load wrong spawn in node");
                            else if (iTreeValues[referencedVal].name != spawn.name)
                                FC_LOG_DEBUG("maps", "StaticMapTree::LoadMapTile() : name collision on GUID=%u", spawn.ID);
#endif
                        }
                    }
                    else
