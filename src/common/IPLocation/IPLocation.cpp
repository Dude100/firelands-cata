...
void IpLocationStore::Load()
{
    ...
    FC_LOG_INFO("server.loading", "Loading IP Location Database...");
    ...
    if (!databaseFile)
    {
        FC_LOG_ERROR("server.loading", "IPLocation: No ip database file exists (%s).", databaseFilePath.c_str());
        return;
    }

    if (!databaseFile.is_open())
    {
        FC_LOG_ERROR("server.loading", "IPLocation: Ip database file (%s) can not be opened.", databaseFilePath.c_str());
        return;
    }
    ...
    FC_LOG_INFO("server.loading", ">> Loaded " SZFMTD " ip location entries.", _ipLocationStore.size());
}

IpLocationRecord const* IpLocationStore::GetLocationRecord(std::string const& ipAddress) const
{
    uint32 ip = Firelands::Net::address_to_uint(Firelands::Net::make_address_v4(ipAddress));
