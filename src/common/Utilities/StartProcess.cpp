...
namespace Firelands
{
        ...
        bool secure)
    {
        ...
        if (!secure)
        {
            FC_LOG_TRACE(logger, "Starting process \"%s\" with arguments: \"%s\".",
        ...
        auto outInfo = MakeTCLogSink([&](std::string const& msg)
            {
                FC_LOG_INFO(logger, "%s", msg.c_str());
            });

        auto outError = MakeTCLogSink([&](std::string const& msg)
            {
                FC_LOG_ERROR(logger, "%s", msg.c_str());
            });
        ...
        if (!secure)
        {
            FC_LOG_TRACE(logger, ">> Process \"%s\" finished with return value %i.",
