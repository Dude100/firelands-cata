...
private:
    ...
    std::unique_ptr<Firelands::Asio::DeadlineTimer> _batchTimer;
    std::unique_ptr<Firelands::Asio::DeadlineTimer> _overallStatusTimer;
...
public:
    ...
    void Initialize(std::string const& realmName, Firelands::Asio::IoContext& ioContext, std::function<void()> overallStatusLogger);
