...
void Metric::Initialize(std::string const& realmName, Firelands::Asio::IoContext& ioContext, std::function<void()> overallStatusLogger)
{
    _dataStream = Firelands::make_unique<boost::asio::ip::tcp::iostream>();
    _realmName = FormatInfluxDBTagValue(realmName);
    _batchTimer = Firelands::make_unique<Firelands::Asio::DeadlineTimer>(ioContext);
    _overallStatusTimer = Firelands::make_unique<Firelands::Asio::DeadlineTimer>(ioContext);
...
bool Metric::Connect()
{
    ...
    if (error)
    {
        FC_LOG_ERROR("metric", "Error connecting to '%s:%s', disabling Metric. Error message : %s",
...
void Metric::LoadFromConfigs()
{
    ...
    {
        FC_LOG_ERROR("metric", "'Metric.Interval' config set to %d, overriding to 1.", _updateInterval);
        _updateInterval = 1;
    }
    ...
    {
        FC_LOG_ERROR("metric", "'Metric.OverallStatusInterval' config set to %d, overriding to 1.", _overallStatusTimerInterval);
    ...
    if (_enabled && !previousValue)
    {
        ...
        {
            FC_LOG_ERROR("metric", "'Metric.ConnectionInfo' not specified in configuration file.");
            return;
        }
        ...
        if (tokens.size() != 3)
        {
            FC_LOG_ERROR("metric", "'Metric.ConnectionInfo' specified with wrong format in configuration file.");
            return;
        }
...
void Metric::SendBatch()
{
    ...
    if (status_code != 204)
    {
        FC_LOG_ERROR("metric", "Error sending data, returned HTTP code: %u", status_code);
    }
...
void Metric::Unload()
{
    ...
    if (_enabled && Firelands::Asio::get_io_context(*_batchTimer).stopped())
    {
        _enabled = false;
        SendBatch();
    }
