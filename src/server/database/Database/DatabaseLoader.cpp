    ...
    _open.push([this, name, updatesEnabledForThis, &pool]() -> bool
        {
            ...
            if (dbString.empty())
            {
                FC_LOG_ERROR(_logger, "Database %s not specified in configuration file!", name.c_str());
                return false;
            }
            ...
            {
                FC_LOG_ERROR(_logger, "%s database: invalid number of worker threads specified. "
        ...
        _populate.push([this, name, &pool]() -> bool
            {
                ...
                {
                    FC_LOG_ERROR(_logger, "Could not populate the %s database, see log for details.", name.c_str());
                    return false;
                }
                return true;
            });
        ...
        _update.push([this, name, &pool]() -> bool
            {
                ...
                {
                    FC_LOG_ERROR(_logger, "Could not update the %s database, see log for details.", name.c_str());
                    return false;
                }
                return true;
            });
    ...
    _prepare.push([this, name, &pool]() -> bool
        {
            ...
            {
                FC_LOG_ERROR(_logger, "Could not prepare statements of the %s database, see log for details.", name.c_str());
                return false;
            }
            return true;
        });
...
{
    if (!_updateFlags)
        FC_LOG_INFO("sql.updates", "Automatic database updates are disabled for all databases!");
