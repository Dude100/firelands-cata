    ...
    _async_threads(0), _synch_threads(0)
{
    ...
    WPFatal(mysql_get_client_version() == MYSQL_VERSION_ID, "Used MySQL library version (%s id %lu) does not match the version id used to compile FirelandsCore (id %u). Search on forum for TCE00011.",
...
uint32 DatabaseWorkerPool<T>::Open()
{
    ...
    FC_LOG_INFO("sql.driver", "Opening DatabasePool '%s'. "
    ...
    if (!error)
    {
        FC_LOG_INFO("sql.driver", "DatabasePool '%s' opened successfully. " SZFMTD
...
{
    FC_LOG_INFO("sql.driver", "Closing down DatabasePool '%s'.", GetDatabaseName());

    //! Closes the actualy MySQL connection.
    _connections[IDX_ASYNC].clear();

    FC_LOG_INFO("sql.driver", "Asynchronous connections on DatabasePool '%s' terminated. "
    ...
    FC_LOG_INFO("sql.driver", "All connections on DatabasePool '%s' closed.", GetDatabaseName());
}
        ...
        for (auto& connection : connections)
        {
            ...
            for (size_t i = 0; i < preparedSize; ++i)
            {
                ...
                if (MySQLPreparedStatement* stmt = connection->m_stmts[i].get())
                {
                    ...
                    // FC only supports uint8 indices.
...
void DatabaseWorkerPool<T>::CommitTransaction(SQLTransaction<T> transaction)
{
    ...
    {
    case 0:
        FC_LOG_DEBUG("sql.driver", "Transaction contains 0 queries. Not executing.");
        return;
    case 1:
        FC_LOG_DEBUG("sql.driver", "Warning: Transaction only holds 1 query, consider removing Transaction context in code.");
        break;
    default:
        break;
    }
...
uint32 DatabaseWorkerPool<T>::OpenConnections(InternalIndex type, uint8 numConnections)
{
    for (uint8 i = 0; i < numConnections; ++i)
    {
        ...
        {
            FC_LOG_ERROR("sql.driver", "FirelandsCore does not support MySQL versions below 5.1");
            return 1;
        }
        else
        {
