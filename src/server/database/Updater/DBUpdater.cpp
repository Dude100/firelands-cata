...
bool DBUpdaterUtil::CheckExecutable()
{
    ...
    if (!is_regular_file(exe))
    {
        ...
        FC_LOG_FATAL("sql.updates", "Didn't find any executable MySQL binary at \'%s\' or in path, correct the path in the *.conf (\"MySQLExecutable\").",
...
{
    FC_LOG_INFO("sql.updates", "Database \"%s\" does not exist, do you want to create it? [yes (default) / no]: ",
    ...
    FC_LOG_INFO("sql.updates", "Creating database \"%s\"...", pool.GetConnectionInfo()->database.c_str());
    ...
    if (!file.is_open())
    {
        FC_LOG_FATAL("sql.updates", "Failed to create temporary query file \"%s\"!", temp.generic_string().c_str());
        return false;
    }
    ...
    {
        FC_LOG_FATAL("sql.updates", "Failed to create database %s! Does the user (named in *.conf) have `CREATE` privileges on the MySQL server?", pool.GetConnectionInfo()->database.c_str());
        boost::filesystem::remove(temp);
        return false;
    }

    FC_LOG_INFO("sql.updates", "Done.");
    boost::filesystem::remove(temp);
    return true;
}
...
bool DBUpdater<T>::Update(DatabaseWorkerPool<T>& pool)
{
    ...
    FC_LOG_INFO("sql.updates", "Updating %s database...", DBUpdater<T>::GetTableName().c_str());

    Path const sourceDirectory(BuiltInConfig::GetSourceDirectory());

    if (!is_directory(sourceDirectory))
    {
        FC_LOG_ERROR("sql.updates", "DBUpdater: Given source directory %s does not exist, skipped!", sourceDirectory.generic_string().c_str());
        return false;
    }
    ...
    if (!result.updated)
        FC_LOG_INFO("sql.updates", ">> %s database is up-to-date! %s", DBUpdater<T>::GetTableName().c_str(), info.c_str());
    else
        FC_LOG_INFO("sql.updates", ">> Applied " SZFMTD " %s. %s", result.updated, result.updated == 1 ? "query" : "queries", info.c_str());
...
bool DBUpdater<T>::Populate(DatabaseWorkerPool<T>& pool)
{
    ...
    FC_LOG_INFO("sql.updates", "Database %s is empty, auto populating it...", DBUpdater<T>::GetTableName().c_str());

    std::string const p = DBUpdater<T>::GetBaseFile();
    if (p.empty())
    {
        FC_LOG_INFO("sql.updates", ">> No base file provided, skipped!");
        return true;
    }
    ...
    if (!exists(base))
    {
        ...
        {
            FC_LOG_ERROR("sql.updates", ">> Base file \"%s\" is missing, try to clone the source again.",
        ...
        case LOCATION_DOWNLOAD:
        {
            ...
            FC_LOG_ERROR("sql.updates", ">> File \"%s\" is missing, download it from \"https://github.com/The-Cataclysm-Preservation-Project/FirelandsCore/releases\"" \
    ...
    // Update database
    FC_LOG_INFO("sql.updates", ">> Applying \'%s\'...", base.generic_string().c_str());
    try
    {
        ApplyFile(pool, base);
    }
    ...
    std::string const& password, std::string const& port_or_socket, std::string const& database, Path const& path)
{
    ...
    if (ret != EXIT_SUCCESS)
    {
        FC_LOG_FATAL("sql.updates", "Applying of file \'%s\' to database \'%s\' failed!" \
