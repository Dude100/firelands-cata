...
UpdateFetcher::UpdateFetcher(Path const& sourceDirectory,
    ...
    _sourceDirectory(Firelands::make_unique<Path>(sourceDirectory)), _apply(apply), _applyFile(applyFile),
    _retrieve(retrieve)
{
}
...
void UpdateFetcher::FillFileListRecursively(Path const& path, LocaleFileStorage& storage, State const state, uint32 const depth) const
{
    ...
    for (directory_iterator itr(path); itr != end; ++itr)
    {
        ...
        {
            FC_LOG_TRACE("sql.updates", "Added locale file \"%s\".", itr->path().filename().generic_string().c_str());
            ...
            {
                FC_LOG_FATAL("sql.updates", "Duplicate filename \"%s\" occurred. Because updates are ordered " \
...
UpdateFetcher::DirectoryStorage UpdateFetcher::ReceiveIncludedDirectories() const
{
    ...
    do
    {
        ...
        if (!is_directory(p))
        {
            FC_LOG_WARN("sql.updates", "DBUpdater: Given update include directory \"%s\" does not exist, skipped!", p.generic_string().c_str());
            continue;
        }
        ...
        FC_LOG_TRACE("sql.updates", "Added applied file \"%s\" from remote.", p.filename().generic_string().c_str());
...
std::string UpdateFetcher::ReadSQLUpdate(boost::filesystem::path const& file) const
{
    ...
    if (!in.is_open())
    {
        FC_LOG_FATAL("sql.updates", "Failed to open the sql update \"%s\" for reading! "
    ...
    int32 const cleanDeadReferencesMaxCount) const
{
    ...
    {
        FC_LOG_DEBUG("sql.updates", "Checking update \"%s\"...", availableQuery.first.filename().generic_string().c_str());
        ...
        if (iter != applied.end())
        {
            ...
            if (!redundancyChecks)
            {
                FC_LOG_DEBUG("sql.updates", ">> Update is already applied, skipping redundancy checks.");
                applied.erase(iter);
                continue;
            }
            ...
            {
                FC_LOG_DEBUG("sql.updates", ">> Update is archived and marked as archived in database, skipping redundancy checks.");
                applied.erase(iter);
                continue;
            }
        }
        ...
        if (iter == applied.end())
        {
            ...
            if (hashIter != hashToName.end())
            {
                ...
                {
                    FC_LOG_WARN("sql.updates", ">> It seems like the update \"%s\" \'%s\' was renamed, but the old file is still there! " \
                ...
                {
                    FC_LOG_INFO("sql.updates", ">> Renaming update \"%s\" to \"%s\" \'%s\'.",
            ...
            {
                FC_LOG_INFO("sql.updates", ">> Applying update \"%s\" \'%s\'...",
                    availableQuery.first.filename().string().c_str(), hash.substr(0, 7).c_str());
            }
        }
        ...
        else if (allowRehash && iter->second.hash.empty())
        {
            ...
            FC_LOG_INFO("sql.updates", ">> Re-hashing update \"%s\" \'%s\'...", availableQuery.first.filename().string().c_str(),
        ...
        else
        {
            ...
            {
                FC_LOG_INFO("sql.updates", ">> Reapplying update \"%s\" \'%s\' -> \'%s\' (it changed)...", availableQuery.first.filename().string().c_str(),
            ...
            else
            {
                ...
                {
                    FC_LOG_DEBUG("sql.updates", ">> Updating the state of \"%s\" to \'%s\'...",
                ...
                FC_LOG_DEBUG("sql.updates", ">> Update is already applied and matches the hash \'%s\'.", hash.substr(0, 7).c_str());
    ...
    if (!applied.empty())
    {
        ...
        for (auto const& entry : applied)
        {
            FC_LOG_WARN("sql.updates", ">> The file \'%s\' was applied to the database, but is missing in" \
                " your update directory now!", entry.first.c_str());

            if (doCleanup)
                FC_LOG_INFO("sql.updates", "Deleting orphaned entry \'%s\'...", entry.first.c_str());
        }
        ...
        {
            FC_LOG_ERROR("sql.updates", "Cleanup is disabled! There were  " SZFMTD " dirty files applied to your database, " \
