...
public:
        ...
        _acceptor.async_accept(*socket, [this, socket, threadIndex](boost::system::error_code error)
            {
                if (!error)
                {
                    ...
                    {
                        FC_LOG_INFO("network", "Failed to initialize client's socket %s", err.what());
                    }
                }
                if (!_closed)
    ...
    bool Bind()
    {
        ...
        if (errorCode)
        {
            FC_LOG_INFO("network", "Failed to open acceptor %s", errorCode.message().c_str());
            return false;
        }
        ...
        if (errorCode)
        {
            FC_LOG_INFO("network", "Failed to set reuse_address option on acceptor %s", errorCode.message().c_str());
            return false;
        }
#endif
        ...
        if (errorCode)
        {
            FC_LOG_INFO("network", "Could not bind to %s:%u %s", _endpoint.address().to_string().c_str(), _endpoint.port(), errorCode.message().c_str());
            return false;
        }

        _acceptor.listen(FIRELANDS_MAX_LISTEN_CONNECTIONS, errorCode);
        if (errorCode)
        {
            FC_LOG_INFO("network", "Failed to start listening on %s:%u %s", _endpoint.address().to_string().c_str(), _endpoint.port(), errorCode.message().c_str());
            return false;
        }
    ...
    _acceptor.async_accept(_socket, [this](boost::system::error_code error)
        {
            if (!error)
            {
                ...
                {
                    FC_LOG_INFO("network", "Failed to retrieve client's remote address %s", err.what());
                }
            }
