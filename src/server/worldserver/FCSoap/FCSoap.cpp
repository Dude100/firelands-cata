...
void TCSoapThread(const std::string& host, uint16 port)
{
    ...
    {
        FC_LOG_ERROR("network.soap", "Couldn't bind to %s:%d", host.c_str(), port);
        exit(-1);
    }

    FC_LOG_INFO("network.soap", "Bound to http://%s:%d", host.c_str(), port);
    ...
    while (!World::IsStopped())
    {
        ...
        FC_LOG_DEBUG("network.soap", "Accepted connection from IP=%d.%d.%d.%d", (int)(soap.ip >> 24) & 0xFF, (int)(soap.ip >> 16) & 0xFF, (int)(soap.ip >> 8) & 0xFF, (int)soap.ip & 0xFF);
...
{
    FC_LOG_TRACE("network.soap", "SOAPWorkingThread::process_message");
...
int ns1__executeCommand(soap* soap, char* command, char** result)
{
    ...
    {
        FC_LOG_INFO("network.soap", "Client didn't provide login information");
        return 401;
    }
    ...
    if (!accountId)
    {
        FC_LOG_INFO("network.soap", "Client used invalid username '%s'", soap->userid);
        return 401;
    }

    if (!AccountMgr::CheckPassword(accountId, soap->passwd))
    {
        FC_LOG_INFO("network.soap", "Invalid password for account '%s'", soap->userid);
        return 401;
    }

    if (AccountMgr::GetSecurity(accountId, realm.Id.Realm) < SEC_ADMINISTRATOR)
    {
        FC_LOG_INFO("network.soap", "%s's gmlevel is too low", soap->userid);
        return 403;
    }
    ...
    FC_LOG_INFO("network.soap", "Received command '%s'", command);
...
{ { "SOAP-ENV", "http://schemas.xmlsoap.org/soap/envelope/", nullptr, nullptr }, // must be first
    ...
    { "ns1", "urn:FC", nullptr, nullptr },     // "ns1" namespace prefix
