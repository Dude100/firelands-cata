...
void RASession::Start()
{
    ...
    FC_LOG_INFO("commands.ra", "Accepting RA connection from user %s (IP: %s)", username.c_str(), GetRemoteIpAddress().c_str());
    ...
    FC_LOG_INFO("commands.ra", "User %s (IP: %s) authenticated correctly to RA", username.c_str(), GetRemoteIpAddress().c_str());
...
bool RASession::CheckAccessLevel(const std::string& user)
{
    ...
    if (!result)
    {
        FC_LOG_INFO("commands.ra", "User %s does not exist in database", user.c_str());
        return false;
    }
    ...
    {
        FC_LOG_INFO("commands.ra", "User %s has no privilege to login", user.c_str());
        return false;
    }
    else if (fields[2].GetInt32() != -1)
    {
        FC_LOG_INFO("commands.ra", "User %s has to be assigned on all realms (with RealmID = '-1')", user.c_str());
        return false;
    }
...
bool RASession::CheckPassword(const std::string& user, const std::string& pass)
{
    ...
    if (!result)
    {
        FC_LOG_INFO("commands.ra", "Wrong password for user: %s", user.c_str());
        return false;
    }
...
bool RASession::ProcessCommand(std::string& command)
{
    ...
    FC_LOG_INFO("commands.ra", "Received command: %s", command.c_str());
