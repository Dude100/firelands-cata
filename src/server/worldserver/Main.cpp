...
extern int main(int argc, char** argv)
{
    ...
    Firelands::Banner::Show("worldserver-daemon",
        [](char const* text)
        {
            FC_LOG_INFO("server.worldserver", "%s", text);
        },
        []()
        {
            FC_LOG_INFO("server.worldserver", "Using configuration file %s.", sConfigMgr->GetFilename().c_str());
            FC_LOG_INFO("server.worldserver", "Using SSL version: %s (library: %s)", OPENSSL_VERSION_TEXT, SSLeay_version(SSLEAY_VERSION));
            FC_LOG_INFO("server.worldserver", "Using Boost version: %i.%i.%i", BOOST_VERSION / 100000, BOOST_VERSION / 100 % 1000, BOOST_VERSION % 100);
        }
        );
    ...
    if (!pidFile.empty())
    {
        if (uint32 pid = CreatePIDFile(pidFile))
            FC_LOG_INFO("server.worldserver", "Daemon PID: %u\n", pid);
        else
        {
            FC_LOG_ERROR("server.worldserver", "Cannot create PID file %s.\n", pidFile.c_str());
            return 1;
        }
    }
    ...
    if (networkThreads <= 0)
    {
        FC_LOG_ERROR("server.worldserver", "Network.Threads must be greater than 0");
    ...
    {
        FC_LOG_ERROR("server.worldserver", "Failed to initialize network");
        World::StopNow(ERROR_EXIT_CODE);
        return 1;
    }
    ...
    if (int coreStuckTime = sConfigMgr->GetIntDefault("MaxCoreStuckTime", 0))
    {
        ...
        FC_LOG_INFO("server.worldserver", "Starting up anti-freeze thread (%u seconds max stuck time)...", coreStuckTime);
    }

    sIpcContext->Initialize();
    FC_LOG_INFO("server.worldserver", "%s (worldserver-daemon) ready...", GitRevision::GetFullVersion());
    ...
    FC_LOG_INFO("server.worldserver", "Halting process...");

    // 0 - normal shutdown
    // 1 - shutdown at error
    // 2 - restart command used, this code can be used by restarter for restart Firelandsd
...
void ShutdownCLIThread(std::thread* cliThread)
{
    if (cliThread != nullptr)
    {
        ...
        if (!CancelSynchronousIo(cliThread->native_handle()))
        {
            ...
            FC_LOG_DEBUG("server.worldserver", "Error cancelling I/O of CliThread, error code %u, detail: %s", uint32(errorCode), errorBuffer);
...
void FreezeDetector::Handler(std::weak_ptr<FreezeDetector> freezeDetectorRef, boost::system::error_code const& error)
{
    if (!error)
    {
        if (std::shared_ptr<FreezeDetector> freezeDetector = freezeDetectorRef.lock())
        {
            ...
            {
                FC_LOG_ERROR("server.worldserver", "World Thread hangs, kicking out server!");
                ABORT();
            }
...
AsyncAcceptor* StartRaSocketAcceptor(Firelands::Asio::IoContext& ioContext)
{
    ...
    if (!acceptor->Bind())
    {
        FC_LOG_ERROR("server.worldserver", "Failed to bind RA socket acceptor");
        delete acceptor;
        return nullptr;
    }
...
bool LoadRealmInfo(Firelands::Asio::IoContext& ioContext)
{
    ...
    if (!externalAddress)
    {
        FC_LOG_ERROR("server.worldserver", "Could not resolve address %s", fields[2].GetString().c_str());
        return false;
    }
    ...
    if (!localAddress)
    {
        FC_LOG_ERROR("server.worldserver", "Could not resolve address %s", fields[3].GetString().c_str());
        return false;
    }
    ...
    if (!localSubmask)
    {
        FC_LOG_ERROR("server.worldserver", "Could not resolve address %s", fields[4].GetString().c_str());
        return false;
    }
...
bool StartDB()
{
    ...
    if (!realm.Id.Realm)
    {
        FC_LOG_ERROR("server.worldserver", "Realm ID not defined in configuration file");
        return false;
    }
    ...
    if (!realmIdQuery)
    {
        FC_LOG_ERROR("server.worldserver", "Realm id %u not defined in realmlist table", realm.Id.Realm);
        return false;
    }
    ...
    FC_LOG_INFO("server.worldserver", "Realm running as realm ID %u region %u battlegroup %u", realm.Id.Realm, uint32(realm.Id.Region), uint32(realm.Id.Site));
    ...
    FC_LOG_INFO("server.worldserver", "Using World DB: %s", sWorld->GetDBVersion());
    return true;
}
...
variables_map GetConsoleArguments(int argc, char** argv, fs::path& configFile, std::string& configService)
{
    ...
    all.add_options()
        ...
        ("config,c", value<fs::path>(&configFile)->default_value(fs::absolute(_FIRELANDS_CORE_CONFIG)),
