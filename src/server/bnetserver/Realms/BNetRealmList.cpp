...
void BNetRealmList::Initialize(Firelands::Asio::IoContext& ioContext, uint32 updateInterval, uint16 worldListenPort)
{
    _updateInterval = updateInterval;
    _updateTimer = Firelands::make_unique<Firelands::Asio::DeadlineTimer>(ioContext);
    _resolver = Firelands::make_unique<Firelands::Asio::Resolver>(ioContext);

    // Get the content of the realmlist table in the database
    UpdateRealms(boost::system::error_code());

    _worldListener = Firelands::make_unique<WorldListener>(worldListenPort);
...
inline void UpdateAddress(std::unique_ptr<boost::asio::ip::address>& out, boost::asio::ip::address&& in, bool& changed)
{
    if (!out || *out != in)
    {
        out = Firelands::make_unique<boost::asio::ip::address>(std::move(in));
        changed = true;
    }
}
...
void BNetRealmList::UpdateRealms(boost::system::error_code const& error)
{
    ...
    FC_LOG_DEBUG("server.authserver", "Updating Realm List...");
    ...
    if (result)
    {
        do
        {
            try
            {
                ...
                if (!externalAddress)
                {
                    FC_LOG_ERROR("realmlist", "Could not resolve address %s", externalAddressString.c_str());
                    continue;
                }
                ...
                if (!localAddress)
                {
                    FC_LOG_ERROR("realmlist", "Could not resolve address %s", localAddressString.c_str());
                    continue;
                }
                ...
                if (!localSubmask)
                {
                    FC_LOG_ERROR("realmlist", "Could not resolve address %s", localSubmaskString.c_str());
                    continue;
                }
                ...
                FC_LOG_TRACE("server.bnetserver", "Realm \"%s\" at %s:%u.", name.c_str(), externalAddressString.c_str(), port);
            }
            catch (std::exception& ex)
            {
                FC_LOG_ERROR("server.bnetserver", "BNetRealmList::UpdateRealms has thrown an exception: %s", ex.what());
                ABORT();
            }
