...
public:
    ...
    struct instance_stratholme_InstanceMapScript : public InstanceScript
    {
        ...
        bool StartSlaugtherSquare()
        {
            ...
            FC_LOG_DEBUG("scripts", "Instance Stratholme: Cannot open slaugther square yet.");
            return false;
        }
        ...
        void SetData(uint32 type, uint32 data) override
        {
            ...
            case TYPE_BARON_RUN:
                ...
                case IN_PROGRESS:
                    ...
                    FC_LOG_DEBUG("scripts", "Instance Stratholme: Baron run in progress.");
                    break;
                case FAIL:
            ...
            case TYPE_RAMSTEIN:
                if (data == IN_PROGRESS)
                {
                    ...
                    if (!count)
                    {
                        ...
                        FC_LOG_DEBUG("scripts", "Instance Stratholme: Ramstein spawned.");
                    }
                    else
                        FC_LOG_DEBUG("scripts", "Instance Stratholme: %u Abomnation left to kill.", count);
                }
                ...
                if (data == DONE)
                {
                    ...
                    FC_LOG_DEBUG("scripts", "Instance Stratholme: Slaugther event will continue in 1 minute.");
                }
        ...
        void Update(uint32 diff) override
        {
            ...
            while (uint32 eventId = events.ExecuteEvent())
            {
                ...
                case EVENT_BARON_RUN:
                    ...
                    FC_LOG_DEBUG("scripts", "Instance Stratholme: Baron run event reached end. Event has state %u.", GetData(TYPE_BARON_RUN));
                    break;
                case EVENT_SLAUGHTER_SQUARE:
                    if (Creature* baron = instance->GetCreature(baronGUID))
                    {
                        ...
                        HandleGameObject(ziggurat4GUID, true);
                        HandleGameObject(ziggurat5GUID, true);
                        FC_LOG_DEBUG("scripts", "Instance Stratholme: Black guard sentries spawned. Opening gates to baron.");
                    }
                    break;
                default:
                    break;
