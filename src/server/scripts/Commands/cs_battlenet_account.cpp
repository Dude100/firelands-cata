...
public:
    ...
    static bool HandleAccountCreateCommand(ChatHandler* handler, char const* args)
    {
        ...
        case AccountOpResult::AOR_OK:
        {
            ...
            {
                FC_LOG_INFO("entities.player.character", "Account: %u (IP: %s) Character:[%s] (%s) created Battle.net account %s%s%s",
        ...
        if (!param.empty())
        {
            if (param == "on")
            {
                ...
                else
                {
                    ...
                    FC_LOG_DEBUG("server.bnetserver", "IP2Location] No information");
                }
            }
            else if (param == "off")
            {
    ...
    static bool HandleAccountPasswordCommand(ChatHandler* handler, char const* args)
    {
        ...
        if (!Battlenet::AccountMgr::CheckPassword(handler->GetSession()->GetBattlenetAccountId(), std::string(oldPassword)))
        {
            ...
            handler->SetSentErrorMessage(true);
            FC_LOG_INFO("entities.player.character", "Battle.net account: %u (IP: %s) Character:[%s] (GUID: %u) Tried to change password, but the provided old password is wrong.",
        ...
        case AccountOpResult::AOR_OK:
            ...
            FC_LOG_INFO("entities.player.character", "Battle.net account: %u (IP: %s) Character:[%s] (GUID: %u) Changed Password.",
                handler->GetSession()->GetBattlenetAccountId(), handler->GetSession()->GetRemoteAddress().c_str(),
                handler->GetSession()->GetPlayer()->GetName().c_str(), handler->GetSession()->GetPlayer()->GetGUID().GetCounter());
            break;
        case AccountOpResult::AOR_PASS_TOO_LONG:
    ...
    static bool HandleGameAccountCreateCommand(ChatHandler* handler, char const* args)
    {
        ...
        case AccountOpResult::AOR_OK:
            ...
            if (handler->GetSession())
            {
                FC_LOG_INFO("entities.player.character", "Account: %u (IP: %s) Character:[%s] (%s) created Account %s (Email: '%s')",
                    handler->GetSession()->GetAccountId(), handler->GetSession()->GetRemoteAddress().c_str(),
                    handler->GetSession()->GetPlayer()->GetName().c_str(), handler->GetSession()->GetPlayer()->GetGUID().ToString().c_str(),
