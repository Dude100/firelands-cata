...
public:
        ...
        case DATA_HORSEMEN_IS_TIMED_KILL:
        {
            ...
            for (Horseman boss : horsemen)
                if (Creature* cBoss = getHorsemanHandle(boss))
                {
                    ...
                    if (!deathTime)
                    {
                        FC_LOG_WARN("scripts", "FourHorsemenAI: Checking for achievement credit but horseman %s is reporting not dead", cBoss->GetName().c_str());
                        return 0;
                    }
                ...
                {
                    FC_LOG_WARN("scripts", "FourHorsemenAI: Checking for achievement credit but horseman with id %u is not present", uint32(boss));
                    return 0;
                }
    ...
    void CheckIsMovementFinished()
    {
        for (Horseman boss : horsemen)
        {
            ...
            {
                FC_LOG_WARN("scripts", "FourHorsemenAI: Checking if movement is finished but horseman with id %u is not present", uint32(boss));
                ResetEncounter();
                return;
            }
        }
    ...
    void BeginEncounter()
    {
        ...
        for (Horseman boss : horsemen)
        {
            ...
            {
                FC_LOG_WARN("scripts", "FourHorsemenAI: Encounter starting but horseman with id %u is not present", uint32(boss));
                ResetEncounter();
                return;
            }
        }
    ...
    void ResetEncounter()
    {
        ...
        for (Horseman boss : horsemen)
        {
            if (Creature* cBoss = getHorsemanHandle(boss))
                ...
                FC_LOG_WARN("scripts", "FourHorsemenAI: Encounter resetting but horseman with id %u is not present", uint32(boss));
        }
    }
    ...
    void JustDied(Unit* /*killer*/) override
    {
        ...
        for (Horseman boss : horsemen)
        {
            ...
            {
                FC_LOG_WARN("scripts", "FourHorsemenAI: %s died but horseman with id %u is not present", me->GetName().c_str(), uint32(boss));
                ResetEncounter();
            }
        }
...
public:
    ...
    class spell_four_horsemen_mark_AuraScript : public AuraScript
    {
        ...
        {
            AfterEffectApply.Register(&spell_four_horsemen_mark_AuraScript::OnApply, EFFECT_0, SPELL_AURA_DUMMY, AURA_EFFECT_HANDLE_REAL_OR_REAPPLY_MASK);
        }
    };
