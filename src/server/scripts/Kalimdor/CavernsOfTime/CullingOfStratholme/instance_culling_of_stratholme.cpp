...
public:
    ...
    struct instance_culling_of_stratholme_InstanceMapScript : public InstanceScript
    {
        ...
        void ReadSaveDataMore(std::istringstream& data) override
        {
            ...
            FC_LOG_DEBUG("scripts.cos", "instance_culling_of_stratholme::ReadSaveDataMore: Loaded with state %u and guardian timeout at %zu minutes %zu seconds from now", (uint32)loadState, timediff / MINUTE, timediff % MINUTE);
        }
        ...
        void Update(uint32 diff) override
        {
            ...
            while (uint32 eventId = events.ExecuteEvent())
            {
                ...
                case EVENT_RESPAWN_ARTHAS:
                    FC_LOG_DEBUG("scripts.cos", "instance_culling_of_stratholme::Update: Spawning new Arthas for instance...");
        ...
        void OnCreatureCreate(Creature* creature) override
        {
            ...
            case NPC_CRATE_HELPER:
                ...
                break;
            case NPC_ARTHAS:
                FC_LOG_DEBUG("scripts.cos", "instance_culling_of_stratholme::OnCreatureCreate: Arthas spawned at %s", creature->GetPosition().ToString().c_str());
        ...
        {
            FC_LOG_DEBUG("scripts.cos", "instance_culling_of_stratholme::SetInstanceProgress: Instance progress is now 0x%X", (uint32)state);
            _currentState = state;
    ...
    private:
        ...
        {
            FC_LOG_DEBUG("scripts.cos", "instance_culling_of_stratholme::SetWorldState: %u %u", uint32(state), value);
        ...
        {
            FC_LOG_DEBUG("scripts.cos", "instance_culling_of_stratholme::PropagateWorldStateUpdate: Propagate world states");
            for (WorldStateMap::const_iterator it = _currentWorldStates.begin(); it != _currentWorldStates.end(); ++it)
            {
                ...
                if (sent != it->second)
                {
                    FC_LOG_DEBUG("scripts.cos", "instance_culling_of_stratholme::PropagateWorldStateUpdate: Sending world state %u (%u)", it->first, it->second);
