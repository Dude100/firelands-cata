            ...
            if (Unit* target = GetHitUnit())
            {
                ...
                for (uint8 slot = 0; slot < MAX_ENCHANTMENT_SLOT; ++slot)
                {
                    ...
                    for (uint8 s = 0; s < 3; ++s)
                    {
                        ...
                        if (!spellInfo)
                        {
                            FC_LOG_ERROR("spells", "Player::CastItemCombatSpell Enchant %i, player (Name: %s, GUID: %u) cast unknown spell %i", enchant->ID, player->GetName().c_str(), player->GetGUID().GetCounter(), enchant->EffectArg[s]);
                            continue;
                        }
...
public:
    ...
    class spell_rog_killing_spree_AuraScript : public AuraScript
    {
        ...
        void HandleEffectPeriodic(AuraEffect const* /*aurEff*/)
        {
            ...
            {
                ObjectGuid guid = Firelands::Containers::SelectRandomContainerElement(_targets);
