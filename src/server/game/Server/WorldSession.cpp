...
void WorldSession::SendPacket(WorldPacket const* packet, bool forced /*= false*/)
{
    ...
    {
        FC_LOG_ERROR("network.opcode", "Prevented sending of NULL_OPCODE to %s", GetPlayerInfo().c_str());
        return;
    }
    else if (packet->GetOpcode() == UNKNOWN_OPCODE)
    {
        FC_LOG_ERROR("network.opcode", "Prevented sending of UNKNOWN_OPCODE to %s", GetPlayerInfo().c_str());
        return;
    }
    ...
    if (!handler)
    {
        FC_LOG_ERROR("network.opcode", "Prevented sending of opcode %u with non existing handler to %s", packet->GetOpcode(), GetPlayerInfo().c_str());
        return;
    }
    ...
    if (packet->GetConnection() != CONNECTION_TYPE_DEFAULT)
    {
        ...
        {
            FC_LOG_ERROR("network.opcode", "Prevented sending of instance only opcode %u with connection type %u to %s", packet->GetOpcode(), uint32(packet->GetConnection()), GetPlayerInfo().c_str());
            return;
        }
    ...
    if (!m_Socket[conIdx])
    {
        FC_LOG_ERROR("network.opcode", "Prevented sending of %s to non existent socket %u to %s", GetOpcodeNameForLogging(static_cast<OpcodeServer>(packet->GetOpcode())).c_str(), uint32(conIdx), GetPlayerInfo().c_str());
        return;
    }
    ...
    if (!forced)
    {
        ...
        {
            FC_LOG_ERROR("network.opcode", "Prevented sending disabled opcode %s to %s", GetOpcodeNameForLogging(static_cast<OpcodeServer>(packet->GetOpcode())).c_str(), GetPlayerInfo().c_str());
            return;
        }
    }
    ...
    else
    {
        ...
        FC_LOG_DEBUG("misc", "Send all time packets count: " UI64FMTD " bytes: " UI64FMTD " avr.count/sec: %f avr.bytes/sec: %f time: %u", sendPacketCount, sendPacketBytes, float(sendPacketCount) / fullTime, float(sendPacketBytes) / fullTime, uint32(fullTime));
        FC_LOG_DEBUG("misc", "Send last min packets count: " UI64FMTD " bytes: " UI64FMTD " avr.count/sec: %f avr.bytes/sec: %f", sendLastPacketCount, sendLastPacketBytes, float(sendLastPacketCount) / minTime, float(sendLastPacketBytes) / minTime);
    ...
    FC_LOG_TRACE("network.opcode", "S->C: %s %s", GetPlayerInfo().c_str(), GetOpcodeNameForLogging(static_cast<OpcodeServer>(packet->GetOpcode())).c_str());
...
{
    FC_LOG_ERROR("network.opcode", "Received unexpected opcode %s Status: %s Reason: %s from %s",
...
void WorldSession::LogUnprocessedTail(WorldPacket const* packet)
{
    ...
    FC_LOG_TRACE("network.opcode", "Unprocessed tail data (read stop at %u from %u) Opcode %s from %s",
    ...
    while (m_Socket[CONNECTION_TYPE_REALM] && _recvQueue.next(packet, updater))
    {
        ...
        try
        {
            ...
            case STATUS_LOGGEDIN:
                if (!_player)
                {
                    ...
                    if (!m_playerRecentlyLogout)
                    {
                        ...
                        deletePacket = false;
                        FC_LOG_DEBUG("network", "Re-enqueueing packet with opcode %s with with status STATUS_LOGGEDIN. "
            ...
            case STATUS_AUTHED:
                ...
                break;
            case STATUS_NEVER:
                FC_LOG_ERROR("network.opcode", "Received not allowed opcode %s from %s", GetOpcodeNameForLogging(static_cast<OpcodeClient>(packet->GetOpcode())).c_str()
                    , GetPlayerInfo().c_str());
                break;
            case STATUS_UNHANDLED:
                FC_LOG_ERROR("network.opcode", "Received not handled opcode %s from %s", GetOpcodeNameForLogging(static_cast<OpcodeClient>(packet->GetOpcode())).c_str()
                    , GetPlayerInfo().c_str());
                break;
            }
        }
        ...
        {
            FC_LOG_ERROR("network", "PacketArrayMaxCapacityException: %s while parsing %s from %s.",
                pamce.what(), GetOpcodeNameForLogging(static_cast<OpcodeClient>(packet->GetOpcode())).c_str(), GetPlayerInfo().c_str());
        }
        catch (ByteBufferException const&)
        {
            FC_LOG_ERROR("network", "WorldSession::Update ByteBufferException occured while parsing a packet (opcode: %u) from client %s, accountid=%i. Skipped packet.",
...
void WorldSession::LogoutPlayer(bool save)
{
    ...
    if (_player)
    {
        ...
        FC_LOG_INFO("entities.player.character", "Account: %d (IP: %s) Logout Character:[%s] (GUID: %u) Level: %d",
        ...
        SendPacket(&data);
        FC_LOG_DEBUG("network", "SESSION: Sent SMSG_LOGOUT_COMPLETE Message");
...
{
    FC_LOG_ERROR("network.opcode", "Received unhandled opcode %s from %s"
        , GetOpcodeNameForLogging(static_cast<OpcodeClient>(null.GetOpcode())).c_str(), GetPlayerInfo().c_str());
}

void WorldSession::Handle_EarlyProccess(WorldPacket& recvPacket)
{
    FC_LOG_ERROR("network.opcode", "Received opcode %s that must be processed in WorldSocket::OnRead from %s"
        , GetOpcodeNameForLogging(static_cast<OpcodeClient>(recvPacket.GetOpcode())).c_str(), GetPlayerInfo().c_str());
}

void WorldSession::Handle_ServerSide(WorldPacket& recvPacket)
{
    FC_LOG_ERROR("network.opcode", "Received server-side opcode %s from %s"
        , GetOpcodeNameForLogging(static_cast<OpcodeClient>(recvPacket.GetOpcode())).c_str(), GetPlayerInfo().c_str());
}

void WorldSession::Handle_Deprecated(WorldPacket& recvPacket)
{
    FC_LOG_ERROR("network.opcode", "Received deprecated opcode %s from %s"
        , GetOpcodeNameForLogging(static_cast<OpcodeClient>(recvPacket.GetOpcode())).c_str(), GetPlayerInfo().c_str());
}
...
void WorldSession::SendConnectToInstance(WorldPackets::Auth::ConnectToSerial serial)
{
    ...
    boost::asio::ip::tcp::endpoint instanceAddress = realm.GetAddressForClient(Firelands::Net::make_address(GetRemoteAddress(), ignored_error));
...
void WorldSession::LoadAccountData(PreparedQueryResult result, uint32 mask)
{
    ...
    do
    {
        ...
        {
            FC_LOG_ERROR("misc", "Table `%s` have invalid account data type (%u), ignore.",
        ...
        if ((mask & (1 << type)) == 0)
        {
            FC_LOG_ERROR("misc", "Table `%s` have non appropriate for table  account data type (%u), ignore.",
                mask == GLOBAL_CACHE_MASK ? "account_data" : "character_account_data", type);
            continue;
        }
...
void WorldSession::ReadAddonsInfo(ByteBuffer& data)
{
    ...
    if (size > 0xFFFFF)
    {
        FC_LOG_DEBUG("addon", "WorldSession::ReadAddonsInfo: AddOnInfo too big, size %u", size);
        return;
    }
    ...
    if (uncompress(addonInfo.contents(), &uSize, data.contents() + pos, data.size() - pos) == Z_OK)
    {
        try
        {
            ...
            for (uint32 i = 0; i < addonsCount; ++i)
            {
                ...
                FC_LOG_DEBUG("addon", "AddOn: %s (CRC: 0x%x) - has key: 0x%x - URL CRC: 0x%x", addon.Name.c_str(), publicKeyCrc, addon.HasKey, urlCrc);
                ...
                if (savedAddon)
                {
                    if (publicKeyCrc != savedAddon->CRC)
                    {
                        if (addon.HasKey)
                        {
                            ...
                            FC_LOG_WARN("addon", " Addon: %s: modified (CRC: 0x%x) - accountID %d)", addon.Name.c_str(), savedAddon->CRC, GetAccountId());
                        }
                        else
                    ...
                    {
                        addon.Status = Addons::SecureAddonInfo::SECURE_HIDDEN;
                        FC_LOG_DEBUG("addon", "Addon: %s: validated (CRC: 0x%x) - accountID %d", addon.Name.c_str(), savedAddon->CRC, GetAccountId());
                    }
                }
                else
                {
                    addon.Status = Addons::SecureAddonInfo::BANNED;
                    FC_LOG_WARN("addon", "Addon: %s: not registered as known secure addon - accountId %d", addon.Name.c_str(), GetAccountId());
                }
            }
            ...
            FC_LOG_DEBUG("addon", "AddOn: Newest banned addon timestamp: %u", lastBannedAddOnTimestamp);
        }
        catch (ByteBufferException const& e)
        {
            FC_LOG_DEBUG("addon", "AddOn: Addon packet read error! %s", e.what());
        }
    }
    else
        FC_LOG_DEBUG("addon", "AddOn: Addon packet uncompress error!");
}
...
void WorldSession::SendAddonsInfo()
{
    ...
    for (Addons::SecureAddonInfo const& addonInfo : _addons.SecureAddons)
    {
        ...
        if (infoProvided)
        {
            ...
            {
                FC_LOG_DEBUG("addon", "AddOn: %s: key missing: sending pubkey to accountID %d", addonInfo.Name.c_str(), GetAccountId());
...
{
    FC_LOG_DEBUG("network", "WORLD: Received CMSG_UNREGISTER_ALL_ADDON_PREFIXES");
...
{
    FC_LOG_DEBUG("network", "WORLD: Received CMSG_ADDON_REGISTERED_PREFIXES");
...
QueryCallback WorldSession::LoadPermissionsAsync()
{
    ...
    FC_LOG_DEBUG("rbac", "WorldSession::LoadPermissions [AccountId: %u, Name: %s, realmId: %d, secLevel: %u]",
...
bool WorldSession::HasPermission(uint32 permission)
{
    ...
    FC_LOG_DEBUG("rbac", "WorldSession::HasPermission [AccountId: %u, Name: %s, realmId: %d]",
...
{
    FC_LOG_DEBUG("rbac", "WorldSession::Invalidaterbac::RBACData [AccountId: %u, Name: %s, realmId: %d]",
        _RBACData->GetId(), _RBACData->GetName().c_str(), realm.Id.Realm);
    delete _RBACData;
    _RBACData = nullptr;
}
...
bool WorldSession::DosProtection::EvaluateOpcode(WorldPacket& p, time_t time) const
{
    ...
    FC_LOG_WARN("network", "AntiDOS: Account %u, IP: %s, Ping: %u, Character: %s, flooding packet (opc: %s (0x%X), count: %u)",
    ...
    case POLICY_KICK:
    {
        FC_LOG_WARN("network", "AntiDOS: Player kicked!");
        Session->KickPlayer();
        return false;
    }
    case POLICY_BAN:
    {
        ...
        FC_LOG_WARN("network", "AntiDOS: Player automatically banned for %u seconds.", duration);
        Session->KickPlayer();
        return false;
    }
...
bool WorldSession::IsRightUnitBeingMoved(ObjectGuid guid)
{
    ...
    // edit: this wouldn't happen in retail but it does in FC, even with a legitimate client.
    if (!client->GetActivelyMovedUnit() || client->GetActivelyMovedUnit()->GetGUID() != guid)
    {
        FC_LOG_DEBUG("entities.unit", "Attempt at tampering movement data by Player %s", _player->GetName().c_str());
        return false;
    }
    ...
    {
        FC_LOG_DEBUG("entities.unit", "Bad or outdated movement data by Player %s", _player->GetName().c_str());
        return false;
    }
