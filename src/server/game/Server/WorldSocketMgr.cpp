...
bool WorldSocketMgr::StartWorldNetwork(Firelands::Asio::IoContext& ioContext, std::string const& bindIp, uint16 port, int threadCount)
{
    _tcpNoDelay = sConfigMgr->GetBoolDefault("Network.TcpNodelay", true);

    int const max_connections = FIRELANDS_MAX_LISTEN_CONNECTIONS;
    FC_LOG_DEBUG("misc", "Max allowed socket connections %d", max_connections);
    ...
    {
        FC_LOG_ERROR("misc", "Network.OutUBuff is wrong in your config file");
        return false;
    }
    ...
    {
        FC_LOG_ERROR("network", "Exception caught in WorldSocketMgr::StartNetwork (%s:%u): %s", bindIp.c_str(), port, err.what());
        return false;
    }

    if (!instanceAcceptor->Bind())
    {
        FC_LOG_ERROR("network", "StartNetwork failed to bind instance socket acceptor");
        return false;
    }
...
void WorldSocketMgr::OnSocketOpen(tcp::socket&& sock, uint32 threadIndex)
{
    ...
    if (_socketSystemSendBufferSize >= 0)
    {
        ...
        {
            FC_LOG_ERROR("misc", "WorldSocketMgr::OnSocketOpen sock.set_option(boost::asio::socket_base::send_buffer_size) err = %s", err.message().c_str());
            return;
        }
    }
    ...
    if (_tcpNoDelay)
    {
        ...
        if (err)
        {
            FC_LOG_ERROR("misc", "WorldSocketMgr::OnSocketOpen sock.set_option(boost::asio::ip::tcp::no_delay) err = %s", err.message().c_str());
            return;
        }
    }
