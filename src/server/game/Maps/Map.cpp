...
{
    std::string tileListName = Firelands::StringFormat("%smaps/%03u.tilelist", sWorld->GetDataPath().c_str(), GetId());
    ...
    if (FILE* tileList = fopen(tileListName.c_str(), "rb"))
    {
        ...
        {
            i_gridFileExists = std::bitset<MAX_NUMBER_OF_GRIDS* MAX_NUMBER_OF_GRIDS>(tilesData, Firelands::Containers::Size(tilesData));
            fclose(tileList);
            return;
        }
...
{
    std::string fileName = Firelands::StringFormat("%smaps/%03u%02u%02u.map", sWorld->GetDataPath().c_str(), mapid, gx, gy);
    ...
    if (!file)
    {
        if (log)
        {
            FC_LOG_ERROR("maps", "Map file '%s' does not exist!", fileName.c_str());
            FC_LOG_ERROR("maps", "Please place MAP-files (*.map) in the appropriate directory (%s), or correct the DataDir setting in your worldserver.conf file.", (sWorld->GetDataPath() + "maps/").c_str());
        }
    }
    else
    {
        ...
        if (fread(&header, sizeof(header), 1, file) == 1)
        {
            ...
            {
                if (log)
                    FC_LOG_ERROR("maps", "Map file '%s' is from an incompatible map version (%.*s v%u), %.*s v%u is expected. Please pull your source, recompile tools and recreate maps using the updated mapextractor, then replace your old map files with new files. If you still have problems search on forum for error TCE00018.",
    ...
    if (VMAP::IVMapManager* vmgr = VMAP::VMapFactory::createOrGetVMapManager())
    {
        if (vmgr->isMapLoadingEnabled())
        {
            ...
            case VMAP::LoadResult::FileNotFound:
                FC_LOG_ERROR("maps", "VMap file '%s' does not exist", (sWorld->GetDataPath() + "vmaps/" + name).c_str());
                FC_LOG_ERROR("maps", "Please place VMAP files (*.vmtree and *.vmtile) in the vmap directory (%s), or correct the DataDir setting in your worldserver.conf file.", (sWorld->GetDataPath() + "vmaps/").c_str());
                return false;
            case VMAP::LoadResult::VersionMismatch:
                FC_LOG_ERROR("maps", "VMap file '%s' couldn't be loaded", (sWorld->GetDataPath() + "vmaps/" + name).c_str());
                FC_LOG_ERROR("maps", "This is because the version of the VMap file and the version of this module are different, please re-extract the maps with the tools compiled with this module.");
                return false;
            }
        }
    }
...
void Map::LoadMMap(int gx, int gy)
{
    ...
    if (mmapLoadResult)
        FC_LOG_DEBUG("mmaps", "MMAP loaded name:%s, id:%d, x:%d, y:%d (mmap rep.: x:%d, y:%d)", GetMapName(), GetId(), gx, gy, gx, gy);
    else
        FC_LOG_ERROR("mmaps", "Could not load MMAP name:%s, id:%d, x:%d, y:%d (mmap rep.: x:%d, y:%d)", GetMapName(), GetId(), gx, gy, gx, gy);
}
...
void Map::LoadVMap(int gx, int gy)
{
    ...
    case VMAP::VMAP_LOAD_RESULT_OK:
        FC_LOG_DEBUG("maps", "VMAP loaded name:%s, id:%d, x:%d, y:%d (vmap rep.: x:%d, y:%d)", GetMapName(), GetId(), gx, gy, gx, gy);
        break;
    case VMAP::VMAP_LOAD_RESULT_ERROR:
        FC_LOG_ERROR("maps", "Could not load VMAP name:%s, id:%d, x:%d, y:%d (vmap rep.: x:%d, y:%d)", GetMapName(), GetId(), gx, gy, gx, gy);
        break;
    case VMAP::VMAP_LOAD_RESULT_IGNORED:
        FC_LOG_DEBUG("maps", "Ignored VMAP name:%s, id:%d, x:%d, y:%d (vmap rep.: x:%d, y:%d)", GetMapName(), GetId(), gx, gy, gx, gy);
        break;
    }
}
...
void Map::LoadMapImpl(Map* map, int gx, int gy)
{
    ...
    // map file name
    std::string fileName = Firelands::StringFormat("%smaps/%03u%02u%02u.map", sWorld->GetDataPath().c_str(), map->GetId(), gx, gy);
    FC_LOG_DEBUG("maps", "Loading map %s", fileName.c_str());
    // loading data
    ...
    if (map->GridMaps[gx][gy])
        ...
        FC_LOG_ERROR("maps", "Error loading map file: %s", fileName.c_str());
}
...
void Map::SwitchGridContainers(Creature* obj, bool on)
{
    ...
    CellCoord p = Firelands::ComputeCellCoord(obj->GetPositionX(), obj->GetPositionY());
    if (!p.IsCoordValid())
    {
        FC_LOG_ERROR("maps", "Map::SwitchGridContainers: Object %s has invalid coordinates X:%f Y:%f grid cell [%u:%u]", obj->GetGUID().ToString().c_str(), obj->GetPositionX(), obj->GetPositionY(), p.x_coord, p.y_coord);
        return;
    }
    ...
    if (sLog->ShouldLog("maps", LOG_LEVEL_DEBUG))
    {
        ...
        FC_LOG_DEBUG("maps", "Switch object %s from grid[%u, %u] %u", obj->GetGUID().ToString().c_str(), grid_x, grid_y, on);
    }
...
void Map::EnsureGridCreated_i(const GridCoord& p)
{
    ...
    {
        FC_LOG_DEBUG("maps", "Creating grid[%u, %u] for map %u instance %u", p.x_coord, p.y_coord, GetId(), i_InstanceId);
...
void Map::EnsureGridLoadedForActiveObject(const Cell& cell, WorldObject* object)
{
    ...
    {
        FC_LOG_DEBUG("maps", "Active object %s triggers loading of grid [%u, %u] on map %u", object->GetGUID().ToString().c_str(), cell.GridX(), cell.GridY(), GetId());
...
bool Map::EnsureGridLoaded(const Cell& cell)
{
    ...
    {
        FC_LOG_DEBUG("maps", "Loading grid[%u, %u] for map %u instance %u", cell.GridX(), cell.GridY(), GetId(), i_InstanceId);
...
{
    CellCoord cellCoord = Firelands::ComputeCellCoord(player->GetPositionX(), player->GetPositionY());
    if (!cellCoord.IsCoordValid())
    {
        FC_LOG_ERROR("maps", "Map::Add: Player (GUID: %u) has invalid coordinates X:%f Y:%f grid cell [%u:%u]", player->GetGUID().GetCounter(), player->GetPositionX(), player->GetPositionY(), cellCoord.x_coord, cellCoord.y_coord);
        return false;
    }
...
bool Map::AddToMap(T* obj)
{
    ...
    CellCoord cellCoord = Firelands::ComputeCellCoord(obj->GetPositionX(), obj->GetPositionY());
    ...
    if (!cellCoord.IsCoordValid())
    {
        FC_LOG_ERROR("maps", "Map::Add: Object %s has invalid coordinates X:%f Y:%f grid cell [%u:%u]", obj->GetGUID().ToString().c_str(), obj->GetPositionX(), obj->GetPositionY(), cellCoord.x_coord, cellCoord.y_coord);
    ...
    if (obj->isActiveObject())
        ...
        EnsureGridCreated(GridCoord(cell.GridX(), cell.GridY()));
    AddToGrid(obj, cell);
    FC_LOG_DEBUG("maps", "Object %s enters grid[%u, %u]", obj->GetGUID().ToString().c_str(), cell.GridX(), cell.GridY());
...
void Map::VisitNearbyCellsOf(WorldObject* obj, TypeContainerVisitor<Firelands::ObjectUpdater, GridTypeMapContainer>& gridVisitor, TypeContainerVisitor<Firelands::ObjectUpdater, WorldTypeMapContainer>& worldVisitor)
{
...
void Map::Update(uint32 t_diff)
{
    ...
    Firelands::ObjectUpdater updater(t_diff);
    // for creature
    TypeContainerVisitor<Firelands::ObjectUpdater, GridTypeMapContainer  > grid_object_update(updater);
    // for pets
    TypeContainerVisitor<Firelands::ObjectUpdater, WorldTypeMapContainer > world_object_update(updater);
    ...
    for (GridRefManager<NGridType>::iterator i = GridRefManager<NGridType>::begin(); i != GridRefManager<NGridType>::end(); ++i)
    {
        ...
        for (uint32 x = cell_min.x_coord; x < cell_max.x_coord; ++x)
        {
            for (uint32 y = cell_min.y_coord; y < cell_max.y_coord; ++y)
            {
                ...
                Firelands::DelayedUnitRelocation cell_relocation(cell, pair, *this, MAX_VISIBILITY_DISTANCE);
                TypeContainerVisitor<Firelands::DelayedUnitRelocation, GridTypeMapContainer  > grid_object_relocation(cell_relocation);
                TypeContainerVisitor<Firelands::DelayedUnitRelocation, WorldTypeMapContainer > world_object_relocation(cell_relocation);
...
/*static*/ bool Map::CheckGridIntegrity(T* object, bool moved, char const* objType)
{
    ...
    if (xy_cell != cur_cell)
    {
        FC_LOG_DEBUG("maps", "%s (%s) X: %f Y: %f (%s) is in grid[%u, %u]cell[%u, %u] instead of grid[%u, %u]cell[%u, %u]",
...
void Map::PlayerRelocation(Player* player, float x, float y, float z, float orientation)
{
    ...
    {
        FC_LOG_DEBUG("maps", "Player %s relocation grid[%u, %u]cell[%u, %u]->grid[%u, %u]cell[%u, %u]", player->GetName().c_str(), old_cell.GridX(), old_cell.GridY(), old_cell.CellX(), old_cell.CellY(), new_cell.GridX(), new_cell.GridY(), new_cell.CellX(), new_cell.CellY());
...
void Map::CreatureRelocation(Creature* creature, float x, float y, float z, float ang, bool respawnRelocationOnFail)
{
    ...
    if (old_cell.DiffCell(new_cell) || old_cell.DiffGrid(new_cell))
    {
        ...
        FC_LOG_DEBUG("maps", "Creature (%s Entry: %u) added to moving list from grid[%u, %u]cell[%u, %u] to grid[%u, %u]cell[%u, %u].", creature->GetGUID().ToString().c_str(), creature->GetEntry(), old_cell.GridX(), old_cell.GridY(), old_cell.CellX(), old_cell.CellY(), new_cell.GridX(), new_cell.GridY(), new_cell.CellX(), new_cell.CellY());
#endif
...
void Map::GameObjectRelocation(GameObject* go, float x, float y, float z, float orientation, bool respawnRelocationOnFail)
{
    ...
    {
#ifdef FIRELANDS_DEBUG
        FC_LOG_DEBUG("maps", "GameObject (GUID: %u Entry: %u) added to moving list from grid[%u, %u]cell[%u, %u] to grid[%u, %u]cell[%u, %u].", go->GetGUID().GetCounter(), go->GetEntry(), old_cell.GridX(), old_cell.GridY(), old_cell.CellX(), old_cell.CellY(), new_cell.GridX(), new_cell.GridY(), new_cell.CellX(), new_cell.CellY());
#endif
...
void Map::DynamicObjectRelocation(DynamicObject* dynObj, float x, float y, float z, float orientation)
{
    ...
    {
#ifdef FIRELANDS_DEBUG
        FC_LOG_DEBUG("maps", "GameObject (GUID: %u) added to moving list from grid[%u, %u]cell[%u, %u] to grid[%u, %u]cell[%u, %u].", dynObj->GetGUID().GetCounter(), old_cell.GridX(), old_cell.GridY(), old_cell.CellX(), old_cell.CellY(), new_cell.GridX(), new_cell.GridY(), new_cell.CellX(), new_cell.CellY());
#endif
...
#ifdef FIRELANDS_DEBUG
                FC_LOG_DEBUG("maps", "Creature (GUID: %u Entry: %u) cannot be move to unloaded respawn grid.", c->GetGUID().GetCounter(), c->GetEntry());
#endif
    ...
    for (std::vector<GameObject*>::iterator itr = _gameObjectsToMove.begin(); itr != _gameObjectsToMove.end(); ++itr)
    {
        ...
        else
        {
            ...
            if (!GameObjectRespawnRelocation(go, false))
            {
                // ... or unload (if respawn grid also not loaded)
#ifdef FIRELANDS_DEBUG
                FC_LOG_DEBUG("maps", "GameObject (GUID: %u Entry: %u) cannot be move to unloaded respawn grid.", go->GetGUID().GetCounter(), go->GetEntry());
#endif
...
void Map::MoveAllDynamicObjectsInMoveList()
{
    ...
    for (std::vector<DynamicObject*>::iterator itr = _dynamicObjectsToMove.begin(); itr != _dynamicObjectsToMove.end(); ++itr)
    {
        ...
        {
#ifdef FIRELANDS_DEBUG
            FC_LOG_DEBUG("maps", "DynamicObject (GUID: %u) cannot be moved to unloaded grid.", dynObj->GetGUID().GetCounter());
#endif
        }
    }
...
bool Map::MapObjectCellRelocation(T* object, Cell new_cell, char const* objType)
{
    ...
    if (!old_cell.DiffGrid(new_cell))                       // in same grid
    {
        ...
        {
#ifdef FIRELANDS_DEBUG
            FC_LOG_DEBUG("maps", "%s (%s Entry: %u) moved in grid[%u, %u] from cell[%u, %u] to cell[%u, %u].", objType, object->GetGUID().ToString().c_str(), object->GetEntry(), old_cell.GridX(), old_cell.GridY(), old_cell.CellX(), old_cell.CellY(), new_cell.CellX(), new_cell.CellY());
#endif
        ...
        {
#ifdef FIRELANDS_DEBUG
            FC_LOG_DEBUG("maps", "%s (%s Entry: %u) moved in same grid[%u, %u]cell[%u, %u].", objType, object->GetGUID().ToString().c_str(), object->GetEntry(), old_cell.GridX(), old_cell.GridY(), old_cell.CellX(), old_cell.CellY());
#endif
        }
...
#ifdef FIRELANDS_DEBUG
        FC_LOG_DEBUG("maps", "Active %s (%s Entry: %u) moved from grid[%u, %u]cell[%u, %u] to grid[%u, %u]cell[%u, %u].", objType, object->GetGUID().ToString().c_str(), object->GetEntry(), old_cell.GridX(), old_cell.GridY(), old_cell.CellX(), old_cell.CellY(), new_cell.GridX(), new_cell.GridY(), new_cell.CellX(), new_cell.CellY());
#endif
    ...
    {
#ifdef FIRELANDS_DEBUG
        FC_LOG_DEBUG("maps", "%s (%s Entry: %u) moved from grid[%u, %u]cell[%u, %u] to grid[%u, %u]cell[%u, %u].", objType, object->GetGUID().ToString().c_str(), object->GetEntry(), old_cell.GridX(), old_cell.GridY(), old_cell.CellX(), old_cell.CellY(), new_cell.GridX(), new_cell.GridY(), new_cell.CellX(), new_cell.CellY());
#endif
...
#ifdef FIRELANDS_DEBUG
    FC_LOG_DEBUG("maps", "%s (%s Entry: %u) attempted to move from grid[%u, %u]cell[%u, %u] to unloaded grid[%u, %u]cell[%u, %u].", objType, object->GetGUID().ToString().c_str(), object->GetEntry(), old_cell.GridX(), old_cell.GridY(), old_cell.CellX(), old_cell.CellY(), new_cell.GridX(), new_cell.GridY(), new_cell.CellX(), new_cell.CellY());
#endif
    (void)objType;
    return false;
}
...
#ifdef FIRELANDS_DEBUG
    FC_LOG_DEBUG("maps", "Creature (GUID: %u Entry: %u) moved from grid[%u, %u]cell[%u, %u] to respawn grid[%u, %u]cell[%u, %u].", c->GetGUID().GetCounter(), c->GetEntry(), c->GetCurrentCell().GridX(), c->GetCurrentCell().GridY(), c->GetCurrentCell().CellX(), c->GetCurrentCell().CellY(), resp_cell.GridX(), resp_cell.GridY(), resp_cell.CellX(), resp_cell.CellY());
#endif
...
#ifdef FIRELANDS_DEBUG
    FC_LOG_DEBUG("maps", "GameObject (GUID: %u Entry: %u) moved from grid[%u, %u]cell[%u, %u] to respawn grid[%u, %u]cell[%u, %u].", go->GetGUID().GetCounter(), go->GetEntry(), go->GetCurrentCell().GridX(), go->GetCurrentCell().GridY(), go->GetCurrentCell().CellX(), go->GetCurrentCell().CellY(), resp_cell.GridX(), resp_cell.GridY(), resp_cell.CellX(), resp_cell.CellY());
#endif
...
bool Map::UnloadGrid(NGridType& ngrid, bool unloadAll)
{
...

    {
        ...
        FC_LOG_DEBUG("maps", "Unloading grid[%u, %u] for map %u", x, y, GetId());
    ...
    FC_LOG_DEBUG("maps", "Unloading grid[%u, %u] for map %u finished", x, y, GetId());
    return true;
}
    ...
    if (HavePlayers())
    {
        for (MapRefManager::iterator itr = m_mapRefManager.begin(); itr != m_mapRefManager.end(); ++itr)
        {
            ...
            if (!player->IsBeingTeleportedFar())
            {
                ...
                FC_LOG_ERROR("maps", "Map::UnloadAll: player %s is still in map %u during unload, this should not happen!", player->GetName().c_str(), GetId());
...
GridMap::LoadResult GridMap::loadData(const char* filename)
{
    ...
    if (header.mapMagic.asUInt == MapMagic.asUInt && header.versionMagic == MapVersionMagic)
    {
        ...
        {
            FC_LOG_ERROR("maps", "Error loading map area data\n");
            fclose(in);
            return LoadResult::InvalidFile;
        }
        // load up height data
        ...
        {
            FC_LOG_ERROR("maps", "Error loading map height data\n");
            fclose(in);
            return LoadResult::InvalidFile;
        }
        // load up liquid data
        ...
        {
            FC_LOG_ERROR("maps", "Error loading map liquids data\n");
            fclose(in);
            return LoadResult::InvalidFile;
        }
        ...
        {
            FC_LOG_ERROR("maps", "Error loading map holes data\n");
            fclose(in);
            return LoadResult::InvalidFile;
        }
        fclose(in);
...
float GridMap::getMinHeight(float x, float y) const
{
    ...
    GridCoord gridCoord = Firelands::ComputeGridCoordSimple(x, y);
...
ZLiquidStatus Map::GetLiquidStatus(PhaseShift const& phaseShift, float x, float y, float z, uint8 ReqLiquidType, LiquidData* data, float collisionHeight)
{
    ...
    if (vmgr->GetLiquidLevel(terrainMapId, x, y, z, ReqLiquidType, liquid_level, ground_level, liquid_type, mogpFlags))
    {
        ...
        FC_LOG_DEBUG("maps", "GetLiquidStatus(): vmap liquid level: %f ground: %f type: %u", liquid_level, ground_level, liquid_type);
        // Check water level and ground level
...
{
    FC_LOG_DEBUG("maps", "Creating player data for himself %u", player->GetGUID().GetCounter());
...
inline void Map::setNGrid(NGridType* grid, uint32 x, uint32 y)
{
    ...
    {
        FC_LOG_ERROR("maps", "map::setNGrid() Invalid grid coordinates found: %d, %d!", x, y);
        ABORT();
    }
    i_grids[x][y] = grid;
}
...
size_t Map::DespawnAll(SpawnObjectType type, ObjectGuid::LowType spawnId)
{
    ...
    {
    case SPAWN_TYPE_CREATURE:
        for (auto const& pair : Firelands::Containers::MapEqualRange(GetCreatureBySpawnIdStore(), spawnId))
            toUnload.push_back(pair.second);
        break;
    case SPAWN_TYPE_GAMEOBJECT:
        for (auto const& pair : Firelands::Containers::MapEqualRange(GetGameObjectBySpawnIdStore(), spawnId))
            toUnload.push_back(pair.second);
        break;
    default:
        break;
...
bool Map::AddRespawnInfo(RespawnInfo const& info)
{
    if (!info.spawnId)
    {
        FC_LOG_ERROR("maps", "Attempt to insert respawn info for zero spawn id (type %u)", uint32(info.type));
        return false;
    }
...
bool Map::SpawnGroupSpawn(uint32 groupId, bool ignoreRespawn, bool force, std::vector<WorldObject*>* spawnedObjects)
{
    ...
    {
        FC_LOG_ERROR("maps", "Tried to spawn non-existing (or system) spawn group %u on map %u. Blocked.", groupId, GetId());
        return false;
    }
...
bool Map::SpawnGroupDespawn(uint32 groupId, bool deleteRespawnTimes, size_t* count)
{
    ...
    if (!groupData || groupData->flags & SPAWNGROUP_FLAG_SYSTEM)
    {
        FC_LOG_ERROR("maps", "Tried to despawn non-existing (or system) spawn group %u on map %u. Blocked.", groupId, GetId());
        return false;
    }
...
void Map::SetSpawnGroupActive(uint32 groupId, bool state)
{
    ...
    {
        FC_LOG_ERROR("maps", "Tried to set non-existing (or system) spawn group %u to %s on map %u. Blocked.", groupId, state ? "active" : "inactive", GetId());
        return;
    }
...
bool Map::IsSpawnGroupActive(uint32 groupId) const
{
    ...
    if (!data)
    {
        FC_LOG_ERROR("maps", "Tried to query state of non-existing spawn group %u on map %u.", groupId, GetId());
        return false;
    }
...
void Map::AddObjectToRemoveList(WorldObject* obj)
{
    ...
    //FC_LOG_DEBUG("maps", "Object (GUID: %u TypeId: %u) added to removing list.", obj->GetGUID().GetCounter(), obj->GetTypeId());
}
...
void Map::RemoveAllObjectsInRemoveList()
{
    ...
    //FC_LOG_DEBUG("maps", "Object remover 1 check.");
    while (!i_objectsToRemove.empty())
    {
        ...
        case TYPEID_CORPSE:
        {
            ...
            if (!corpse)
                FC_LOG_ERROR("maps", "Tried to delete corpse/bones %u that is not in map.", obj->GetGUID().GetCounter());
            else
        ...
        case TYPEID_UNIT:
            ...
            break;
        default:
            FC_LOG_ERROR("maps", "Non-grid object (TypeId: %u) is in grid object remove list, ignored.", obj->GetTypeId());
            break;
        }
    ...
    //FC_LOG_DEBUG("maps", "Object remover 2 check.");
}
...
bool Map::ActiveObjectsNearGrid(NGridType const& ngrid) const
{
    ...
    for (MapRefManager::const_iterator iter = m_mapRefManager.begin(); iter != m_mapRefManager.end(); ++iter)
    {
        ...
        CellCoord p = Firelands::ComputeCellCoord(player->GetPositionX(), player->GetPositionY());
...
void Map::AddToActive(WorldObject* obj)
{
    ...
    if (respawnLocation)
    {
        GridCoord p = Firelands::ComputeGridCoord(respawnLocation->GetPositionX(), respawnLocation->GetPositionY());
        if (getNGrid(p.x_coord, p.y_coord))
            getNGrid(p.x_coord, p.y_coord)->incUnloadActiveLock();
        else
        {
            GridCoord p2 = Firelands::ComputeGridCoord(obj->GetPositionX(), obj->GetPositionY());
            FC_LOG_ERROR("maps", "Active object %s added to grid[%u, %u] but spawn grid[%u, %u] was not loaded.",
...
void Map::RemoveFromActive(WorldObject* obj)
{
    ...
    if (respawnLocation)
    {
        ...
        {
            GridCoord p2 = Firelands::ComputeGridCoord(obj->GetPositionX(), obj->GetPositionY());
            FC_LOG_ERROR("maps", "Active object %s removed from to grid[%u, %u] but spawn grid[%u, %u] was not loaded.",
                obj->GetGUID().ToString().c_str(), p.x_coord, p.y_coord, p2.x_coord, p2.y_coord);
        }
    }
}
...
Map::EnterState InstanceMap::CannotEnter(Player* player)
{
    ...
    {
        FC_LOG_ERROR("maps", "InstanceMap::CannotEnter - player %s(%u) already in map %d, %d, %d!", player->GetName().c_str(), player->GetGUID().GetCounter(), GetId(), GetInstanceId(), GetSpawnMode());
        ABORT();
    ...
    {
        FC_LOG_WARN("maps", "MAP: Instance '%u' of map '%s' cannot have more than '%u' players. Player '%s' rejected", GetInstanceId(), GetMapName(), maxPlayers, player->GetName().c_str());
...
bool InstanceMap::AddPlayerToMap(Player* player)
{
...

    {
        ...
        if (IsDungeon())
        {
            ...
            if (!mapSave)
            {
                FC_LOG_DEBUG("maps", "InstanceMap::Add: creating instance save for map %d spawnmode %d with instance id %d", GetId(), GetSpawnMode(), GetInstanceId());
            ...
            else
            {
                if (group)
                {
                    ...
                    if (playerBind && playerBind->save != mapSave)
                    {
                        ...
                        if (groupBind)
                            FC_LOG_ERROR("maps", "InstanceMap::Add: the group is bound to the instance %s %d, %d, %d, %d, %d, %d", GetMapName(), groupBind->save->GetMapId(), groupBind->save->GetInstanceId(), groupBind->save->GetDifficulty(), groupBind->save->GetPlayerCount(), groupBind->save->GetGroupCount(), groupBind->save->CanReset());
                        //ABORT();
                        return false;
                    }
                    ...
                    else
                    {
                        ...
                        if (groupBind->save != mapSave)
                        {
                            ...
                            FC_LOG_ERROR("maps", "MapSave players: %d, group count: %d", mapSave->GetPlayerCount(), mapSave->GetGroupCount());
                            if (groupBind->save)
                                FC_LOG_ERROR("maps", "GroupBind save players: %d, group count: %d", groupBind->save->GetPlayerCount(), groupBind->save->GetGroupCount());
                            else
                                FC_LOG_ERROR("maps", "GroupBind save nullptr");
                            return false;
                        }
        ...
        FC_LOG_DEBUG("maps", "MAP: Player '%s' entered instance '%u' of map '%s'", player->GetName().c_str(), GetInstanceId(), GetMapName());
...
{
    FC_LOG_DEBUG("maps", "MAP: Removing player '%s' from instance '%u' of map '%s' before relocating to another map", player->GetName().c_str(), GetInstanceId(), GetMapName());
...
void InstanceMap::CreateInstanceData(bool load)
{
    ...
    if (load)
    {
        ...
        if (result)
        {
            ...
            if (!data.empty())
            {
                FC_LOG_DEBUG("maps", "Loading instance data for `%s` with id %u", sObjectMgr->GetScriptName(i_script_id).c_str(), i_InstanceId);
...
void InstanceMap::PermBindAllPlayers()
{
    ...
    if (!save)
    {
        FC_LOG_ERROR("maps", "Cannot bind players to instance map (Name: %s, Entry: %u, Difficulty: %u, ID: %u) because no instance save is available!", GetMapName(), GetId(), GetDifficulty(), GetInstanceId());
        return;
    }
    ...
    for (MapRefManager::iterator itr = m_mapRefManager.begin(); itr != m_mapRefManager.end(); ++itr)
    {
        ...
        if (bind && bind->perm)
        {
            ...
            {
                FC_LOG_ERROR("maps", "Player (GUID: %u, Name: %s) is in instance map (Name: %s, Entry: %u, Difficulty: %u, ID: %u) that is being bound, but already has a save for the map on ID %u!", player->GetGUID().GetCounter(), player->GetName().c_str(), GetMapName(), save->GetMapId(), save->GetDifficulty(), save->GetInstanceId(), bind->save->GetInstanceId());
            }
            else if (!bind->save)
            {
                FC_LOG_ERROR("maps", "Player (GUID: %u, Name: %s) is in instance map (Name: %s, Entry: %u, Difficulty: %u, ID: %u) that is being bound, but already has a bind (without associated save) for the map!", player->GetGUID().GetCounter(), player->GetName().c_str(), GetMapName(), save->GetMapId(), save->GetDifficulty(), save->GetInstanceId());
            }
        }
        else
        {
...
void InstanceMap::SetResetSchedule(bool on)
{
    ...
    if (IsDungeon() && !HavePlayers() && !IsRaidOrHeroicDungeon())
    {
        if (InstanceSave* save = sInstanceSaveMgr->GetInstanceSave(GetInstanceId()))
            ...
            FC_LOG_ERROR("maps", "InstanceMap::SetResetSchedule: cannot turn schedule %s, there is no save information for instance (map [id: %u, name: %s], instance id: %u, difficulty: %u)",
...
Map::EnterState BattlegroundMap::CannotEnter(Player* player)
{
    if (player->GetMapRef().getTarget() == this)
    {
        FC_LOG_ERROR("maps", "BGMap::CannotEnter - player %u is already in map!", player->GetGUID().GetCounter());
        ABORT();
        return CANNOT_ENTER_ALREADY_IN_MAP;
    }
...
{
    FC_LOG_DEBUG("maps", "MAP: Removing player '%s' from bg '%u' of map '%s' before relocating to another map", player->GetName().c_str(), GetInstanceId(), GetMapName());
    Map::RemovePlayerFromMap(player, remove);
}
...
void Map::SaveRespawnTime(SpawnObjectType type, ObjectGuid::LowType spawnId, uint32 entry, time_t respawnTime, uint32 gridId, CharacterDatabaseTransaction dbTrans, bool startup)
{
    ...
    if (!data)
    {
        FC_LOG_ERROR("maps", "Map %u attempt to save respawn time for nonexistant spawnid (%u,%u).", GetId(), type, spawnId);
        return;
    }
    ...
    {
        if (!success)
            FC_LOG_ERROR("maps", "Attempt to load saved respawn %" PRIu64 " for (%u,%u) failed - duplicate respawn? Skipped.", respawnTime, uint32(type), spawnId);
    }
    else if (success)
        ...
        do
        {
            ...
            if (SpawnData::TypeHasData(type))
            {
                if (SpawnData const* data = sObjectMgr->GetSpawnData(type, spawnId))
                    SaveRespawnTime(type, spawnId, data->id, time_t(respawnTime), Firelands::ComputeGridCoord(data->spawnPoint.GetPositionX(), data->spawnPoint.GetPositionY()).GetId(), nullptr, true);
                else
                    FC_LOG_ERROR("maps", "Loading saved respawn time of %" PRIu64 " for spawnid (%u,%u) - spawn does not exist, ignoring", respawnTime, uint32(type), spawnId);
            }
            else
            {
                FC_LOG_ERROR("maps", "Loading saved respawn time of %" PRIu64 " for spawnid (%u,%u) - invalid spawn type, ignoring", respawnTime, uint32(type), spawnId);
            }
...
void Map::LoadCorpseData()
{
    ...
    do
    {
        ...
        {
            FC_LOG_ERROR("misc", "Corpse (guid: %u) have wrong corpse type (%u), not loading.", guid, type);
            continue;
        }
...
Weather* Map::GetOrGenerateZoneDefaultWeather(uint32 zoneId)
{
    ...
    {
        info.DefaultWeather = Firelands::make_unique<Weather>(zoneId, weatherData);
