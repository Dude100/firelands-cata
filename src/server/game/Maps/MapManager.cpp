...
Map::EnterState MapManager::PlayerCannotEnter(uint32 mapid, Player* player, bool loginCheck)
{
    ...
    if (!player->IsAlive())
    {
        if (player->HasCorpse())
        {
            ...
            FC_LOG_DEBUG("maps", "MAP: Player '%s' has corpse in instance '%s' and can enter.", player->GetName().c_str(), mapName);
        }
        else
            FC_LOG_DEBUG("maps", "Map::CanPlayerEnter - player '%s' is dead but does not have a corpse!", player->GetName().c_str());
    }
...
uint32 MapManager::GenerateInstanceId()
{
    ...
    {
        FC_LOG_ERROR("maps", "Instance ID overflow!! Can't continue, shutting down server. ");
