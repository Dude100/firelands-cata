...
inline Player* Map::_GetScriptPlayerSourceOrTarget(Object* source, Object* target, ScriptInfo const* scriptInfo) const
{
    ...
    if (!source && !target)
        FC_LOG_ERROR("scripts", "%s source and target objects are nullptr.", scriptInfo->GetDebugInfo().c_str());
    else
    {
        ...
        if (!player)
            FC_LOG_ERROR("scripts", "%s neither source nor target object is player (source: TypeId: %u, Entry: %u, GUID: %u; target: TypeId: %u, Entry: %u, GUID: %u), skipping.",
...
inline Creature* Map::_GetScriptCreatureSourceOrTarget(Object* source, Object* target, ScriptInfo const* scriptInfo, bool bReverse) const
{
    ...
    else
    {
        ...
        if (!creature)
            FC_LOG_ERROR("scripts", "%s neither source nor target are creatures (source: TypeId: %u, Entry: %u, GUID: %u; target: TypeId: %u, Entry: %u, GUID: %u), skipping.",
                scriptInfo->GetDebugInfo().c_str(),
                source ? source->GetTypeId() : 0, source ? source->GetEntry() : 0, source ? source->GetGUID().GetCounter() : 0,
                target ? target->GetTypeId() : 0, target ? target->GetEntry() : 0, target ? target->GetGUID().GetCounter() : 0);
    }
...
inline Unit* Map::_GetScriptUnit(Object* obj, bool isSource, ScriptInfo const* scriptInfo) const
{
    Unit* unit = nullptr;
    if (!obj)
        FC_LOG_ERROR("scripts", "%s %s object is nullptr.", scriptInfo->GetDebugInfo().c_str(), isSource ? "source" : "target");
    else if (!obj->isType(TYPEMASK_UNIT))
        FC_LOG_ERROR("scripts", "%s %s object is not unit (TypeId: %u, Entry: %u, GUID: %u), skipping.",
            scriptInfo->GetDebugInfo().c_str(), isSource ? "source" : "target", obj->GetTypeId(), obj->GetEntry(), obj->GetGUID().GetCounter());
    else
    {
        unit = obj->ToUnit();
        if (!unit)
            FC_LOG_ERROR("scripts", "%s %s object could not be cast to unit.",
...
inline Player* Map::_GetScriptPlayer(Object* obj, bool isSource, ScriptInfo const* scriptInfo) const
{
    ...
    else
    {
        player = obj->ToPlayer();
        if (!player)
            FC_LOG_ERROR("scripts", "%s %s object is not a player (TypeId: %u, Entry: %u, GUID: %u).",
                scriptInfo->GetDebugInfo().c_str(), isSource ? "source" : "target", obj->GetTypeId(), obj->GetEntry(), obj->GetGUID().GetCounter());
    }
    return player;
}
...
inline Creature* Map::_GetScriptCreature(Object* obj, bool isSource, ScriptInfo const* scriptInfo) const
{
    ...
    else
    {
        ...
        if (!creature)
            FC_LOG_ERROR("scripts", "%s %s object is not a creature (TypeId: %u, Entry: %u, GUID: %u).", scriptInfo->GetDebugInfo().c_str(),
...
inline WorldObject* Map::_GetScriptWorldObject(Object* obj, bool isSource, ScriptInfo const* scriptInfo) const
{
    ...
    if (!obj)
        FC_LOG_ERROR("scripts", "%s %s object is nullptr.",
            scriptInfo->GetDebugInfo().c_str(), isSource ? "source" : "target");
    else
    {
        pWorldObject = dynamic_cast<WorldObject*>(obj);
        if (!pWorldObject)
            FC_LOG_ERROR("scripts", "%s %s object is not a world object (TypeId: %u, Entry: %u, GUID: %u).",
                scriptInfo->GetDebugInfo().c_str(), isSource ? "source" : "target", obj->GetTypeId(), obj->GetEntry(), obj->GetGUID().GetCounter());
    }
    return pWorldObject;
}
...
inline void Map::_ScriptProcessDoor(Object* source, Object* target, ScriptInfo const* scriptInfo) const
{
    ...
    default:
        FC_LOG_ERROR("scripts", "%s unknown command for _ScriptProcessDoor.", scriptInfo->GetDebugInfo().c_str());
        return;
    }
    if (!guid)
        FC_LOG_ERROR("scripts", "%s door guid is not specified.", scriptInfo->GetDebugInfo().c_str());
    else if (!source)
        FC_LOG_ERROR("scripts", "%s source object is nullptr.", scriptInfo->GetDebugInfo().c_str());
    else if (!source->isType(TYPEMASK_UNIT))
        FC_LOG_ERROR("scripts", "%s source object is not unit (TypeId: %u, Entry: %u, GUID: %u), skipping.", scriptInfo->GetDebugInfo().c_str(),
            source->GetTypeId(), source->GetEntry(), source->GetGUID().GetCounter());
    else
    {
        WorldObject* wSource = dynamic_cast <WorldObject*> (source);
        if (!wSource)
            FC_LOG_ERROR("scripts", "%s source object could not be cast to world object (TypeId: %u, Entry: %u, GUID: %u), skipping.",
        ...
        else
        {
            ...
            if (!pDoor)
                FC_LOG_ERROR("scripts", "%s gameobject was not found (guid: %u).", scriptInfo->GetDebugInfo().c_str(), guid);
            else if (pDoor->GetGoType() != GAMEOBJECT_TYPE_DOOR)
                FC_LOG_ERROR("scripts", "%s gameobject is not a door (GoType: %u, Entry: %u, GUID: %u).",
    ...
    while (!m_scriptSchedule.empty() && (iter->first <= GameTime::GetGameTime()))
    {
        ...
        if (step.sourceGUID)
        {
            ...
            case HighGuid::Mo_Transport:
                ...
                break;
            default:
                FC_LOG_ERROR("scripts", "%s source with unsupported high guid %s.",
        ...
        if (step.targetGUID)
        {
            ...
            case HighGuid::Mo_Transport:
                ...
                break;
            default:
                FC_LOG_ERROR("scripts", "%s target with unsupported high guid %s.",
        ...
        case SCRIPT_COMMAND_TALK:
        {
            ...
            {
                FC_LOG_ERROR("scripts", "%s invalid chat type (%u) specified, skipping.", step.script->GetDebugInfo().c_str(), step.script->Talk.ChatType);
                break;
            }
            ...
            if (source)
            {
                ...
                if (!sourceUnit)
                {
                    FC_LOG_ERROR("scripts", "%s source object (%s) is not an unit, skipping.", step.script->GetDebugInfo().c_str(), source->GetGUID().ToString().c_str());
                    break;
                }
                ...
                case CHAT_TYPE_BOSS_WHISPER:
                {
                    ...
                    if (!receiver)
                        FC_LOG_ERROR("scripts", "%s attempt to whisper to non-player unit, skipping.", step.script->GetDebugInfo().c_str());
                    else
        ...
        case SCRIPT_COMMAND_FIELD_SET:
            ...
            if (Creature* cSource = _GetScriptCreatureSourceOrTarget(source, target, step.script))
            {
                ...
                if (step.script->FieldSet.FieldID <= OBJECT_FIELD_ENTRY || step.script->FieldSet.FieldID >= cSource->GetValuesCount())
                    FC_LOG_ERROR("scripts", "%s wrong field %u (max count: %u) in object (TypeId: %u, %s) specified, skipping.",
        ...
        case SCRIPT_COMMAND_FLAG_SET:
            ...
            if (Creature* cSource = _GetScriptCreatureSourceOrTarget(source, target, step.script))
            {
                ...
                if (step.script->FlagToggle.FieldID <= OBJECT_FIELD_ENTRY || step.script->FlagToggle.FieldID >= cSource->GetValuesCount())
                    FC_LOG_ERROR("scripts", "%s wrong field %u (max count: %u) in object (TypeId: %u, Entry: %u, GUID: %u) specified, skipping.",
        ...
        case SCRIPT_COMMAND_QUEST_EXPLORED:
        {
            if (!source)
            {
                FC_LOG_ERROR("scripts", "%s source object is nullptr.", step.script->GetDebugInfo().c_str());
                break;
            }
            if (!target)
            {
                FC_LOG_ERROR("scripts", "%s target object is nullptr.", step.script->GetDebugInfo().c_str());
                break;
            }
            ...
            if (player)
            {
                ...
                {
                    FC_LOG_ERROR("scripts", "%s source is not unit, gameobject or player (TypeId: %u, Entry: %u, GUID: %u), skipping.",
            ...
            else
            {
                ...
                if (player)
                {
                    ...
                    {
                        FC_LOG_ERROR("scripts", "%s target is not unit, gameobject or player (TypeId: %u, Entry: %u, GUID: %u), skipping.",
                ...
                {
                    FC_LOG_ERROR("scripts", "%s neither source nor target is player (source: TypeId: %u, Entry: %u, GUID: %u; target: TypeId: %u, Entry: %u, GUID: %u), skipping.",
                        step.script->GetDebugInfo().c_str(), source->GetTypeId(), source->GetEntry(), source->GetGUID().GetCounter(),
        ...
        case SCRIPT_COMMAND_RESPAWN_GAMEOBJECT:
            ...
            {
                FC_LOG_ERROR("scripts", "%s gameobject guid (datalong) is not specified.", step.script->GetDebugInfo().c_str());
                break;
            }
            ...
            if (WorldObject* pSummoner = _GetScriptWorldObject(source, true, step.script))
            {
                ...
                if (!pGO)
                {
                    FC_LOG_ERROR("scripts", "%s gameobject was not found (guid: %u).", step.script->GetDebugInfo().c_str(), step.script->RespawnGameobject.GOGuid);
                    break;
                }
                ...
                {
                    FC_LOG_ERROR("scripts", "%s can not be used with gameobject of type %u (guid: %u).",
        ...
        case SCRIPT_COMMAND_TEMP_SUMMON_CREATURE:
        {
            ...
            if (WorldObject* pSummoner = _GetScriptWorldObject(source, true, step.script))
            {
                if (!step.script->TempSummonCreature.CreatureEntry)
                    FC_LOG_ERROR("scripts", "%s creature entry (datalong) is not specified.", step.script->GetDebugInfo().c_str());
                else
                {
                    ...
                    if (!pSummoner->SummonCreature(step.script->TempSummonCreature.CreatureEntry, x, y, z, o, TEMPSUMMON_TIMED_OR_DEAD_DESPAWN, step.script->TempSummonCreature.DespawnDelay))
                        FC_LOG_ERROR("scripts", "%s creature was not spawned (entry: %u).", step.script->GetDebugInfo().c_str(), step.script->TempSummonCreature.CreatureEntry);
                }
            }
            break;
        }
        ...
        case SCRIPT_COMMAND_ACTIVATE_OBJECT:
            ...
            if (Unit* unit = _GetScriptUnit(source, true, step.script))
            {
                ...
                {
                    FC_LOG_ERROR("scripts", "%s target object is not gameobject (TypeId: %u, Entry: %u, GUID: %u), skipping.",
                        step.script->GetDebugInfo().c_str(), target->GetTypeId(), target->GetEntry(), target->GetGUID().GetCounter());
                    break;
                }
        ...
        case SCRIPT_COMMAND_CAST_SPELL:
        {
            ...
            if (!source && !target)
            {
                FC_LOG_ERROR("scripts", "%s source and target objects are nullptr.", step.script->GetDebugInfo().c_str());
                break;
            }
            ...
            {
                FC_LOG_ERROR("scripts", "%s no source unit found for spell %u", step.script->GetDebugInfo().c_str(), step.script->CastSpell.SpellID);
                break;
            }

            if (!uTarget || !uTarget->isType(TYPEMASK_UNIT))
            {
                FC_LOG_ERROR("scripts", "%s no target unit found for spell %u", step.script->GetDebugInfo().c_str(), step.script->CastSpell.SpellID);
                break;
            }
        ...
        case SCRIPT_COMMAND_LOAD_PATH:
            ...
            if (Unit* unit = _GetScriptUnit(source, true, step.script))
            {
                if (!sWaypointMgr->GetPath(step.script->LoadPath.PathID))
                    FC_LOG_ERROR("scripts", "%s source object has an invalid path (%u), skipping.", step.script->GetDebugInfo().c_str(), step.script->LoadPath.PathID);
                else
        ...
        case SCRIPT_COMMAND_CALLSCRIPT_TO_UNIT:
        {
            ...
            {
                FC_LOG_ERROR("scripts", "%s creature entry is not specified, skipping.", step.script->GetDebugInfo().c_str());
                break;
            }
            if (!step.script->CallScript.ScriptID)
            {
                FC_LOG_ERROR("scripts", "%s script id is not specified, skipping.", step.script->GetDebugInfo().c_str());
                break;
            }
            ...
            if (!cTarget)
            {
                FC_LOG_ERROR("scripts", "%s target was not found (entry: %u)", step.script->GetDebugInfo().c_str(), step.script->CallScript.CreatureEntry);
                break;
            }
            ...
            if (!datamap)
            {
                FC_LOG_ERROR("scripts", "%s unknown scriptmap (%u) specified, skipping.", step.script->GetDebugInfo().c_str(), step.script->CallScript.ScriptType);
                break;
            }
        ...
        case SCRIPT_COMMAND_KILL:
            ...
            {
                if (cSource->isDead())
                    FC_LOG_ERROR("scripts", "%s creature is already dead (Entry: %u, GUID: %u)",
        ...
        default:
            FC_LOG_ERROR("scripts", "Unknown script command %s.", step.script->GetDebugInfo().c_str());
            break;
        }
