...
void TransportMgr::LoadTransportTemplates()
{
    ...
    if (!result)
    {
        FC_LOG_INFO("server.loading", ">> Loaded 0 transport templates. DB table `gameobject_template` has no transports!");
        return;
    }
    ...
    do
    {
        ...
        if (goInfo == nullptr)
        {
            FC_LOG_ERROR("sql.sql", "Transport %u has no associated GameObjectTemplate from `gameobject_template` , skipped.", entry);
            continue;
        }

        if (goInfo->moTransport.taxiPathId >= sTaxiPathNodesByPath.size())
        {
            FC_LOG_ERROR("sql.sql", "Transport %u (name: %s) has an invalid path specified in `gameobject_template`.`data0` (%u) field, skipped.", entry, goInfo->name.c_str(), goInfo->moTransport.taxiPathId);
            continue;
        }
    ...
    FC_LOG_INFO("server.loading", ">> Loaded %u transport templates in %u ms", count, GetMSTimeDiffToNow(oldMSTime));
}
...
void TransportMgr::LoadTransportSpawns()
{
    ...
    if (result)
    {
        do
        {
            ...
            {
                FC_LOG_ERROR("sql.sql", "Table `transports` have transport (GUID: %u Entry: %u) with unknown gameobject `entry` set, skipped.", guid, entry);
                continue;
            }

            if (phaseUseFlags & ~PHASE_USE_FLAGS_ALL)
            {
                FC_LOG_ERROR("sql.sql", "Table `transports` have transport (GUID: %u Entry: %u) with unknown `phaseUseFlags` set, removed unknown value.", guid, entry);
                phaseUseFlags &= PHASE_USE_FLAGS_ALL;
            }

            if (phaseUseFlags & PHASE_USE_FLAGS_ALWAYS_VISIBLE && phaseUseFlags & PHASE_USE_FLAGS_INVERSE)
            {
                FC_LOG_ERROR("sql.sql", "Table `transports` have transport (GUID: %u Entry: %u) has both `phaseUseFlags` PHASE_USE_FLAGS_ALWAYS_VISIBLE and PHASE_USE_FLAGS_INVERSE,"
            ...
            {
                FC_LOG_ERROR("sql.sql", "Table `transports` have transport (GUID: %u Entry: %u) with both `phaseid` and `phasegroup` set, `phasegroup` set to 0", guid, entry);
                phaseGroupId = 0;
            }
            ...
            if (phaseId)
            {
                ...
                {
                    FC_LOG_ERROR("sql.sql", "Table `transports` have transport (GUID: %u Entry: %u) with `phaseid` %u does not exist, set to 0", guid, entry, phaseId);
                    phaseId = 0;
                }
            }
            ...
            if (phaseGroupId)
            {
                ...
                {
                    FC_LOG_ERROR("sql.sql", "Table `transports` have transport (GUID: %u Entry: %u) with `phaseGroup` %u does not exist, set to 0", guid, entry, phaseGroupId);
                    phaseGroupId = 0;
                }
            }
    ...
    FC_LOG_INFO("server.loading", ">> Spawned %u continent transports in %u ms", count, GetMSTimeDiffToNow(oldMSTime));
}
...
Transport* TransportMgr::CreateTransport(uint32 entry, Map* map, ObjectGuid::LowType guid /*= 0*/, uint8 phaseUseFlags /*= 0*/, uint32 phaseId /*= 0*/, uint32 phaseGroupId /*= 0*/)
{
    ...
    if (!tInfo)
    {
        FC_LOG_ERROR("sql.sql", "Transport %u will not be loaded, `transport_template` missing", entry);
        return nullptr;
    }
    ...
    if (!startingPosition)
    {
        FC_LOG_ERROR("sql.sql", "Transport %u will not be loaded, failed to compute starting position", entry);
        return nullptr;
    }
    ...
    if (MapEntry const* mapEntry = sMapStore.LookupEntry(mapId))
    {
        ...
        {
            FC_LOG_ERROR("entities.transport", "Transport %u (name: %s) attempted creation in instance map (id: %u) but it is not an instanced transport!", entry, trans->GetName().c_str(), mapId);
            delete trans;
            return nullptr;
        }
    }
...
{
    return Firelands::Containers::MapGetValuePtr(_transportTemplates, entry);
}

TransportAnimation const* TransportMgr::GetTransportAnimInfo(uint32 entry) const
{
    return Firelands::Containers::MapGetValuePtr(_transportAnimations, entry);
}

TransportSpawn const* TransportMgr::GetTransportSpawn(ObjectGuid::LowType spawnId) const
{
    return Firelands::Containers::MapGetValuePtr(_transportSpawns, spawnId);
}
