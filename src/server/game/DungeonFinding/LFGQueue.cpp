...
namespace lfg
{
    ...
    */
    ...
    void LFGQueue::AddToQueue(ObjectGuid guid, bool reAdd)
    {
        ...
        if (itQueue == QueueDataStore.end())
        {
            FC_LOG_ERROR("lfg.queue.add", "Queue data not found for [%s]", guid.ToString().c_str());
            return;
        }
    ...
    */
    void LFGQueue::RemoveFromCompatibles(ObjectGuid guid)
    {
        ...
        FC_LOG_DEBUG("lfg.queue.data.compatibles.remove", "Removing %s", guid.ToString().c_str());
    ...
    */
    ...
    uint8 LFGQueue::FindGroups()
    {
        ...
        while (!newToQueueStore.empty())
        {
            ...
            FC_LOG_DEBUG("lfg.queue.match.check.new", "Checking [%s] newToQueue(%u), currentQueue(%u)", frontguid.ToString().c_str(),
    ...
    */
    LfgCompatibility LFGQueue::FindNewGroups(GuidList& check, GuidList& all)
    {
        ...
        FC_LOG_DEBUG("lfg.queue.match.check", "Guids: (%s): %s - all(%s)", GetDetailedMatchRoles(check).c_str(), GetCompatibleString(compatibles), GetDetailedMatchRoles(all).c_str());
        ...
        {
            FC_LOG_DEBUG("lfg.queue.match.check", "Guids: (%s) compatibles (cached) changed from bad states to match", GetDetailedMatchRoles(check).c_str());
    ...
    */
    LfgCompatibility LFGQueue::CheckCompatibility(GuidList check)
    {
        ...
        if (check.empty())
        {
            FC_LOG_DEBUG("lfg.queue.match.compatibility.check", "Guids: (%s): Size wrong - Not compatibles", GetDetailedMatchRoles(check).c_str());
        ...
        if (check.size() > 2)
        {
            ...
            {
                FC_LOG_DEBUG("lfg.queue.match.compatibility.check", "Guids: (%s) child %s not compatibles", strGuids.c_str(), GetDetailedMatchRoles(check).c_str());
        ...
        do
        {
            ...
            {
                FC_LOG_DEBUG("lfg.queue.match.compatibility.check", "LFGQueue::CheckCompatibility: (%s) No compatible dungeons%s", strGuids.c_str(), o.str().c_str());
                SetCompatibles(strGuids, LFG_INCOMPATIBLES_NO_DUNGEONS);
                return LFG_INCOMPATIBLES_NO_DUNGEONS;
            }

            proposal.dungeonId = Firelands::Containers::SelectRandomContainerElement(proposalDungeons);
        ...
        {
            FC_LOG_DEBUG("lfg.queue.match.compatibility.check", "LFGQueue::CheckCompatibility: (%s): Size wrong - Not compatibles", strGuids.c_str());
            return LFG_INCOMPATIBLES_WRONG_GROUP_SIZE;
        }
        ...
        for (GuidList::const_iterator it = check.begin(); it != check.end() && numLfgGroups < 1 && numPlayers <= dungeon->GetMaxGroupSize(); ++it)
        {
            ObjectGuid guid = *it;
            LfgQueueDataContainer::iterator itQueue = QueueDataStore.find(guid);
            if (itQueue == QueueDataStore.end())
            {
                FC_LOG_ERROR("lfg.queue.match.compatibility.check", "Guid: [%s] is not queued but listed as queued!", guid.ToString().c_str());
        ...
        if (numLfgGroups > 1)
        {
            FC_LOG_DEBUG("lfg.queue.match.compatibility.check", "Guids: (%s) More than one Lfggroup (%u)", GetDetailedMatchRoles(check).c_str(), numLfgGroups);
        ...
        {
            FC_LOG_DEBUG("lfg.queue.match.compatibility.check", "Guids: (%s) Too many players (%u)", GetDetailedMatchRoles(check).c_str(), numPlayers);
        ...
        if (check.size() > 1)
        {
                ...
                for (LfgRolesMap::const_iterator itRoles = roles.begin(); itRoles != roles.end(); ++itRoles)
                {
                    ...
                    for (itPlayer = proposalRoles.begin(); itPlayer != proposalRoles.end(); ++itPlayer)
                    {
                        if (itRoles->first == itPlayer->first)
                            FC_LOG_ERROR("lfg.queue.match.compatibility.check", "Guids: ERROR! Player multiple times in queue! [%s]", itRoles->first.ToString().c_str());
            ...
            {
                FC_LOG_DEBUG("lfg.queue.match.compatibility.check", "Guids: (%s) not compatible, %u players are ignoring each other", GetDetailedMatchRoles(check).c_str(), playersize);
                SetCompatibles(strGuids, LFG_INCOMPATIBLES_HAS_IGNORES);
                return LFG_INCOMPATIBLES_HAS_IGNORES;
            }

            LfgRolesMap debugRoles = proposalRoles;
            if (!LFGMgr::CheckGroupRoles(proposalRoles, dungeon))
            {
                std::ostringstream o;
                for (LfgRolesMap::const_iterator it = debugRoles.begin(); it != debugRoles.end(); ++it)
                    o << ", " << it->first.GetRawValue() << ": " << GetRolesString(it->second);

                FC_LOG_DEBUG("lfg.queue.match.compatibility.check", "Guids: (%s) Roles not compatible%s", GetDetailedMatchRoles(check).c_str(), o.str().c_str());
        ...
        {
            proposalRoles = QueueDataStore[check.front()].roles;
            LFGMgr::CheckGroupRoles(proposalRoles, dungeon);    // assign new roles
        }

        // Enough players?
        if (numPlayers != dungeon->GetMaxGroupSize())
        {
            FC_LOG_DEBUG("lfg.queue.match.compatibility.check", "Guids: (%s) Compatibles but not enough players(%u)", GetDetailedMatchRoles(check).c_str(), numPlayers);
        ...
        {
            FC_LOG_DEBUG("lfg.queue.match.compatibility.check", "Guids: (%s) Group MATCH but can't create proposal!", GetDetailedMatchRoles(check).c_str());
        ...
        FC_LOG_DEBUG("lfg.queue.match.compatibility.check", "Guids: (%s) MATCH! Group formed", GetDetailedMatchRoles(check).c_str());
        SetCompatibles(strGuids, LFG_COMPATIBLES_MATCH);
        return LFG_COMPATIBLES_MATCH;
    }
    ...
    {
        FC_LOG_TRACE("lfg.queue.timers.update", "Updating queue timers...");
    ...
    {
        FC_LOG_DEBUG("lfg.queue.compatibles.find", "%s", itrQueue->first.ToString().c_str());
        std::ostringstream o;
    ...
    void LFGQueue::UpdateBestCompatibleInQueue(LfgQueueDataContainer::iterator itrQueue, std::string const& key, LfgRolesMap const& roles)
    {
        ...
        FC_LOG_DEBUG("lfg.queue.compatibles.update", "Changed (%s) to (%s) as best compatible group for %s",
            queueData.bestCompatible.c_str(), key.c_str(), itrQueue->first.ToString().c_str());

        queueData.bestCompatible = key;
        queueData.InitializeGroupSetup();
