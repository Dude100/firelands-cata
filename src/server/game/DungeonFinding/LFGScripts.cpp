...
namespace lfg
{
    ...
    void LFGPlayerScript::OnLogin(Player* player, bool /*loginFirst*/)
    {
        ...
        if (Group const* group = player->GetGroup())
        {
            ...
            if (gguid != gguid2)
            {
                FC_LOG_ERROR("lfg", "%s on group %s but LFG has group %s saved... Fixing.",
    ...
    void LFGPlayerScript::OnMapChanged(Player* player)
    {
        ...
        if (sLFGMgr->inLfgDungeonMap(player->GetGUID(), map->GetId(), map->GetDifficulty()))
        {
            ...
            if (!group)
            {
                ...
                FC_LOG_ERROR("lfg", "LFGPlayerScript::OnMapChanged, Player %s (%u) is in LFG dungeon map but does not have a valid group! "
        ...
        else
        {
            ...
            if (group && group->GetMembersCount() == 1)
            {
                ...
                group->Disband();
                FC_LOG_DEBUG("lfg", "LFGPlayerScript::OnMapChanged, Player %s(%s) is last in the lfggroup so we disband the group.",
    ...
    void LFGGroupScript::OnAddMember(Group* group, ObjectGuid guid)
    {
        ...
        if (leader == guid)
        {
            FC_LOG_DEBUG("lfg", "LFGScripts::OnAddMember [%s]: added [%s] leader [%s]", gguid.ToString().c_str(), guid.ToString().c_str(), leader.ToString().c_str());
        ...
        else
        {
            ...
            FC_LOG_DEBUG("lfg", "LFGScripts::OnAddMember [%s]: added [%s] leader [%s] gstate: %u, state: %u", gguid.ToString().c_str(), guid.ToString().c_str(), leader.ToString().c_str(), gstate, state);
    ...
    void LFGGroupScript::OnRemoveMember(Group* group, ObjectGuid guid, RemoveMethod method, ObjectGuid kicker, char const* reason)
    {
        ...
        ObjectGuid gguid = group->GetGUID();
        FC_LOG_DEBUG("lfg", "LFGScripts::OnRemoveMember [%s]: remove [%s] Method: %d Kicker: [%s] Reason: %s",
    ...
    void LFGGroupScript::OnDisband(Group* group)
    {
        ...
        ObjectGuid gguid = group->GetGUID();
        FC_LOG_DEBUG("lfg", "LFGScripts::OnDisband [%s]", gguid.ToString().c_str());
    ...
    void LFGGroupScript::OnChangeLeader(Group* group, ObjectGuid newLeaderGuid, ObjectGuid oldLeaderGuid)
    {
        ...
        FC_LOG_DEBUG("lfg", "LFGScripts::OnChangeLeader [%s]: old [%s] new [%s]",
    ...
    void LFGGroupScript::OnInviteMember(Group* group, ObjectGuid guid)
    {
        ...
        ObjectGuid gguid = group->GetGUID();
        ObjectGuid leader = group->GetLeaderGUID();
        FC_LOG_DEBUG("lfg", "LFGScripts::OnInviteMember [%s]: invite [%s] leader [%s]",
