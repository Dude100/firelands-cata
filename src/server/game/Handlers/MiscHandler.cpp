...
{
    FC_LOG_DEBUG("network", "WORLD: Recvd CMSG_REPOP_REQUEST Message");
    ...
    {
        FC_LOG_DEBUG("network", "HandleRepopRequestOpcode: got request after player %s(%d) was killed and before he was updated",
...
{
    FC_LOG_DEBUG("network", "WORLD: CMSG_GOSSIP_SELECT_OPTION");
    ...
    if (guid.IsCreatureOrVehicle())
    {
        ...
        if (!unit)
        {
            FC_LOG_DEBUG("network", "WORLD: HandleGossipSelectOptionOpcode - %s not found or you can't interact with him.", guid.ToString().c_str());
            return;
        }
    }
    else if (guid.IsGameObject())
    {
        ...
        if (!go)
        {
            FC_LOG_DEBUG("network", "WORLD: HandleGossipSelectOptionOpcode - %s not found or you can't interact with it.", guid.ToString().c_str());
            return;
        }
    }
    else
    {
        FC_LOG_DEBUG("network", "WORLD: HandleGossipSelectOptionOpcode - unsupported %s.", guid.ToString().c_str());
        return;
    }
    ...
    {
        FC_LOG_DEBUG("network", "WORLD: HandleGossipSelectOptionOpcode - Script reloaded while in use, ignoring and set new scipt id");
        if (unit)
...
void WorldSession::HandleWhoOpcode(WorldPackets::Who::WhoRequestPkt& whoRequest)
{
    ...
    {
        FC_LOG_DEBUG("network", "WorldSession::HandleWhoOpcode: Word: %s", request.Words[i].Word.c_str());
...
{
    FC_LOG_DEBUG("network", "WORLD: Recvd CMSG_LOGOUT_REQUEST Message, security - %u", GetSecurity());
...
{
    FC_LOG_DEBUG("network", "WORLD: Recvd CMSG_PLAYER_LOGOUT Message");
}

void WorldSession::HandleLogoutCancelOpcode(WorldPacket& /*recvData*/)
{
    FC_LOG_DEBUG("network", "WORLD: Recvd CMSG_LOGOUT_CANCEL Message");
    ...
    FC_LOG_DEBUG("network", "WORLD: Sent SMSG_LOGOUT_CANCEL_ACK Message");
}
...
void WorldSession::HandleZoneUpdateOpcode(WorldPacket& recvData)
{
    ...
    FC_LOG_DEBUG("network", "WORLD: Recvd ZONE_UPDATE: %u", newZone);
...
void WorldSession::HandleRequestCemeteryList(WorldPacket& /*recvPacket*/)
{
    ...
    {
        FC_LOG_DEBUG("network", "No graveyards found for zone %u for player %u (team %u) in CMSG_REQUEST_CEMETERY_LIST",
...
void WorldSession::HandleBugOpcode(WorldPacket& recvData)
{
    ...
    if (suggestion == 0)
        FC_LOG_DEBUG("network", "WORLD: Received CMSG_BUG [Bug Report]");
    else
        FC_LOG_DEBUG("network", "WORLD: Received CMSG_BUG [Suggestion]");

    FC_LOG_DEBUG("network", "%s", type.c_str());
    FC_LOG_DEBUG("network", "%s", content.c_str());
...
{
    FC_LOG_DEBUG("network", "WORLD: Received CMSG_RECLAIM_CORPSE");
...
{
    FC_LOG_DEBUG("network", "WORLD: Received CMSG_RESURRECT_RESPONSE");
...
void WorldSession::HandleAreaTriggerOpcode(WorldPacket& recvData)
{
    ...
    FC_LOG_DEBUG("network", "CMSG_AREATRIGGER. Trigger ID: %u", triggerId);

    Player* player = GetPlayer();
    if (player->IsInFlight())
    {
        FC_LOG_DEBUG("network", "HandleAreaTriggerOpcode: Player '%s' (GUID: %u) in flight, ignore Area Trigger ID:%u",
    ...
    if (!atEntry)
    {
        FC_LOG_DEBUG("network", "HandleAreaTriggerOpcode: Player '%s' (GUID: %u) send unknown (by DBC) Area Trigger ID:%u",
            player->GetName().c_str(), player->GetGUID().GetCounter(), triggerId);
        return;
    }
    ...
    {
        FC_LOG_DEBUG("network", "HandleAreaTriggerOpcode: Player '%s' (GUID: %u) too far, ignore Area Trigger ID: %u",
            player->GetName().c_str(), player->GetGUID().GetCounter(), triggerId);
        return;
    }
    ...
    if (player->GetMapId() != at->target_mapId)
    {
        if (Map::EnterState denyReason = sMapMgr->PlayerCannotEnter(at->target_mapId, player, false))
        {
            ...
            case Map::CANNOT_ENTER_NO_ENTRY:
                FC_LOG_DEBUG("maps", "MAP: Player '%s' attempted to enter map with id %d which has no entry", player->GetName().c_str(), at->target_mapId);
                break;
            case Map::CANNOT_ENTER_UNINSTANCED_DUNGEON:
                FC_LOG_DEBUG("maps", "MAP: Player '%s' attempted to enter dungeon map %d but no instance template was found", player->GetName().c_str(), at->target_mapId);
                break;
            case Map::CANNOT_ENTER_DIFFICULTY_UNAVAILABLE:
                FC_LOG_DEBUG("maps", "MAP: Player '%s' attempted to enter instance map %d but the requested difficulty was not found", player->GetName().c_str(), at->target_mapId);
                if (MapEntry const* entry = sMapStore.LookupEntry(at->target_mapId))
                    player->SendTransferAborted(entry->ID, TRANSFER_ABORT_DIFFICULTY, player->GetDifficulty(entry->IsRaid()));
                break;
            case Map::CANNOT_ENTER_NOT_IN_RAID:
            {
                FC_LOG_DEBUG("maps", "MAP: Player '%s' must be in a raid group to enter instance map %d", player->GetName().c_str(), at->target_mapId);
            ...
            case Map::CANNOT_ENTER_CORPSE_IN_DIFFERENT_INSTANCE:
            {
                ...
                FC_LOG_DEBUG("maps", "MAP: Player '%s' does not have a corpse in instance map %d and cannot enter", player->GetName().c_str(), at->target_mapId);
                break;
            }
            case Map::CANNOT_ENTER_INSTANCE_BIND_MISMATCH:
                if (MapEntry const* entry = sMapStore.LookupEntry(at->target_mapId))
                {
                    ...
                    FC_LOG_DEBUG("maps", "MAP: Player '%s' cannot enter instance map '%s' because their permanent bind is incompatible with their group's", player->GetName().c_str(), mapName);
            ...
            case Map::CANNOT_ENTER_TOO_MANY_INSTANCES:
                ...
                FC_LOG_DEBUG("maps", "MAP: Player '%s' cannot enter instance map %d because he has exceeded the maximum number of instances per hour.", player->GetName().c_str(), at->target_mapId);
                reviveAtTrigger = true;
                break;
...
{
    FC_LOG_DEBUG("network", "WORLD: Received CMSG_UPDATE_ACCOUNT_DATA");

    uint32 type, timestamp, decompressedSize;
    recvData >> type >> timestamp >> decompressedSize;

    FC_LOG_DEBUG("network", "UAD: type %u, time %u, decompressedSize %u", type, timestamp, decompressedSize);
    ...
    if (decompressedSize > 0xFFFF)
    {
        ...
        FC_LOG_ERROR("network", "UAD: Account data packet too big, size %u", decompressedSize);
        return;
    }
    ...
    {
        recvData.rfinish();                   // unnneded warning spam in this case
        FC_LOG_ERROR("network", "UAD: Failed to decompress account data");
        return;
    }
...
{
    FC_LOG_DEBUG("network", "WORLD: Received CMSG_REQUEST_ACCOUNT_DATA");

    uint32 type;
    recvData >> type;

    FC_LOG_DEBUG("network", "RAD: type %u", type);
    ...
    {
        FC_LOG_DEBUG("network", "RAD: Failed to compress account data");
        return;
    }
...
void WorldSession::HandleSetActionButtonOpcode(WorldPacket& recvData)
{
    ...
    FC_LOG_DEBUG("network", "CMSG_SET_ACTION_BUTTON Button: %u Data: %u", button, packetData);
...
void WorldSession::HandleSetActionBarToggles(WorldPacket& recvData)
{
    ...
    {
        if (actionBar != 0)
            FC_LOG_ERROR("network", "WorldSession::HandleSetActionBarToggles in not logged state with value: %u, ignored", uint32(actionBar));
        return;
    }
...
void WorldSession::HandleInspectOpcode(WorldPacket& recvData)
{
    ...
    FC_LOG_DEBUG("network", "WORLD: Received CMSG_INSPECT");

    Player* player = ObjectAccessor::GetPlayer(*_player, guid);
    if (!player)
    {
        FC_LOG_DEBUG("network", "CMSG_INSPECT: No player found from %s", guid.ToString().c_str());
        return;
    }
...
void WorldSession::HandleWorldTeleportOpcode(WorldPacket& recvData)
{
    ...
    FC_LOG_DEBUG("network", "WORLD: Received CMSG_WORLD_TELEPORT");

    if (GetPlayer()->IsInFlight())
    {
        FC_LOG_DEBUG("network", "Player '%s' (GUID: %u) in flight, ignore worldport command.",
            GetPlayer()->GetName().c_str(), GetPlayer()->GetGUID().GetCounter());
        return;
    }

    FC_LOG_DEBUG("network", "CMSG_WORLD_TELEPORT: Player = %s, Time = %u, map = %u, x = %f, y = %f, z = %f, o = %f",
...
{
    FC_LOG_DEBUG("network", "Received opcode CMSG_WHOIS");
    std::string charname;
    recvData >> charname;
    ...
    FC_LOG_DEBUG("network", "Received whois command from player %s for character %s",
        GetPlayer()->GetName().c_str(), charname.c_str());
}

void WorldSession::HandleComplainOpcode(WorldPacket& recvData)
{
    FC_LOG_DEBUG("network", "WORLD: CMSG_COMPLAIN");
    ...
    FC_LOG_DEBUG("network", "REPORT SPAM: type %u, %s, unk1 %u, unk2 %u, unk3 %u, unk4 %u, message %s",
        spam_type, spammer_guid.ToString().c_str(), unk1, unk2, unk3, unk4, description.c_str());
}

void WorldSession::HandleRealmSplitOpcode(WorldPacket& recvData)
{
    FC_LOG_DEBUG("network", "CMSG_REALM_SPLIT");
    ...
    data << split_date;
    SendPacket(&data);
    //FC_LOG_DEBUG("response sent %u", unk);
}

void WorldSession::HandleFarSightOpcode(WorldPacket& recvData)
{
    FC_LOG_DEBUG("network", "WORLD: CMSG_FAR_SIGHT");
    ...
    if (apply)
    {
        FC_LOG_DEBUG("network", "Added FarSight %s to player %u", _player->GetGuidValue(PLAYER_FARSIGHT).ToString().c_str(), _player->GetGUID().GetCounter());
        if (WorldObject* target = _player->GetViewpoint())
            _player->SetSeer(target);
        else
            FC_LOG_DEBUG("network", "Player %s (%s) requests non-existing seer %s", _player->GetName().c_str(), _player->GetGUID().ToString().c_str(), _player->GetGuidValue(PLAYER_FARSIGHT).ToString().c_str());
    }
    else
    {
        FC_LOG_DEBUG("network", "Player %u set vision to self", _player->GetGUID().GetCounter());
...
{
    FC_LOG_DEBUG("network", "CMSG_SET_TITLE");
...
{
    FC_LOG_DEBUG("network", "WORLD: CMSG_RESET_INSTANCES");
...
{
    FC_LOG_DEBUG("network", "MSG_SET_DUNGEON_DIFFICULTY");
    ...
    {
        FC_LOG_DEBUG("network", "WorldSession::HandleSetDungeonDifficultyOpcode: player %d sent an invalid instance mode %d!", _player->GetGUID().GetCounter(), mode);
        return;
    }
    ...
    {
        FC_LOG_DEBUG("network", "WorldSession::HandleSetDungeonDifficultyOpcode: player (Name: %s, GUID: %u) tried to reset the instance while player is inside!",
    ...
    if (group)
    {
        if (group->IsLeader(_player->GetGUID()))
        {
            for (GroupReference* itr = group->GetFirstMember(); itr != nullptr; itr = itr->next())
            {
                ...
                {
                    FC_LOG_DEBUG("network", "WorldSession::HandleSetDungeonDifficultyOpcode: player %d tried to reset the instance while group member (Name: %s, GUID: %u) is inside!",
...
{
    FC_LOG_DEBUG("network", "MSG_SET_RAID_DIFFICULTY");
    ...
    {
        FC_LOG_ERROR("network", "WorldSession::HandleSetRaidDifficultyOpcode: player %d sent an invalid instance mode %d!", _player->GetGUID().GetCounter(), mode);
        return;
    }
    ...
    if (map && map->IsDungeon())
    {
        FC_LOG_DEBUG("network", "WorldSession::HandleSetRaidDifficultyOpcode: player %d tried to reset the instance while inside!", _player->GetGUID().GetCounter());
        return;
    }
...
{
    FC_LOG_DEBUG("network", "WORLD: CMSG_CANCEL_MOUNT_AURA");
...
{
    FC_LOG_DEBUG("network", "WORLD: CMSG_SET_TAXI_BENCHMARK_MODE");
    ...
    FC_LOG_DEBUG("network", "Client used \"/timetest %d\" command", mode);
}
...
void WorldSession::HandleQueryInspectAchievements(WorldPacket& recvData)
{
    ...
    FC_LOG_DEBUG("network", "CMSG_QUERY_INSPECT_ACHIEVEMENTS [%s] Inspected Player [%s]", _player->GetGUID().ToString().c_str(), guid.ToString().c_str());
    Player* player = ObjectAccessor::GetPlayer(*_player, guid);
    if (!player)
        return;
...
{
    // empty opcode
    FC_LOG_DEBUG("network", "WORLD: CMSG_WORLD_STATE_UI_TIMER_UPDATE");
...
{
    // empty opcode
    FC_LOG_DEBUG("network", "WORLD: CMSG_READY_FOR_ACCOUNT_DATA_TIMES");
...
{
    FC_LOG_DEBUG("network", "WORLD: CMSG_AREA_SPIRIT_HEALER_QUERY");
...
{
    FC_LOG_DEBUG("network", "WORLD: CMSG_AREA_SPIRIT_HEALER_QUEUE");
...
void WorldSession::HandleInstanceLockResponse(WorldPacket& recvPacket)
{
    ...
    {
        FC_LOG_INFO("network", "InstanceLockResponse: Player %s (guid %u) tried to bind himself/teleport to graveyard without a pending bind!",
            _player->GetName().c_str(), _player->GetGUID().GetCounter());
        return;
    }
...
void WorldSession::HandleObjectUpdateFailedOpcode(WorldPacket& recvPacket)
{
    ...
    FC_LOG_ERROR("network", "Object update failed for object " UI64FMTD " (%s) for player %s (%u)", uint64(guid), obj ? obj->GetName().c_str() : "object-not-found", GetPlayerName().c_str(), GetGUIDLow());
...
{
    FC_LOG_DEBUG("network", "WORLD: CMSG_SAVE_CUF_PROFILES");

    uint8 count = (uint8)recvPacket.ReadBits(20);

    if (count > MAX_CUF_PROFILES)
    {
        FC_LOG_ERROR("entities.player", "HandleSaveCUFProfiles - %s tried to save more than %i CUF profiles. Hacking attempt?", GetPlayerName().c_str(), MAX_CUF_PROFILES);
        recvPacket.rfinish();
        return;
    }
    ...
    {
        profiles[i] = Firelands::make_unique<CUFProfile>();
...
{
    FC_LOG_DEBUG("network", "Received CMSG_CHANGEPLAYER_DIFFICULTY");
