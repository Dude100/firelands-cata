...
{
    //FC_LOG_DEBUG("network", "WORLD: CMSG_SPLIT_ITEM");
    uint8 srcbag, srcslot, dstbag, dstslot;
    uint32 count;

    recvData >> srcbag >> srcslot >> dstbag >> dstslot >> count;
    //FC_LOG_DEBUG("STORAGE: receive srcbag = %u, srcslot = %u, dstbag = %u, dstslot = %u, count = %u", srcbag, srcslot, dstbag, dstslot, count);
...
{
    //FC_LOG_DEBUG("network", "WORLD: CMSG_SWAP_INV_ITEM");
    uint8 srcslot, dstslot;

    recvData >> dstslot >> srcslot;
    //FC_LOG_DEBUG("STORAGE: receive srcslot = %u, dstslot = %u", srcslot, dstslot);
    ...
    {
        FC_LOG_DEBUG("network", "WORLD: HandleSwapInvItemOpcode - Unit (%s) not found or you can't interact with him.", m_currentBankerGUID.ToString().c_str());
        return;
    }
...
{
    //FC_LOG_DEBUG("network", "WORLD: CMSG_SWAP_ITEM");
    uint8 dstbag, dstslot, srcbag, srcslot;

    recvData >> dstbag >> dstslot >> srcbag >> srcslot;
    //FC_LOG_DEBUG("STORAGE: receive srcbag = %u, srcslot = %u, dstbag = %u, dstslot = %u", srcbag, srcslot, dstbag, dstslot);
    ...
    {
        FC_LOG_DEBUG("network", "WORLD: HandleSwapItem - Unit (%s) not found or you can't interact with him.", m_currentBankerGUID.ToString().c_str());
        return;
    }
...
{
    //FC_LOG_DEBUG("network", "WORLD: CMSG_AUTOEQUIP_ITEM");
    uint8 srcbag, srcslot;

    recvData >> srcbag >> srcslot;
    //FC_LOG_DEBUG("STORAGE: receive srcbag = %u, srcslot = %u", srcbag, srcslot);
...
void WorldSession::HandleDestroyItemOpcode(WorldPackets::Item::DestroyItem& destroyItem)
{
    ...
    if (destroyItem.Count)
        _player->DestroyItemCount(pItem, destroyItem.Count, true);
    else
...
void WorldSession::HandleReadItem(WorldPacket& recvData)
{
    ...
    if (pItem && pItem->GetTemplate()->GetPageText())
    {
        ...
        if (msg == EQUIP_ERR_OK)
        {
            ...
            FC_LOG_INFO("network", "STORAGE: Item page sent");
        }
        else
        {
            data.Initialize(SMSG_READ_ITEM_FAILED, 8);
            FC_LOG_INFO("network", "STORAGE: Unable to read item");
...
{
    FC_LOG_DEBUG("network", "WORLD: Received CMSG_SELL_ITEM");
    ...
    if (!creature)
    {
        FC_LOG_DEBUG("network", "WORLD: HandleSellItemOpcode - %s not found or you can not interact with him.", vendorguid.ToString().c_str());
        ...
        if (pProto)
        {
            if (pProto->GetSellPrice() > 0)
            {
                ...
                if (count < pItem->GetCount())               // need split items
                {
                    ...
                    if (!pNewItem)
                    {
                        FC_LOG_ERROR("network", "WORLD: HandleSellItemOpcode - could not create clone of item %u; count = %u", pItem->GetEntry(), count);
                        _player->SendSellError(SELL_ERR_CANT_SELL_ITEM, creature, itemguid);
                        return;
                    }
...
{
    FC_LOG_DEBUG("network", "WORLD: Received CMSG_BUYBACK_ITEM");
    ...
    if (!creature)
    {
        FC_LOG_DEBUG("network", "WORLD: HandleBuybackItem - Unit (%s) not found or you can not interact with him.", vendorguid.ToString().c_str());
...
{
    FC_LOG_DEBUG("network", "WORLD: Received CMSG_BUY_ITEM_IN_SLOT");
...
{
    FC_LOG_DEBUG("network", "WORLD: Received CMSG_BUY_ITEM");
    ObjectGuid vendorguid, bagGuid;
    uint32 item, slot, count;
    ...
    if (itemType == ITEM_VENDOR_TYPE_ITEM)
    {
        ...
        FC_LOG_DEBUG("network", "WORLD: received wrong itemType (%u) in HandleBuyItemOpcode", itemType);
}
...
void WorldSession::HandleListInventoryOpcode(WorldPackets::NPC::Hello& packet)
{
    ...
    FC_LOG_DEBUG("network", "WORLD: Recvd CMSG_LIST_INVENTORY");
...
{
    FC_LOG_DEBUG("network", "WORLD: Sent SMSG_VENDOR_INVENTORY");

    Creature* vendor = GetPlayer()->GetNPCIfCanInteractWith(vendorGuid, UNIT_NPC_FLAG_VENDOR);
    if (!vendor)
    {
        FC_LOG_DEBUG("network", "WORLD: SendListInventory - %s not found or you can not interact with him.", vendorGuid.ToString().c_str());
        _player->SendSellError(SELL_ERR_CANT_FIND_VENDOR, nullptr, ObjectGuid::Empty);
        return;
    }
    ...
    for (uint32 slot = 0; slot < rawItemCount; ++slot)
    {
        ...
        */
        ...
        if (vendorItem->Type == ITEM_VENDOR_TYPE_ITEM)
        {
            ...
            {
                FC_LOG_DEBUG("condition", "SendListInventory: conditions not met for creature entry %u item %u", vendor->GetEntry(), vendorItem->item);
                continue;
            }
...
{
    //FC_LOG_DEBUG("network", "WORLD: CMSG_AUTOSTORE_BAG_ITEM");
    uint8 srcbag, srcslot, dstbag;

    recvData >> srcbag >> srcslot >> dstbag;
    //FC_LOG_DEBUG("STORAGE: receive srcbag = %u, srcslot = %u, dstbag = %u", srcbag, srcslot, dstbag);
...
{
    FC_LOG_DEBUG("network", "WORLD: CMSG_BUY_BANK_SLOT");
    ...
    if (!CanUseBank(guid))
    {
        ...
        SendPacket(&data);
        FC_LOG_DEBUG("network", "WORLD: HandleBuyBankSlotOpcode - %s not found or you can't interact with him.", guid.ToString().c_str());
        return;
    }
    ...
    FC_LOG_INFO("network", "PLAYER: Buy bank bag slot, slot number = %u", slot);
...
{
    FC_LOG_DEBUG("network", "WORLD: CMSG_AUTOBANK_ITEM");
    uint8 srcbag, srcslot;

    recvPacket >> srcbag >> srcslot;
    FC_LOG_DEBUG("network", "STORAGE: receive srcbag = %u, srcslot = %u", srcbag, srcslot);

    if (!CanUseBank())
    {
        FC_LOG_DEBUG("network", "WORLD: HandleAutoBankItemOpcode - Unit (%s) not found or you can't interact with him.", m_currentBankerGUID.ToString().c_str());
        return;
    }
...
{
    FC_LOG_DEBUG("network", "WORLD: CMSG_AUTOSTORE_BANK_ITEM");
    uint8 srcbag, srcslot;
    ...
    if (!CanUseBank())
    {
        FC_LOG_DEBUG("network", "WORLD: HandleAutoStoreBankItemOpcode - Unit (%s) not found or you can't interact with him.", m_currentBankerGUID.ToString().c_str());
        return;
    }
...
{
    FC_LOG_DEBUG("network", "Received opcode CMSG_WRAP_ITEM");
    ...
    FC_LOG_DEBUG("network", "WRAP: receive gift_bag = %u, gift_slot = %u, item_bag = %u, item_slot = %u", gift_bag, gift_slot, item_bag, item_slot);
...
{
    FC_LOG_DEBUG("network", "WORLD: CMSG_SOCKET_GEMS");
...
{
    FC_LOG_DEBUG("network", "WORLD: CMSG_CANCEL_TEMP_ENCHANTMENT");
...
{
    FC_LOG_DEBUG("network", "WORLD: CMSG_ITEM_REFUND_INFO");
    ...
    if (!item)
    {
        FC_LOG_DEBUG("network", "Item refund: item not found!");
        return;
    }
...
{
    FC_LOG_DEBUG("network", "WORLD: CMSG_ITEM_REFUND");
    ObjectGuid guid;
    recvData >> guid;                                      // item guid
...
void WorldSession::HandleItemTextQuery(WorldPacket& recvData)
{
    ...
    FC_LOG_DEBUG("network", "CMSG_ITEM_TEXT_QUERY %s", itemGuid.ToString().c_str());
...
void WorldSession::HandleTransmogrifyItems(WorldPackets::Item::TransmogrifyItems& transmogrifyItems)
{
    ...
    if (transmogrifyItems.Items.size() >= EQUIPMENT_SLOT_END)
    {
        FC_LOG_DEBUG("network", "WORLD: HandleTransmogrifyItems - Player (GUID: %u, name: %s) tried to transmogrify more items than possible. Amount: (%u)", player->GetGUID().GetCounter(), player->GetName().c_str(), uint32(transmogrifyItems.Items.size()));
        return;
    }
    ...
    {
        FC_LOG_DEBUG("network", "WORLD: HandleTransmogrifyItems - %s not found or player can't interact with it.", transmogrifyItems.Npc.ToString().c_str());
        return;
    }

    int64 cost = 0;
    for (WorldPackets::Item::TransmogrifyItem& item : transmogrifyItems.Items)
    {
        // slot of the transmogrified item
        if (item.Slot >= EQUIPMENT_SLOT_END)
        {
            FC_LOG_DEBUG("network", "WORLD: HandleTransmogrifyItems - Player (GUID: %u, name: %s) tried to transmogrify %s with a wrong slot (%u) when transmogrifying items.", player->GetGUID().GetCounter(), player->GetName().c_str(), item.SrcItemGUID.ToString().c_str(), item.Slot);
            return;
        }

        // transmogrified item
        Item* targetItem = player->GetItemByPos(INVENTORY_SLOT_BAG_0, item.Slot);
        if (!targetItem)
        {
            FC_LOG_DEBUG("network", "WORLD: HandleTransmogrifyItems - Player (GUID: %u, name: %s) tried to transmogrify an invalid item in a valid slot (slot: %u).", player->GetGUID().GetCounter(), player->GetName().c_str(), item.Slot);
            return;
        }

        // Applying the appearance of a different item onto another
        if (item.ItemID)
        {
            ...
            if (!proto)
            {
                FC_LOG_DEBUG("network", "WORLD: HandleTransmogrifyItems - Player (GUID: %u, name: %s) tried to transmogrify to an invalid item (entry: %u).", player->GetGUID().GetCounter(), player->GetName().c_str(), item.ItemID);
                return;
            }

            // guid of the transmogrifier item
            Item const* sourceItem = player->GetItemByGuid(item.SrcItemGUID);
            if (!sourceItem)
            {
                FC_LOG_DEBUG("network", "WORLD: HandleTransmogrifyItems - Player (GUID: %u, name: %s) tried to transmogrify with an invalid item (%s).", player->GetGUID().GetCounter(), player->GetName().c_str(), item.SrcItemGUID.ToString().c_str());
                return;
            }
            ...
            {
                FC_LOG_DEBUG("network", "WORLD: HandleTransmogrifyItems - Player (GUID: %u, name: %s) tried to transmogrify with an invalid entry (entry: %u) for %s.", player->GetGUID().GetCounter(), player->GetName().c_str(), item.ItemID, item.SrcItemGUID.ToString().c_str());
                return;
            }

            // validity of the transmogrification items
            if (!Item::CanTransmogrifyItemWithItem(targetItem, sourceItem))
            {
                FC_LOG_DEBUG("network", "WORLD: HandleTransmogrifyItems - Player (GUID: %u, name: %s) failed CanTransmogrifyItemWithItem (%u with %u).", player->GetGUID().GetCounter(), player->GetName().c_str(), targetItem->GetEntry(), sourceItem->GetEntry());
                return;
            }

            // add cost
            cost += targetItem->GetSpecialPrice();
        }
    }
    ...
    for (WorldPackets::Item::TransmogrifyItem& item : transmogrifyItems.Items)
    {
        Item* targetItem = ASSERT_NOTNULL(player->GetItemByPos(INVENTORY_SLOT_BAG_0, item.Slot));

        if (item.ItemID)
        {
            Item* sourceitem = ASSERT_NOTNULL(player->GetItemByGuid(item.SrcItemGUID));

            // Transmogrify
            targetItem->SetEnchantment(TRANSMOGRIFY_ENCHANTMENT_SLOT, item.ItemID, 0, 0);
            player->SetVisibleItemSlot(item.Slot, targetItem);
            ...
            if (sourceitem->GetTemplate()->GetBonding() == BIND_ON_EQUIP || sourceitem->GetTemplate()->GetBonding() == BIND_ON_USE)
        ...
        {
            // Reset
            targetItem->ClearEnchantment(TRANSMOGRIFY_ENCHANTMENT_SLOT);
            player->SetVisibleItemSlot(item.Slot, targetItem);
        }
    }
}
...
void WorldSession::HandleReforgeItemOpcode(WorldPacket& recvData)
{
    ...
    {
        FC_LOG_DEBUG("network", "WORLD: HandleReforgeItemOpcode - %s not found or player can't interact with it.", guid.ToString().c_str());
    ...
    if (!item)
    {
        FC_LOG_DEBUG("network", "WORLD: HandleReforgeItemOpcode - Player (Guid: %u Name: %s) tried to reforge an invalid/non-existant item.", player->GetGUID().GetCounter(), player->GetName().c_str());
        SendReforgeResult(false);
        return;
    }
    ...
    if (!reforgeEntry)
    {
        ...
        {
            FC_LOG_ERROR("network", "WORLD: HandleReforgeItemOpcode - Player (Guid: %u Name: %s) tried to remove reforge from non-reforged item (Entry: %u)", player->GetGUID().GetCounter(), player->GetName().c_str(), item->GetEntry());
            SendReforgeResult(false);
            return;
        }
    ...
    if (item->GetEnchantmentId(REFORGE_ENCHANTMENT_SLOT))
    {
        FC_LOG_ERROR("network", "WORLD: HandleReforgeItemOpcode - Player (Guid: %u Name: %s) tried to reforge an already reforged item (Entry: %u)", player->GetGUID().GetCounter(), player->GetName().c_str(), item->GetEntry());
        SendReforgeResult(false);
        return;
    }
    ...
    if (!stats)
    {
        FC_LOG_DEBUG("network", "WORLD: HandleReforgeItemOpcode - Player (Guid: %u Name: %s) tried to reforge an item with invalid reforge entry (%u).", player->GetGUID().GetCounter(), player->GetName().c_str(), reforgeEntry);
        SendReforgeResult(false);
        return;
    }
