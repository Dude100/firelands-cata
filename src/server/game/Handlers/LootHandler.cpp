...
{
    FC_LOG_DEBUG("network", "WORLD: CMSG_AUTOSTORE_LOOT_ITEM");
...
{
    FC_LOG_DEBUG("network", "WORLD: CMSG_LOOT_CURRENCY");
...
{
    FC_LOG_DEBUG("network", "WORLD: CMSG_LOOT_MONEY");
...
{
    FC_LOG_DEBUG("network", "WORLD: CMSG_LOOT");
...
{
    FC_LOG_DEBUG("network", "WORLD: CMSG_LOOT_RELEASE");
...
void WorldSession::HandleLootMasterGiveOpcode(WorldPacket& recvData)
{
    ...
    FC_LOG_DEBUG("network", "WorldSession::HandleLootMasterGiveOpcode (CMSG_LOOT_MASTER_GIVE, 0x02A3) Target = [%s].", target->GetName().c_str());
    ...
    if (!_player->IsInRaidWith(target) || !_player->IsInMap(target))
    {
        ...
        FC_LOG_INFO("entities.player.cheat", "MasterLootItem: Player %s tried to give an item to ineligible player %s !", GetPlayer()->GetName().c_str(), target->GetName().c_str());
        return;
    }
    ...
    {
        FC_LOG_INFO("entities.player.cheat", "MasterLootItem: Player %s might be using a hack! (slot %d, size %lu)",
