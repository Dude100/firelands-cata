...
void WorldSession::HandleDismissCritter(WorldPacket& recvData)
{
    ...
    FC_LOG_DEBUG("network.opcode", "WORLD: Received CMSG_DISMISS_CRITTER for %s", guid.ToString().c_str());

    Unit* pet = ObjectAccessor::GetCreatureOrPetOrVehicle(*_player, guid);

    if (!pet)
    {
        FC_LOG_DEBUG("entities.pet", "Vanitypet (%s) does not exist - player '%s' (guid: %u / account: %u) attempted to dismiss it (possibly lagged out)",
...
void WorldSession::HandlePetAction(WorldPacket& recvData)
{
    ...
    FC_LOG_DEBUG("entities.pet", "HandlePetAction: %s - flag: %u, spellid: %u, target: %s.", guid1.ToString().c_str(), uint32(flag), spellid, guid2.ToString().c_str());

    if (!pet)
    {
        FC_LOG_DEBUG("entities.pet", "HandlePetAction: %s doesn't exist for %s %s", guid1.ToString().c_str(), GetPlayer()->GetGUID().ToString().c_str(), GetPlayer()->GetName().c_str());
        return;
    }

    if (pet != GetPlayer()->GetFirstControlled())
    {
        FC_LOG_DEBUG("entities.pet", "HandlePetAction: %s does not belong to %s %s", guid1.ToString().c_str(), GetPlayer()->GetGUID().ToString().c_str(), GetPlayer()->GetName().c_str());
        return;
    }
...
void WorldSession::HandlePetStopAttack(WorldPacket& recvData)
{
    ...
    FC_LOG_DEBUG("network.opcode", "WORLD: Received CMSG_PET_STOP_ATTACK for %s", guid.ToString().c_str());

    Unit* pet = ObjectAccessor::GetCreatureOrPetOrVehicle(*_player, guid);

    if (!pet)
    {
        FC_LOG_ERROR("entities.pet", "HandlePetStopAttack: %s does not exist", guid.ToString().c_str());
        return;
    }

    if (pet != GetPlayer()->GetPet() && pet != GetPlayer()->GetCharmed())
    {
        FC_LOG_ERROR("entities.pet", "HandlePetStopAttack: %s isn't a pet or charmed creature of player %s",
...
void WorldSession::HandlePetActionHelper(Unit* pet, ObjectGuid guid1, uint32 spellid, uint16 flag, ObjectGuid guid2, float x, float y, float z)
{
    ...
    if (!charmInfo)
    {
        FC_LOG_DEBUG("entities.pet", "WorldSession::HandlePetAction(petGuid: %s, tagGuid: %s, spellId: %u, flag: %u): object (GUID: %u Entry: %u TypeId: %u) is considered pet-like but doesn't have a charminfo!",
    ...
    case ACT_COMMAND:                                   //0x07
        ...
        case COMMAND_MOVE_TO:
            ...
            break;
        default:
            FC_LOG_ERROR("entities.pet", "WORLD: unknown PET flag Action %i and spellid %i.", uint32(flag), spellid);
        }
        break;
    ...
    case ACT_ENABLED:                                   // 0xC1    spell
    {
        ...
        if (!spellInfo)
        {
            FC_LOG_ERROR("spells.pet", "WORLD: unknown PET spell id %i", spellid);
            return;
        }
        ...
        if (result == SPELL_FAILED_UNIT_NOT_INFRONT && !pet->isPossessed() && !pet->IsVehicle())
        {
            if (unit_target)
            {
                pet->SetOrientationTowards(unit_target);
                if (Player* player = unit_target->ToPlayer())
                    pet->SendUpdateToPlayer(player);
            }
            else if (Unit* unit_target2 = spell->m_targets.GetUnitTarget())
            {
                pet->SetOrientationTowards(unit_target2);
...
{
    FC_LOG_DEBUG("network.opcode", "WORLD: Received CMSG_PET_NAME_QUERY");
...
bool WorldSession::CheckStableMaster(ObjectGuid guid)
{
    ...
    if (guid == GetPlayer()->GetGUID())
    {
        ...
        {
            FC_LOG_DEBUG("entities.player.cheat", "%s attempt open stable in cheating way.", guid.ToString().c_str());
            return false;
        }
    }
    // stable master case
    else
    {
        ...
        {
            FC_LOG_DEBUG("entities.player", "Stablemaster %s not found or you can't interact with him.", guid.ToString().c_str());
            return false;
        }
    }
    return true;
...
{
    FC_LOG_DEBUG("network.opcode", "WORLD: Received CMSG_PET_SET_ACTION");
    ...
    {
        FC_LOG_ERROR("entities.pet", "HandlePetSetAction: Unknown %s or owner (%s)", petguid.ToString().c_str(), _player->GetGUID().ToString().c_str());
        return;
    }
    ...
    if (!charmInfo)
    {
        FC_LOG_ERROR("entities.pet", "WorldSession::HandlePetSetAction: object (GUID: %u TypeId: %u) is considered pet-like but doesn't have a charminfo!", pet->GetGUID().GetCounter(), pet->GetTypeId());
        return;
    }
    ...
    for (uint8 i = 0; i < count; ++i)
    {
        ...
        FC_LOG_DEBUG("entities.pet", "Player %s has changed pet spell action. Position: %u, Spell: %u, State: 0x%X",
...
{
    FC_LOG_DEBUG("network.opcode", "WORLD: Received CMSG_PET_RENAME");
...
void WorldSession::HandlePetAbandon(WorldPacket& recvData)
{
    ...
    FC_LOG_DEBUG("network.opcode", "WORLD: Received CMSG_PET_ABANDON %s", guid.ToString().c_str());
...
{
    FC_LOG_DEBUG("network.opcode", "WORLD: Received CMSG_PET_SPELL_AUTOCAST");
    ObjectGuid guid;
    uint32 spellid;
    ...
    {
        FC_LOG_ERROR("entities.pet", "HandlePetSpellAutocastOpcode. %s isn't pet of player %s (%s).", guid.ToString().c_str(), GetPlayer()->GetName().c_str(), GetPlayer()->GetGUID().ToString().c_str());
        return;
    }
    ...
    if (!spellInfo)
    {
        FC_LOG_ERROR("spells.pet", "WORLD: unknown PET spell id %u", spellid);
        return;
    }
    ...
    if (!charmInfo)
    {
        FC_LOG_ERROR("entities.pet", "WorldSession::HandlePetSpellAutocastOpcod: object (GUID: %u TypeId: %u) is considered pet-like but doesn't have a charminfo!", pet->GetGUID().GetCounter(), pet->GetTypeId());
        return;
    }
...
{
    FC_LOG_DEBUG("network.opcode", "WORLD: Received CMSG_PET_CAST_SPELL");
    ...
    FC_LOG_DEBUG("entities.pet", "WORLD: CMSG_PET_CAST_SPELL, %s, castCount: %u, spellId %u, castFlags %u", guid.ToString().c_str(), castCount, spellId, castFlags);
    ...
    {
        FC_LOG_ERROR("entities.pet", "HandlePetCastSpellOpcode: %s isn't pet of player %s (%s).", guid.ToString().c_str(), GetPlayer()->GetName().c_str(), GetPlayer()->GetGUID().ToString().c_str());
        return;
    }
...
{
    FC_LOG_DEBUG("network.opcode", "WORLD: Received CMSG_PET_LEARN_TALENT");
...
{
    FC_LOG_DEBUG("network.opcode", "WORLD: Received CMSG_LEARN_PREVIEW_TALENTS_PET");
