...
{
    FC_LOG_DEBUG("network", "WORLD: got MSG_MOVE_WORLDPORT_ACK.");
...
void WorldSession::HandleMoveWorldportAck()
{
    ...
    {
        FC_LOG_ERROR("network", "%s %s is still in world when teleported from map %s (%u) to new map %s (%u)", GetPlayer()->GetGUID().ToString().c_str(), GetPlayer()->GetName().c_str(), oldMap->GetMapName(), oldMap->GetId(), newMap ? newMap->GetMapName() : "Unknown", loc.GetMapId());
    ...
    {
        FC_LOG_ERROR("network", "Map %d (%s) could not be created for player %d (%s), porting player to homebind", loc.GetMapId(), newMap ? newMap->GetMapName() : "Unknown", GetPlayer()->GetGUID().GetCounter(), GetPlayer()->GetName().c_str());
    ...
    {
        FC_LOG_ERROR("network", "WORLD: failed to teleport player %s (%d) to map %d (%s) because of unknown reason!",
...
void WorldSession::HandleMovementOpcode(uint16 opcode, MovementInfo& movementInfo)
{
    ...
    {
        FC_LOG_ERROR("network", "HandleMovementOpcodes: guid error");
        return;
    }
    if (!movementInfo.pos.IsPositionValid())
    {
        FC_LOG_ERROR("network", "HandleMovementOpcodes: Invalid Position");
        return;
    }
    ...
    if (!movementInfo.transport.guid.IsEmpty())
    {
        ...
        if (!Firelands::IsValidMapCoord(movementInfo.pos.GetPositionX() + movementInfo.transport.pos.GetPositionX(), movementInfo.pos.GetPositionY() + movementInfo.transport.pos.GetPositionY(),
    ...
    {
        FC_LOG_WARN("misc", "The computed movement time using clockDelta is erronous. Using fallback instead");
...
void WorldSession::HandleForceSpeedChangeAck(WorldPacket& recvData)
{
    ...
    {
        FC_LOG_ERROR("entities.unit", "Ignoring ACK. Bad or outdated movement data by Player %s", _player->GetName().c_str());
        return;
    }
    ...
    default:
        FC_LOG_ERROR("network", "WorldSession::HandleForceSpeedChangeAck: Unknown move type opcode: %s", GetOpcodeNameForLogging(static_cast<OpcodeClient>(recvData.GetOpcode())).c_str());
        return;
    }
    ...
    {
        FC_LOG_INFO("cheat", "WorldSession::HandleForceSpeedChangeAck: Player %s from account id %u kicked for incorrect data returned in an ack",
...
void WorldSession::HandleSetActiveMoverOpcode(WorldPackets::Movement::SetActiveMover& packet)
{
    ...
    if (!client->IsAllowedToMove(packet.ActiveMover))
    {
        ...
        FC_LOG_DEBUG("entities.unit", "set active mover FAILED for client of player %s. GUID %s.", _player->GetName().c_str(), packet.ActiveMover.ToString().c_str());
        return;
    }

    // step 2:
    FC_LOG_DEBUG("entities.unit", "set active mover OK for client of player %s. GUID %s.", _player->GetName().c_str(), packet.ActiveMover.ToString().c_str());
...
{
    FC_LOG_DEBUG("network", "WORLD: Recvd CMSG_MOVE_NOT_ACTIVE_MOVER");
    ...
    {
        FC_LOG_DEBUG("entities.unit", "unset active mover FAILED for client of player %s. GUID %s.", _player->GetName().c_str(), movementInfo.guid.ToString().c_str());
        return;
    }

    FC_LOG_DEBUG("entities.unit", "unset active mover OK for client of player %s. GUID %s.", _player->GetName().c_str(), movementInfo.guid.ToString().c_str());
...
{
    FC_LOG_DEBUG("network", "CMSG_MOVE_KNOCK_BACK_ACK");
...
{
    FC_LOG_DEBUG("network", "CMSG_MOVE_HOVER_ACK");
    MovementInfo movementInfo;
    GetPlayer()->ReadMovementInfo(recvData, &movementInfo);
}
...
{
    FC_LOG_DEBUG("network", "CMSG_FORCE_MOVE_ROOT_ACK");
    MovementInfo movementInfo;
    GetPlayer()->ReadMovementInfo(recvData, &movementInfo);
}
...
{
    FC_LOG_DEBUG("network", "CMSG_MOVE_FEATHER_FALL_ACK");
    MovementInfo movementInfo;
    GetPlayer()->ReadMovementInfo(recvData, &movementInfo);
}
...
{
    FC_LOG_DEBUG("network", "CMSG_FORCE_MOVE_UNROOT_ACK");
    MovementInfo movementInfo;
    GetPlayer()->ReadMovementInfo(recvData, &movementInfo);
}
...
{
    FC_LOG_DEBUG("network", "CMSG_MOVE_SET_CAN_TRANSITION_BETWEEN_SWIM_AND_FLY_ACK");
    MovementInfo movementInfo;
    GetPlayer()->ReadMovementInfo(recvData, &movementInfo);
...
{
    FC_LOG_DEBUG("network", "received %s", GetOpcodeNameForLogging(static_cast<OpcodeClient>(recvData.GetOpcode())).c_str());
    MovementInfo movementInfo;
    GetPlayer()->ReadMovementInfo(recvData, &movementInfo);
    ...
    {
        FC_LOG_INFO("cheat", "WorldSession::HandleMoveGravityDisableAck: Player %s from account id %u kicked for incorrect data returned in an ack",
            _player->GetName().c_str(), _player->GetSession()->GetAccountId());
        _player->GetSession()->KickPlayer();
        return;
    }
...
void WorldSession::HandleSetCollisionHeightAck(WorldPacket& recvData)
{
    ...
    {
        FC_LOG_INFO("cheat", "WorldSession::HandleSetCollisionHeightAck: Player %s from account id %u kicked for incorrect data returned in an ack",
            _player->GetName().c_str(), _player->GetSession()->GetAccountId());
        _player->GetSession()->KickPlayer();
        return;
    }
...
{
    FC_LOG_DEBUG("network", "CMSG_MOVE_SET_CAN_FLY_ACK");
    MovementInfo movementInfo;
    GetPlayer()->ReadMovementInfo(recvData, &movementInfo);
...
{
    FC_LOG_DEBUG("network", "WORLD: Received CMSG_MOVE_TIME_SKIPPED");
    ...
    if (!mover)
    {
        FC_LOG_WARN("entities.player", "WorldSession::HandleMoveTimeSkippedOpcode wrong mover state from the unit moved by the player %s", GetPlayer()->GetGUID().ToString().c_str());
        return;
    }
    ...
    {
        FC_LOG_WARN("entities.player", "WorldSession::HandleMoveTimeSkippedOpcode wrong guid from the unit moved by the player %s", GetPlayer()->GetGUID().ToString().c_str());
        return;
    }
...
{
    FC_LOG_DEBUG("network", "CMSG_TIME_SYNC_RESP");
