...
{
    FC_LOG_DEBUG("network", "CMSG_LEARN_PREVIEW_TALENTS");
...
{
    FC_LOG_DEBUG("network", "MSG_TALENT_WIPE_CONFIRM");
    ObjectGuid guid;
    recvData >> guid;
    ...
    if (!unit)
    {
        FC_LOG_DEBUG("network", "WORLD: HandleTalentWipeConfirmOpcode - %s not found or you can't interact with him.", guid.ToString().c_str());
        return;
    }
