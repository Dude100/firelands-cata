...
bool WorldSession::CanOpenMailBox(ObjectGuid guid)
{
    if (guid == _player->GetGUID())
    {
        ...
        {
            FC_LOG_WARN("cheat", "%s attempted to open mailbox by using a cheat.", _player->GetName().c_str());
            return false;
        }
    }
...
void WorldSession::HandleSendMail(WorldPacket& recvData)
{
    ...
    if (!receiverGuid)
    {
        FC_LOG_INFO("network", "Player %u is sending mail to %s (GUID: non-existing!) with subject %s "
    ...
    FC_LOG_INFO("network", "Player %u is sending mail to %s (%s) with subject %s and body %s "
...
void WorldSession::HandleMailCreateTextItem(WorldPacket& recvData)
{
    ...
    FC_LOG_INFO("network", "HandleMailCreateTextItem mailid=%u", mailId);
