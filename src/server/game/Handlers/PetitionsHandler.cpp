...
{
    FC_LOG_DEBUG("network", "Received opcode CMSG_PETITION_BUY");
    ...
    FC_LOG_DEBUG("network", "Petitioner %s tried sell petition: name %s", guidNPC.ToString().c_str(), name.c_str());

    // prevent cheating
    Creature* creature = GetPlayer()->GetNPCIfCanInteractWith(guidNPC, UNIT_NPC_FLAG_PETITIONER);
    if (!creature)
    {
        FC_LOG_DEBUG("network", "WORLD: HandlePetitionBuyOpcode - %s not found or you can't interact with him.", guidNPC.ToString().c_str());
        return;
    }
    ...
    else
    {
        ...
        case 3:
            ...
            break;
        default:
            FC_LOG_DEBUG("network", "unknown selection at buy arena petition: %u", clientIndex);
            return;
        }
    ...
    if (Petition const* petition = sPetitionMgr->GetPetitionByOwnerWithType(_player->GetGUID(), type))
    {
        ...
        FC_LOG_DEBUG("network", "Invalid petition GUID: %u", petition->petitionGuid.GetCounter());
    }
...
{
    FC_LOG_DEBUG("network", "Received opcode CMSG_PETITION_SHOW_SIGNATURES");
    ...
    if (!petition)
    {
        FC_LOG_DEBUG("entities.player.items", "Petition %s is not found for player %u %s", petitionGuid.ToString().c_str(), GetPlayer()->GetGUID().GetCounter(), GetPlayer()->GetName().c_str());
        return;
    }
    ...
    FC_LOG_DEBUG("network", "CMSG_PETITION_SHOW_SIGNATURES petition entry: '%u'", petitionGuid.GetCounter());
...
{
    FC_LOG_DEBUG("network", "Received opcode CMSG_PETITION_QUERY");   // ok

    ObjectGuid::LowType guildguid;
    ObjectGuid petitionguid;
    recvData >> guildguid;                                 // in Firelands always same as GUID_LOPART(petitionguid)
    recvData >> petitionguid;                              // petition guid
    FC_LOG_DEBUG("network", "CMSG_PETITION_QUERY Petition %s Guild GUID %u", petitionguid.ToString().c_str(), guildguid);
...
void WorldSession::SendPetitionQueryOpcode(ObjectGuid petitionguid)
{
    ...
    if (!petition)
    {
        FC_LOG_DEBUG("network", "CMSG_PETITION_QUERY failed for petition (%s)", petitionguid.ToString().c_str());
        return;
    }

    WorldPacket data(SMSG_PETITION_QUERY_RESPONSE, (4 + 8 + petition->petitionName.size() + 1 + 1 + 4 * 12 + 2 + 10));
    data << uint32(petitionguid.GetCounter());              // guild/team guid (in Firelands always same as GUID_LOPART(petition guid)
...
{
    FC_LOG_DEBUG("network", "Received opcode MSG_PETITION_RENAME");
    ...
    FC_LOG_DEBUG("network", "Petition %s renamed to '%s'", petitionGuid.ToString().c_str(), newName.c_str());
...
{
    FC_LOG_DEBUG("network", "Received opcode CMSG_PETITION_SIGN");    // ok
    ...
    if (!petition)
    {
        FC_LOG_ERROR("network", "Petition %s is not found for player %u %s", petitionGuid.ToString().c_str(), GetPlayer()->GetGUID().GetCounter(), GetPlayer()->GetName().c_str());
        return;
    }
    ...
    FC_LOG_DEBUG("network", "PETITION SIGN: %s by player: %s (GUID: %u Account: %u)", petitionGuid.ToString().c_str(), _player->GetName().c_str(), playerGuid.GetCounter(), GetAccountId());
...
{
    FC_LOG_DEBUG("network", "Received opcode MSG_PETITION_DECLINE");  // ok

    ObjectGuid petitionguid;
    recvData >> petitionguid;                              // petition guid
    FC_LOG_DEBUG("network", "Petition %s declined by %u", petitionguid.ToString().c_str(), _player->GetGUID().GetCounter());
...
{
    FC_LOG_DEBUG("network", "Received opcode CMSG_OFFER_PETITION");   // ok
    ...
    FC_LOG_DEBUG("network", "OFFER PETITION: type %u, %s, to %s", static_cast<uint32>(type), petitionGuid.ToString().c_str(), offererGuid.ToString().c_str());
...
{
    FC_LOG_DEBUG("network", "Received opcode CMSG_TURN_IN_PETITION");
    ...
    FC_LOG_DEBUG("network", "Petition %s turned in by %u", petitionGuid.ToString().c_str(), _player->GetGUID().GetCounter());

    Petition const* petition = sPetitionMgr->GetPetition(petitionGuid);
    if (!petition)
    {
        FC_LOG_ERROR("entities.player.cheat", "Player %s (guid: %u) tried to turn in petition (%s) that is not present in the database", _player->GetName().c_str(), _player->GetGUID().GetCounter(), petitionGuid.ToString().c_str());
        return;
    }
    ...
    else
    {
        ...
        FC_LOG_DEBUG("network", "PetitonsHandler: Arena team (guid: %u) added to ObjectMgr", arenaTeam->GetId());

        // Add members
        for (Signature const& signature : signatures)
        {
            FC_LOG_DEBUG("network", "PetitionsHandler: Adding arena team (guid: %u) member %s", arenaTeam->GetId(), signature.second.ToString().c_str());
    ...
    // created
    FC_LOG_DEBUG("network", "Player %s (%s) turning in petition %s", _player->GetName().c_str(), _player->GetGUID().ToString().c_str(), petitionGuid.ToString().c_str());
...
{
    FC_LOG_DEBUG("network", "Received CMSG_PETITION_SHOWLIST");
...
void WorldSession::SendPetitionShowList(ObjectGuid guid)
{
    ...
    if (!creature)
    {
        FC_LOG_DEBUG("network", "WORLD: HandlePetitionShowListOpcode - %s not found or you can't interact with him.", guid.ToString().c_str());
        return;
    }
    ...
    SendPacket(&data);
    FC_LOG_DEBUG("network", "Sent SMSG_PETITION_SHOWLIST");
}
