...
void WorldSession::HandleCharEnum(PreparedQueryResult result)
{
    ...
    if (result)
    {
        do
        {
            ...
            FC_LOG_INFO("network", "Loading char guid %s from account %u.", charInfo.Guid.ToString().c_str(), GetAccountId());
            ...
            {
                FC_LOG_ERROR("entities.player.loading", "Player %s has wrong Appearance values (Hair/Skin/Color), forcing recustomize", charInfo.Guid.ToString().c_str());
...
void WorldSession::HandleCharCreateOpcode(WorldPacket& recvData)
{
    ...
    if (!classEntry)
    {
        FC_LOG_ERROR("network", "Class (%u) not found in DBC while creating new char for account (ID: %u): wrong DBC files or cheater?", createInfo->Class, GetAccountId());
    ...
    if (!raceEntry)
    {
        FC_LOG_ERROR("network", "Race (%u) not found in DBC while creating new char for account (ID: %u): wrong DBC files or cheater?", createInfo->Race, GetAccountId());
        SendCharCreate(CHAR_CREATE_FAILED);
        return;
    }
    ...
    {
        FC_LOG_ERROR("entities.player.cheat", "Expansion %u account:[%d] tried to Create character with expansion %u race (%u)", GetAccountExpansion(), GetAccountId(), raceEntry->Race_related, createInfo->Race);
    ...
    {
        FC_LOG_ERROR("entities.player.cheat", "Expansion %u account:[%d] tried to Create character with expansion %u class (%u)", GetAccountExpansion(), GetAccountId(), classEntry->Required_expansion, createInfo->Class);
    ...
    {
        FC_LOG_INFO("entities.player.cheat", "Account:[%d] but tried to Create character with empty [name] ", GetAccountId());
                            ...
                            AddTransactionCallback(CharacterDatabase.AsyncCommitTransaction(characterTransaction)).AfterComplete([this, newChar = std::move(newChar)](bool success)
                                {
                                    if (success)
                                    {
                                        FC_LOG_INFO("entities.player.character", "Account: %d (IP: %s) Create Character:[%s] (GUID: %u)", GetAccountId(), GetRemoteAddress().c_str(), newChar->GetName().c_str(), newChar->GetGUID().GetCounter());
...
void WorldSession::HandleCharDeleteOpcode(WorldPackets::Character::CharDelete& packet)
{
    ...
    FC_LOG_INFO("entities.player.character", "Account: %d, IP: %s deleted character: %s, %s, Level: %u", accountId, GetRemoteAddress().c_str(), name.c_str(), packet.Guid.ToString().c_str(), level);
...
void WorldSession::HandlePlayerLoginOpcode(WorldPackets::Character::PlayerLogin& packet)
{
    ...
    {
        FC_LOG_ERROR("network", "Player tries to login again, AccountId = %d", GetAccountId());
        KickPlayer();
        return;
    }
    ...
    {
        FC_LOG_ERROR("network", "Account (%u) can't login with that character (%s).", GetAccountId(), packet.Guid.ToString().c_str());
        KickPlayer();
        return;
    }
...
void WorldSession::HandlePlayerLogin(LoginQueryHolder const& holder)
{
    ...
    //FC_LOG_DEBUG("Player %s added to Map.", pCurrChar->GetName().c_str());
    ...
    if (pCurrChar->GetGuildId() != 0)
    {
        ...
        else
        {
            ...
            FC_LOG_ERROR("misc", "Player %s (GUID: %u) marked as member of not existing guild (id: %u), removing guild membership for player.", pCurrChar->GetName().c_str(), pCurrChar->GetGUID().GetCounter(), pCurrChar->GetGuildId());
            pCurrChar->SetInGuild(0);
        }
    }
    ...
    FC_LOG_INFO("entities.player.character", "Account: %d (IP: %s) Login Character:[%s] (GUID: %u) Level: %d",
...
{
    FC_LOG_DEBUG("network", "WORLD: Received CMSG_SET_FACTION_ATWAR");
...
{
    FC_LOG_ERROR("network", "WORLD SESSION: HandleSetFactionCheat, not expected call, please report.");
...
{
    FC_LOG_DEBUG("network", "WORLD: Received CMSG_SET_WATCHED_FACTION");
    uint32 fact;
    recvData >> fact;
...
{
    FC_LOG_DEBUG("network", "WORLD: Received CMSG_SET_FACTION_INACTIVE");
    uint32 replistid;
    uint8 inactive;
...
void WorldSession::HandleCharRenameCallback(std::shared_ptr<CharacterRenameInfo> renameInfo, PreparedQueryResult result)
{
    ...
    FC_LOG_INFO("entities.player.character", "Account: %d (IP: %s) Character:[%s] (%s) Changed name to: %s", GetAccountId(), GetRemoteAddress().c_str(), oldName.c_str(), renameInfo->Guid.ToString().c_str(), renameInfo->Name.c_str());
...
{
    FC_LOG_DEBUG("network", "CMSG_ALTER_APPEARANCE");
...
void WorldSession::HandleRemoveGlyph(WorldPacket& recvData)
{
    ...
    {
        FC_LOG_DEBUG("network", "Client sent wrong glyph slot number in opcode CMSG_REMOVE_GLYPH %u", slot);
        return;
    }
...
void WorldSession::HandleCharCustomize(WorldPacket& recvData)
{
    ...
    {
        FC_LOG_INFO("entities.player.cheat", "Account %u, IP: %s tried to customise %s, but it does not belong to their account!",
...
void WorldSession::HandleCharCustomizeCallback(std::shared_ptr<CharacterCustomizeInfo> customizeInfo, PreparedQueryResult result)
{
    ...
    FC_LOG_INFO("entities.player.character", "Account: %d (IP: %s), Character[%s] (%s) Customized to: %s",
        GetAccountId(), GetRemoteAddress().c_str(), oldName.c_str(), customizeInfo->Guid.ToString().c_str(), customizeInfo->Name.c_str());
}

void WorldSession::HandleEquipmentSetSave(WorldPacket& recvData)
{
    FC_LOG_DEBUG("network", "CMSG_EQUIPMENT_SET_SAVE");
...
{
    FC_LOG_DEBUG("network", "CMSG_EQUIPMENT_SET_DELETE");
...
{
    FC_LOG_DEBUG("network", "CMSG_EQUIPMENT_SET_USE");
    ...
    for (uint32 i = 0; i < EQUIPMENT_SLOT_END; ++i)
    {
        ...
        FC_LOG_DEBUG("entities.player.items", "%s: srcbag %u, srcslot %u", itemGuid.ToString().c_str(), srcbag, srcslot);
...
void WorldSession::HandleCharFactionOrRaceChange(WorldPacket& recvData)
{
    ...
    {
        FC_LOG_INFO("entities.player.cheat", "Account %u, IP: %s tried to factionchange character %s, but it does not belong to their account!",
...
void WorldSession::HandleCharFactionOrRaceChangeCallback(std::shared_ptr<CharacterFactionChangeInfo> factionChangeInfo, PreparedQueryResult result)
{
    ...
    FC_LOG_DEBUG("entities.player", "%s (IP: %s) changed race from %u to %u", GetPlayerInfo().c_str(), GetRemoteAddress().c_str(), oldRace, factionChangeInfo->Race);
...
void WorldSession::HandleRandomizeCharNameOpcode(WorldPackets::Character::GenerateRandomCharacterName& packet)
{
    ...
    {
        FC_LOG_ERROR("misc", "Invalid race (%u) sent by accountId: %u", packet.Race, GetAccountId());
        return;
    }

    if (!Player::IsValidGender(packet.Sex))
    {
        FC_LOG_ERROR("misc", "Invalid gender (%u) sent by accountId: %u", packet.Sex, GetAccountId());
        return;
    }
