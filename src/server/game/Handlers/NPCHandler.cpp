...
void WorldSession::HandleTabardVendorActivateOpcode(WorldPacket& recvData)
{
    ...
    if (!unit)
    {
        FC_LOG_DEBUG("network", "WORLD: HandleTabardVendorActivateOpcode - %s not found or you can not interact with him.", guid.ToString().c_str());
        return;
    }
...
void WorldSession::HandleBankerActivateOpcode(WorldPackets::NPC::Hello& packet)
{
    ...
    if (!unit)
    {
        FC_LOG_DEBUG("network", "WORLD: HandleBankerActivateOpcode - %s not found or you can not interact with him.", packet.Unit.ToString().c_str());
        return;
    }
...
void WorldSession::HandleTrainerListOpcode(WorldPackets::NPC::Hello& packet)
{
    ...
    if (!npc)
    {
        FC_LOG_DEBUG("network", "WorldSession::SendTrainerList - %s not found or you can not interact with him.", packet.Unit.ToString().c_str());
        return;
    }
    ...
    if (uint32 trainerId = sObjectMgr->GetCreatureDefaultTrainer(npc->GetEntry()))
        ...
        FC_LOG_DEBUG("network", "WorldSession::SendTrainerList - Creature id %u has no trainer data.", npc->GetEntry());
}
...
void WorldSession::SendTrainerList(Creature* npc, uint32 trainerId)
{
    ...
    if (!trainer)
    {
        FC_LOG_DEBUG("network", "WorldSession::SendTrainerList - trainer spells not found for trainer %s id %d", npc->GetGUID().ToString().c_str(), trainerId);
        return;
    }
...
{
    FC_LOG_DEBUG("network", "WORLD: Received CMSG_TRAINER_BUY_SPELL %s, learn spell id is: %u", packet.TrainerGUID.ToString().c_str(), packet.SpellID);

    Creature* npc = GetPlayer()->GetNPCIfCanInteractWith(packet.TrainerGUID, UNIT_NPC_FLAG_TRAINER);
    if (!npc)
    {
        FC_LOG_DEBUG("network", "WORLD: HandleTrainerBuySpellOpcode - %s not found or you can not interact with him.", packet.TrainerGUID.ToString().c_str());
        return;
    }
...
void WorldSession::HandleGossipHelloOpcode(WorldPackets::NPC::Hello& packet)
{
    ...
    if (!unit)
    {
        FC_LOG_DEBUG("network", "WORLD: HandleGossipHelloOpcode - %s not found or you can not interact with him.", packet.Unit.ToString().c_str());
        return;
    }
...
{
    FC_LOG_DEBUG("network", "WORLD: CMSG_SPIRIT_HEALER_ACTIVATE");
    ...
    if (!unit)
    {
        FC_LOG_DEBUG("network", "WORLD: HandleSpiritHealerActivateOpcode - %s not found or you can not interact with him.", guid.ToString().c_str());
        return;
    }
...
void WorldSession::HandleBinderActivateOpcode(WorldPackets::NPC::Hello& packet)
{
    ...
    if (!unit)
    {
        FC_LOG_DEBUG("network", "WORLD: HandleBinderActivateOpcode - %s not found or you can not interact with him.", packet.Unit.ToString().c_str());
        return;
    }
...
{
    FC_LOG_DEBUG("network", "WORLD: Recv MSG_LIST_STABLED_PETS");
    ObjectGuid npcGUID;
...
{
    FC_LOG_DEBUG("network", "WORLD: Recv CMSG_STABLE_PET");
    ObjectGuid guid;
    uint32 petId;
    uint8 new_slot;
...
{
    FC_LOG_DEBUG("network", "HandleStableRevivePet: Not implemented");
}

void WorldSession::HandleRepairItemOpcode(WorldPacket& recvData)
{
    FC_LOG_DEBUG("network", "WORLD: CMSG_REPAIR_ITEM");
    ...
    if (!unit)
    {
        FC_LOG_DEBUG("network", "WORLD: HandleRepairItemOpcode - %s not found or you can not interact with him.", npcGUID.ToString().c_str());
        return;
    }
    ...
    if (itemGUID)
    {
        FC_LOG_DEBUG("network", "ITEM: Repair %s, at %s", itemGUID.ToString().c_str(), npcGUID.ToString().c_str());
    ...
    {
        FC_LOG_DEBUG("network", "ITEM: Repair all items at %s", npcGUID.ToString().c_str());
