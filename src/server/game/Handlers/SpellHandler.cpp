...
{
    FC_LOG_DEBUG("network", "WORLD: CMSG_OPEN_ITEM packet, data length = %i", (uint32)recvPacket.size());
    ...
    FC_LOG_INFO("network", "bagIndex: %u, slot: %u", bagIndex, slot);
    ...
    if (!(proto->GetFlags() & ITEM_FLAG_HAS_LOOT) && !item->HasFlag(ITEM_FIELD_FLAGS, ITEM_FIELD_FLAG_WRAPPED))
    {
        ...
        FC_LOG_INFO("entities.player.cheat", "Possible hacking attempt: Player %s [guid: %u] tried to open item [guid: %u, entry: %u] which is not openable!",
    ...
    if (lockId)
    {
        ...
        if (!lockInfo)
        {
            ...
            FC_LOG_ERROR("network", "WORLD::OpenItem: item [guid = %u] has an unknown lockId: %u!", item->GetGUID().GetCounter(), lockId);
            return;
        }
...
void WorldSession::HandleOpenWrappedItemCallback(uint16 pos, ObjectGuid itemGuid, PreparedQueryResult result)
{
    ...
    if (!result)
    {
        FC_LOG_ERROR("network", "Wrapped item %u don't have record in character_gifts table and will deleted", itemGuid.GetCounter());
...
void WorldSession::HandleGameObjectUseOpcode(WorldPacket& recvData)
{
    ...
    FC_LOG_DEBUG("network", "WORLD: Recvd CMSG_GAMEOBJ_USE Message [%s]", guid.ToString().c_str());
...
void WorldSession::HandleGameobjectReportUse(WorldPacket& recvPacket)
{
    ...
    FC_LOG_DEBUG("network", "WORLD: Recvd CMSG_GAMEOBJ_REPORT_USE Message [%s]", guid.ToString().c_str());
...
void WorldSession::HandlePetCancelAuraOpcode(WorldPacket& recvPacket)
{
    ...
    if (!spellInfo)
    {
        FC_LOG_ERROR("network", "WORLD: unknown PET spell id %u", spellId);
        return;
    }
    ...
    if (!pet)
    {
        FC_LOG_ERROR("network", "HandlePetCancelAura: Attempt to cancel an aura for non-existant %s by player '%s'", guid.ToString().c_str(), GetPlayer()->GetName().c_str());
        return;
    }

    if (pet != GetPlayer()->GetGuardianPet() && pet != GetPlayer()->GetCharmed())
    {
        FC_LOG_ERROR("network", "HandlePetCancelAura: %s is not a pet of player '%s'", guid.ToString().c_str(), GetPlayer()->GetName().c_str());
        return;
    }
...
{
    FC_LOG_DEBUG("network", "WORLD: CMSG_SELF_RES");                  // empty opcode
...
{
    FC_LOG_DEBUG("network", "WORLD: CMSG_GET_MIRRORIMAGE_DATA");
    ObjectGuid guid;
    recvData >> guid;
...
{
    FC_LOG_DEBUG("network", "WORLD: CMSG_UPDATE_PROJECTILE_POSITION");
