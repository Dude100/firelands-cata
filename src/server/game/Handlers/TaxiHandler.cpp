...
{
    FC_LOG_DEBUG("network", "WORLD: Received CMSG_TAXINODE_STATUS_QUERY");
...
void WorldSession::SendTaxiStatus(ObjectGuid guid)
{
    ...
    {
        FC_LOG_DEBUG("network", "WorldSession::SendTaxiStatus - %s not found.", guid.ToString().c_str());
        return;
    }
    ...
    SendPacket(&data);
    FC_LOG_DEBUG("network", "WORLD: Sent SMSG_TAXINODE_STATUS");
}

void WorldSession::HandleTaxiQueryAvailableNodes(WorldPacket& recvData)
{
    FC_LOG_DEBUG("network", "WORLD: Received CMSG_TAXIQUERYAVAILABLENODES");
    ...
    if (!unit)
    {
        FC_LOG_DEBUG("network", "WORLD: HandleTaxiQueryAvailableNodes - %s not found or you can't interact with him.", guid.ToString().c_str());
        return;
    }
...
void WorldSession::SendTaxiMenu(Creature* unit)
{
    ...
    FC_LOG_DEBUG("network", "WORLD: CMSG_TAXINODE_STATUS_QUERY %u ", curloc);
    ...
    FC_LOG_DEBUG("network", "WORLD: Sent SMSG_SHOWTAXINODES");
...
{
    FC_LOG_DEBUG("network", "WORLD: Received CMSG_ACTIVATETAXIEXPRESS");
    ...
    if (!npc)
    {
        FC_LOG_DEBUG("network", "WORLD: HandleActivateTaxiExpressOpcode - %s not found or you can't interact with it.", guid.ToString().c_str());
    ...
    FC_LOG_DEBUG("network", "WORLD: Received CMSG_ACTIVATETAXIEXPRESS from %d to %d", nodes.front(), nodes.back());
...
{
    FC_LOG_DEBUG("network", "WORLD: Received CMSG_MOVE_SPLINE_DONE");
...
{
    FC_LOG_DEBUG("network", "WORLD: Received CMSG_ACTIVATETAXI");
    ...
    FC_LOG_DEBUG("network", "WORLD: Received CMSG_ACTIVATETAXI from %d to %d", nodes[0], nodes[1]);
    Creature* npc = GetPlayer()->GetNPCIfCanInteractWith(guid, UNIT_NPC_FLAG_FLIGHTMASTER);
    if (!npc)
    {
        FC_LOG_DEBUG("network", "WORLD: HandleActivateTaxiOpcode - %s not found or you can't interact with it.", guid.ToString().c_str());
        SendActivateTaxiReply(ERR_TAXITOOFARAWAY);
        return;
    }
...
void WorldSession::SendActivateTaxiReply(ActivateTaxiReply reply)
{
    ...
    FC_LOG_DEBUG("network", "WORLD: Sent SMSG_ACTIVATETAXIREPLY");
}
