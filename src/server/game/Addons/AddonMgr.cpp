...
namespace AddonMgr
{
    ...
    void LoadFromDB()
    {
        ...
        if (result)
        {
            ...
            FC_LOG_INFO("server.loading", ">> Loaded %u known addons in %u ms", count, GetMSTimeDiffToNow(oldMSTime));
        }
        else
            FC_LOG_INFO("server.loading", ">> Loaded 0 known addons. DB table `addons` is empty!");
        ...
        if (result)
        {
            ...
            FC_LOG_INFO("server.loading", ">> Loaded %u banned addons in %u ms", count, GetMSTimeDiffToNow(oldMSTime));
        }
    }
