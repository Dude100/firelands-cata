...
void CharacterCache::LoadCharacterCacheStorage()
{
    ...
    if (!result)
    {
        FC_LOG_INFO("server.loading", "No character name data loaded, empty query");
        return;
    }
    ...
    FC_LOG_INFO("server.loading", "Loaded character infos for " SZFMTD " characters in %u ms", _characterCacheStore.size(), GetMSTimeDiffToNow(oldMSTime));
}
