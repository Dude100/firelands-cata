...
void Guild::SendCommandResult(WorldSession* session, GuildCommandType type, GuildCommandError errCode, std::string const& param)
{
    ...
    FC_LOG_DEBUG("guild", "SMSG_GUILD_COMMAND_RESULT [%s]: Type: %u, code: %u, param: %s", session->GetPlayerInfo().c_str(), type, errCode, param.c_str());
}
...
void Guild::SendSaveEmblemResult(WorldSession* session, GuildEmblemError errCode)
{
    ...
    FC_LOG_DEBUG("guild", "MSG_SAVE_GUILD_EMBLEM [%s] Code: %u", session->GetPlayerInfo().c_str(), errCode);
}
...
void Guild::RankInfo::CreateMissingTabsIfNeeded(uint8 tabs, CharacterDatabaseTransaction& trans, bool logOnCreate /* = false */)
{
    for (uint8 i = 0; i < tabs; ++i)
    {
        ...
        if (logOnCreate)
            FC_LOG_ERROR("guild", "Guild %u has broken Tab %u for rank %u. Created default tab.", m_guildId, i, m_rankId);
...
bool Guild::BankTab::LoadItemFromDB(Field* fields)
{
    ...
    {
        FC_LOG_ERROR("guild", "Invalid slot for item (GUID: %u, id: %u) in guild bank, skipped.", itemGuid, itemEntry);
        return false;
    }
    ...
    if (!proto)
    {
        FC_LOG_ERROR("guild", "Unknown item (GUID: %u, id: %u) in guild bank, skipped.", itemGuid, itemEntry);
        return false;
    }
    ...
    {
        FC_LOG_ERROR("guild", "Item (GUID %u, id: %u) not found in item_instance, deleting from guild bank!", itemGuid, itemEntry);
...
void Guild::BankTab::SendText(Guild const* guild, WorldSession* session) const
{
    ...
    if (session)
    {
        FC_LOG_DEBUG("guild", "SMSG_GUILD_BANK_QUERY_TEXT_RESULT [%s]: Tabid: %u, Text: %s"
            , session->GetPlayerInfo().c_str(), m_tabId, m_text.c_str());
        session->SendPacket(textQuery.Write());
    }
    else
    {
        FC_LOG_DEBUG("guild", "SMSG_GUILD_BANK_QUERY_TEXT_RESULT [Broadcast]: Tabid: %u, Text: %s", m_tabId, m_text.c_str());
...
bool Guild::Member::LoadFromDB(Field* fields)
{
    ...
    if (!m_zoneId)
    {
        FC_LOG_DEBUG("guild", "%s has broken zone-data", m_guid.ToString().c_str());
...
bool Guild::Member::CheckStats() const
{
    if (m_level < 1)
    {
        FC_LOG_ERROR("guild", "%s has a broken data in field `characters`.`level`, deleting him from guild!", m_guid.ToString().c_str());
        return false;
    }

    if (m_class < CLASS_WARRIOR || m_class >= MAX_CLASSES)
    {
        FC_LOG_ERROR("guild", "%s has a broken data in field `characters`.`class`, deleting him from guild!", m_guid.ToString().c_str());
        return false;
    }
    return true;
}
...
Item* Guild::BankMoveItemData::StoreItem(CharacterDatabaseTransaction& trans, Item* pItem)
{
    ...
    for (auto itr = m_vec.begin(); itr != m_vec.end(); )
    {
        ...
        FC_LOG_DEBUG("guild", "GUILD STORAGE: StoreItem tab = %u, slot = %u, item = %u, count = %u",
...
{
    FC_LOG_DEBUG("guild", "GUILD STORAGE: CanStore() tab = %u, slot = %u, item = %u, count = %u",
        m_container, m_slotId, pItem->GetEntry(), pItem->GetCount());
...
bool Guild::Create(Player* pLeader, std::string const& name)
{
    ...
    FC_LOG_DEBUG("guild", "GUILD: creating guild [%s] for leader %s (%u)",
...
void Guild::UpdateMemberData(Player* player, uint8 dataid, uint32 value)
{
    if (Member* member = GetMember(player->GetGUID()))
    {
        ...
        case GUILD_MEMBER_DATA_PROFESSIONS:
            ...
            break;
        default:
            FC_LOG_ERROR("guild", "Guild::UpdateMemberData: Called with incorrect DATAID %u (value %u)", dataid, value);
            return;
        }
        //HandleRoster();
    }
...
void Guild::HandleRoster(WorldSession* session)
{
    ...
    FC_LOG_DEBUG("guild", "SMSG_GUILD_ROSTER [%s]", session->GetPlayerInfo().c_str());
...
void Guild::SendQueryResponse(WorldSession* session)
{
    ...
    FC_LOG_DEBUG("guild", "SMSG_QUERY_GUILD_INFO_RESPONSE [%s]", session->GetPlayerInfo().c_str());
}
...
void Guild::SendGuildRankInfo(WorldSession* session) const
{
    ...
    FC_LOG_DEBUG("guild", "SMSG_GUILD_RANKS [%s]", session->GetPlayerInfo().c_str());
}
...
void Guild::HandleSetBankTabInfo(WorldSession* session, uint8 tabId, std::string const& name, std::string const& icon)
{
    ...
    if (!tab)
    {
        FC_LOG_ERROR("guild", "Guild::HandleSetBankTabInfo: Player %s trying to change bank tab info from unexisting tab %d.",
...
void Guild::HandleSetRankInfo(WorldSession* session, uint8 rankId, std::string const& name, uint32 rights, uint32 moneyPerDay, GuildBankRightsAndSlotsVec const& rightsAndSlots)
{
    ...
    {
        FC_LOG_DEBUG("guild", "Changed RankName to '%s', rights to 0x%08X", name.c_str(), rights);
...
void Guild::HandleInviteMember(WorldSession* session, std::string const& name)
{
    ...
    FC_LOG_DEBUG("guild", "Player %s invited %s to join his Guild", player->GetName().c_str(), name.c_str());
    ...
    FC_LOG_DEBUG("guild", "SMSG_GUILD_INVITE [%s]", pInvitee->GetName().c_str());
}
...
void Guild::HandleDisband(WorldSession* session)
{
    ...
    {
        Disband();
        FC_LOG_DEBUG("guild", "Guild Successfully Disbanded");
        delete this;
    }
}
...
void Guild::HandleGuildPartyRequest(WorldSession* session)
{
    ...
    session->SendPacket(&data);
    FC_LOG_DEBUG("guild", "SMSG_GUILD_PARTY_STATE_RESPONSE [%s]", session->GetPlayerInfo().c_str());
}
...
void Guild::SendEventLog(WorldSession* session) const
{
    ...
    FC_LOG_DEBUG("guild", "SMSG_GUILD_EVENT_LOG_QUERY_RESULT [%s]", session->GetPlayerInfo().c_str());
}
...
void Guild::SendNewsUpdate(WorldSession* session)
{
    ...
    session->SendPacket(packet.Write());
    FC_LOG_DEBUG("guild", "SMSG_GUILD_NEWS_UPDATE [%s]", session->GetPlayerInfo().c_str());
}
...
void Guild::SendBankLog(WorldSession* session, uint8 tabId) const
{
    ...
    if (tabId < _GetPurchasedTabsSize() || tabId == GUILD_BANK_MAX_TABS)
    {
        ...
        FC_LOG_DEBUG("guild", "SMSG_GUILD_BANK_LOG_QUERY_RESULT [%s] TabId: %u", session->GetPlayerInfo().c_str(), tabId);
    }
}
...
void Guild::SendPermissions(WorldSession* session) const
{
    ...
    session->SendPacket(&data);
    FC_LOG_DEBUG("guild", "SMSG_GUILD_PERMISSIONS_QUERY_RESULTS [%s] Rank: %u", session->GetPlayerInfo().c_str(), rankId);
}
...
void Guild::SendMoneyInfo(WorldSession* session) const
{
    ...
    data << int64(amount);
    session->SendPacket(&data);
    FC_LOG_DEBUG("guild", "SMSG_GUILD_BANK_MONEY_WITHDRAWN [%s] Money: " SI64FMTD, session->GetPlayerInfo().c_str(), amount);
}
...
void Guild::SendLoginInfo(WorldSession* session)
{
    ...
    */
    ...
    FC_LOG_DEBUG("guild", "SMSG_GUILD_EVENT [%s] MOTD", session->GetPlayerInfo().c_str());
    ...
    if (dbTabId < _GetPurchasedTabsSize() || isMoneyTab)
    {
        ...
        if (pLog->CanInsert())
        {
            ...
            if (BankEventLogEntry::IsMoneyEvent(eventType))
            {
                if (!isMoneyTab)
                {
                    FC_LOG_ERROR("guild", "GuildBankEventLog ERROR: MoneyEvent(LogGuid: %u, Guild: %u) does not belong to money tab (%u), ignoring...", guid, m_id, dbTabId);
                    return false;
                }
            }
            else if (isMoneyTab)
            {
                FC_LOG_ERROR("guild", "GuildBankEventLog ERROR: non-money event (LogGuid: %u, Guild: %u) belongs to money tab, ignoring...", guid, m_id);
                return false;
            }
...
void Guild::LoadBankTabFromDB(Field* fields)
{
    ...
    if (tabId >= _GetPurchasedTabsSize())
        FC_LOG_ERROR("guild", "Invalid tab (tabId: %u) in guild bank, skipped.", tabId);
    else
...
bool Guild::LoadBankItemFromDB(Field* fields)
{
    ...
    if (tabId >= _GetPurchasedTabsSize())
    {
        FC_LOG_ERROR("guild", "Invalid tab for item (GUID: %u, id: #%u) in guild bank, skipped.",
...
bool Guild::Validate()
{
    ...
    {
        FC_LOG_ERROR("guild", "Guild %u has invalid number of ranks, creating new...", m_id);
        broken_ranks = true;
    }
    else
    {
        for (uint8 rankId = 0; rankId < ranks; ++rankId)
        {
            ...
            {
                FC_LOG_ERROR("guild", "Guild %u has broken rank id %u, creating default set of ranks...", m_id, rankId);
                broken_ranks = true;
            }
            else
...
void Guild::_CreateDefaultGuildRanks(CharacterDatabaseTransaction& trans, LocaleConstant loc)
{
    ...
    _CreateRank(trans, sObjectMgr->GetFirelandsString(LANG_GUILD_MASTER, loc), GR_RIGHT_ALL);
    _CreateRank(trans, sObjectMgr->GetFirelandsString(LANG_GUILD_OFFICER, loc), GR_RIGHT_ALL);
    _CreateRank(trans, sObjectMgr->GetFirelandsString(LANG_GUILD_VETERAN, loc), GR_RIGHT_GCHATLISTEN | GR_RIGHT_GCHATSPEAK);
    _CreateRank(trans, sObjectMgr->GetFirelandsString(LANG_GUILD_MEMBER, loc), GR_RIGHT_GCHATLISTEN | GR_RIGHT_GCHATSPEAK);
    _CreateRank(trans, sObjectMgr->GetFirelandsString(LANG_GUILD_INITIATE, loc), GR_RIGHT_GCHATLISTEN | GR_RIGHT_GCHATSPEAK);
}
...
void Guild::_MoveItems(MoveItemData* pSrc, MoveItemData* pDest, uint32 splitedAmount)
{
    ...
    {
        FC_LOG_FATAL("guild", "Guild::SwapItems: Player %s(GUIDLow: %u) tried to move item %u from tab %u slot %u to tab %u slot %u, but item %u has a stack of zero!",
...
void Guild::_BroadcastEvent(GuildEvents guildEvent, ObjectGuid guid, char const* param1, char const* param2, char const* param3) const
{
    ...
    if (sLog->ShouldLog("guild", LOG_LEVEL_DEBUG))
        FC_LOG_DEBUG("guild", "SMSG_GUILD_EVENT [Broadcast] Event: %s (%u)", _GetGuildEventString(guildEvent).c_str(), guildEvent);
}
...
void Guild::SendBankList(WorldSession* session, uint8 tabId, bool fullUpdate) const
{
    ...
    FC_LOG_DEBUG("guild", "WORLD: Sent (SMSG_GUILD_BANK_LIST)");
}
...
void Guild::SendGuildRanksUpdate(ObjectGuid setterGuid, ObjectGuid targetGuid, uint32 rank)
{
    ...
    FC_LOG_DEBUG("network", "SMSG_GUILD_RANKS_UPDATE [Broadcast] Target: %s, Issuer: %s, RankId: %u",
...
void Guild::SendGuildReputationWeeklyCap(WorldSession* session, uint32 reputation) const
{
    ...
    packet.Cap = cap;
    session->SendPacket(packet.Write());
    FC_LOG_DEBUG("guild", "SMSG_GUILD_REPUTATION_WEEKLY_CAP [%s]: Left: %u",
...
void Guild::HandleNewsSetSticky(WorldSession* session, uint32 newsId, bool sticky)
{
    ...
    if (itr == logs->end())
    {
        FC_LOG_DEBUG("guild", "HandleNewsSetSticky: [%s] requested unknown newsId %u - Sticky: %u",
    ...
    FC_LOG_DEBUG("guild", "HandleNewsSetSticky: [%s] chenged newsId %u sticky to %u",
        session->GetPlayerInfo().c_str(), newsId, sticky);
