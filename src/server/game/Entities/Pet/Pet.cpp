...
bool Pet::LoadPetData(Player* owner, uint32 petEntry, uint32 petnumber, bool current)
{
    ...
    if (IsCritter())
    {
        ...
        if (!IsPositionValid())
        {
            FC_LOG_ERROR("entities.pet", "Pet (guidlow %d, entry %d) not loaded. Suggested coordinates isn't valid (X: %f Y: %f)",
    ...
    default:
        if (!IsPetGhoul())
            FC_LOG_ERROR("entities.pet", "Pet have incorrect type (%u) for pet loading.", getPetType());
        break;
    }
    ...
    FC_LOG_DEBUG("entities.pet", "New Pet has guid %u", GetGUID().GetCounter());
...
void Pet::SavePetToDB(PetSaveMode mode)
{
    ...
    if (mode >= PET_SAVE_CURRENT_STATE)
    {
        ...
        if (m_petSlot > PET_SLOT_LAST)
            FC_LOG_ERROR("sql.sql", "Pet::SavePetToDB: bad slot %u for pet %u!", m_petSlot, petId);
...
void Pet::Update(uint32 diff)
{
    ...
    case ALIVE:
    {
        ...
        if (isControlled())
        {
            ...
            {
                FC_LOG_ERROR("entities.pet", "Pet %u is not pet of owner %s, removed", GetEntry(), GetOwner()->GetName().c_str());
...
bool Pet::CreateBaseAtCreature(Creature* creature)
{
    ...
    if (!IsPositionValid())
    {
        FC_LOG_ERROR("entities.pet", "Pet (guidlow %d, entry %d) not created base at creature. Suggested coordinates isn't valid (X: %f Y: %f)",
            GetGUID().GetCounter(), GetEntry(), GetPositionX(), GetPositionY());
        return false;
    }
    ...
    if (!cinfo)
    {
        FC_LOG_ERROR("entities.pet", "CreateBaseAtCreature() failed, creatureInfo is missing!");
        return false;
    }
...
{
    FC_LOG_DEBUG("entities.pet", "Pet::CreateBaseForTamed");
    ObjectGuid::LowType guid = map->GenerateLowGuid<HighGuid::Pet>();
...
bool Guardian::InitStatsForLevel(uint8 petlevel)
{
    ...
    if (IsPet() && GetOwner()->GetTypeId() == TYPEID_PLAYER)
    {
        ...
        case CLASS_HUNTER:
            ...
            break;
        default:
            FC_LOG_ERROR("entities.pet", "Unknown type pet %u is summoned by player class %u",
...
{
    FC_LOG_DEBUG("entities.pet", "Loading auras for pet %u", GetGUID().GetCounter());
    ...
    if (result)
    {
        do
        {
            ...
            if (!spellInfo)
            {
                FC_LOG_ERROR("entities.pet", "Unknown aura (spellid %u), ignore.", spellid);
                continue;
            }
            ...
            if (Aura* aura = Aura::TryCreate(spellInfo, effmask, this, nullptr, &baseDamage[0], nullptr, caster_guid))
            {
                ...
                FC_LOG_DEBUG("entities.pet", "Added aura spellid %u, effectmask %u", spellInfo->Id, effmask);
            }
        } while (result->NextRow());
    }
}
...
bool Pet::addSpell(uint32 spellId, ActiveStates active /*= ACT_DECIDE*/, PetSpellState state /*= PETSPELL_NEW*/, PetSpellType type /*= PETSPELL_NORMAL*/)
{
    ...
    if (!spellInfo)
    {
        ...
        {
            FC_LOG_ERROR("entities.pet", "Pet::addSpell: Non-existed in SpellStore spell #%u request, deleting for all pets in `pet_spell`.", spellId);
            ...
            FC_LOG_ERROR("entities.pet", "Pet::addSpell: Non-existed in SpellStore spell #%u request.", spellId);
