...
void AddItemsSetItem(Player* player, Item* item)
{
    ...
    if (!set)
    {
        FC_LOG_ERROR("sql.sql", "Item set %u for item (id %u) not found, mods not applied.", setid, proto->GetId());
        return;
    }
    ...
    for (uint32 x = 0; x < MAX_ITEM_SET_SPELLS; ++x)
    {
        ...
        for (uint32 y = 0; y < MAX_ITEM_SET_SPELLS; ++y)
        {
            if (!eff->spells[y])                             // free slot
            {
                ...
                if (!spellInfo)
                {
                    FC_LOG_ERROR("entities.player.items", "WORLD: unknown spell id %u in items set %u effects", set->SetSpellID[x], setid);
                    break;
                }
...
void RemoveItemsSetItem(Player* player, ItemTemplate const* proto)
{
    ...
    if (!set)
    {
        FC_LOG_ERROR("sql.sql", "Item set #%u for item #%u not found, mods not removed.", setid, proto->GetId());
        return;
    }
...
void Item::UpdateDuration(Player* owner, uint32 diff)
{
    ...
    FC_LOG_DEBUG("entities.player.items", "Item::UpdateDuration Item (Entry: %u Duration %u Diff %u)", GetEntry(), GetUInt32Value(ITEM_FIELD_DURATION), diff);
...
void AddItemToUpdateQueueOf(Item* item, Player* player)
{
    ...
    {
        FC_LOG_DEBUG("entities.player.items", "AddItemToUpdateQueueOf - Owner's guid (%s) and player's guid (%s) don't match!",
...
void RemoveItemFromUpdateQueueOf(Item* item, Player* player)
{
    ...
    if (player->GetGUID() != item->GetOwnerGUID())
    {
        FC_LOG_DEBUG("entities.player.items", "RemoveItemFromUpdateQueueOf - Owner's guid (%s) and player's guid (%s) don't match!",
            item->GetOwnerGUID().ToString().c_str(), player->GetGUID().ToString().c_str());
        return;
    }
