    ...
    if (result)
    {
        ...
        do
        {
            ...
            case ItemRandomEnchantmentType::Property:
                ...
                {
                    FC_LOG_ERROR("sql.sql", "Property %u used in `item_enchantment_template` by entry %u doesn't have exist in ItemRandomProperties.db2", ench, entry);
                    continue;
                }
                break;
            case ItemRandomEnchantmentType::Suffix:
                ...
                {
                    FC_LOG_ERROR("sql.sql", "Suffix %u used in `item_enchantment_template` by entry %u doesn't have exist in ItemRandomSuffix.db2", ench, entry);
                    continue;
                }
                break;
            default:
                FC_LOG_ERROR("sql.sql", "Invalid random enchantment type specified in `item_enchantment_template` table for `entry` %u `ench` %u", entry, ench);
                break;
            }

            if (chance < 0.000001f || chance > 100.0f)
            {
                FC_LOG_ERROR("sql.sql", "Random item enchantment for entry %u type %u ench %u has invalid chance %f", entry, uint32(type), ench, chance);
                continue;
            }
        ...
        FC_LOG_INFO("server.loading", ">> Loaded %u Item Enchantment definitions in %u ms", count, GetMSTimeDiffToNow(oldMSTime));
    }
    else
        FC_LOG_ERROR("server.loading", ">> Loaded 0 Item Enchantment definitions. DB table `item_enchantment_template` is empty.");
}
...
ItemRandomEnchantmentId GetItemEnchantMod(int32 entry, ItemRandomEnchantmentType type)
{
    ...
    {
        FC_LOG_ERROR("sql.sql", "Item RandomProperty / RandomSuffix id #%u used in `item_template` but it does not have records in `item_enchantment_template` table.", entry);
        return{};
    }

    auto selectedItr = Firelands::Containers::SelectRandomWeightedContainerElement(tab->second, [](EnchStoreItem const& enchant)
        {
...
ItemRandomEnchantmentId GenerateItemRandomPropertyId(uint32 item_id)
{
    ...
    if (itemProto->GetRandomProperty() && itemProto->GetRandomSuffix())
    {
        FC_LOG_ERROR("sql.sql", "Item template %u have RandomProperty == %u and RandomSuffix == %u, but must have one from field =0", itemProto->GetId(), itemProto->GetRandomProperty(), itemProto->GetRandomSuffix());
        return{};
    }
