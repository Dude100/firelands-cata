...
void FormationMgr::AddCreatureToGroup(uint32 leaderGuid, Creature* creature)
{
    ...
    {
        FC_LOG_DEBUG("entities.unit", "Group found: %u, inserting creature GUID: %u, Group InstanceID %u", leaderGuid, creature->GetGUID().GetCounter(), creature->GetInstanceId());
        itr->second->AddMember(creature);
    }
    //Create new group
    else
    {
        FC_LOG_DEBUG("entities.unit", "Group not found: %u. Creating new group.", leaderGuid);
...
{
    FC_LOG_DEBUG("entities.unit", "Deleting member pointer to GUID: %u from group %u", group->GetId(), member->GetSpawnId());
    ...
    if (group->isEmpty())
    {
        ...
        FC_LOG_DEBUG("entities.unit", "Deleting group with InstanceID %u", member->GetInstanceId());
...
void FormationMgr::LoadCreatureFormations()
{
    ...
    if (!result)
    {
        FC_LOG_ERROR("server.loading", ">>  Loaded 0 creatures in formations. DB table `creature_formations` is empty!");
        return;
    }
    ...
    do
    {
        ...
        // check data correctness
        {
            ...
            {
                FC_LOG_ERROR("sql.sql", "creature_formations table leader guid %u incorrect (not exist)", group_member.LeaderGUID);
                continue;
            }

            if (!sObjectMgr->GetCreatureData(memberGUID))
            {
                FC_LOG_ERROR("sql.sql", "creature_formations table member guid %u incorrect (not exist)", memberGUID);
                continue;
            }
        }
    ...
    FC_LOG_INFO("server.loading", ">> Loaded %u creatures in formations in %u ms", count, GetMSTimeDiffToNow(oldMSTime));
}

void CreatureGroup::AddMember(Creature* member)
{
    FC_LOG_DEBUG("entities.unit", "CreatureGroup::AddMember: Adding unit GUID: %u.", member->GetGUID().GetCounter());

    //Check if it is a leader
    if (member->GetSpawnId() == m_groupID)
    {
        FC_LOG_DEBUG("entities.unit", "Unit GUID: %u is formation leader. Adding group.", member->GetGUID().GetCounter());
        m_leader = member;
    }
...
void CreatureGroup::FormationReset(bool dismiss)
{
    for (CreatureGroupMemberType::iterator itr = m_members.begin(); itr != m_members.end(); ++itr)
    {
        if (itr->first != m_leader && itr->first->IsAlive())
        {
            ...
            FC_LOG_DEBUG("entities.unit", "Set %s movement for member GUID: %u", dismiss ? "default" : "idle", itr->first->GetGUID().GetCounter());
        }
    }
    m_Formed = !dismiss;
}
