...
void Creature::AddToWorld()
{
    ...
    if (!IsInWorld())
    {
        ...
        FC_LOG_DEBUG("entities.unit", "Adding creature %u with entry %u and DBGUID %u to world in map %u", GetGUID().GetCounter(), GetEntry(), m_spawnId, GetMap()->GetId());
...
void Creature::RemoveFromWorld()
{
    if (IsInWorld())
    {
        ...
        if (m_spawnId)
            Firelands::Containers::MultimapErasePair(GetMap()->GetCreatureBySpawnIdStore(), m_spawnId, this);

        FC_LOG_DEBUG("entities.unit", "Removing creature %u with entry %u and DBGUID %u to world in map %u", GetGUID().GetCounter(), GetEntry(), m_spawnId, GetMap()->GetId());
...
bool Creature::InitEntry(uint32 entry, CreatureData const* data /*= nullptr*/)
{
    ...
    if (!normalInfo)
    {
        FC_LOG_ERROR("sql.sql", "Creature::InitEntry creature entry %u does not exist.", entry);
        return false;
    }
    ...
    {
        FC_LOG_ERROR("sql.sql", "Creature (Entry: %u) has no model defined in table `creature_template`, can't load. ", entry);
        return false;
    }
    ...
    {
        FC_LOG_ERROR("sql.sql", "Creature (Entry: %u) has invalid model %u defined in table `creature_template`, can't load.", entry, displayID);
        return false;
    }
...
void Creature::Update(uint32 diff)
{
    ...
    case JUST_RESPAWNED:
        ...
        FC_LOG_ERROR("entities.unit", "Creature (GUID: %u Entry: %u) in wrong state: JUST_RESPAWNED (4)", GetGUID().GetCounter(), GetEntry());
        break;
    case JUST_DIED:
        // Must not be called, see Creature::setDeathState JUST_DIED -> CORPSE promoting.
        FC_LOG_ERROR("entities.unit", "Creature (GUID: %u Entry: %u) in wrong state: JUST_DIED (1)", GetGUID().GetCounter(), GetEntry());
        break;
    case DEAD:
    {
        if (!m_respawnCompatibilityMode)
        {
            FC_LOG_ERROR("entities.unit", "Creature (GUID: %u Entry: %u) in wrong state: DEAD (3)", GetGUID().GetCounter(), GetEntry());
            break;
        }
    ...
    case CORPSE:
    {
        ...
        {
            RemoveCorpse(false);
            FC_LOG_DEBUG("entities.unit", "Removing corpse... %u ", GetUInt32Value(OBJECT_FIELD_ENTRY));
        }
        break;
    }
    case ALIVE:
...
void Creature::DoFleeToGetAssistance()
{
    ...
    if (radius > 0)
    {
        ...
        Firelands::NearestAssistCreatureInCreatureRangeCheck u_check(this, GetVictim(), radius);
        Firelands::CreatureLastSearcher<Firelands::NearestAssistCreatureInCreatureRangeCheck> searcher(this, creature, u_check);
...
bool Creature::Create(ObjectGuid::LowType guidlow, Map* map, uint32 entry, Position const& pos, CreatureData const* data /*= nullptr*/, uint32 vehId /*= 0*/, bool dynamic)
{
    ...
    if (!cinfo)
    {
        FC_LOG_ERROR("sql.sql", "Creature::Create(): creature template (guidlow: %u, entry: %u) does not exist.", guidlow, entry);
        return false;
    }
    ...
    if (!IsPositionValid())
    {
        FC_LOG_ERROR("entities.unit", "Creature::Create(): given coordinates for creature (guidlow %d, entry %d) are not valid (X: %f, Y: %f, Z: %f, O: %f)", guidlow, entry, pos.GetPositionX(), pos.GetPositionY(), pos.GetPositionZ(), pos.GetOrientation());
        return false;
    }
...
void Creature::SaveToDB()
{
    ...
    if (!data)
    {
        FC_LOG_ERROR("entities.unit", "Creature::SaveToDB failed, cannot get creature data!");
        return;
    }
...
bool Creature::CreateFromProto(ObjectGuid::LowType guidlow, uint32 entry, CreatureData const* data /*= nullptr*/, uint32 vehId /*= 0*/)
{
    ...
    if (!cinfo)
    {
        FC_LOG_ERROR("sql.sql", "Creature::CreateFromProto(): creature template (guidlow: %u, entry: %u) does not exist.", guidlow, entry);
        return false;
    }
...
bool Creature::LoadFromDB(ObjectGuid::LowType spawnId, Map* map, bool addToMap, bool allowDuplicate)
{
    if (!allowDuplicate)
    {
        ...
        if (creatureBounds.first != creatureBounds.second)
        {
            for (auto itr = creatureBounds.first; itr != creatureBounds.second; ++itr)
            {
                ...
                {
                    FC_LOG_DEBUG("maps", "Would have spawned %u but %s already exists", spawnId, creatureBounds.first->second->GetGUID().ToString().c_str());
                    return false;
                }
                else
                {
                    despawnList.push_back(itr->second);
                    FC_LOG_DEBUG("maps", "Despawned dead instance of spawn %u (%s)", spawnId, itr->second->GetGUID().ToString().c_str());
                }
            }
    ...
    if (!data)
    {
        FC_LOG_ERROR("sql.sql", "Creature (SpawnID %u) not found in table `creature`, can't load. ", spawnId);
        return false;
    }
    ...
    if (!m_respawnTime && !map->IsSpawnGroupActive(data->spawnGroupData->groupId))
    {
        if (!m_respawnCompatibilityMode)
        {
            ...
            {
                FC_LOG_ERROR("entities.unit", "Creature (SpawnID %u) trying to load in inactive spawn group '%s':\n", spawnId, data->spawnGroupData->name.c_str());
                return false;
            }
        }
    ...
    if (m_respawnTime)
    {
        if (!m_respawnCompatibilityMode)
        {
            ...
            if (!sPoolMgr->IsPartOfAPool<Creature>(spawnId))
            {
                FC_LOG_ERROR("entities.unit", "Creature (SpawnID %u) trying to load despite a respawn timer in progress:\n", spawnId);
                return false;
            }
        }
        else
        ...
        if (CanFly())
        {
            ...
            if (data->spawnPoint.GetPositionZ() - tz > 0.1f && Firelands::IsValidMapCoord(tz))
...
/*static*/ bool Creature::DeleteFromDB(ObjectGuid::LowType spawnId)
{
    ...
    sMapMgr->DoForAllMapsWithMapId(data->mapId,
        [spawnId, trans](Map* map) -> void
        {
            ...
            for (auto const& pair : Firelands::Containers::MapEqualRange(map->GetCreatureBySpawnIdStore(), spawnId))
...
{
    if (Firelands::XP::GetColorCode(playerLevel, creatureLevel) != XP_GRAY)
        return false;
...
void Creature::Respawn(bool force)
{
    ...
    if (m_respawnCompatibilityMode)
    {
        ...
        {
            FC_LOG_DEBUG("entities.unit", "Respawning creature %s (%s)", GetName().c_str(), GetGUID().ToString().c_str());
            m_respawnTime = 0;
    ...
    FC_LOG_DEBUG("entities.unit", "Respawning creature %s (%s)",
...
SpellInfo const* Creature::reachWithSpellAttack(Unit* victim)
{
    ...
    for (uint32 i = 0; i < MAX_CREATURE_SPELLS; ++i)
    {
        ...
        if (!spellInfo)
        {
            FC_LOG_ERROR("entities.unit", "WORLD: unknown spell id %i", m_spells[i]);
            continue;
        }
...
Unit* Creature::SelectNearestTarget(float dist, bool playerOnly /* = false */) const
{
    ...
    Unit* target = nullptr;
    Firelands::NearestHostileUnitCheck u_check(this, dist, playerOnly);
    Firelands::UnitLastSearcher<Firelands::NearestHostileUnitCheck> searcher(this, target, u_check);
...
Unit* Creature::SelectNearestTargetInAttackDistance(float dist) const
{
    ...
    {
        FC_LOG_ERROR("entities.unit", "Creature (GUID: %u Entry: %u) SelectNearestTargetInAttackDistance called with dist > MAX_VISIBILITY_DISTANCE. Distance set to ATTACK_DISTANCE.", GetGUID().GetCounter(), GetEntry());
        dist = ATTACK_DISTANCE;
    }

    Unit* target = nullptr;
    Firelands::NearestHostileUnitInAttackDistanceCheck u_check(this, dist);
    Firelands::UnitLastSearcher<Firelands::NearestHostileUnitInAttackDistanceCheck> searcher(this, target, u_check);
...
void Creature::SendAIReaction(AiReaction reactionType)
{
    ...
    FC_LOG_DEBUG("network", "WORLD: Sent SMSG_AI_REACTION, type %u.", reactionType);
}
...
void Creature::CallAssistance()
{
    if (!m_AlreadyCallAssistance && GetVictim() && !IsPet() && !IsCharmed())
    {
        ...
        if (radius > 0)
        {
            ...
            Firelands::AnyAssistCreatureInRangeCheck u_check(this, GetVictim(), radius);
            Firelands::CreatureListSearcher<Firelands::AnyAssistCreatureInRangeCheck> searcher(this, assistList, u_check);
            Cell::VisitGridObjects(this, searcher, radius);
...
void Creature::CallForHelp(float radius)
{
    ...
    if (!target)
    {
        FC_LOG_ERROR("entities.unit", "Creature %u (%s) trying to call for help without being in combat.", GetEntry(), GetName().c_str());
        return;
    }

    Firelands::CallOfHelpCreatureInRangeDo u_do(this, target, radius);
    Firelands::CreatureWorker<Firelands::CallOfHelpCreatureInRangeDo> worker(this, u_do);
...
void Creature::SaveRespawnTime(uint32 forceDelay)
{
    ...
    GetMap()->SaveRespawnTime(SPAWN_TYPE_CREATURE, m_spawnId, GetOriginalEntry(), thisRespawnTime, Firelands::ComputeGridCoord(GetHomePosition().GetPositionX(), GetHomePosition().GetPositionY()).GetId());
}
...
bool Creature::LoadCreaturesAddon()
{
    ...
    if (!cainfo->auras.empty())
    {
        for (std::vector<uint32>::const_iterator itr = cainfo->auras.begin(); itr != cainfo->auras.end(); ++itr)
        {
            ...
            {
                FC_LOG_ERROR("sql.sql", "Creature (GUID: %u Entry: %u) has wrong spell %u defined in `auras` field.", GetGUID().GetCounter(), GetEntry(), *itr);
                continue;
            }
            ...
            AddAura(*itr, this);
            FC_LOG_DEBUG("entities.unit", "Spell: %u added to creature (GUID: %u Entry: %u)", *itr, GetGUID().GetCounter(), GetEntry());
        }
    }
...
void Creature::SetInCombatWithZone()
{
    if (!CanHaveThreatList())
    {
        FC_LOG_ERROR("entities.unit", "Creature entry %u call SetInCombatWithZone but creature cannot have threat list.", GetEntry());
        return;
    }
    ...
    if (!map->IsDungeon())
    {
        FC_LOG_ERROR("entities.unit", "Creature entry %u call SetInCombatWithZone for map (id: %u) that isn't an instance.", GetEntry(), map->GetId());
        return;
    }
...
void Creature::SetPosition(float x, float y, float z, float o)
{
    ...
    {
        FC_LOG_DEBUG("entities.unit", "Creature::SetPosition(%f, %f, %f) .. bad coordinates!", x, y, z);
        return;
    }
...
Unit* Creature::SelectNearestHostileUnitInAggroRange(bool useLOS) const
{
    ...
    Firelands::NearestHostileUnitInAggroRangeCheck u_check(this, useLOS);
    Firelands::UnitSearcher<Firelands::NearestHostileUnitInAggroRangeCheck> searcher(this, target, u_check);
...
bool Creature::HasSpellFocus(Spell const* focusSpell) const
{
    if (isDead()) // dead creatures cannot focus
    {
        ...
        {
            FC_LOG_WARN("entities.unit", "Creature '%s' (entry %u) has spell focus (spell id %u, delay %ums) despite being dead.",
...
void Creature::ReacquireSpellFocusTarget()
{
    if (!HasSpellFocus())
    {
        FC_LOG_ERROR("entities.unit", "Creature::ReacquireSpellFocusTarget() being alled with HasSpellFocus() return false.");
        return;
    }
...
void Creature::SetTextRepeatId(uint8 textGroup, uint8 id)
{
    ...
    if (std::find(repeats.begin(), repeats.end(), id) == repeats.end())
        ...
        FC_LOG_ERROR("sql.sql", "CreatureTextMgr: TextGroup %u for Creature(%s) GuidLow %u Entry %u, id %u already added", uint32(textGroup), GetName().c_str(), GetGUID().GetCounter(), GetEntry(), uint32(id));
}
