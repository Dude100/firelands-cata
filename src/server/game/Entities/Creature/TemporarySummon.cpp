...
void TempSummon::Update(uint32 diff)
{
    ...
    default:
        UnSummon();
        FC_LOG_ERROR("entities.unit", "Temporary summoned creature (entry: %u) have unknown type %u of ", GetEntry(), m_type);
        break;
    }
}
...
void TempSummon::RemoveFromWorld()
{
    ...
    //if (GetOwnerGUID())
    //    FC_LOG_ERROR("entities.unit", "Unit %u has owner guid when removed from world", GetEntry());
...
void Minion::RemoveFromWorld()
{
    ...
    else if (!IsPet() && !IsHunterPet())
    {
        if (owner->m_Controlled.find(this) != owner->m_Controlled.end())
            ...
            FC_LOG_FATAL("entities.unit", "Minion::RemoveFromWorld: Owner %s tried to remove a non-existing controlled unit %s from controlled unit set.", owner->GetGUID().ToString().c_str(), GetGUID().ToString().c_str());
