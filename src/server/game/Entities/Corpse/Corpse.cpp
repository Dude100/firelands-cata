...
bool Corpse::Create(ObjectGuid::LowType guidlow, Player* owner)
{
    ...
    if (!IsPositionValid())
    {
        FC_LOG_ERROR("entities.player", "Corpse (guidlow %d, owner %s) not created. Suggested coordinates isn't valid (X: %f Y: %f)",
...
bool Corpse::LoadCorpseFromDB(ObjectGuid::LowType guid, Field* fields)
{
    ...
    if (!IsPositionValid())
    {
        FC_LOG_ERROR("entities.player", "Corpse (%s, owner: %s) is not created, given coordinates are not valid (X: %f, Y: %f, Z: %f)",
