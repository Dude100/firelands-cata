...
namespace GameObjectType
{
    ...
    {
    public:
        // @todo: chronofy timer trackers for 4.x
        //static constexpr Milliseconds PositionUpdateInterval = 50ms;
        static constexpr uint8 PositionUpdateInterval = 50;
        ...
        void Update(uint32 diff) override
        {
            ...
            else
            {
                ...
                {
                    int32 timeToStop = _owner.GetInt32Value(GAMEOBJECT_LEVEL) - _stateChangeTime;
        ...
        void OnStateChanged(GOState oldState, GOState newState) override
        {
            ...
            _owner.SetUInt32Value(GAMEOBJECT_LEVEL, GameTime::GetGameTimeMS() + timeToStop);
        ...
        void AddPassenger(WorldObject* passenger) override
        {
            ...
            if (_passengers.insert(passenger).second)
            {
                ...
                FC_LOG_DEBUG("entities.transport", "Object %s boarded transport %s.", passenger->GetName().c_str(), _owner.GetName().c_str());
            }
        }
        ...
        TransportBase* RemovePassenger(WorldObject* passenger) override
        {
            if (_passengers.erase(passenger) > 0)
            {
                ...
                FC_LOG_DEBUG("entities.transport", "Object %s removed from transport %s.", passenger->GetName().c_str(), _owner.GetName().c_str());
    ...
    private:
        ...
        TimeTrackerSmall _positionUpdateTimer;
...
void GameObject::RemoveFromOwner()
{
    ...
    FC_LOG_DEBUG("misc", "Removed GameObject (GUID: %u Entry: %u SpellId: %u LinkedGO: %u) that just lost any reference to the owner (%s) GO list",
...
void GameObject::RemoveFromWorld()
{
    ...
    if (IsInWorld())
    {
        ...
        if (m_spawnId)
            Firelands::Containers::MultimapErasePair(GetMap()->GetGameObjectBySpawnIdStore(), m_spawnId, this);
        GetMap()->GetObjectsStore().Remove<GameObject>(GetGUID());
    }
}

bool GameObject::Create(ObjectGuid::LowType guidlow, uint32 name_id, Map* map, Position const& pos, QuaternionData const& rotation, uint32 animprogress, GOState go_state, uint32 artKit /*= 0*/, bool dynamic, ObjectGuid::LowType spawnid)
{
    ASSERT(map);
    SetMap(map);
    ...
    if (!IsPositionValid())
    {
        FC_LOG_ERROR("misc", "Gameobject (GUID: %u Entry: %u) not created. Suggested coordinates isn't valid (X: %f Y: %f)", guidlow, name_id, pos.GetPositionX(), pos.GetPositionY());
        return false;
    }
    ...
    if (!goinfo)
    {
        FC_LOG_ERROR("sql.sql", "Gameobject (GUID: %u Entry: %u) not created: non-existing entry in `gameobject_template`. Map: %u (X: %f Y: %f Z: %f)", guidlow, name_id, map->GetId(), pos.GetPositionX(), pos.GetPositionY(), pos.GetPositionZ());
        return false;
    }

    if (goinfo->type == GAMEOBJECT_TYPE_MO_TRANSPORT)
    {
        FC_LOG_ERROR("sql.sql", "Gameobject (GUID: %u Entry: %u) not created: gameobject type GAMEOBJECT_TYPE_MO_TRANSPORT cannot be manually created.", guidlow, name_id);
        return false;
    }
    ...
    {
        FC_LOG_ERROR("sql.sql", "Gameobject (GUID: %u Entry: %u) not created: non-existing GO type '%u' in `gameobject_template`. It will crash client if created.", guidlow, name_id, goinfo->type);
        return false;
    }
    ...
    if (uint32 linkedEntry = GetGOInfo()->GetLinkedGameObjectEntry())
    {
        ...
        if (linkedGO->Create(map->GenerateLowGuid<HighGuid::GameObject>(), linkedEntry, map, pos, rotation, 255, GO_STATE_READY))
        {
...
void GameObject::Update(uint32 diff)
{
    if (AI())
        ...
        FC_LOG_ERROR("misc", "Could not initialize GameObjectAI");
        ...
        if (isSpawned())
        {
            ...
            if (goInfo->type == GAMEOBJECT_TYPE_TRAP)
            {
                ...
                if (Unit* owner = GetOwner())
                {
                    ...
                    Firelands::NearestAttackableNoTotemUnitInObjectRangeCheck checker(this, owner, radius);
                    Firelands::UnitLastSearcher<Firelands::NearestAttackableNoTotemUnitInObjectRangeCheck> searcher(this, target, checker);
                ...
                else
                {
                    ...
                    Firelands::AnyPlayerInObjectRangeCheck checker(this, radius);
                    Firelands::PlayerSearcher<Firelands::AnyPlayerInObjectRangeCheck> searcher(this, player, checker);
...
void GameObject::SaveToDB()
{
    ...
    if (!data)
    {
        FC_LOG_ERROR("misc", "GameObject::SaveToDB failed, cannot get gameobject data!");
        return;
    }
...
bool GameObject::LoadFromDB(ObjectGuid::LowType spawnId, Map* map, bool addToMap, bool)
{
    ...
    if (!data)
    {
        FC_LOG_ERROR("sql.sql", "Gameobject (GUID: %u) not found in table `gameobject`, can't load. ", spawnId);
        return false;
    }
    ...
    if (!Create(map->GenerateLowGuid<HighGuid::GameObject>(), entry, map, data->spawnPoint, data->rotation, animprogress, go_state, artKit, !m_respawnCompatibilityMode))
        return false;
    ...
    else
    {
        if (!m_respawnCompatibilityMode)
        {
            FC_LOG_WARN("sql.sql", "GameObject %u (SpawnID %u) is not spawned by default, but tries to use a non-hack spawn system. This will not work. Defaulting to compatibility mode.", entry, spawnId);
...
/*static*/ bool GameObject::DeleteFromDB(ObjectGuid::LowType spawnId)
{
    ...
    sMapMgr->DoForAllMapsWithMapId(data->mapId,
        [spawnId, trans](Map* map) -> void
        {
            ...
            for (auto const& pair : Firelands::Containers::MapEqualRange(map->GetGameObjectBySpawnIdStore(), spawnId))
...
bool GameObject::IsDynTransport() const
{
    ...
    return gInfo->type == GAMEOBJECT_TYPE_MO_TRANSPORT || gInfo->type == GAMEOBJECT_TYPE_TRANSPORT;
}
...
void GameObject::SaveRespawnTime(uint32 forceDelay)
{
    if (m_goData && (forceDelay || m_respawnTime > GameTime::GetGameTime()) && m_spawnedByDefault)
    {
        ...
        GetMap()->SaveRespawnTime(SPAWN_TYPE_GAMEOBJECT, m_spawnId, GetEntry(), thisRespawnTime, Firelands::ComputeGridCoord(GetPositionX(), GetPositionY()).GetId());
    }
}
...
GameObject* GameObject::LookupFishingHoleAround(float range)
{
    ...
    Firelands::NearestGameObjectFishingHole u_check(*this, range);
    Firelands::GameObjectSearcher<Firelands::NearestGameObjectFishingHole> checker(this, ok, u_check);
...
void GameObject::Use(Unit* user)
{
    ...
    case GAMEOBJECT_TYPE_GOOBER:                        //10
    {
        ...
        if (Player* player = user->ToPlayer())
        {
            ...
            {
                FC_LOG_DEBUG("maps.script", "Goober ScriptStart id %u for GO entry %u (GUID %u).", info->goober.eventId, GetEntry(), GetSpawnId());
    ...
    case GAMEOBJECT_TYPE_FISHINGNODE:                   //17
    {
        ...
        case GO_READY:                              // ready for loot
        {
            ...
            if (!zone_skill)
                FC_LOG_ERROR("sql.sql", "Fishable areaId %u are not properly defined in `skill_fishing_base_level`.", subzone);
            ...
            FC_LOG_DEBUG("misc", "Fishing check (skill: %i zone min skill: %i chance %i roll: %i", skill, zone_skill, chance, roll);
        ...
        if (GetUniqueUseCount() == info->summoningRitual.reqParticipants)
        {
            ...
            if (info->summoningRitual.casterTargetSpell && info->summoningRitual.casterTargetSpell != 1) // No idea why this field is a bool in some cases
                for (uint32 i = 0; i < info->summoningRitual.casterTargetSpellTargets; i++)
                    // m_unique_users can contain only player GUIDs
                    if (Player* target = ObjectAccessor::GetPlayer(*this, Firelands::Containers::SelectRandomContainerElement(m_unique_users)))
    ...
    default:
        if (GetGoType() >= MAX_GAMEOBJECT_TYPE)
            FC_LOG_ERROR("misc", "GameObject::Use(): unit (type: %u, guid: %u, name: %s) tries to use object (guid: %u, entry: %u, name: %s) of unknown type (%u)",
    ...
    if (!sSpellMgr->GetSpellInfo(spellId))
    {
        if (user->GetTypeId() != TYPEID_PLAYER || !sOutdoorPvPMgr->HandleCustomSpell(user->ToPlayer(), spellId, this))
            FC_LOG_ERROR("misc", "WORLD: unknown spell id %u at use action for gameobject (Entry: %u GoType: %u)", spellId, GetEntry(), GetGoType());
        else
            FC_LOG_DEBUG("outdoorpvp", "WORLD: %u non-dbc spell was handled by OutdoorPvP", spellId);
        return;
    }
...
{
    return GameObjectModel::Create(Firelands::make_unique<GameObjectModelOwnerImpl>(this), sWorld->GetDataPath());
}
