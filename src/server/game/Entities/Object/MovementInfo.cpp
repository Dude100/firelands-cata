...
{
    FC_LOG_DEBUG("misc", "MOVEMENT INFO");
    FC_LOG_DEBUG("misc", "%s", guid.ToString().c_str());
    FC_LOG_DEBUG("misc", "flags %u", flags);
    FC_LOG_DEBUG("misc", "flags2 %u", flags2);
    FC_LOG_DEBUG("misc", "time %u current time " UI64FMTD "", flags2, uint64(::GameTime::GetGameTime()));
    FC_LOG_DEBUG("misc", "position: `%s`", pos.ToString().c_str());
    if (transport.guid)
    {
        FC_LOG_DEBUG("misc", "TRANSPORT:");
        FC_LOG_DEBUG("misc", "%s", transport.guid.ToString().c_str());
        FC_LOG_DEBUG("misc", "position: `%s`", transport.pos.ToString().c_str());
        FC_LOG_DEBUG("misc", "seat: %i", transport.seat);
        FC_LOG_DEBUG("misc", "time: %u", transport.time);
        if (flags2 & MOVEMENTFLAG2_INTERPOLATED_MOVEMENT)
            FC_LOG_DEBUG("misc", "time2: %u", transport.time2);
        if (transport.vehicleId)
            FC_LOG_INFO("misc", "vehicleId: %u", transport.vehicleId);
    }

    if ((flags & (MOVEMENTFLAG_SWIMMING | MOVEMENTFLAG_FLYING)) || (flags2 & MOVEMENTFLAG2_ALWAYS_ALLOW_PITCHING))
        FC_LOG_DEBUG("misc", "pitch: %f", pitch);

    if (flags & MOVEMENTFLAG_FALLING || jump.fallTime)
    {
        FC_LOG_INFO("misc", "fallTime: %u j_zspeed: %f", jump.fallTime, jump.zspeed);
        if (flags & MOVEMENTFLAG_FALLING)
            FC_LOG_DEBUG("misc", "j_zspeed: %f j_sinAngle: %f j_cosAngle: %f j_xyspeed: %f", jump.zspeed, jump.sinAngle, jump.cosAngle, jump.xyspeed);
    }

    if (flags & MOVEMENTFLAG_SPLINE_ELEVATION)
        FC_LOG_DEBUG("misc", "splineElevation: %f", splineElevation);
}
