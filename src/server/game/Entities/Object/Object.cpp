...
WorldObject::~WorldObject()
{
    ...
    if (IsWorldObject() && m_currMap)
    {
        ...
        {
            FC_LOG_FATAL("misc", "WorldObject::~WorldObject Corpse Type: %d (%s) deleted but still in map!!",
...
Object::~Object()
{
    if (IsInWorld())
    {
        FC_LOG_FATAL("misc", "Object::~Object %s deleted but still in world!!", GetGUID().ToString().c_str());
        if (isType(TYPEMASK_ITEM))
            FC_LOG_FATAL("misc", "Item slot %u", ((Item*)this)->GetSlot());
        ABORT();
    }

    if (m_objectUpdated)
    {
        FC_LOG_FATAL("misc", "Object::~Object %s deleted but still in update list!!", GetGUID().ToString().c_str());
        ABORT();
    }
...
void Object::SetByteValue(uint16 index, uint8 offset, uint8 value)
{
    ...
    if (offset > 3)
    {
        FC_LOG_ERROR("misc", "Object::SetByteValue: wrong offset %u", offset);
        return;
    }
...
void Object::SetUInt16Value(uint16 index, uint8 offset, uint16 value)
{
    ...
    if (offset > 1)
    {
        FC_LOG_ERROR("misc", "Object::SetUInt16Value: wrong offset %u", offset);
        return;
    }
...
void Object::SetByteFlag(uint16 index, uint8 offset, uint8 newFlag)
{
    ...
    if (offset > 3)
    {
        FC_LOG_ERROR("misc", "Object::SetByteFlag: wrong offset %u", offset);
        return;
    }
...
void Object::RemoveByteFlag(uint16 index, uint8 offset, uint8 oldFlag)
{
    ...
    if (offset > 3)
    {
        FC_LOG_ERROR("misc", "Object::RemoveByteFlag: wrong offset %u", offset);
        return;
    }
...
{
    FC_LOG_ERROR("misc", "Attempt to %s non-existing value field: %u (count: %u) for object typeid: %u type mask: %u", (set ? "set value to" : "get value from"), index, m_valuesCount, GetTypeId(), m_objectType);
...
void WorldObject::GetRandomPoint(const Position& pos, float distance, float& rand_x, float& rand_y, float& rand_z) const
{
    ...
    Firelands::NormalizeMapCoord(rand_x);
    Firelands::NormalizeMapCoord(rand_y);
...
{
    Firelands::MessageDistDeliverer notifier(this, data, GetVisibilityRange(), false, skipped_rcvr);
...
void WorldObject::SetMap(Map* map)
{
    ...
    if (m_currMap)
    {
        FC_LOG_FATAL("misc", "WorldObject::SetMap: obj %u new map %u %u, old map %u %u", (uint32)GetTypeId(), map->GetId(), map->GetInstanceId(), m_currMap->GetId(), m_currMap->GetInstanceId());
        ABORT();
    }
    m_currMap = map;
    m_mapId = map->GetId();
...
void WorldObject::AddObjectToRemoveList()
{
    ...
    if (!map)
    {
        FC_LOG_ERROR("misc", "Object (TypeId: %u Entry: %u GUID: %u) at attempt add to move list not have valid map (Id: %u).", GetTypeId(), GetEntry(), GetGUID().GetCounter(), GetMapId());
        return;
    }
...
TempSummon* Map::SummonCreature(uint32 entry, Position const& pos, SummonCreatureExtraArgs const& summonArgs /*= { }*/)
{
    ...
    Firelands::AIRelocationNotifier notifier(*summon);
...
GameObject* WorldObject::SummonGameObject(uint32 entry, Position const& pos, QuaternionData const& rot, uint32 respawnTime, GOSummonType summonType)
{
    ...
    if (!goinfo)
    {
        FC_LOG_ERROR("sql.sql", "Gameobject template %u not found in database!", entry);
        return nullptr;
    }
...
void WorldObject::SummonCreatureGroup(uint8 group, std::list<TempSummon*>* list /*= nullptr*/)
{
    ...
    if (!data)
    {
        FC_LOG_WARN("scripts", "%s (%s) tried to summon non-existing summon group %u.", GetName().c_str(), GetGUID().ToString().c_str(), group);
        return;
    }
...
Creature* WorldObject::FindNearestCreature(uint32 entry, float range, bool alive) const
{
    ...
    Firelands::NearestCreatureEntryWithLiveStateInObjectRangeCheck checker(*this, entry, alive, range);
    Firelands::CreatureLastSearcher<Firelands::NearestCreatureEntryWithLiveStateInObjectRangeCheck> searcher(this, creature, checker);
...
GameObject* WorldObject::FindNearestGameObject(uint32 entry, float range) const
{
    ...
    Firelands::NearestGameObjectEntryInObjectRangeCheck checker(*this, entry, range);
...
{
    GameObject* go = nullptr;
    Firelands::NearestGameObjectTypeInObjectRangeCheck checker(*this, type, range);
    Firelands::GameObjectLastSearcher<Firelands::NearestGameObjectTypeInObjectRangeCheck> searcher(this, go, checker);
    Cell::VisitGridObjects(this, searcher, range);
    return go;
}
...
Player* WorldObject::SelectNearestPlayer(float distance) const
{
    ...
    Firelands::NearestPlayerInObjectRangeCheck checker(this, distance);
    Firelands::PlayerLastSearcher<Firelands::NearestPlayerInObjectRangeCheck> searcher(this, target, checker);
...
{
    Firelands::AllGameObjectsWithEntryInRange check(this, entry, maxSearchRange);
    Firelands::GameObjectListSearcher<Firelands::AllGameObjectsWithEntryInRange> searcher(this, gameObjectContainer, check);
...
{
    Firelands::AllCreaturesOfEntryInRange check(this, entry, maxSearchRange);
    Firelands::CreatureListSearcher<Firelands::AllCreaturesOfEntryInRange> searcher(this, creatureContainer, check);
    Cell::VisitGridObjects(this, searcher, maxSearchRange);
}
...
{
    Firelands::AnyPlayerInObjectRangeCheck checker(this, maxSearchRange);
    Firelands::PlayerListSearcher<Firelands::AnyPlayerInObjectRangeCheck> searcher(this, playerContainer, checker);
...
void WorldObject::MovePosition(Position& pos, float dist, float angle)
{
    ...
    if (!Firelands::IsValidMapCoord(destx, desty, pos.m_positionZ))
    {
        FC_LOG_FATAL("misc", "WorldObject::MovePosition: Object (TypeId: %u Entry: %u GUID: %u) has invalid coordinates X: %f and Y: %f were passed!",
    ...
    Firelands::NormalizeMapCoord(pos.m_positionX);
    Firelands::NormalizeMapCoord(pos.m_positionY);
...
void WorldObject::MovePositionToFirstCollision(Position& pos, float dist, float angle)
{
    ...
    // Prevent invalid coordinates here, position is unchanged
    if (!Firelands::IsValidMapCoord(destx, desty))
    {
        FC_LOG_FATAL("misc", "WorldObject::MovePositionToFirstCollision invalid coordinates X: %f and Y: %f were passed!", destx, desty);
        return;
    }
...
void WorldObject::DestroyForNearbyPlayers()
{
    ...
    Firelands::AnyPlayerInObjectRangeCheck check(this, GetVisibilityRange(), false);
    Firelands::PlayerListSearcher<Firelands::AnyPlayerInObjectRangeCheck> searcher(this, targets, check);
...
void WorldObject::UpdateObjectVisibility(bool /*forced*/)
{
    ...
    Firelands::VisibleChangesNotifier notifier(*this);
    Cell::VisitWorldObjects(this, notifier, GetVisibilityRange());
}
