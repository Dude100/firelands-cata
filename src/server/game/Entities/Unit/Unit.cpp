...
uint32 Unit::DealDamage(Unit* victim, uint32 damage, CleanDamage const* cleanDamage, DamageEffectType damagetype, SpellSchoolMask damageSchoolMask, SpellInfo const* spellProto, bool durabilityLoss)
{
    ...
    if (UnitAI* victimAI = victim->GetAI())
        victimAI->DamageTaken(this, damage);

    if (UnitAI* attackerAI = GetAI())
            ...
            for (Unit* controlled : victim->m_Controlled)
                if (Creature* cControlled = controlled->ToCreature())
                    if (CreatureAI* controlledAI = cControlled->AI())
                        controlledAI->OwnerAttackedBy(this);
        }
    ...
    FC_LOG_DEBUG("entities.unit", "DealDamageStart");

    uint32 health = victim->GetHealth();
    FC_LOG_DEBUG("entities.unit", "%s dealt %u damage to %s", GetGUID().ToString().c_str(), damage, victim->GetGUID().ToString().c_str());
    ...
    if (victim->GetTypeId() == TYPEID_PLAYER && victim->ToPlayer()->duel && damage >= (health - 1))
    {
        ...
        if (victim->ToPlayer()->duel->opponent == GetControllingPlayer())
            damage = health - 1;
    ...
    else if (victim->IsVehicle() && damage >= (health - 1) && victim->GetCharmer() && victim->GetCharmer()->GetTypeId() == TYPEID_PLAYER)
    {
        ...
        {
            // prevent kill only if killed in duel and killed by opponent or opponent controlled creature
            if (victimRider->duel->opponent == GetControllingPlayer())
                damage = health - 1;
    ...
    if (health <= damage)
    {
        FC_LOG_DEBUG("entities.unit", "DealDamage: victim just died");
    ...
    {
        FC_LOG_DEBUG("entities.unit", "DealDamageAlive");
    ...
    FC_LOG_DEBUG("entities.unit", "DealDamageEnd returned %d damage", damage);
...
void Unit::CastSpell(SpellCastTargets const& targets, uint32 spellId, CastSpellExtraArgs const& args)
{
    ...
    if (!info)
    {
        FC_LOG_ERROR("entities.unit", "CastSpell: unknown spell %u by caster %s", spellId, GetGUID().ToString().c_str());
        return;
    }
...
void Unit::CastSpell(WorldObject* target, uint32 spellId, CastSpellExtraArgs const& args)
{
    ...
    if (target)
    {
        ...
        {
            FC_LOG_ERROR("entities.unit", "CastSpell: Invalid target %s passed to spell cast by %s", target->GetGUID().ToString().c_str(), GetGUID().ToString().c_str());
            return;
        }
    }
    ...
    if (!spellInfo->HasAttribute(SPELL_ATTR4_FIXED_DAMAGE))
    {
        ...
        case SPELL_DAMAGE_CLASS_MELEE:
        {
            ...
            if (damageSchoolMask & SPELL_SCHOOL_MASK_NORMAL)
            {
                ...
                if (spellInfo->HasAttribute(SPELL_ATTR8_CANT_BLOCK))
                {
                    // Get blocked status
...
void Unit::DealSpellDamage(SpellNonMeleeDamage* damageInfo, bool durabilityLoss)
{
    ...
    {
        FC_LOG_DEBUG("entities.unit", "Unit::DealSpellDamage has wrong damageInfo->SpellID: %u", damageInfo->SpellID);
        return;
    }
...
void Unit::CalculateMeleeDamage(Unit* victim, uint32 damage, CalcDamageInfo* damageInfo, WeaponAttackType attackType)
{
    ...
    case MELEE_HIT_GLANCING:
    {
        ...
        if (leveldif < 0)
        {
            FC_LOG_DEBUG("entities.unit", "Unit::CalculateMeleeDamage: (Player) %s attacked %s. Glancing should never happen against lower level target", GetGUID().ToString().c_str(), victim->GetGUID().ToString().c_str());
            break;
        }
        if (leveldif == 0)
            leveldif = 1;
...
uint32 Unit::CalcSpellResistedDamage(Unit* victim, uint32 damage, SpellSchoolMask schoolMask, SpellInfo const* spellInfo) const
{
    ...
    {
        int32 ignoredResistance = GetTotalAuraModifierByMiscMask(SPELL_AURA_MOD_IGNORE_TARGET_RESIST, schoolMask);
...
void Unit::AttackerStateUpdate(Unit* victim, WeaponAttackType attType, bool extra)
{
    ...
    else
    {
        ...
        if (!meleeAttackAuraEffect)
        {
            ...
            if (GetTypeId() == TYPEID_PLAYER)
                FC_LOG_DEBUG("entities.unit", "AttackerStateUpdate: (Player) %u attacked %u (TypeId: %u) for %u dmg, absorbed %u, blocked %u, resisted %u.",
                ...
                FC_LOG_DEBUG("entities.unit", "AttackerStateUpdate: (NPC) %u attacked %u (TypeId: %u) for %u dmg, absorbed %u, blocked %u, resisted %u.",
                    GetGUID().GetCounter(), victim->GetGUID().GetCounter(), victim->GetTypeId(), damageInfo.damage, damageInfo.absorb, damageInfo.blocked_amount, damageInfo.resist);
        }
        else
        {
...
void Unit::SendMeleeAttackStop(Unit* victim)
{
    ...
    if (victim)
        FC_LOG_DEBUG("entities.unit", "%s %u stopped attacking %s %u", (GetTypeId() == TYPEID_PLAYER ? "Player" : "Creature"), GetGUID().GetCounter(), (victim->GetTypeId() == TYPEID_PLAYER ? "player" : "creature"), victim->GetGUID().GetCounter());
    else
        FC_LOG_DEBUG("entities.unit", "%s %u stopped attacking", (GetTypeId() == TYPEID_PLAYER ? "Player" : "Creature"), GetGUID().GetCounter());
}
...
SpellMissInfo Unit::MeleeSpellHitResult(Unit* victim, SpellInfo const* spellInfo)
{
    ...
    for (AuraEffect const* aurEff : ignore)
    {
        ...
        case MELEE_HIT_PARRY:
            ...
            break;
        default:
            FC_LOG_DEBUG("entities.unit", "Spell %u SPELL_AURA_IGNORE_COMBAT_RESULT has unhandled state %d", aurEff->GetId(), aurEff->GetMiscValue());
            break;
        }
    }
...
void Unit::_UnapplyAura(AuraApplicationMap::iterator& i, AuraRemoveFlags removeMode)
{
    ...
    Aura* aura = aurApp->GetBase();
    FC_LOG_DEBUG("spells", "Aura %u now is remove mode %u", aura->GetId(), static_cast<uint32>(removeMode));
...
void Unit::RemoveOwnedAura(Aura* aura, AuraRemoveFlags removeMode)
{
    ...
    {
        FC_LOG_ERROR("spells", "Unit::RemoveOwnedAura() called with unallowed removeMode AuraRemoveFlags::None, spellId %u", aura->GetId());
        return;
    }
...
void Unit::DelayOwnedAuras(uint32 spellId, ObjectGuid caster, int32 delaytime)
{
    ...
    for (; range.first != range.second; ++range.first)
    {
        ...
        if (!caster || aura->GetCasterGUID() == caster)
        {
            ...
            FC_LOG_DEBUG("spells", "Aura %u partially interrupted on unit %u, new duration: %u ms", aura->GetId(), GetGUID().GetCounter(), aura->GetDuration());
        }
    }
}
...
void Unit::SendPeriodicAuraLog(SpellPeriodicAuraLogInfo* pInfo)
{
    ...
    case SPELL_AURA_PERIODIC_MANA_LEECH:
        ...
        break;
    default:
        FC_LOG_ERROR("entities.unit", "Unit::SendPeriodicAuraLog: unknown aura %u", uint32(aura->GetAuraType()));
        return;
    }
...
FactionTemplateEntry const* Unit::GetFactionTemplateEntry() const
{
    ...
    {
        if (Player const* player = ToPlayer())
            FC_LOG_ERROR("entities.unit", "Player %s has invalid faction (faction template id) #%u", player->GetName().c_str(), GetFaction());
        else if (Creature const* creature = ToCreature())
            FC_LOG_ERROR("entities.unit", "Creature (template id: %u) has invalid faction (faction template id) #%u", creature->GetCreatureTemplate()->Entry, GetFaction());
        else
            FC_LOG_ERROR("entities.unit", "Unit (name=%s, type=%u) has invalid faction (faction template id) #%u", GetName().c_str(), uint32(GetTypeId()), GetFaction());
...
void Unit::RemoveAllAttackers()
{
    while (!m_attackers.empty())
    {
        ...
        if (!(*iter)->AttackStop())
        {
            FC_LOG_ERROR("entities.unit", "WORLD: Unit has an attacker that isn't attacking it!");
...
Minion* Unit::GetFirstMinion() const
{
    if (ObjectGuid pet_guid = GetMinionGUID())
    {
        ...
        FC_LOG_ERROR("entities.unit", "Unit::GetFirstMinion: Minion %s not exist.", pet_guid.ToString().c_str());
...
Guardian* Unit::GetGuardianPet() const
{
    if (ObjectGuid pet_guid = GetPetGUID())
    {
        ...
        FC_LOG_FATAL("entities.unit", "Unit::GetGuardianPet: Guardian %s not exist.", pet_guid.ToString().c_str());
...
void Unit::SetMinion(Minion* minion, bool apply)
{
    if (!minion)
    {
        FC_LOG_ERROR("entities.unit", "Unit::SetMinion: Unit %s tried to reference a non existing minion", GetGUID().ToString().c_str());
        return;
    }

    FC_LOG_DEBUG("entities.unit", "SetMinion %u for %u, apply %u", minion->GetEntry(), GetEntry(), apply);

    if (apply)
    {
        if (minion->GetOwnerGUID())
        {
            FC_LOG_FATAL("entities.unit", "SetMinion: Minion %u is not the minion of owner %u", minion->GetEntry(), GetEntry());
            return;
        }
...
void Unit::DealHeal(HealInfo& healInfo)
{
    ...
    if (UnitAI* victimAI = victim->GetAI())
        victimAI->HealReceived(this, addhealth);

    if (UnitAI* healerAI = GetAI())
...
void Unit::RemoveAllControlled()
{
    ...
    while (!m_Controlled.empty())
    {
        ...
        if (target->GetCharmerGUID() == GetGUID())
            ...
            FC_LOG_ERROR("entities.unit", "Unit %u is trying to release unit %u which is neither charmed nor owned by it", GetEntry(), target->GetEntry());
    }
    if (GetPetGUID())
        FC_LOG_FATAL("entities.unit", "Unit %u is not able to release its pet %s", GetEntry(), GetPetGUID().ToString().c_str());
    if (GetMinionGUID())
        FC_LOG_FATAL("entities.unit", "Unit %u is not able to release its minion %s", GetEntry(), GetMinionGUID().ToString().c_str());
    if (GetCharmedGUID())
        FC_LOG_FATAL("entities.unit", "Unit %u is not able to release its charm %s", GetEntry(), GetCharmedGUID().ToString().c_str());
...
void Unit::UpdateSpeed(UnitMoveType mtype)
{
    ...
    default:
        FC_LOG_ERROR("entities.unit", "Unit::UpdateSpeed: Unsupported move type (%d)", mtype);
        return;
    }
...
void Unit::FollowTarget(Unit* target)
{
    if (!target)
    {
        FC_LOG_ERROR("entities.unit", "Unit::FollowTarget: Unit (%s) tried to follow a non-existant target.", GetGUID().ToString().c_str());
        return;
    }
...
void Unit::HandleStatFlatModifier(UnitMods unitMod, UnitModifierFlatType modifierType, float amount, bool apply)
{
    ...
    {
        FC_LOG_ERROR("entities.unit", "ERROR in HandleStatFlatModifier(): non-existing UnitMods or wrong UnitModifierType!");
        return;
    }
...
void Unit::ApplyStatPctModifier(UnitMods unitMod, UnitModifierPctType modifierType, float pct)
{
    ...
    {
        FC_LOG_ERROR("entities.unit", "ERROR in ApplyStatPctModifier(): non-existing UnitMods or wrong UnitModifierType!");
        return;
    }
...
float Unit::GetFlatModifierValue(UnitMods unitMod, UnitModifierFlatType modifierType) const
{
    if (unitMod >= UNIT_MOD_END || modifierType >= MODIFIER_TYPE_FLAT_END)
    {
        FC_LOG_ERROR("entities.unit", "attempt to access non-existing modifier value from UnitMods!");
        return 0.0f;
    }
...
float Unit::GetTotalAuraModValue(UnitMods unitMod) const
{
    ...
    {
        FC_LOG_ERROR("entities.unit", "attempt to access non-existing UnitMods in GetTotalAuraModValue()!");
        return 0.0f;
    }
...
void Unit::RemoveFromWorld()
{
    ...
    if (IsInWorld())
    {
        ...
        if (Unit* owner = GetOwner())
        {
            ...
            {
                FC_LOG_FATAL("entities.unit", "Unit %u is in controlled list of %u when removed from world", GetEntry(), owner->GetEntry());
                ABORT();
            }
        }
        ...
        if (IsPlayer())
        {
            if (Unit* charmer = GetCharmer())
            {
                ...
                if (Creature* creatureCharmer = charmer->ToCreature())
                {
                    ...
                    FC_LOG_ERROR("misc", "Attempt to assign charm AI to player %s who is charmed by non-creature %s.", GetGUID().ToString().c_str(), GetCharmerGUID().ToString().c_str());
            }
...
Unit* Unit::SelectNearbyTarget(Unit* exclude, float dist) const
{
    ...
    Firelands::AnyUnfriendlyUnitInObjectRangeCheck u_check(this, this, dist);
    Firelands::UnitListSearcher<Firelands::AnyUnfriendlyUnitInObjectRangeCheck> searcher(this, targets, u_check);
...
bool Unit::InitTamedPet(Pet* pet, uint8 level, uint32 spell_id)
{
    ...
    {
        FC_LOG_ERROR("entities.unit", "Pet::InitStatsForLevel() failed for creature (Entry: %u)!", pet->GetEntry());
        return false;
    }
...
void Unit::Kill(Unit* victim, bool durabilityLoss)
{
    ...
    FC_LOG_DEBUG("entities.unit", "SET JUST_DIED");
    ...
    if (player)
    {
        ...
        if (pet && pet->IsAlive() && pet->isControlled())
        {
            if (pet->IsAIEnabled())
                ...
                FC_LOG_ERROR("entities.unit", "Pet doesn't have any AI in Unit::Kill(). %s", pet->GetGUID().ToString().c_str());
        }
    }
    ...
    if (Player* plrVictim = victim->ToPlayer())
    {
        ...
        if ((durabilityLoss && !player && !victim->ToPlayer()->InBattleground()) || (player && sWorld->getBoolConfig(CONFIG_DURABILITY_LOSS_IN_PVP)))
        {
            ...
            FC_LOG_DEBUG("entities.unit", "We are dead, losing %u percent durability", loss);
    ...
    {
        FC_LOG_DEBUG("entities.unit", "DealDamageNotPlayer");
...
bool Unit::SetCharmedBy(Unit* charmer, CharmType type, AuraApplication const* aurApp)
{
    ...
    FC_LOG_DEBUG("entities.unit", "SetCharmedBy: charmer %u (GUID %u), charmed %u (GUID %u), type %u.", charmer->GetEntry(), charmer->GetGUID().GetCounter(), GetEntry(), GetGUID().GetCounter(), uint32(type));

    if (this == charmer)
    {
        FC_LOG_FATAL("entities.unit", "Unit::SetCharmedBy: Unit %u (GUID %u) is trying to charm itself!", GetEntry(), GetGUID().GetCounter());
        return false;
    }
    ...
    {
        FC_LOG_FATAL("entities.unit", "Unit::SetCharmedBy: Player on transport is trying to charm %u (GUID %u)", GetEntry(), GetGUID().GetCounter());
        return false;
    }
    ...
    if (GetCharmerGUID())
    {
        FC_LOG_FATAL("entities.unit", "Unit::SetCharmedBy: %u (GUID %u) has already been charmed but %u (GUID %u) is trying to charm it!", GetEntry(), GetGUID().GetCounter(), charmer->GetEntry(), charmer->GetGUID().GetCounter());
        return false;
    }
    ...
    if (!IsInWorld())
    {
        FC_LOG_FATAL("entities.unit", "Unit::SetCharmedBy: %u (GUID %u) is not in world but %u (GUID %u) is trying to charm it!", GetEntry(), GetGUID().GetCounter(), charmer->GetEntry(), charmer->GetGUID().GetCounter());
        return false;
    }
            ...
            if (GetTypeId() == TYPEID_UNIT && charmer->getClass() == CLASS_WARLOCK)
            {
                ...
                if (cinfo && cinfo->type == CREATURE_TYPE_DEMON)
                {
                    ...
                    if (GetCharmInfo())
                        ...
                        FC_LOG_ERROR("entities.unit", "Aura::HandleModCharm: %s has a charm aura but no charm info!", GetGUID().ToString().c_str());
                }
            }
            break;
        case CHARM_TYPE_CONVERT:
...
bool Unit::HandleSpellClick(Unit* clicker, int8 seatId)
{
    ...
    for (SpellClickInfoContainer::const_iterator itr = clickPair.first; itr != clickPair.second; ++itr)
    {
        ...
        if (seatId > -1)
        {
            ...
            if (!valid)
            {
                FC_LOG_ERROR("sql.sql", "Spell %u specified in npc_spellclick_spells is not a valid vehicle enter aura!", itr->second.spellId);
                continue;
            }
...
void Unit::_EnterVehicle(Vehicle* vehicle, int8 seatId, AuraApplication const* aurApp)
{
    ...
    if (m_vehicle)
    {
        ...
        {
            FC_LOG_DEBUG("entities.vehicle", "EnterVehicle: %u exit %u and enter %u.", GetEntry(), m_vehicle->GetBase()->GetEntry(), vehicle->GetBase()->GetEntry());
            ExitVehicle();
        }
...
void Unit::_ExitVehicle(Position const* exitPosition)
{
    ...
    if (!vehicle)
    {
        FC_LOG_ERROR("entities.vehicle", "RemovePassenger() couldn't remove current unit from vehicle.");
        return;
    }
...
void Unit::WriteMovementInfo(WorldPacket& data, Movement::ExtraMovementStatusElement* extras /*= nullptr*/, uint32* movementCounter /*= nullptr*/)
{
    ...
    if (!sequence)
    {
        FC_LOG_ERROR("network", "Unit::WriteMovementInfo: No movement sequence found for opcode %s", GetOpcodeNameForLogging(static_cast<OpcodeClient>(data.GetOpcode())).c_str());
        return;
    }
...
bool Unit::UpdatePosition(float x, float y, float z, float orientation, bool teleport)
{
    ...
    if (!Firelands::IsValidMapCoord(x, y, z, orientation))
    {
        FC_LOG_DEBUG("entities.unit", "Unit::UpdatePosition(%f, %f, %f) .. bad coordinates!", x, y, z);
        return false;
    }
    ...
    for (std::pair<MovementChangeType const, PlayerMovementPendingChange> const& pendingChange : m_pendingMovementChanges)
    {
        if (GameTime::GetGameTimeMS() > pendingChange.second.time + sWorld->getIntConfig(CONFIG_PENDING_MOVE_CHANGES_TIMEOUT))
        {
            ...
            */
            ...
            FC_LOG_INFO("cheat", "Unit::CheckPendingMovementAcks: Player GUID: %s took too long to acknowledge a movement change. He was therefore kicked.", controller->GetBasePlayer()->GetGUID().ToString().c_str());
            break;
        }
    }
}
...
{
    FC_LOG_ERROR("entities.unit", "Unit::OutDebugInfo");
    FC_LOG_DEBUG("entities.unit", "%s name %s", GetGUID().ToString().c_str(), GetName().c_str());
    FC_LOG_DEBUG("entities.unit", "Owner %s, Minion %s, Charmer %s, Charmed %s", GetOwnerOrCreatorGUID().ToString().c_str(), GetMinionGUID().ToString().c_str(), GetCharmerGUID().ToString().c_str(), GetCharmedGUID().ToString().c_str());
    FC_LOG_DEBUG("entities.unit", "In world %u, unit type mask %u", (uint32)(IsInWorld() ? 1 : 0), m_unitTypeMask);
    if (IsInWorld())
        FC_LOG_DEBUG("entities.unit", "Mapid %u", GetMapId());
    ...
    FC_LOG_DEBUG("entities.unit", "%s", o.str().c_str());
    o.str("");
    ...
    if (GetVehicle())
        FC_LOG_DEBUG("entities.unit", "On vehicle %u.", GetVehicleBase()->GetEntry());
}
...
{
    Firelands::CustomChatTextBuilder builder(this, msgType, text, language, target);
    Firelands::LocalizedPacketDo<Firelands::CustomChatTextBuilder> localizer(builder);
    Firelands::PlayerDistWorker<Firelands::LocalizedPacketDo<Firelands::CustomChatTextBuilder> > worker(this, textRange, localizer);
...
void Unit::Talk(uint32 textId, ChatMsg msgType, float textRange, WorldObject const* target)
{
    ...
    {
        FC_LOG_ERROR("entities.unit", "WorldObject::MonsterText: `broadcast_text` (ID: %u) was not found", textId);
        return;
    }

    Firelands::BroadcastTextBuilder builder(this, msgType, textId, getGender(), target);
    Firelands::LocalizedPacketDo<Firelands::BroadcastTextBuilder> localizer(builder);
    Firelands::PlayerDistWorker<Firelands::LocalizedPacketDo<Firelands::BroadcastTextBuilder> > worker(this, textRange, localizer);
    Cell::VisitWorldObjects(this, worker, textRange);
}
...
void Unit::Whisper(uint32 textId, Player* target, bool isBossWhisper /*= false*/)
{
    ...
    if (!bct)
    {
        FC_LOG_ERROR("entities.unit", "WorldObject::MonsterWhisper: `broadcast_text` was not %u found", textId);
        return;
    }
