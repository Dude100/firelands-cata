...
void PetAI::UpdateAI(uint32 diff)
{
    ...
    if (me->GetVictim() && me->EnsureVictim()->IsAlive())
    {
        ...
        if (_needToStop())
        {
            FC_LOG_DEBUG("misc", "Pet AI stopped attacking [guid=%u]", me->GetGUID().GetCounter());
            _stopAttack();
            return;
        }
...
void PetAI::OwnerAttacked(Unit* target)
{
    ...
    // Passive and Defesnive pets don't do anything
...
void PetAI::HandleReturnMovement()
{
    ...
    if (!me->GetCharmInfo())
    {
        FC_LOG_WARN("scripts.ai.petai", "me->GetCharmInfo() is NULL in PetAI::HandleReturnMovement()");
        return;
    }
...
bool PetAI::CanAttack(Unit* target)
{
    ...
    if (!me->GetCharmInfo())
    {
        FC_LOG_ERROR("scripts.ai.petai", "me->GetCharmInfo() is NULL in PetAI::CanAttack()");
        return false;
    }
