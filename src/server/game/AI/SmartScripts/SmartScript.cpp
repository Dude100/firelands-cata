...
bool SmartScript::IsSmart(Creature* c, bool silent)
{
    ...
    if (!smart && !silent)
        FC_LOG_ERROR("sql.sql", "SmartScript: Action target Creature (GUID: %u Entry: %u) is not using SmartAI, action called by Creature (GUID: %u Entry: %u) skipped to prevent crash.", c ? c->GetSpawnId() : 0, c ? c->GetEntry() : 0, me ? me->GetSpawnId() : 0, me ? me->GetEntry() : 0);
...
bool SmartScript::IsSmart(GameObject* g, bool silent)
{
    ...
    if (!smart && !silent)
        FC_LOG_ERROR("sql.sql", "SmartScript: Action target GameObject (GUID: %u Entry: %u) is not using SmartGameObjectAI, action called by GameObject (GUID: %u Entry: %u) skipped to prevent crash.", g ? g->GetSpawnId() : 0, g ? g->GetEntry() : 0, go ? go->GetSpawnId() : 0, go ? go->GetEntry() : 0);
...
void SmartScript::ProcessAction(SmartScriptHolder& e, Unit* unit, uint32 var0, uint32 var1, bool bvar, SpellInfo const* spell, GameObject* gob)
{
    ...
    if (Unit* tempInvoker = GetLastInvoker())
        FC_LOG_DEBUG("scripts.ai", "SmartScript::ProcessAction: Invoker: %s (guidlow: %u)", tempInvoker->GetName().c_str(), tempInvoker->GetGUID().GetCounter());
    ...
    case SMART_ACTION_TALK:
    {
        ...
        FC_LOG_DEBUG("scripts.ai", "SmartScript::ProcessAction: SMART_ACTION_TALK: talker: %s (GuidLow: %u), textGuid: %u",
    ...
    case SMART_ACTION_SIMPLE_TALK:
    {
        for (WorldObject* target : targets)
        {
            ...
            FC_LOG_DEBUG("scripts.ai", "SmartScript::ProcessAction:: SMART_ACTION_SIMPLE_TALK: talker: %s (GuidLow: %u), textGroupId: %u",
    ...
    case SMART_ACTION_PLAY_EMOTE:
    {
        for (WorldObject* target : targets)
        {
            if (IsUnit(target))
            {
                ...
                FC_LOG_DEBUG("scripts.ai", "SmartScript::ProcessAction:: SMART_ACTION_PLAY_EMOTE: target: %s (GuidLow: %u), emote: %u",
    ...
    case SMART_ACTION_SOUND:
    {
        for (WorldObject* target : targets)
        {
            if (IsUnit(target))
            {
                ...
                FC_LOG_DEBUG("scripts.ai", "SmartScript::ProcessAction:: SMART_ACTION_SOUND: target: %s (GuidLow: %u), sound: %u, onlyself: %u",
        ...
        for (WorldObject* target : targets)
        {
            if (IsCreature(target))
            {
                if (e.action.faction.factionID)
                {
                    ...
                    FC_LOG_DEBUG("scripts.ai", "SmartScript::ProcessAction:: SMART_ACTION_SET_FACTION: Creature entry %u, GuidLow %u set faction to %u",
    ...
    case SMART_ACTION_MORPH_TO_ENTRY_OR_MODEL:
    {
        for (WorldObject* target : targets)
        {
            ...
            if (e.action.morphOrMount.creature || e.action.morphOrMount.model)
            {
                ...
                if (e.action.morphOrMount.creature)
                {
                    if (CreatureTemplate const* ci = sObjectMgr->GetCreatureTemplate(e.action.morphOrMount.creature))
                    {
                        ...
                        FC_LOG_DEBUG("scripts.ai", "SmartScript::ProcessAction:: SMART_ACTION_MORPH_TO_ENTRY_OR_MODEL: Creature entry %u, GuidLow %u set displayid to %u",
            ...
            else
            {
                ...
                FC_LOG_DEBUG("scripts.ai", "SmartScript::ProcessAction:: SMART_ACTION_MORPH_TO_ENTRY_OR_MODEL: Creature entry %u, GuidLow %u demorphs.",
    ...
    case SMART_ACTION_FAIL_QUEST:
    {
        for (WorldObject* target : targets)
        {
            if (IsPlayer(target))
            {
                ...
                FC_LOG_DEBUG("scripts.ai", "SmartScript::ProcessAction:: SMART_ACTION_FAIL_QUEST: Player guidLow %u fails quest %u",
                ...
                if (Quest const* q = sObjectMgr->GetQuestTemplate(e.action.questOffer.questID))
                {
                    if (me && e.action.questOffer.directAdd == 0)
                    {
                        if (pTarget->CanTakeQuest(q, true))
                        {
                            ...
                            FC_LOG_DEBUG("scripts.ai", "SmartScript::ProcessAction:: SMART_ACTION_OFFER_QUEST: Player guidLow %u - offered quest %u",
                    ...
                    else
                    {
                        ...
                        FC_LOG_DEBUG("scripts.ai", "SmartScript::ProcessAction:: SMART_ACTION_OFFER_QUEST: Player guidLow %u - quest %u added",
                            pTarget->GetGUID().GetCounter(), e.action.questOffer.questID);
                    }
                }
            }
    ...
    case SMART_ACTION_RANDOM_EMOTE:
    {
        ...
        for (WorldObject* target : targets)
        {
            if (IsUnit(target))
            {
                ...
                FC_LOG_DEBUG("scripts.ai", "SmartScript::ProcessAction:: SMART_ACTION_RANDOM_EMOTE: Creature guidLow %u handle random emote %u",
    ...
    case SMART_ACTION_THREAT_ALL_PCT:
    {
        ...
        for (auto* ref : me->GetThreatManager().GetModifiableThreatList())
        {
            ...
            FC_LOG_DEBUG("scripts.ai", "SmartScript::ProcessAction:: SMART_ACTION_THREAT_ALL_PCT: Creature guidLow %u modify threat for unit %u, value %i",
    ...
    case SMART_ACTION_THREAT_SINGLE_PCT:
    {
        ...
        for (WorldObject* target : targets)
        {
            if (IsUnit(target))
            {
                ...
                FC_LOG_DEBUG("scripts.ai", "SmartScript::ProcessAction:: SMART_ACTION_THREAT_SINGLE_PCT: Creature guidLow %u modify threat for unit %u, value %i",
    ...
    case SMART_ACTION_CALL_AREAEXPLOREDOREVENTHAPPENS:
    {
        for (WorldObject* target : targets)
        {
            ...
            if (IsPlayer(target))
            {
                ...
                FC_LOG_DEBUG("scripts.ai", "SmartScript::ProcessAction:: SMART_ACTION_CALL_AREAEXPLOREDOREVENTHAPPENS: Player guidLow %u credited quest %u",
                    target->GetGUID().GetCounter(), e.action.quest.quest);
            }
        }
        break;
    ...
    case SMART_ACTION_CAST:
    {
        ...
        if (e.action.cast.targetsLimit > 0 && targets.size() > e.action.cast.targetsLimit)
            Firelands::Containers::RandomResize(targets, e.action.cast.targetsLimit);
        ...
        for (WorldObject* target : targets)
        {
            ...
            if (!(e.action.cast.castFlags & SMARTCAST_AURA_NOT_PRESENT) || !target->ToUnit()->HasAura(e.action.cast.spell))
            {
                ...
                FC_LOG_DEBUG("scripts.ai", "SmartScript::ProcessAction:: SMART_ACTION_CAST:: %s: %u casts spell %u on target %u with castflags %u",
                ...
                FC_LOG_DEBUG("scripts.ai", "Spell %u not cast because it has flag SMARTCAST_AURA_NOT_PRESENT and the target (%s) already has the aura", e.action.cast.spell, target->GetGUID().ToString().c_str());
        }
        break;
    }
    ...
    case SMART_ACTION_INVOKER_CAST:
    {
        ...
        for (WorldObject* target : targets)
        {
            ...
            if (!(e.action.cast.castFlags & SMARTCAST_AURA_NOT_PRESENT) || !target->ToUnit()->HasAura(e.action.cast.spell))
            {
                ...
                FC_LOG_DEBUG("scripts.ai", "SmartScript::ProcessAction:: SMART_ACTION_INVOKER_CAST: Invoker %u casts spell %u on target %u with castflags %u",
    ...
    case SMART_ACTION_ADD_AURA:
    {
        for (WorldObject* target : targets)
        {
            if (IsUnit(target))
            {
                ...
                FC_LOG_DEBUG("scripts.ai", "SmartScript::ProcessAction:: SMART_ACTION_ADD_AURA: Adding aura %u to unit %u",
    ...
    case SMART_ACTION_ACTIVATE_GOBJECT:
    {
        for (WorldObject* target : targets)
        {
            if (IsGameObject(target))
            {
                ...
                FC_LOG_DEBUG("scripts.ai", "SmartScript::ProcessAction:: SMART_ACTION_ACTIVATE_GOBJECT. Gameobject %u (entry: %u) activated",
    ...
    case SMART_ACTION_RESET_GOBJECT:
    {
        for (WorldObject* target : targets)
        {
            if (IsGameObject(target))
            {
                ...
                FC_LOG_DEBUG("scripts.ai", "SmartScript::ProcessAction:: SMART_ACTION_RESET_GOBJECT. Gameobject %u (entry: %u) reset",
                    target->GetGUID().GetCounter(), target->GetEntry());
            }
        }
        break;
    ...
    case SMART_ACTION_SET_EMOTE_STATE:
    {
        for (WorldObject* target : targets)
        {
            if (IsUnit(target))
            {
                ...
                FC_LOG_DEBUG("scripts.ai", "SmartScript::ProcessAction:: SMART_ACTION_SET_EMOTE_STATE. Unit %u set emotestate to %u",
        ...
        for (WorldObject* target : targets)
        {
            if (IsUnit(target))
            {
                if (!e.action.unitFlag.type)
                {
                    ...
                    FC_LOG_DEBUG("scripts.ai", "SmartScript::ProcessAction:: SMART_ACTION_SET_UNIT_FLAG. Unit %u added flag %u to UNIT_FIELD_FLAGS",
                        target->GetGUID().GetCounter(), e.action.unitFlag.flag);
                }
                else
                {
                    target->ToUnit()->SetFlag(UNIT_FIELD_FLAGS_2, e.action.unitFlag.flag);
                    FC_LOG_DEBUG("scripts.ai", "SmartScript::ProcessAction:: SMART_ACTION_SET_UNIT_FLAG. Unit %u added flag %u to UNIT_FIELD_FLAGS_2",
                        target->GetGUID().GetCounter(), e.action.unitFlag.flag);
                }
            }
        }
        ...
        for (WorldObject* target : targets)
        {
            if (IsUnit(target))
            {
                if (!e.action.unitFlag.type)
                {
                    ...
                    FC_LOG_DEBUG("scripts.ai", "SmartScript::ProcessAction:: SMART_ACTION_REMOVE_UNIT_FLAG. Unit %u removed flag %u to UNIT_FIELD_FLAGS",
                        target->GetGUID().GetCounter(), e.action.unitFlag.flag);
                }
                else
                {
                    target->ToUnit()->RemoveFlag(UNIT_FIELD_FLAGS_2, e.action.unitFlag.flag);
                    FC_LOG_DEBUG("scripts.ai", "SmartScript::ProcessAction:: SMART_ACTION_REMOVE_UNIT_FLAG. Unit %u removed flag %u to UNIT_FIELD_FLAGS_2",
                        target->GetGUID().GetCounter(), e.action.unitFlag.flag);
                }
            }
        }
    ...
    case SMART_ACTION_AUTO_ATTACK:
    {
        ...
        FC_LOG_DEBUG("scripts.ai", "SmartScript::ProcessAction:: SMART_ACTION_AUTO_ATTACK: Creature: %u bool on = %u",
    ...
    case SMART_ACTION_ALLOW_COMBAT_MOVEMENT:
    {
        ...
        FC_LOG_DEBUG("scripts.ai", "SmartScript::ProcessAction:: SMART_ACTION_ALLOW_COMBAT_MOVEMENT: Creature %u bool on = %u",
    ...
    case SMART_ACTION_SET_EVENT_PHASE:
    {
        ...
        FC_LOG_DEBUG("scripts.ai", "SmartScript::ProcessAction:: SMART_ACTION_SET_EVENT_PHASE: Creature %u set event phase %u",
    ...
    case SMART_ACTION_INC_EVENT_PHASE:
    {
        ...
        FC_LOG_DEBUG("scripts.ai", "SmartScript::ProcessAction:: SMART_ACTION_INC_EVENT_PHASE: Creature %u inc event phase by %u, "
    ...
    case SMART_ACTION_EVADE:
    {
        ...
        FC_LOG_DEBUG("scripts.ai", "SmartScript::ProcessAction:: SMART_ACTION_EVADE: Creature %u EnterEvadeMode", me->GetGUID().GetCounter());
        break;
    }
    case SMART_ACTION_FLEE_FOR_ASSIST:
    {
        ...
        {
            Firelands::BroadcastTextBuilder builder(me, CHAT_MSG_MONSTER_EMOTE, BROADCAST_TEXT_FLEE_FOR_ASSIST, me->getGender());
            sCreatureTextMgr->SendChatPacket(me, builder, CHAT_MSG_MONSTER_EMOTE);
        }
        FC_LOG_DEBUG("scripts.ai", "SmartScript::ProcessAction:: SMART_ACTION_FLEE_FOR_ASSIST: Creature %u DoFleeToGetAssistance", me->GetGUID().GetCounter());
        break;
    }
    case SMART_ACTION_CALL_GROUPEVENTHAPPENS:
    {
        ...
        if (player && GetBaseObject())
        {
            ...
            FC_LOG_DEBUG("scripts.ai", "SmartScript::ProcessAction: SMART_ACTION_CALL_GROUPEVENTHAPPENS: Player %u, group credit for quest %u",
    ...
    case SMART_ACTION_COMBAT_STOP:
    {
        ...
        me->CombatStop(true);
        FC_LOG_DEBUG("scripts.ai", "SmartScript::ProcessAction:: SMART_ACTION_COMBAT_STOP: %s CombatStop", me->GetGUID().ToString().c_str());
        break;
    }
    case SMART_ACTION_REMOVEAURASFROMSPELL:
    {
        for (WorldObject* target : targets)
        {
            ...
            FC_LOG_DEBUG("scripts.ai", "SmartScript::ProcessAction: SMART_ACTION_REMOVEAURASFROMSPELL: Unit %u, spell %u",
    ...
    case SMART_ACTION_FOLLOW:
    {
        ...
        for (WorldObject* target : targets)
        {
            if (IsUnit(target))
            {
                ...
                FC_LOG_DEBUG("scripts.ai", "SmartScript::ProcessAction: SMART_ACTION_FOLLOW: Creature %u following target %u",
    ...
    case SMART_ACTION_RANDOM_PHASE:
    {
        ...
        uint32 phase = Firelands::Containers::SelectRandomContainerElement(phases);
        SetPhase(phase);
        FC_LOG_DEBUG("scripts.ai", "SmartScript::ProcessAction: SMART_ACTION_RANDOM_PHASE: Creature %u sets event phase to %u",
    ...
    case SMART_ACTION_RANDOM_PHASE_RANGE:
    {
        ...
        SetPhase(phase);
        FC_LOG_DEBUG("scripts.ai", "SmartScript::ProcessAction: SMART_ACTION_RANDOM_PHASE_RANGE: Creature %u sets event phase to %u",
            GetBaseObject()->GetGUID().GetCounter(), phase);
        break;
    }
    case SMART_ACTION_CALL_KILLEDMONSTER:
    {
        if (e.target.type == SMART_TARGET_NONE || e.target.type == SMART_TARGET_SELF) // Loot recipient and his group members
        {
            ...
            if (Player* player = me->GetLootRecipient())
            {
                ...
                FC_LOG_DEBUG("scripts.ai", "SmartScript::ProcessAction: SMART_ACTION_CALL_KILLEDMONSTER: Player %u, Killcredit: %u",
    ...
    case SMART_ACTION_SET_INST_DATA:
    {
        ...
        if (!instance)
        {
            FC_LOG_ERROR("sql.sql", "SmartScript: Event %u attempt to set instance data without instance script. EntryOrGuid %d", e.GetEventType(), e.entryOrGuid);
            break;
        }
        ...
        case 0:
            ...
            FC_LOG_DEBUG("scripts.ai", "SmartScript::ProcessAction: SMART_ACTION_SET_INST_DATA: SetData Field: %u, data: %u",
        ...
        case 1:
            ...
            FC_LOG_DEBUG("scripts.ai", "SmartScript::ProcessAction: SMART_ACTION_SET_INST_DATA: SetBossState BossId: %u, State: %u (%s)",
    ...
    case SMART_ACTION_SET_INST_DATA64:
    {
        ...
        FC_LOG_DEBUG("scripts.ai", "SmartScript::ProcessAction: SMART_ACTION_SET_INST_DATA64: Field: %u, data: %s",
    ...
    case SMART_ACTION_DIE:
    {
        ...
        {
            me->KillSelf();
            FC_LOG_DEBUG("scripts.ai", "SmartScript::ProcessAction: SMART_ACTION_DIE: Creature %u", me->GetGUID().GetCounter());
        }
        break;
    }
    case SMART_ACTION_SET_IN_COMBAT_WITH_ZONE:
    {
        if (me && me->IsAIEnabled())
        {
            ...
            FC_LOG_DEBUG("scripts.ai", "SmartScript::ProcessAction: SMART_ACTION_SET_IN_COMBAT_WITH_ZONE: Creature %u", me->GetGUID().GetCounter());
        }
        break;
    }
    case SMART_ACTION_CALL_FOR_HELP:
    {
        if (me)
        {
            ...
            {
                Firelands::BroadcastTextBuilder builder(me, CHAT_MSG_MONSTER_EMOTE, BROADCAST_TEXT_CALL_FOR_HELP, me->getGender());
                sCreatureTextMgr->SendChatPacket(me, builder, CHAT_MSG_MONSTER_EMOTE);
            }
            FC_LOG_DEBUG("scripts.ai", "SmartScript::ProcessAction: SMART_ACTION_CALL_FOR_HELP: Creature %u", me->GetGUID().GetCounter());
        }
        break;
    }
    case SMART_ACTION_SET_SHEATH:
    {
        if (me)
        {
            ...
            FC_LOG_DEBUG("scripts.ai", "SmartScript::ProcessAction: SMART_ACTION_SET_SHEATH: Creature %u, State: %u",
    ...
    case SMART_ACTION_ATTACK_START:
    {
        ...
        if (Unit* target = Firelands::Containers::SelectRandomContainerElement(targets)->ToUnit())
            ...
            for (WorldObject* target : targets)
            {
                if (IsCreature(target))
                {
                    if (SmartAI* ai = CAST_AI(SmartAI, target->ToCreature()->AI()))
                        ...
                        FC_LOG_ERROR("sql.sql", "SmartScript: Action target for SMART_ACTION_SET_COUNTER is not using SmartAI, skipping");
                }
                else if (IsGameObject(target))
                {
                    if (SmartGameObjectAI* ai = CAST_AI(SmartGameObjectAI, target->ToGameObject()->AI()))
                        ...
                        FC_LOG_ERROR("sql.sql", "SmartScript: Action target for SMART_ACTION_SET_COUNTER is not using SmartGameObjectAI, skipping");
                }
            }
        }
        else
    ...
    case SMART_ACTION_MOVE_TO_POS:
    {
        ...
        e.GetTargetType() == SMART_TARGET_SELF || e.GetTargetType() == SMART_TARGET_STORED)*/
        {
            ...
            if (!targets.empty())
                target = Firelands::Containers::SelectRandomContainerElement(targets);
        }
    ...
    case SMART_ACTION_ENABLE_TEMP_GOBJ:
    {
        ...
        {
            if (IsCreature(target))
                FC_LOG_WARN("sql.sql", "Invalid creature target '%s' (entry %u, spawnId %u) specified for SMART_ACTION_ENABLE_TEMP_GOBJ", target->GetName().c_str(), target->GetEntry(), target->ToCreature()->GetSpawnId());
            else if (IsGameObject(target))
            {
                if (target->ToGameObject()->isSpawnedByDefault())
                    FC_LOG_WARN("sql.sql", "Invalid gameobject target '%s' (entry %u, spawnId %u) for SMART_ACTION_ENABLE_TEMP_GOBJ - the object is spawned by default", target->GetName().c_str(), target->GetEntry(), target->ToGameObject()->GetSpawnId());
                else
        ...
        for (WorldObject* target : targets)
        {
            if (Creature* npc = target->ToCreature())
            {
                ...
                if (equipId)
                {
                    ...
                    if (!einfo)
                    {
                        FC_LOG_ERROR("sql.sql", "SmartScript: SMART_ACTION_EQUIP uses non-existent equipment info id %u for creature %u", equipId, npc->GetEntry());
                        break;
                    }
    ...
    case SMART_ACTION_CALL_TIMED_ACTIONLIST:
    {
        ...
        {
            FC_LOG_ERROR("sql.sql", "SmartScript: Entry %d SourceType %u Event %u Action %u is using TARGET_NONE(0) for Script9 target. Please correct target_type in database.", e.entryOrGuid, e.GetScriptType(), e.GetEventType(), e.GetActionType());
            break;
        }
        ...
        for (WorldObject* caster : casters)
        {
            ...
            for (WorldObject* target : targets)
            {
                ...
                if (!(e.action.crossCast.castFlags & SMARTCAST_AURA_NOT_PRESENT) || !target->ToUnit()->HasAura(e.action.crossCast.spell))
                {
                    ...
                    FC_LOG_DEBUG("scripts.ai", "Spell %u not cast because it has flag SMARTCAST_AURA_NOT_PRESENT and the target (%s) already has the aura", e.action.crossCast.spell, target->GetGUID().ToString().c_str());
            }
        }
        break;
    }
    case SMART_ACTION_CALL_RANDOM_TIMED_ACTIONLIST:
    {
        ...
        uint32 id = Firelands::Containers::SelectRandomContainerElement(actionLists);
        if (e.GetTargetType() == SMART_TARGET_NONE)
        {
            FC_LOG_ERROR("sql.sql", "SmartScript: Entry %d SourceType %u Event %u Action %u is using TARGET_NONE(0) for Script9 target. Please correct target_type in database.", e.entryOrGuid, e.GetScriptType(), e.GetEventType(), e.GetActionType());
            break;
        ...
        for (WorldObject* target : targets)
        {
            if (IsCreature(target))
            {
                if (SmartAI* ai = CAST_AI(SmartAI, target->ToCreature()->AI()))
                    ...
                    FC_LOG_ERROR("sql.sql", "SmartScript: Action target for SMART_ACTION_SEND_TARGET_TO_TARGET is not using SmartAI, skipping");
            }
            else if (IsGameObject(target))
            {
                if (SmartGameObjectAI* ai = CAST_AI(SmartGameObjectAI, target->ToGameObject()->AI()))
                    ...
                    FC_LOG_ERROR("sql.sql", "SmartScript: Action target for SMART_ACTION_SEND_TARGET_TO_TARGET is not using SmartGameObjectAI, skipping");
            }
        }
        break;
    }
    case SMART_ACTION_SEND_GOSSIP_MENU:
    {
        ...
        FC_LOG_DEBUG("scripts.ai", "SmartScript::ProcessAction:: SMART_ACTION_SEND_GOSSIP_MENU: gossipMenuId %d, gossipNpcTextId %d",
        ...
        for (WorldObject* target : targets)
        {
            if (IsCreature(target))
            {
                    ...
                    e.GetTargetType() == SMART_TARGET_CLOSEST_ENEMY || e.GetTargetType() == SMART_TARGET_CLOSEST_FRIENDLY)
                {
                    ...
                    FC_LOG_ERROR("sql.sql", "SmartScript: Action target for SMART_ACTION_SET_HOME_POS is invalid, skipping");
            }
        }
    ...
    case SMART_ACTION_GAME_EVENT_STOP:
    {
        ...
        {
            FC_LOG_ERROR("sql.sql", "SmartScript::ProcessAction: At case SMART_ACTION_GAME_EVENT_STOP, inactive event (id: %u)", eventId);
            break;
        }
    ...
    case SMART_ACTION_GAME_EVENT_START:
    {
        ...
        if (sGameEventMgr->IsActiveEvent(eventId))
        {
            FC_LOG_ERROR("sql.sql", "SmartScript::ProcessAction: At case SMART_ACTION_GAME_EVENT_START, already activated event (id: %u)", eventId);
            break;
        }
    ...
    case SMART_ACTION_RANDOM_SOUND:
    {
        ...
        for (WorldObject* target : targets)
        {
            if (IsUnit(target))
            {
                uint32 sound = Firelands::Containers::SelectRandomContainerElement(sounds);
                ...
                FC_LOG_DEBUG("scripts.ai", "SmartScript::ProcessAction:: SMART_ACTION_RANDOM_SOUND: target: %s (%s), sound: %u, onlyself: %s",
    ...
    case SMART_ACTION_PLAY_ANIMKIT:
    {
        for (WorldObject* target : targets)
        {
            if (IsCreature(target))
            {
                ...
                {
                    FC_LOG_ERROR("sql.sql", "SmartScript: Invalid type for SMART_ACTION_PLAY_ANIMKIT, skipping");
                    break;
                }

                FC_LOG_DEBUG("scripts.ai", "SmartScript::ProcessAction:: SMART_ACTION_PLAY_ANIMKIT: target: %s (%s), AnimKit: %u, Type: %u",
    ...
    case SMART_ACTION_RESPAWN_BY_SPAWNID:
    {
        ...
        if (map)
            ...
            FC_LOG_ERROR("sql.sql", "SmartScript::ProcessAction: Entry %d SourceType %u, Event %u - tries to respawn by spawnId but does not provide a map", e.entryOrGuid, e.GetScriptType(), e.event_id);
        break;
    }
    ...
    case SMART_ACTION_OVERRIDE_LIGHT:
    {
        if (WorldObject* obj = GetBaseObject())
        {
            ...
            FC_LOG_DEBUG("scripts.ai", "SmartScript::ProcessAction: SMART_ACTION_OVERRIDE_LIGHT: %s sets zone override light (zoneId: %u, areaLightId: %u, overrideLightId: %u, transitionMilliseconds: %u)",
    ...
    case SMART_ACTION_OVERRIDE_WEATHER:
    {
        if (WorldObject* obj = GetBaseObject())
        {
            ...
            FC_LOG_DEBUG("scripts.ai", "SmartScript::ProcessAction: SMART_ACTION_OVERRIDE_WEATHER: %s sets zone weather (zoneId: %u, weatherId: %u, intensity: %u)",
    ...
    case SMART_ACTION_SET_HOVER:
        ...
        break;
    default:
        FC_LOG_ERROR("sql.sql", "SmartScript::ProcessAction: Entry %d SourceType %u, Event %u, Unhandled Action type %u", e.entryOrGuid, e.GetScriptType(), e.event_id, e.GetActionType());
        break;
    }
    ...
    if (e.link && e.link != e.event_id)
    {
        ...
        if (linked)
            ...
            FC_LOG_DEBUG("sql.sql", "SmartScript::ProcessAction: Entry %d SourceType %u, Event %u, Link Event %u not found or invalid, skipped.", e.entryOrGuid, e.GetScriptType(), e.event_id, e.link);
    }
}
...
void SmartScript::InstallTemplate(SmartScriptHolder const& e)
{
    ...
    if (mTemplate)
    {
        FC_LOG_ERROR("sql.sql", "SmartScript::InstallTemplate: Entry %d SourceType %u AI Template can not be set more then once, skipped.", e.entryOrGuid, e.GetScriptType());
        return;
    }
...
void SmartScript::GetTargets(ObjectVector& targets, SmartScriptHolder const& e, Unit* invoker /*= nullptr*/)
{
    ...
    case SMART_TARGET_CREATURE_GUID:
    {
        ...
        {
            FC_LOG_ERROR("sql.sql", "SMART_TARGET_CREATURE_GUID can not be used without invoker");
            break;
        }
    ...
    case SMART_TARGET_GAMEOBJECT_GUID:
    {
        if (!scriptTrigger && !baseObject)
        {
            FC_LOG_ERROR("sql.sql", "SMART_TARGET_GAMEOBJECT_GUID can not be used without invoker");
            break;
        }
...
void SmartScript::GetWorldObjectsInDist(ObjectVector& targets, float dist)
{
    WorldObject* obj = GetBaseObject();
    if (!obj)
        return;

    Firelands::AllWorldObjectsInRange u_check(obj, dist);
    Firelands::WorldObjectListSearcher<Firelands::AllWorldObjectsInRange> searcher(obj, targets, u_check);
...
void SmartScript::ProcessEvent(SmartScriptHolder& e, Unit* unit, uint32 var0, uint32 var1, bool bvar, SpellInfo const* spell, GameObject* gob)
{
    ...
    case SMART_EVENT_FRIENDLY_IS_CC:
    {
        ...
        ProcessTimedAction(e, e.event.friendlyCC.repeatMin, e.event.friendlyCC.repeatMax, Firelands::Containers::SelectRandomContainerElement(pList));
        break;
    }
    case SMART_EVENT_FRIENDLY_MISSING_BUFF:
    {
        ...
        ProcessTimedAction(e, e.event.missingBuff.repeatMin, e.event.missingBuff.repeatMax, Firelands::Containers::SelectRandomContainerElement(pList));
        break;
    }
    ...
    {
        FC_LOG_DEBUG("scripts.ai", "SmartScript: Gossip Select:  menu %u action %u", var0, var1);//little help for scripters
    ...
    case SMART_EVENT_COUNTER_SET:
        ...
        break;
    default:
        FC_LOG_ERROR("sql.sql", "SmartScript::ProcessEvent: Unhandled Event type %u", e.GetEventType());
        break;
    }
}
...
void SmartScript::FillScript(SmartAIEventList e, WorldObject* obj, AreaTriggerEntry const* at)
{
    ...
    {
        if (obj)
            FC_LOG_DEBUG("scripts.ai", "SmartScript: EventMap for Entry %u is empty but is using SmartScript.", obj->GetEntry());
        if (at)
            FC_LOG_DEBUG("scripts.ai", "SmartScript: EventMap for AreaTrigger %u is empty but is using SmartScript.", at->ID);
        return;
    }
...
void SmartScript::OnInitialize(WorldObject* obj, AreaTriggerEntry const* at)
{
    if (obj)//handle object based scripts
    {
        ...
        case TYPEID_UNIT:
            mScriptType = SMART_SCRIPT_TYPE_CREATURE;
            me = obj->ToCreature();
            FC_LOG_DEBUG("scripts.ai", "SmartScript::OnInitialize: source is Creature %u", me->GetEntry());
            break;
        case TYPEID_GAMEOBJECT:
            mScriptType = SMART_SCRIPT_TYPE_GAMEOBJECT;
            go = obj->ToGameObject();
            FC_LOG_DEBUG("scripts.ai", "SmartScript::OnInitialize: source is GameObject %u", go->GetEntry());
            break;
        default:
            FC_LOG_ERROR("misc", "SmartScript::OnInitialize: Unhandled TypeID !WARNING!");
            return;
        }
    }
    else if (at)
    {
        ...
        trigger = at;
        FC_LOG_DEBUG("scripts.ai", "SmartScript::OnInitialize: source is AreaTrigger %u", trigger->ID);
    }
    else
    {
        FC_LOG_ERROR("misc", "SmartScript::OnInitialize: !WARNING! Initialized objects are nullptr.");
        return;
    }
...
Unit* SmartScript::DoSelectLowestHpFriendly(float range, uint32 MinHPDiff)
{
    ...
    Firelands::FriendlyMostHPMissingInRange u_check(me, range, MinHPDiff);
    Firelands::UnitLastSearcher<Firelands::FriendlyMostHPMissingInRange> searcher(me, unit, u_check);
...
Unit* SmartScript::DoSelectLowestHpPctFriendly(float range, uint8 hpPctMin, uint8 hpPctMax)
{
    ...
    Firelands::FriendlyMostHPPctMissingInRange u_check(me, range, hpPctMin, hpPctMax);
    Firelands::UnitLastSearcher<Firelands::FriendlyMostHPPctMissingInRange> searcher(me, unit, u_check);
    Cell::VisitGridObjects(me, searcher, range);
    return unit;
}
...
void SmartScript::DoFindFriendlyCC(std::list<Creature*>& _list, float range)
{
    ...
    Firelands::FriendlyCCedInRange u_check(me, range);
    Firelands::CreatureListSearcher<Firelands::FriendlyCCedInRange> searcher(me, _list, u_check);
    Cell::VisitGridObjects(me, searcher, range);
}
...
void SmartScript::DoFindFriendlyMissingBuff(std::list<Creature*>& list, float range, uint32 spellid)
{
    ...
    Firelands::FriendlyMissingBuffInRange u_check(me, range, spellid);
    Firelands::CreatureListSearcher<Firelands::FriendlyMissingBuffInRange> searcher(me, list, u_check);
    Cell::VisitGridObjects(me, searcher, range);
}
...
Unit* SmartScript::DoFindClosestFriendlyInRange(float range, bool playerOnly)
{
    ...
    Unit* unit = nullptr;
    Firelands::AnyFriendlyUnitInObjectRangeCheck u_check(me, me, range, playerOnly);
    Firelands::UnitLastSearcher<Firelands::AnyFriendlyUnitInObjectRangeCheck> searcher(me, unit, u_check);
    Cell::VisitGridObjects(me, searcher, range);
    return unit;
}
...
void SmartScript::SetScript9(SmartScriptHolder& e, uint32 entry)
{
    ...
    {
        FC_LOG_ERROR("scripts.ai", "Entry %d SourceType %u Event %u Action %u is trying to overwrite timed action list from a timed action, this is not allowed!.", e.entryOrGuid, e.GetScriptType(), e.GetEventType(), e.GetActionType());
        return;
    }
