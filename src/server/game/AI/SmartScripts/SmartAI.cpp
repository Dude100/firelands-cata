...
void SmartAI::PausePath(uint32 delay, bool forced)
{
    ...
    {
        FC_LOG_ERROR("misc", "SmartAI::PausePath: Creature entry %u wanted to pause waypoint (current waypoint: %u) movement while already paused, ignoring.", me->GetEntry(), _currentWaypointNode);
        return;
    }
...
{
    FC_LOG_DEBUG("scripts.ai", "SmartGameObjectAI::OnReportUse");
...
public:
    ...
    bool OnTrigger(Player* player, AreaTriggerEntry const* trigger) override
    {
        ...
        FC_LOG_DEBUG("scripts.ai", "AreaTrigger %u is using SmartTrigger script", trigger->ID);
        SmartScript script;
