...
void ScriptedAI::DoPlaySoundToSet(WorldObject* source, uint32 soundId)
{
    ...
    {
        FC_LOG_ERROR("scripts", "Invalid soundId %u used in DoPlaySoundToSet (Source: TypeId %u, GUID %u)", soundId, source->GetTypeId(), source->GetGUID().GetCounter());
        return;
    }
...
void ScriptedAI::DoTeleportPlayer(Unit* unit, float x, float y, float z, float o)
{
    ...
    if (Player* player = unit->ToPlayer())
        ...
        FC_LOG_ERROR("scripts", "Creature %s Tried to teleport non-player unit (%s) to x: %f y:%f z: %f o: %f. Aborted.",
...
{
    Unit* unit = nullptr;
    Firelands::FriendlyBelowHpPctEntryInRange u_check(me, entry, range, minHPDiff, excludeSelf);
    Firelands::UnitLastSearcher<Firelands::FriendlyBelowHpPctEntryInRange> searcher(me, unit, u_check);
    Cell::VisitAllObjects(me, searcher, range);
...
std::list<Creature*> ScriptedAI::DoFindFriendlyCC(float range)
{
    ...
    Firelands::CreatureListSearcher<Firelands::FriendlyCCedInRange> searcher(me, list, u_check);
    Cell::VisitAllObjects(me, searcher, range);
...
{
    std::list<Creature*> list;
    Firelands::FriendlyMissingBuffInRange u_check(me, range, uiSpellid);
...
Player* ScriptedAI::GetPlayerAtMinimumRange(float minimumRange)
{
    ...
    Firelands::PlayerAtMinimumRangeAway check(me, minimumRange);
    Firelands::PlayerSearcher<Firelands::PlayerAtMinimumRangeAway> searcher(me, player, check);
...
void BossAI::_DespawnAtEvade(Seconds delayToRespawn, Creature* who)
{
    ...
    {
        FC_LOG_ERROR("scripts", "_DespawnAtEvade called with delay of %ld seconds, defaulting to 2.", delayToRespawn.count());
    ...
    {
        FC_LOG_WARN("scripts", "_DespawnAtEvade called on a temporary summon.");
        whoSummon->UnSummon();
        return;
    }
