...
void EscortAI::EnterEvadeMode(EvadeReason /*why*/)
{
    ...
    if (HasEscortState(STATE_ESCORT_ESCORTING))
    {
        ...
        ReturnToLastPoint();
        FC_LOG_DEBUG("scripts", "EscortAI::EnterEvadeMode: left combat and is now returning to last point");
    }
    else
    {
...
void EscortAI::UpdateAI(uint32 diff)
{
        ...
        if (_pauseTimer <= diff)
        {
            if (!HasEscortState(STATE_ESCORT_PAUSED))
            {
                ...
                if (_ended)
                {
                    ...
                    if (_despawnAtEnd)
                    {
                        FC_LOG_DEBUG("scripts", "EscortAI::UpdateAI: reached end of waypoints, despawning at end");
                        if (_returnToStart)
                        {
                            ...
                            FC_LOG_DEBUG("scripts", "EscortAI::UpdateAI: returning to spawn location: %s", respawnPosition.ToString().c_str());
                        }
                    ...
                    FC_LOG_DEBUG("scripts", "EscortAI::UpdateAI: reached end of waypoints");
    ...
    if (_despawnAtFar && HasEscortState(STATE_ESCORT_ESCORTING) && _playerGUID && !me->IsEngaged() && !HasEscortState(STATE_ESCORT_RETURNING))
    {
        if (_playerCheckTimer <= diff)
        {
            ...
            {
                FC_LOG_DEBUG("scripts", "EscortAI::UpdateAI: failed because player/group was to far away or not found");
...
void EscortAI::MovementInform(uint32 type, uint32 id)
{
    ...
    if (type == POINT_MOTION_TYPE)
    {
        ...
        {
            FC_LOG_DEBUG("scripts", "EscortAI::MovementInform: returned to before combat position");
            me->SetWalk(!_running);
            RemoveEscortState(STATE_ESCORT_RETURNING);
        }
        else if (id == POINT_HOME)
        {
            FC_LOG_DEBUG("scripts", "EscortAI::MovementInform: returned to home location and restarting waypoint path");
            _started = false;
        }
    }
    else if (type == WAYPOINT_MOTION_TYPE)
    {
        ...
        FC_LOG_DEBUG("scripts", "EscortAI::MovementInform: waypoint node %u reached", waypoint.Id);
...
void EscortAI::FillPointMovementListForCreature()
{
    ...
    for (WaypointNode const& value : path->Nodes)
    {
        ...
        Firelands::NormalizeMapCoord(node.X);
        Firelands::NormalizeMapCoord(node.Y);
...
void EscortAI::Start(bool isActiveAttacker /* = true*/, bool run /* = false */, ObjectGuid playerGUID /* = 0 */, Quest const* quest /* = nullptr */, bool instantRespawn /* = false */, bool canLoopPath /* = false */, bool resetWaypoints /* = true */)
{
    ...
    if (me->IsEngaged())
    {
        FC_LOG_ERROR("scripts", "EscortAI::Start: (script: %s, creature entry: %u) attempts to Start while in combat", me->GetScriptName().c_str(), me->GetEntry());
        return;
    }

    if (HasEscortState(STATE_ESCORT_ESCORTING))
    {
        FC_LOG_ERROR("scripts", "EscortAI::Start: (script: %s, creature entry: %u) attempts to Start while already escorting", me->GetScriptName().c_str(), me->GetEntry());
        return;
    }
    ...
    {
        FC_LOG_ERROR("scripts", "EscortAI::Start: (script: %s, creature entry: %u) starts with 0 waypoints (possible missing entry in script_waypoint. Quest: %u).", me->GetScriptName().c_str(), me->GetEntry(), quest ? quest->GetQuestId() : 0);
        return;
    }
    ...
    if (_returnToStart && _instantRespawn)
        FC_LOG_DEBUG("scripts", "EscortAI::Start: (script: %s, creature entry: %u) is set to return home after waypoint end and instant respawn at waypoint end. Creature will never despawn.", me->GetScriptName().c_str(), me->GetEntry());
    ...
    FC_LOG_DEBUG("scripts", "EscortAI::Start: (script: %s, creature entry: %u) started with %u waypoints. ActiveAttacker = %d, Run = %d, Player = %s", me->GetScriptName().c_str(), me->GetEntry(), uint32(_path.Nodes.size()), _activeAttacker, _running, _playerGUID.ToString().c_str());
