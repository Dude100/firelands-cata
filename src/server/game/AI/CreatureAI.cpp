...
void CreatureAI::DoZoneInCombat(Creature* creature /*= nullptr*/)
{
    ...
    {
        FC_LOG_ERROR("misc", "DoZoneInCombat call for map that isn't an instance (creature entry = %d)", creature->GetTypeId() == TYPEID_UNIT ? creature->ToCreature()->GetEntry() : 0);
        return;
    }
...
void CreatureAI::EnterEvadeMode(EvadeReason why)
{
    ...
    FC_LOG_DEBUG("entities.unit", "Creature %u enters evade mode.", me->GetEntry());
...
void CreatureAI::EngagementStart(Unit* who)
{
    if (_isEngaged)
    {
        FC_LOG_ERROR("scripts.ai", "CreatureAI::EngagementStart called even though creature is already engaged.");
        return;
    }
    _isEngaged = true;
...
void CreatureAI::EngagementOver()
{
    if (!_isEngaged)
    {
        FC_LOG_ERROR("scripts.ai", "CreatureAI::EngagementOver called even though creature is not currently engaged.");
        return;
    }
    _isEngaged = false;
