...
void ArchaeologyMgr::LoadData()
{
    ...
    if (!result)
    {
        FC_LOG_INFO("server.loading", ">> Loaded 0 archaeology sites. DB table `archaeology_site` is empty!");
        return;
    }
    ...
    FC_LOG_INFO("server.loading", ">> Loaded %u archaeology sites in %u ms", count, GetMSTimeDiffToNow(oldMSTime));
    ...
    for (uint32 i = 0; i < sQuestPOIPointStore.GetNumRows(); i++)
    {
        ...
        for (std::list<std::pair<uint16, uint32> >::iterator itr = sites.begin(); itr != sites.end();)
        {
            if (itr->second == entry->QuestPOIBlobID)
            {
                ...
                if (_polygonMap[site])
                {
                    FC_LOG_ERROR("server.loading", "Archaeology: Tried to overwrite polygonData of site %u", site);
                    break;
                }
                ...
                for (uint32 j = 1; j < 12; j++)
                {
                    ...
                    {
                        FC_LOG_ERROR("server.loading", "Archaeology: Tried to use POI %u not related to site %u", entry->ID, poiRel);
                        break;
                    }
    ...
    for (std::list<std::pair<uint16, uint32> >::iterator itr = sites.begin(); itr != sites.end(); itr++)
        FC_LOG_ERROR("player.skills", "Archaeology: SiteEntry:%u links at POIBlob:%u which does not exist", itr->first, itr->second);

    FC_LOG_INFO("server.loading", ">> Loaded %u archaeology site polygons in %u ms", count, GetMSTimeDiffToNow(oldMSTime));
    ...
    FC_LOG_INFO("server.loading", ">> Loaded %u archaeology branches in %u ms", count, GetMSTimeDiffToNow(oldMSTime));
    ...
    if (!result)
    {
        FC_LOG_INFO("server.loading", ">> Loaded 0 archaeology projects. DB table `archaeology_projects` is empty!");
        return;
    }
    ...
    FC_LOG_INFO("server.loading", ">> Loaded %u archaeology projects in %u ms", count, GetMSTimeDiffToNow(oldMSTime));
    ...
    FC_LOG_INFO("server.loading", ">> Calculated %u archaeology branch chances in %u ms", count, GetMSTimeDiffToNow(oldMSTime));
}
...
{
    if (!_objectMap[entry])
        FC_LOG_ERROR("player.skills", "Archaeology: Entry %u has no find object Type!", entry);
