...
void CalendarMgr::LoadFromDB()
{
    ...
    if (QueryResult result = CharacterDatabase.Query("SELECT id, creator, title, description, type, dungeon, eventtime, flags, time2 FROM calendar_events"))
        ...
        FC_LOG_INFO("server.loading", ">> Loaded %u calendar events in %u ms", count, GetMSTimeDiffToNow(oldMSTime));
        count = 0;
        oldMSTime = getMSTime();
        ...
        if (QueryResult result = CharacterDatabase.Query("SELECT id, event, invitee, sender, status, statustime, `rank`, text FROM calendar_invites"))
            ...
            FC_LOG_INFO("server.loading", ">> Loaded %u calendar invites in %u ms", count, GetMSTimeDiffToNow(oldMSTime));
...
CalendarEvent* CalendarMgr::GetEvent(uint64 eventId) const
{
    ...
    FC_LOG_DEBUG("calendar", "CalendarMgr::GetEvent: [" UI64FMTD "] not found!", eventId);
    return nullptr;
}
...
CalendarInvite* CalendarMgr::GetInvite(uint64 inviteId) const
{
    ...
    FC_LOG_DEBUG("calendar", "CalendarMgr::GetInvite: [" UI64FMTD "] not found!", inviteId);
    return nullptr;
}
