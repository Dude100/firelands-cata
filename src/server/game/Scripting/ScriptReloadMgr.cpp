...
public:
    ...
    void operator() (HandleType handle) const
    {
        ...
        if (!success)
        {
            FC_LOG_ERROR("scripts.hotswap", "Failed to unload (syscall) the shared library \"%s\".",
        ...
        if (cache_path_)
        {
            ...
            {
                FC_LOG_ERROR("scripts.hotswap", "Failed to delete the cached shared library \"%s\" (%s).",
                    cache_path_->generic_string().c_str(), error.message().c_str());

                return;
            }

            FC_LOG_TRACE("scripts.hotswap", "Lazy unloaded the shared library \"%s\" "
...
ScriptModule::CreateFromPath(fs::path const& path, Optional<fs::path> cache_path)
{
    ...
    if (!handle)
    {
        if (cache_path)
        {
            FC_LOG_ERROR("scripts.hotswap", "Could not dynamic load the shared library \"%s\" "
    ...
    {
        FC_LOG_ERROR("scripts.hotswap", "Could not extract all required functions from the shared library \"%s\"!",
            path.generic_string().c_str());
...
static bool HasValidScriptModuleName(std::string const& name)
{
    ...
    static Firelands::regex const regex(
        Firelands::StringFormat("^%s[sS]cripts_[a-zA-Z0-9_]+\\.%s$",
            GetSharedLibraryPrefix(),
            GetSharedLibraryExtension()));

    return Firelands::regex_match(name, regex);
}
...
static int InvokeCMakeCommand(T&&... args)
{
    ...
    return Firelands::StartProcess(executable, {
        std::forward<T>(args)...
        }, "scripts.hotswap");
}
...
template<typename... T>
static std::shared_ptr<Firelands::AsyncProcessResult> InvokeAsyncCMakeCommand(T&&... args)
{
    auto const executable = BuiltInConfig::GetCMakeCommand();
    return Firelands::StartAsyncProcess(executable, {
        std::forward<T>(args)...
        }, "scripts.hotswap");
}
    ...
    : public ScriptReloadMgr
{
    ...
    class BuildJob
    {
        ...
        std::shared_ptr<Firelands::AsyncProcessResult> async_result_;
    ...
    public:
        ...
        std::shared_ptr<Firelands::AsyncProcessResult> const& GetProcess() const
        {
        ...
        void UpdateCurrentJob(BuildJobType type,
            std::shared_ptr<Firelands::AsyncProcessResult> async_result)
        {
...
public:
    ...
    void Initialize() final override
    {
        ...
        {
            FC_LOG_ERROR("scripts.hotswap", "Your build directory path \"%s\" "
...

        {
            ...
            {
                FC_LOG_ERROR("scripts.hotswap", "Library directory \"%s\" doesn't exist!.",
        ...
        {
            FC_LOG_ERROR("scripts.hotswap", "Couldn't create the cache directory at \"%s\".",
...
private:
    ...
    void InitializeDefaultLibraries()
    {
        ...
        for (fs::directory_iterator dir_itr(libraryDirectory); dir_itr != dir_end; ++dir_itr)
            ...
            {
                FC_LOG_INFO("scripts.hotswap", "Loading script module \"%s\"...",
        ...
        FC_LOG_INFO("scripts.hotswap", ">> Loaded %u script modules.", count);
    }
    ...
    void InitializeFileWatchers()
    {
        ...
        if (_libraryWatcher >= 0)
        {
            FC_LOG_INFO("scripts.hotswap", ">> Library reloader is listening on \"%s\".",
                GetLibraryDirectory().generic_string().c_str());
        }
        else
        {
            FC_LOG_ERROR("scripts.hotswap", "Failed to initialize the library reloader on \"%s\".",
                GetLibraryDirectory().generic_string().c_str());
        }
    ...
    static fs::path CalculateTemporaryCachePath()
    {
        ...
        path /= Firelands::StringFormat("tc_script_cache_%s_%s",
    ...
    fs::path GenerateUniquePathForLibraryInCache(fs::path path)
    {
        ...
        cache_path /= Firelands::StringFormat("%s.%u%s",
    ...
    void ProcessLoadScriptModule(fs::path const& path, bool swap_context = true)
    {
...

        {
            ...
            if (code)
            {
                FC_LOG_FATAL("scripts.hotswap", ">> Failed to create cache entry for module "
            ...
            FC_LOG_TRACE("scripts.hotswap", ">> Copied the shared library \"%s\" to \"%s\" for caching.",
                path.filename().generic_string().c_str(), cache_path.generic_string().c_str());
        }
        ...
        if (!module)
        {
            FC_LOG_FATAL("scripts.hotswap", ">> Failed to load script module \"%s\"!",
        ...
        FC_LOG_INFO("scripts.hotswap", ">> Loaded script module \"%s\" (\"%s\" - %s).",
            path.filename().generic_string().c_str(), module_name.c_str(), module_revision.c_str());

        if (module_revision.empty())
        {
            FC_LOG_WARN("scripts.hotswap", ">> Script module \"%s\" has an empty revision hash!",
                path.filename().generic_string().c_str());
        }
        else
        {
            ...
            {
                FC_LOG_WARN("scripts.hotswap", ">> Script module \"%s\" has a different revision hash! "
...

        {
            ...
            {
                FC_LOG_ERROR("scripts.hotswap", ">> Attempt to load a module twice \"%s\" (loaded module is at %s)!",
        ...
        auto listener = Firelands::make_unique<SourceUpdateListener>(
        ...
        (*module)->AddScripts();
        FC_LOG_TRACE("scripts.hotswap", ">> Registered all scripts of module %s.", module_name.c_str());
    ...
    void ProcessUnloadScriptModule(fs::path const& path, bool finish = true)
    {
        ...
        FC_LOG_INFO("scripts.hotswap", "Released script module \"%s\" (\"%s\")...",
    ...
    void DispatchRunningBuildJobs()
    {
        if (_build_job)
        {
            ...
            if (sWorld->getBoolConfig(CONFIG_HOTSWAP_EARLY_TERMINATION_ENABLED))
            {
                if (!terminate_early && _sources_changed.find(_build_job->GetModuleName()) != _sources_changed.end())
                {
                    ...
                    FC_LOG_INFO("scripts.hotswap", "Terminating the running build of module \"%s\"...",
        ...
        if (IsDebuggerBlockingRebuild())
        {
                ...
                (GetMSTimeDiffToNow(_last_time_user_informed) > 7500))
            {
                ...
                FC_LOG_INFO("scripts.hotswap", "Your attached debugger is blocking the FirelandsCore "
        ...
        auto module_name = [&]
        {
            ...
            if (sLog->ShouldLog("scripts.hotswap", LogLevel::LOG_LEVEL_TRACE))
                ...
                {
                    FC_LOG_TRACE("scripts.hotswap", "Source file %s was %s.",
        ...
        FC_LOG_INFO("scripts.hotswap", "Recompiling Module \"%s\"...",
            module_name.c_str());
    ...
    void ProcessReadyBuildJob()
    {
        ...
        case BuildJobType::BUILD_JOB_RERUN_CMAKE:
        {
            if (!error)
            {
                FC_LOG_INFO("scripts.hotswap", ">> Successfully updated the build files!");
            }
            else
            {
                FC_LOG_INFO("scripts.hotswap", ">> Failed to update the build files at \"%s\", "
        ...
        case BuildJobType::BUILD_JOB_INSTALL:
        {
            if (!error)
            {
                ...
                FC_LOG_INFO("scripts.hotswap", ">> Successfully installed module %s in %us",
                    _build_job->GetModuleName().c_str(),
    ...
    void DoRerunCMake()
    {
        ...
        FC_LOG_INFO("scripts.hotswap", "Rerunning CMake because there were sources added or removed...");
    ...
    void DoCompileCurrentProcessedModule()
    {
        ...
        FC_LOG_INFO("scripts.hotswap", "Starting asynchronous build job for module %s...",
            _build_job->GetModuleName().c_str());
    ...
    void DoInstallCurrentProcessedModule()
    {
        ...
        FC_LOG_INFO("scripts.hotswap", "Starting asynchronous install job for module %s...",
            _build_job->GetModuleName().c_str());
    ...
    {
        FC_LOG_INFO("scripts.hotswap", "Correcting your CMAKE_INSTALL_PREFIX in \"%s\"...",
            BuiltInConfig::GetBuildDirectory().c_str());
        ...
        if (![&]
            {
                boost::system::error_code error;
                    ...
                    {
                        FC_LOG_ERROR("scripts.hotswap", ">> CMake cache \"%s\" doesn't exist, "
            ...
            FC_LOG_TRACE("scripts.hotswap", "Checking CMake cache (\"%s\") "
            ...
            std::string cmake_cache_content;
            {
                ...
                if (!in.is_open())
                {
                    FC_LOG_ERROR("scripts.hotswap", ">> Failed to read the CMake cache at \"%s\"!",
                        cmake_cache_path.generic_string().c_str());
            ...
            if (begin != std::string::npos)
            {
                ...
                if (end != std::string::npos)
                {
                    ...
                    if (value != current_path)
                    {
                        ...
                        FC_LOG_INFO("scripts.hotswap", ">> Found outdated CMAKE_INSTALL_PREFIX (\"%s\"), "
                            "worldserver is currently installed at %s",
                            value.generic_string().c_str(), current_path.generic_string().c_str());
                    }
                    else
                    {
                        FC_LOG_INFO("scripts.hotswap", ">> CMAKE_INSTALL_PREFIX is equal to the current path of execution.");
                        return;
                    }
                }
            }

            FC_LOG_INFO("scripts.hotswap", "Invoking CMake cache correction...");
            ...
            if (error)
            {
                FC_LOG_ERROR("scripts.hotswap", ">> Failed to update the CMAKE_INSTALL_PREFIX! "
                    "This could lead to unexpected behaviour!");
            }
            else
            {
                FC_LOG_ERROR("scripts.hotswap", ">> Successfully corrected your CMAKE_INSTALL_PREFIX variable"
...
{
    // FC_LOG_TRACE("scripts.hotswap", "Library listener detected change on possible module \"%s\ (%s)".", filename.c_str(), ActionToString(action));
    ...
    sScriptReloadMgr->QueueMessage([=](HotSwapScriptReloadMgr* reloader) mutable
        {
            ...
            case efsw::Actions::Add:
                FC_LOG_TRACE("scripts.hotswap", ">> Loading \"%s\" (%s)...",
                    path.generic_string().c_str(), ActionToString(action));
                reloader->QueueSharedLibraryChanged(path);
                break;
            case efsw::Actions::Delete:
                FC_LOG_TRACE("scripts.hotswap", ">> Unloading \"%s\" (%s)...",
                    path.generic_string().c_str(), ActionToString(action));
                reloader->QueueSharedLibraryChanged(path);
                break;
            case efsw::Actions::Modified:
                FC_LOG_TRACE("scripts.hotswap", ">> Reloading \"%s\" (%s)...",
                    path.generic_string().c_str(), ActionToString(action));
                reloader->QueueSharedLibraryChanged(path);
                break;
            default:
...
{
    static Firelands::regex const regex("^\\.(h|hpp|c|cc|cpp)$");
    return Firelands::regex_match(path.extension().generic_string(), regex);
}
    ...
    watcher_id_(sScriptReloadMgr->_fileWatcher.addWatch(path_.generic_string(), this, true))
{
    if (watcher_id_ >= 0)
    {
        FC_LOG_TRACE("scripts.hotswap", ">> Attached the source recompiler to \"%s\".",
            path_.generic_string().c_str());
    }
    else
    {
        FC_LOG_ERROR("scripts.hotswap", "Failed to initialize thesource recompiler on \"%s\".",
            path_.generic_string().c_str());
    }
}
...
SourceUpdateListener::~SourceUpdateListener()
{
    if (watcher_id_ >= 0)
    {
        ...
        FC_LOG_TRACE("scripts.hotswap", ">> Detached the source recompiler from \"%s\".",
            path_.generic_string().c_str());
    }
}
...
void SourceUpdateListener::handleFileAction(efsw::WatchID watchid, std::string const& dir,
    std::string const& filename, efsw::Action action, std::string oldFilename)
{
    // FC_LOG_TRACE("scripts.hotswap", "Source listener detected change on possible file \"%s/%s\" (%s).", dir.c_str(), filename.c_str(), ActionToString(action));
    ...
    sScriptReloadMgr->QueueMessage([=](HotSwapScriptReloadMgr* reloader)
        {
            FC_LOG_TRACE("scripts.hotswap", "Detected source change on module \"%s\", "
                "queued for recompilation...", script_module_name_.c_str());

            switch (action)
            {
            case efsw::Actions::Add:
                FC_LOG_TRACE("scripts.hotswap", "Source file %s of module %s was added.",
                    path.generic_string().c_str(), script_module_name_.c_str());
                reloader->QueueAddSourceFile(script_module_name_, path);
                break;
            case efsw::Actions::Delete:
                FC_LOG_TRACE("scripts.hotswap", "Source file %s of module %s was deleted.",
                    path.generic_string().c_str(), script_module_name_.c_str());
                reloader->QueueRemoveSourceFile(script_module_name_, path);
                break;
            case efsw::Actions::Modified:
                FC_LOG_TRACE("scripts.hotswap", "Source file %s of module %s was modified.",
                    path.generic_string().c_str(), script_module_name_.c_str());
