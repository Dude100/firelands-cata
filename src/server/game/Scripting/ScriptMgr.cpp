...
public:
    ...
    void LogDuplicatedScriptPointerError(ScriptType const* first, ScriptType const* second)
    {
        ...
        FC_LOG_ERROR("scripts", "Script '%s' has same memory pointer as '%s'.",
...
public:
    ...
    void AddScript(ScriptType* script)
    {
        ...
        if (uint32 const id = sObjectMgr->GetScriptId(script->GetName()))
        {
            ...
            for (auto const& stored_script : _scripts)
            {
                ...
                if (stored_script.second->GetName() == script->GetName())
                {
                    ...
                    FC_LOG_ERROR("scripts", "Script '%s' already assigned with the same script name, "
        ...
        else
        {
            ...
            FC_LOG_ERROR("sql.sql", "Script named '%s' does not have a script name assigned in database.",
...
void ScriptMgr::Initialize()
{
    ...
    FC_LOG_INFO("server.loading", "Loading C++ scripts");
    ...
    for (std::string const& scriptName : unusedScriptNames)
    {
        ...
        FC_LOG_ERROR("sql.sql", "ScriptName '%s' exists in database, "
            "but no core script found!", scriptName.c_str());
    }

    FC_LOG_INFO("server.loading", ">> Loaded %u C++ scripts in %u ms",
...
{
    if (!GetEntry())
        FC_LOG_ERROR("scripts", "Invalid WorldMapScript for %u; no such map ID.", mapId);

    if (GetEntry() && !GetEntry()->IsWorldMap())
        FC_LOG_ERROR("scripts", "WorldMapScript for map %u is invalid.", mapId);
...
{
    if (!GetEntry())
        FC_LOG_ERROR("scripts", "Invalid InstanceMapScript for %u; no such map ID.", mapId);

    if (GetEntry() && !GetEntry()->IsDungeon())
        FC_LOG_ERROR("scripts", "InstanceMapScript for map %u is invalid.", mapId);
...
{
    if (!GetEntry())
        FC_LOG_ERROR("scripts", "Invalid BattlegroundMapScript for %u; no such map ID.", mapId);

    if (GetEntry() && !GetEntry()->IsBattleground())
        FC_LOG_ERROR("scripts", "BattlegroundMapScript for map %u is invalid.", mapId);
