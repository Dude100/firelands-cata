...
void PetitionMgr::LoadPetitions()
{
    ...
    if (!result)
    {
        FC_LOG_INFO("server.loading", ">> Loaded 0 petitions.");
        return;
    }
    ...
    FC_LOG_INFO("server.loading", ">> Loaded %u petitions in: %u ms.", count, GetMSTimeDiffToNow(oldMSTime));
}
...
void PetitionMgr::LoadSignatures()
{
    ...
    if (!result)
    {
        FC_LOG_INFO("server.loading", ">> Loaded 0 Petition signs!");
        return;
    }
    ...
    FC_LOG_INFO("server.loading", ">> Loaded %u Petition signs in %u ms.", count, GetMSTimeDiffToNow(oldMSTime));
}
