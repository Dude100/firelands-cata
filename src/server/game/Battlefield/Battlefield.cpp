...
void Battlefield::InitStalker(uint32 entry, Position const& pos)
{
    if (Creature* creature = SpawnCreature(entry, pos))
        ...
        FC_LOG_ERROR("bg.battlefield", "Battlefield::InitStalker: Could not spawn Stalker (Creature entry %u), zone messages will be unavailable!", entry);
}
...
BfGraveyard* Battlefield::GetGraveyardById(uint32 id) const
{
    if (id < m_GraveyardList.size())
    {
        if (BfGraveyard* graveyard = m_GraveyardList.at(id))
            ...
            FC_LOG_ERROR("bg.battlefield", "Battlefield::GetGraveyardById Id:%u does not exist.", id);
    }
    else
        FC_LOG_ERROR("bg.battlefield", "Battlefield::GetGraveyardById Id:%u could not be found.", id);
...
void BfGraveyard::SetSpirit(Creature* spirit, TeamId team)
{
    if (!spirit)
    {
        FC_LOG_ERROR("bg.battlefield", "BfGraveyard::SetSpirit: Invalid Spirit.");
        return;
    }
...
Creature* Battlefield::SpawnCreature(uint32 entry, Position const& pos)
{
    ...
    {
        FC_LOG_ERROR("bg.battlefield", "Battlefield::SpawnCreature: Can't create creature entry: %u", entry);
        delete creature;
        return nullptr;
    }
...
GameObject* Battlefield::SpawnGameObject(uint32 entry, Position const& pos, QuaternionData const& rot)
{
    ...
    {
        FC_LOG_ERROR("bg.battlefield", "Battlefield::SpawnGameObject: Gameobject template %u could not be found in the database! Battlefield has not been created!", entry);
        FC_LOG_ERROR("bg.battlefield", "Battlefield::SpawnGameObject: Could not create gameobject template %u! Battlefield has not been created!", entry);
        delete go;
        return nullptr;
    }
...
bool BfCapturePoint::SetCapturePointData(GameObject* capturePoint)
{
    ...
    FC_LOG_DEBUG("bg.battlefield", "Creating capture point %u", capturePoint->GetEntry());
    ...
    {
        FC_LOG_ERROR("misc", "OutdoorPvP: GO %u is not a capture point!", capturePoint->GetEntry());
        return false;
    }
...
bool BfCapturePoint::Update(uint32 diff)
{
    ...
    if (GameObject* capturePoint = m_Bf->GetGameObject(m_capturePointSpawnId))
    {
        ...
        Firelands::AnyPlayerInObjectRangeCheck checker(capturePoint, radius);
        Firelands::PlayerListSearcher<Firelands::AnyPlayerInObjectRangeCheck> searcher(capturePoint, players, checker);
    ...
    {
        //FC_LOG_ERROR("bg.battlefield", "%u->%u", m_OldState, m_State);
        if (oldTeam != m_team)
            ChangeTeam(oldTeam);
        return true;
    }
