...
Group::~Group()
{
    if (m_bgGroup)
    {
        FC_LOG_DEBUG("bg.battleground", "Group::~Group: battleground group being deleted.");
        ...
        else FC_LOG_ERROR("misc", "Group::~Group: battleground group is not linked to the correct battleground.");
    }
    Rolls::iterator itr;
    while (!RollId.empty())
    {
    ...
    if (Map* playerMap = player->GetMap())
        if (!switchLeader && playerMap->IsNonRaidDungeon())
            if (InstanceSave* save = sInstanceSaveMgr->GetInstanceSave(playerMap->GetInstanceId()))
                ...
                {
                    FC_LOG_DEBUG("maps", "Group::ConvertLeaderInstancesToGroup: Group for player %s is taking over unbound instance map %d with Id %d", player->GetName().c_str(), playerMap->GetId(), playerMap->GetInstanceId());
...
void Group::GroupLoot(Loot* loot, WorldObject* pLootedObject)
{
    ...
    for (i = loot->items.begin(); i != loot->items.end(); ++i, ++itemSlot)
    {
        ...
        if (!item)
        {
            //FC_LOG_DEBUG("misc", "Group::GroupLoot: missing item prototype for item with id: %d", i->itemid);
            continue;
        }
...
{
    FC_LOG_DEBUG("network", "Group::MasterLoot (SMSG_LOOT_MASTER_LIST)");
        ...
        if (isEmpty || method == INSTANCE_RESET_GROUP_DISBAND || method == INSTANCE_RESET_CHANGE_DIFFICULTY)
        {
            ...
            if (isEmpty && instanceSave->CanReset())
            {
                if (map && this->isRaidGroup() && map->IsDungeon() && SendMsgTo)
                {
                    ...
                    if (!instanceEntrance)
                        FC_LOG_DEBUG("root", "Instance entrance not found for maps %u", map->GetId());
                    else
                    {
...
InstanceGroupBind* Group::BindToInstance(InstanceSave* save, bool permanent, bool load)
{
    ...
    bind.perm = permanent;
    if (!load)
        FC_LOG_DEBUG("maps", "Group::BindToInstance: %s, storage id: %u is now bound to map %d, instance %d, difficulty %d",
...
void Group::BroadcastGroupUpdate(void)
{
    ...
    for (member_citerator citr = m_memberSlots.begin(); citr != m_memberSlots.end(); ++citr)
    {
        ...
        if (pp)
        {
            ...
            FC_LOG_DEBUG("misc", "-- Forced group value update for '%s'", pp->GetName().c_str());
        }
    }
}
