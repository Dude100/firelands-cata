...
void SplineChainMovementGenerator::SendSplineFor(Unit* me, uint32 index, uint32& toNext)
{
    ...
    FC_LOG_DEBUG("movement.splinechain", "%s: Sending spline for %u.", me->GetGUID().ToString().c_str(), index);
    ...
    {
        FC_LOG_DEBUG("movement.splinechain", "%s: Sent spline for %u, duration is %u ms. Expected was %u ms (delta %d ms). Adjusting.", me->GetGUID().ToString().c_str(), index, actualDuration, thisLink.ExpectedDuration, int32(actualDuration) - int32(thisLink.ExpectedDuration));
        toNext = uint32(double(actualDuration) / double(thisLink.ExpectedDuration) * toNext);
    }
    else
    {
        FC_LOG_DEBUG("movement.splinechain", "%s: Sent spline for %u, duration is %u ms.", me->GetGUID().ToString().c_str(), index, actualDuration);
    }
}
...
void SplineChainMovementGenerator::Initialize(Unit* me)
{
    if (!_chainSize)
    {
        FC_LOG_ERROR("movement.splinechain", "SplineChainMovementGenerator::Initialize: couldn't initialize generator, referenced spline is empty! (%s)", me->GetGUID().ToString().c_str());
        return;
    }

    if (_nextIndex >= _chainSize)
    {
        FC_LOG_WARN("movement.splinechain", "SplineChainMovementGenerator::Initialize: couldn't initialize generator, _nextIndex is >= _chainSize (%s)", me->GetGUID().ToString().c_str());
        _msToNext = 0;
        return;
    }
    ...
    if (_nextFirstWP) // this is a resumed movegen that has to start with a partial spline
    {
        ...
        {
            FC_LOG_ERROR("movement.splinechain", "%s: Attempted to resume spline chain from invalid resume state (%u, %u).", me->GetGUID().ToString().c_str(), _nextIndex, _nextFirstWP);
        ...
        FC_LOG_DEBUG("movement.splinechain", "%s: Resumed spline chain generator from resume state.", me->GetGUID().ToString().c_str());
...
bool SplineChainMovementGenerator::Update(Unit* me, uint32 diff)
{
    ...
    {
        // Send next spline
        FC_LOG_DEBUG("movement.splinechain", "%s: Should send spline %u (%u ms late).", me->GetGUID().ToString().c_str(), _nextIndex, diff - _msToNext);
        _msToNext = std::max(_chain[_nextIndex].TimeToNext, 1u);
        SendSplineFor(me, _nextIndex, _msToNext);
        ++_nextIndex;
        if (_nextIndex >= _chainSize)
