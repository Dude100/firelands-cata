    ...
    _navMeshQuery(nullptr)
{
    ...
    FC_LOG_DEBUG("maps.mmaps", "++ PathGenerator::PathGenerator for %u", _source->GetGUID().GetCounter());
...
{
    if (!Firelands::IsValidMapCoord(startPoint.x, startPoint.y, startPoint.z) || !Firelands::IsValidMapCoord(endPoint.x, endPoint.y, endPoint.z))
        return false;
    ...
    FC_LOG_DEBUG("maps.mmaps", "++ PathGenerator::CalculatePath() for %u", _source->GetGUID().GetCounter());
...
void PathGenerator::BuildPolyPath(G3D::Vector3 const& startPos, G3D::Vector3 const& endPos)
{
    ...
    {
        FC_LOG_DEBUG("maps.mmaps", "++ BuildPolyPath :: (startPoly == 0 || endPoly == 0)");
        BuildShortcut();
    ...
    {
        FC_LOG_DEBUG("maps.mmaps", "++ BuildPolyPath :: farFromPoly distToStartPoly=%.3f distToEndPoly=%.3f", distToStartPoly, distToEndPoly);
        ...
        {
            FC_LOG_DEBUG("maps.mmaps", "++ BuildPolyPath :: underWater case");
        ...
        {
            FC_LOG_DEBUG("maps.mmaps", "++ BuildPolyPath :: flying case");
            if (const Unit* _sourceUnit = _source->ToUnit())
            {
    ...
    {
        FC_LOG_DEBUG("maps.mmaps", "++ BuildPolyPath :: (startPoly == endPoly)");
    ...
    if (_polyLength)
    {
        for (; pathStartIndex < _polyLength; ++pathStartIndex)
        {
            ...
            {
                FC_LOG_ERROR("maps.mmaps", "Invalid poly ref in BuildPolyPath. _polyLength: %u, pathStartIndex: %u,"
    ...
    {
        FC_LOG_DEBUG("maps.mmaps", "++ BuildPolyPath :: (startPolyFound && endPolyFound)");
    ...
    else if (startPolyFound && !endPolyFound)
    {
        ...
        if (_useRaycast)
        {
            FC_LOG_ERROR("maps.mmaps", "PathGenerator::BuildPolyPath() called with _useRaycast with a previous path for unit %s", _source->GetGUID().ToString().c_str());
            BuildShortcut();
            _type = PATHFIND_NOPATH;
            return;
        }
        ...
        if (!suffixPolyLength || dtStatusFailed(dtResult))
        {
            ...
            FC_LOG_ERROR("maps", "%u's Path Build failed: 0 length path", _source->GetGUID().GetCounter());
        }

        FC_LOG_DEBUG("maps.mmaps", "++  m_polyLength=%u prefixPolyLength=%u suffixPolyLength=%u", _polyLength, prefixPolyLength, suffixPolyLength);
...
void PathGenerator::BuildPointPath(const float* startPoint, const float* endPoint)
{
    ...
    if (_useRaycast)
    {
        ...
        FC_LOG_ERROR("maps.mmaps", "PathGenerator::BuildPointPath() called with _useRaycast for unit %s", _source->GetGUID().ToString().c_str());
        BuildShortcut();
        _type = PATHFIND_NOPATH;
        return;
    }
    ...
    else if (pointCount < 2 || dtStatusFailed(dtResult))
    {
        ...
        FC_LOG_DEBUG("maps.mmaps", "++ PathGenerator::BuildPointPath FAILED! path sized %d returned", pointCount);
        BuildShortcut();
    ...
    {
        FC_LOG_DEBUG("maps.mmaps", "++ PathGenerator::BuildPointPath FAILED! path sized %d returned, lower than limit set to %d", pointCount, _pointPathLimit);
        BuildShortcut();
    ...
    FC_LOG_DEBUG("maps.mmaps", "++ PathGenerator::BuildPointPath path type %d size %d poly-size %d", _type, pointCount, _polyLength);
}
...
{
    FC_LOG_DEBUG("maps.mmaps", "++ BuildShortcut :: making shortcut");
    ...
    *smoothPathSize = 0;
    ...
    while (npolys && nsmoothPath < maxSmoothPathSize)
    {
        ...
        if (dtStatusFailed(_navMeshQuery->getPolyHeight(polys[0], result, &result[1])))
            FC_LOG_DEBUG("maps.mmaps", "Cannot find height at position X: %f Y: %f Z: %f for unit %u", result[2], result[0], result[1], _source->GetEntry());
        result[1] += 0.5f;
...
void PathGenerator::ShortenPathUntilDist(G3D::Vector3 const& target, float dist)
{
    ...
    {
        FC_LOG_ERROR("maps", "PathGenerator::ReducePathLengthByDist called before path was successfully built");
        return;
    }
