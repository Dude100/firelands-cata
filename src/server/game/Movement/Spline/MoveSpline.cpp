...
namespace Movement {
    ...
    void MoveSpline::init_spline(MoveSplineInitArgs const& args)
    {
        ...
        {
            FC_LOG_DEBUG("misc", "MoveSpline::init_spline: zero length spline, wrong input data?");
...
#define CHECK(exp) \
    ...
    {\
        if (unit)\
            FC_LOG_ERROR("misc.movesplineinitargs", "MoveSplineInitArgs::Validate: expression '%s' failed for GUID: %u Entry: %u", #exp, unit->GetTypeId() == TYPEID_PLAYER ? unit->GetGUID().GetCounter() : unit->ToCreature()->GetSpawnId(), unit->GetEntry());\
        else\
            FC_LOG_ERROR("misc.movesplineinitargs", "MoveSplineInitArgs::Validate: expression '%s' failed for cyclic spline continuation", #exp); \
        return false;\
    }
        CHECK(path.size() > 1);
    ...
    bool MoveSplineInitArgs::_checkPathBounds() const
    {
        if (!(flags & MoveSplineFlag::Catmullrom) && path.size() > 2)
        {
            ...
            for (uint32 i = 1; i < path.size() - 1; ++i)
            {
                ...
                {
                    FC_LOG_ERROR("misc", "MoveSplineInitArgs::_checkPathBounds check failed");
                    return false;
                }
            }
        }
