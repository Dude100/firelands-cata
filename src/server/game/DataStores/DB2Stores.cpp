...
uint32 DB2Manager::LoadStores(std::string const& dataPath, uint32 defaultLocale)
{
    ...
    {
        FC_LOG_ERROR("misc", "Please extract correct db2 files from client 4.3.4 15595.");
        exit(1);
    }

    FC_LOG_INFO("server.loading", ">> Initialized %d DB2 data stores in %u ms", DB2FilesCount, GetMSTimeDiffToNow(oldMSTime));
...
void DB2Manager::LoadHotfixData()
{
    ...
    if (!result)
    {
        FC_LOG_INFO("misc", ">> Loaded 0 hotfix info entries.");
        return;
    }
    ...
    FC_LOG_INFO("misc", ">> Loaded %u hotfix info entries in %u ms", count, GetMSTimeDiffToNow(oldMSTime));
}
