...
void LoadM2Cameras(std::string const& dataPath)
{
    ...
    FC_LOG_INFO("server.loading", ">> Loading Cinematic Camera files");
    ...
    for (CinematicCameraEntry const* dbcentry : sCinematicCameraStore)
    {
        ...
        {
            FC_LOG_ERROR("server.loading", "Camera file %s is damaged. File is smaller than header size", filename.string().c_str());
            m2file.close();
            continue;
        }
        ...
        {
            FC_LOG_ERROR("server.loading", "Camera file %s is damaged. File identifier not found", filename.string().c_str());
            m2file.close();
            continue;
        }
        ...
        {
            FC_LOG_ERROR("server.loading", "Camera file %s is damaged. Camera references position beyond file end", filename.string().c_str());
            continue;
        }
    ...
    FC_LOG_INFO("server.loading", ">> Loaded %u cinematic waypoint sets in %u ms", (uint32)sFlyByCameraStore.size(), GetMSTimeDiffToNow(oldMSTime));
}

std::vector<FlyByCamera> const* GetFlyByCameras(uint32 cinematicCameraId)
{
    return Firelands::Containers::MapGetValuePtr(sFlyByCameraStore, cinematicCameraId);
}
