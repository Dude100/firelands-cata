...
uint64 AuctionHouseMgr::GetAuctionDeposit(AuctionHouseEntry const* entry, uint32 time, Item* pItem, uint32 count)
{
    ...
    FC_LOG_DEBUG("auctionHouse", "MSV:        %u", MSV);
    FC_LOG_DEBUG("auctionHouse", "Items:      %u", count);
    FC_LOG_DEBUG("auctionHouse", "Multiplier: %f", multiplier);
    FC_LOG_DEBUG("auctionHouse", "Deposit:    " UI64FMTD, deposit);
    FC_LOG_DEBUG("auctionHouse", "Deposit rm: %f", remainderbase * count);
...
void AuctionHouseMgr::LoadAuctionItems()
{
    ...
    if (!result)
    {
        FC_LOG_INFO("server.loading", ">> Loaded 0 auction items. DB table `auctionhouse` or `item_instance` is empty!");
    ...
    do
    {
        ...
        if (!proto)
        {
            FC_LOG_ERROR("misc", "AuctionHouseMgr::LoadAuctionItems: Unknown item (GUID: %u item entry: #%u) in auction, skipped.", item_guid, itemEntry);
            continue;
        }
    ...
    FC_LOG_INFO("server.loading", ">> Loaded %u auction items in %u ms", count, GetMSTimeDiffToNow(oldMSTime));
...
void AuctionHouseMgr::LoadAuctions()
{
    ...
    if (!resultAuctions)
    {
        FC_LOG_INFO("server.loading", ">> Loaded 0 auctions. DB table `auctionhouse` is empty.");
    ...
    FC_LOG_INFO("server.loading", ">> Loaded %u auctions with %u bidders in %u ms", countAuctions, countBidders, GetMSTimeDiffToNow(oldMSTime));
}
...
void AuctionHouseMgr::UpdatePendingAuctions()
{
    for (auto itr = pendingAuctionMap.begin(); itr != pendingAuctionMap.end();)
    {
        ...
        else
        {
            ...
            FC_LOG_WARN("auctionHouse", "Player %s was offline, unable to retrieve deposit!", playerGUID.ToString().c_str());
...
bool AuctionEntry::BuildAuctionInfo(WorldPacket& data, Item* sourceItem) const
{
    ...
    if (!item)
    {
        FC_LOG_ERROR("misc", "AuctionEntry::BuildAuctionInfo: Auction %u has a non-existent item: %u", Id, itemGUIDLow);
        return false;
    }
    data << uint32(Id);
...
bool AuctionEntry::LoadFromDB(Field* fields)
{
    ...
    {
        FC_LOG_ERROR("misc", "Auction %u has invalid house id %u", Id, houseId);
        return false;
    }
    ...
    {
        FC_LOG_ERROR("misc", "Auction %u has not a existing item : %u", Id, itemGUIDLow);
        return false;
    }
