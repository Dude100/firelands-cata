...
void InstanceScript::LoadMinionData(MinionData const* data)
{
    ...
    FC_LOG_DEBUG("scripts", "InstanceScript::LoadMinionData: " UI64FMTD " minions loaded.", uint64(minions.size()));
}
...
void InstanceScript::LoadDoorData(DoorData const* data)
{
    ...
    FC_LOG_DEBUG("scripts", "InstanceScript::LoadDoorData: " UI64FMTD " doors loaded.", uint64(doors.size()));
}
...
void InstanceScript::LoadObjectData(ObjectData const* creatureData, ObjectData const* gameObjectData)
{
    ...
    FC_LOG_DEBUG("scripts", "InstanceScript::LoadObjectData: " SZFMTD " objects loaded.", _creatureInfo.size() + _gameObjectInfo.size());
}
...
bool InstanceScript::SetBossState(uint32 id, EncounterState state)
{
    if (id < bosses.size())
    {
        ...
        if (bossInfo->state == TO_BE_DECIDED) // loading
        {
            ...
            FC_LOG_DEBUG("scripts", "InstanceScript: Initialize boss %u state as %s (map %u, %u).", id, GetBossStateName(state), instance->GetId(), instance->GetInstanceId());
            return false;
        }
        ...
        else
        {
            ...
            {
                FC_LOG_ERROR("map", "InstanceScript: Tried to set instance boss %u state from %s back to %s for map %u, instance id %u. Blocked!", id, GetBossStateName(bossInfo->state), GetBossStateName(state), instance->GetId(), instance->GetInstanceId());
                return false;
            }
...
void InstanceScript::HandleGameObject(ObjectGuid guid, bool open, GameObject* go /*= nullptr*/)
{
    ...
    if (go)
        ...
        FC_LOG_DEBUG("scripts", "InstanceScript: HandleGameObject failed");
}
...
void InstanceScript::DoUseDoorOrButton(ObjectGuid guid, uint32 withRestoreTime /*= 0*/, bool useAlternativeState /*= false*/)
{
    ...
    if (GameObject* go = instance->GetGameObject(guid))
    {
        if (go->GetGoType() == GAMEOBJECT_TYPE_DOOR || go->GetGoType() == GAMEOBJECT_TYPE_BUTTON)
        {
            ...
            FC_LOG_ERROR("scripts", "InstanceScript: DoUseDoorOrButton can't use gameobject entry %u, because type is %u.", go->GetEntry(), go->GetGoType());
    }
    else
        FC_LOG_DEBUG("scripts", "InstanceScript: DoUseDoorOrButton failed");
}
...
void InstanceScript::DoCloseDoorOrButton(ObjectGuid guid)
{
    ...
    if (GameObject* go = instance->GetGameObject(guid))
    {
        if (go->GetGoType() == GAMEOBJECT_TYPE_DOOR || go->GetGoType() == GAMEOBJECT_TYPE_BUTTON)
        {
            ...
            FC_LOG_ERROR("scripts", "InstanceScript: DoCloseDoorOrButton can't use gameobject entry %u, because type is %u.", go->GetEntry(), go->GetGoType());
    }
    else
        FC_LOG_DEBUG("scripts", "InstanceScript: DoCloseDoorOrButton failed");
}
...
void InstanceScript::DoRespawnGameObject(ObjectGuid guid, uint32 timeToDespawn /*= MINUTE*/)
{
    if (GameObject* go = instance->GetGameObject(guid))
    {
        ...
        case GAMEOBJECT_TYPE_FISHINGNODE:
            ...
            FC_LOG_ERROR("scripts", "InstanceScript: DoRespawnGameObject can't respawn gameobject entry %u, because type is %u.", go->GetEntry(), go->GetGoType());
            return;
        default:
            break;
        }
        ...
        FC_LOG_DEBUG("scripts", "InstanceScript: DoRespawnGameObject failed");
}
...
void InstanceScript::DoUpdateWorldState(uint32 uiStateId, uint32 uiStateData)
{
    ...
    if (!lPlayers.isEmpty())
    {
        ...
        FC_LOG_DEBUG("scripts", "DoUpdateWorldState attempt send data but no players in map.");
}
...
{
    FC_LOG_ERROR("misc", "Achievement system call InstanceScript::CheckAchievementCriteriaMeet but instance script for map %u not have implementation for achievement criteria %u",
...
void InstanceScript::UpdateEncounterState(EncounterCreditType type, uint32 creditEntry, Unit* /*source*/)
{
    ...
    for (DungeonEncounter const* encounter : *encounters)
    {
        ...
        if (encounter->lastEncounterDungeon)
        {
            ...
            FC_LOG_DEBUG("lfg", "UpdateEncounterState: Instance %s (instanceId %u) completed encounter %s. Credit Dungeon: %u", instance->GetMapName(), instance->GetInstanceId(), encounter->dbcEntry->Name, dungeonId);
            break;
        }
