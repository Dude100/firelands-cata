...
char const* ChatHandler::GetFirelandsString(uint32 entry) const
{
    return m_session->GetFirelandsString(entry);
}
...
{
    SendSysMessage(GetFirelandsString(entry));
}
...
bool ChatHandler::SetDataForCommandInTable(std::vector<ChatCommand>& table, char const* text, uint32 permission, std::string const& help, std::string const& fullcommand)
{
    ...
    for (uint32 i = 0; i < table.size(); i++)
    {
        ...
        {
            FC_LOG_ERROR("sql.sql", "Table `command` contains an unexpected subcommand '%s' in command '%s', skipped.", text, fullcommand.c_str());
            return false;
        }

        if (table[i].Permission != permission)
            FC_LOG_INFO("misc", "Table `command` overwrite for command '%s' default permission (%u) by %u", fullcommand.c_str(), table[i].Permission, permission);
    ...
    if (!cmd.empty())
    {
        if (&table == &getCommandTable())
            FC_LOG_ERROR("sql.sql", "Table `command` contains a non-existing command '%s', skipped.", cmd.c_str());
        else
            FC_LOG_ERROR("sql.sql", "Table `command` contains a non-existing subcommand '%s' in command '%s', skipped.", cmd.c_str(), fullcommand.c_str());
    }
...
GameObject* ChatHandler::GetNearbyGameObject()
{
    ...
    Firelands::NearestGameObjectCheck check(*pl);
    Firelands::GameObjectLastSearcher<Firelands::NearestGameObjectCheck> searcher(pl, obj, check);
...
char const* CliHandler::GetFirelandsString(uint32 entry) const
{
...
{
    return GetFirelandsString(LANG_CONSOLE_COMMAND);
}
...
{
    if (memcmp(str, "FirelandsCore\t", 12))
        return false;
    char opcode = str[12];
...
void AddonChannelCommandHandler::SendSysMessage(char const* str, bool escapeCharacters)
{
    ...
    std::string msg = "FirelandsCore\tm";
    msg.append(echo, 4);
    std::string body(str);
    if (escapeCharacters)
