    ...
    _zoneEntry(nullptr)
{
    ...
    if (sWorld->getBoolConfig(CONFIG_PRESERVE_CUSTOM_CHANNELS))
    {
        ...
        if (PreparedQueryResult result = CharacterDatabase.Query(stmt)) // load
        {
            ...
            if (!db_BannedList.empty())
            {
                ...
                for (auto const& token : tokens)
                {
                    ...
                    if (banned_guid)
                    {
                        FC_LOG_DEBUG("chat.system", "Channel(%s) loaded player %s into bannedStore", name.c_str(), banned_guid.ToString().c_str());
        ...
        else // save
        {
            ...
            FC_LOG_DEBUG("chat.system", "Channel(%s) saved in database", name.c_str());
        }
...
void Channel::UpdateChannelInDB() const
{
    if (_persistentChannel)
    {
        ...
        FC_LOG_DEBUG("chat.system", "Channel(%s) updated in database", _channelName.c_str());
    }
}
...
void Channel::CleanOldChannelsInDB()
{
    if (sWorld->getIntConfig(CONFIG_PRESERVE_CUSTOM_CHANNEL_DURATION) > 0)
    {
        ...
        FC_LOG_DEBUG("chat.system", "Cleaned out unused custom chat channels.");
    }
}
...
void Channel::List(Player const* player) const
{
    ...
    FC_LOG_DEBUG("chat.system", "SMSG_CHANNEL_LIST %s Channel: %s",
