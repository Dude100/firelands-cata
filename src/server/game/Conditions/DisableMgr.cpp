...
namespace DisableMgr
{
    ...
    void LoadDisables()
    {
        ...
        if (!result)
        {
            FC_LOG_INFO("server.loading", ">> Loaded 0 disables. DB table `disables` is empty!");
            return;
        }
        ...
        do
        {
            ...
            {
                FC_LOG_ERROR("sql.sql", "Invalid type %u specified in `disables` table, skipped.", type);
                continue;
            }
            ...
            case DISABLE_TYPE_SPELL:
                ...
                {
                    FC_LOG_ERROR("sql.sql", "Spell entry %u from `disables` doesn't exist in dbc, skipped.", entry);
                    continue;
                }

                if (!flags || flags > MAX_SPELL_DISABLE_TYPE)
                {
                    FC_LOG_ERROR("sql.sql", "Disable flags for spell %u are invalid, skipped.", entry);
                    continue;
                }
            ...
            case DISABLE_TYPE_LFG_MAP:
            {
                ...
                if (!mapEntry)
                {
                    FC_LOG_ERROR("sql.sql", "Map entry %u from `disables` doesn't exist in dbc, skipped.", entry);
                    continue;
                }
                ...
                case MAP_ARENA:
                    FC_LOG_ERROR("sql.sql", "Battleground map %u specified to be disabled in map case, skipped.", entry);
                    continue;
                }
                if (isFlagInvalid)
                {
                    FC_LOG_ERROR("sql.sql", "Disable flags for map %u are invalid, skipped.", entry);
                    continue;
                }
                break;
            }
            case DISABLE_TYPE_BATTLEGROUND:
                ...
                {
                    FC_LOG_ERROR("sql.sql", "Battleground entry %u from `disables` doesn't exist in dbc, skipped.", entry);
                    continue;
                }
                if (flags)
                    FC_LOG_ERROR("sql.sql", "Disable flags specified for battleground %u, useless data.", entry);
                break;
            case DISABLE_TYPE_OUTDOORPVP:
                if (entry > MAX_OUTDOORPVP_TYPES)
                {
                    FC_LOG_ERROR("sql.sql", "OutdoorPvPTypes value %u from `disables` is invalid, skipped.", entry);
                    continue;
                }
                if (flags)
                    FC_LOG_ERROR("sql.sql", "Disable flags specified for outdoor PvP %u, useless data.", entry);
                break;
            case DISABLE_TYPE_ACHIEVEMENT_CRITERIA:
                if (!sAchievementMgr->GetAchievementCriteria(entry))
                {
                    FC_LOG_ERROR("sql.sql", "Achievement Criteria entry %u from `disables` doesn't exist in dbc, skipped.", entry);
                    continue;
                }
                if (flags)
                    FC_LOG_ERROR("sql.sql", "Disable flags specified for Achievement Criteria %u, useless data.", entry);
                break;
            case DISABLE_TYPE_VMAP:
            {
                ...
                case MAP_COMMON:
                    if (flags & VMAP::VMAP_DISABLE_AREAFLAG)
                        FC_LOG_INFO("misc", "Areaflag disabled for world map %u.", entry);
                    if (flags & VMAP::VMAP_DISABLE_LIQUIDSTATUS)
                        FC_LOG_INFO("misc", "Liquid status disabled for world map %u.", entry);
                    break;
                case MAP_INSTANCE:
                case MAP_RAID:
                    if (flags & VMAP::VMAP_DISABLE_HEIGHT)
                        FC_LOG_INFO("misc", "Height disabled for instance map %u.", entry);
                    if (flags & VMAP::VMAP_DISABLE_LOS)
                        FC_LOG_INFO("misc", "LoS disabled for instance map %u.", entry);
                    break;
                case MAP_BATTLEGROUND:
                    if (flags & VMAP::VMAP_DISABLE_HEIGHT)
                        FC_LOG_INFO("misc", "Height disabled for battleground map %u.", entry);
                    if (flags & VMAP::VMAP_DISABLE_LOS)
                        FC_LOG_INFO("misc", "LoS disabled for battleground map %u.", entry);
                    break;
                case MAP_ARENA:
                    if (flags & VMAP::VMAP_DISABLE_HEIGHT)
                        FC_LOG_INFO("misc", "Height disabled for arena map %u.", entry);
                    if (flags & VMAP::VMAP_DISABLE_LOS)
                        FC_LOG_INFO("misc", "LoS disabled for arena map %u.", entry);
                    break;
                default:
                    break;
                }
            ...
            case DISABLE_TYPE_MMAP:
            {
                ...
                {
                case MAP_COMMON:
                    FC_LOG_INFO("misc", "Pathfinding disabled for world map %u.", entry);
                    break;
                case MAP_INSTANCE:
                case MAP_RAID:
                    FC_LOG_INFO("misc", "Pathfinding disabled for instance map %u.", entry);
                    break;
                case MAP_BATTLEGROUND:
                    FC_LOG_INFO("misc", "Pathfinding disabled for battleground map %u.", entry);
                    break;
                case MAP_ARENA:
                    FC_LOG_INFO("misc", "Pathfinding disabled for arena map %u.", entry);
                    break;
                default:
                    break;
                }
        ...
        FC_LOG_INFO("server.loading", ">> Loaded %u disables in %u ms", total_count, GetMSTimeDiffToNow(oldMSTime));
    }
    ...
    void CheckQuestDisables()
    {
        ...
        if (!count)
        {
            FC_LOG_INFO("server.loading", ">> Checked 0 quest disables.");
            return;
        }
        ...
        for (DisableTypeMap::iterator itr = m_DisableMap[DISABLE_TYPE_QUEST].begin(); itr != m_DisableMap[DISABLE_TYPE_QUEST].end();)
        {
            ...
            {
                FC_LOG_ERROR("sql.sql", "Quest entry %u from `disables` doesn't exist, skipped.", entry);
                m_DisableMap[DISABLE_TYPE_QUEST].erase(itr++);
                continue;
            }
            if (itr->second.flags)
                FC_LOG_ERROR("sql.sql", "Disable flags specified for quest %u, useless data.", entry);
            ++itr;
        }

        FC_LOG_INFO("server.loading", ">> Checked %u quest disables in %u ms", count, GetMSTimeDiffToNow(oldMSTime));
    }
