...
bool Condition::Meets(ConditionSourceInfo& sourceInfo) const
{
    ...
    if (!object)
    {
        FC_LOG_DEBUG("condition", "Condition object not found for %s", ToString().c_str());
        return false;
    }
...
bool ConditionMgr::IsObjectMeetToConditionList(ConditionSourceInfo& sourceInfo, ConditionContainer const& conditions) const
{
    ...
    {
        FC_LOG_DEBUG("condition", "ConditionMgr::IsPlayerMeetToConditionList %s val1: %u", condition->ToString().c_str(), condition->ConditionValue1);
        if (condition->isLoaded())
        {
            ...
            if (condition->ReferenceId)//handle reference
            {
                ...
                {
                    FC_LOG_DEBUG("condition", "ConditionMgr::IsPlayerMeetToConditionList %s Reference template -%u not found",
...
bool ConditionMgr::IsObjectMeetToConditions(ConditionSourceInfo& sourceInfo, ConditionContainer const& conditions) const
{
    ...
    FC_LOG_DEBUG("condition", "ConditionMgr::IsObjectMeetToConditions");
...
bool ConditionMgr::IsObjectMeetingNotGroupedConditions(ConditionSourceType sourceType, uint32 entry, ConditionSourceInfo& sourceInfo) const
{
    if (sourceType > CONDITION_SOURCE_TYPE_NONE && sourceType < CONDITION_SOURCE_TYPE_MAX)
    {
        ...
        {
            FC_LOG_DEBUG("condition", "GetConditionsForNotGroupedEntry: found conditions for type %u and entry %u", uint32(sourceType), entry);
...
bool ConditionMgr::IsObjectMeetingSpellClickConditions(uint32 creatureId, uint32 spellId, WorldObject* clicker, WorldObject* target) const
{
    ...
    if (itr != SpellClickEventConditionStore.end())
    {
        ...
        if (i != itr->second.end())
        {
            FC_LOG_DEBUG("condition", "GetConditionsForSpellClickEvent: found conditions for SpellClickEvent entry %u spell %u", creatureId, spellId);
...
bool ConditionMgr::IsObjectMeetingVehicleSpellConditions(uint32 creatureId, uint32 spellId, Player* player, Unit* vehicle) const
{
    ...
    if (itr != VehicleSpellConditionStore.end())
    {
        ...
        if (i != itr->second.end())
        {
            FC_LOG_DEBUG("condition", "GetConditionsForVehicleSpell: found conditions for Vehicle entry %u spell %u", creatureId, spellId);
...
bool ConditionMgr::IsObjectMeetingSmartEventConditions(int32 entryOrGuid, uint32 eventId, uint32 sourceType, Unit* unit, WorldObject* baseObject) const
{
    ...
    if (itr != SmartEventConditionStore.end())
    {
        ...
        if (i != itr->second.end())
        {
            FC_LOG_DEBUG("condition", "GetConditionsForSmartEvent: found conditions for Smart Event entry or guid %d eventId %u", entryOrGuid, eventId);
...
bool ConditionMgr::IsObjectMeetingVendorItemConditions(uint32 creatureId, uint32 itemId, Player* player, Creature* vendor) const
{
    ...
    if (itr != NpcVendorConditionContainerStore.end())
    {
        ...
        if (i != (*itr).second.end())
        {
            FC_LOG_DEBUG("condition", "GetConditionsForNpcVendorEvent: found conditions for creature entry %u item %u", creatureId, itemId);
...
void ConditionMgr::LoadConditions(bool isReload)
{
    ...
    if (isReload)
    {
        FC_LOG_INFO("misc", "Reseting Loot Conditions...");
        ...
        FC_LOG_INFO("misc", "Re-Loading `gossip_menu` Table for Conditions!");
        sObjectMgr->LoadGossipMenu();

        FC_LOG_INFO("misc", "Re-Loading `gossip_menu_option` Table for Conditions!");
    ...
    if (!result)
    {
        FC_LOG_ERROR("server.loading", ">> Loaded 0 conditions. DB table `conditions` is empty!");
        return;
    }
    ...
    do
    {
        ...
        if (iConditionTypeOrReference < 0)//it has a reference
        {
            ...
            {
                FC_LOG_ERROR("sql.sql", "Condition reference %i is referencing self, skipped", iSourceTypeOrReferenceId);
                delete cond;
                continue;
            }
            ...
            if (cond->ConditionTarget)
                FC_LOG_ERROR("sql.sql", "Condition %s %i has useless data in ConditionTarget (%u)!", rowType, iSourceTypeOrReferenceId, cond->ConditionTarget);
            if (cond->ConditionValue1)
                FC_LOG_ERROR("sql.sql", "Condition %s %i has useless data in value1 (%u)!", rowType, iSourceTypeOrReferenceId, cond->ConditionValue1);
            if (cond->ConditionValue2)
                FC_LOG_ERROR("sql.sql", "Condition %s %i has useless data in value2 (%u)!", rowType, iSourceTypeOrReferenceId, cond->ConditionValue2);
            if (cond->ConditionValue3)
                FC_LOG_ERROR("sql.sql", "Condition %s %i has useless data in value3 (%u)!", rowType, iSourceTypeOrReferenceId, cond->ConditionValue3);
            if (cond->NegativeCondition)
                FC_LOG_ERROR("sql.sql", "Condition %s %i has useless data in NegativeCondition (%u)!", rowType, iSourceTypeOrReferenceId, cond->NegativeCondition);
            if (cond->SourceGroup && iSourceTypeOrReferenceId < 0)
                FC_LOG_ERROR("sql.sql", "Condition %s %i has useless data in SourceGroup (%u)!", rowType, iSourceTypeOrReferenceId, cond->SourceGroup);
            if (cond->SourceEntry && iSourceTypeOrReferenceId < 0)
                FC_LOG_ERROR("sql.sql", "Condition %s %i has useless data in SourceEntry (%u)!", rowType, iSourceTypeOrReferenceId, cond->SourceEntry);
        }
        ...
        {
            FC_LOG_ERROR("sql.sql", "%s has not allowed value of SourceGroup = %u!", cond->ToString().c_str(), cond->SourceGroup);
            delete cond;
            continue;
        }
        if (cond->SourceId && !CanHaveSourceIdSet(cond->SourceType))
        {
            FC_LOG_ERROR("sql.sql", "%s has not allowed value of SourceId = %u!", cond->ToString().c_str(), cond->SourceId);
            delete cond;
            continue;
        }
        ...
        {
            FC_LOG_ERROR("sql.sql", "%s can't have ErrorType (%u), set to 0!", cond->ToString().c_str(), cond->ErrorType);
            cond->ErrorType = 0;
        }

        if (cond->ErrorTextId && !cond->ErrorType)
        {
            FC_LOG_ERROR("sql.sql", "%s has any ErrorType, ErrorTextId (%u) is set, set to 0!", cond->ToString().c_str(), cond->ErrorTextId);
            cond->ErrorTextId = 0;
        }
        ...
        if (cond->SourceGroup)
        {
            ...
            if (!valid)
            {
                FC_LOG_ERROR("sql.sql", "%s Not handled grouped condition.", cond->ToString().c_str());
                delete cond;
            }
            else
            {
    ...
    FC_LOG_INFO("server.loading", ">> Loaded %u conditions in %u ms", count, GetMSTimeDiffToNow(oldMSTime));
}
...
bool ConditionMgr::addToLootTemplate(Condition* cond, LootTemplate* loot) const
{
    if (!loot)
    {
        FC_LOG_ERROR("sql.sql", "%s LootTemplate %u not found.", cond->ToString().c_str(), cond->SourceGroup);
        return false;
    }
    ...
    FC_LOG_ERROR("sql.sql", "%s Item %u not found in LootTemplate %u.", cond->ToString().c_str(), cond->SourceEntry, cond->SourceGroup);
    return false;
}
...
bool ConditionMgr::addToGossipMenus(Condition* cond) const
{
    ...
    FC_LOG_ERROR("sql.sql", "%s GossipMenu %u not found.", cond->ToString().c_str(), cond->SourceGroup);
    return false;
}
...
bool ConditionMgr::addToGossipMenuItems(Condition* cond) const
{
    ...
    FC_LOG_ERROR("sql.sql", "%s GossipMenuId %u Item %u not found.", cond->ToString().c_str(), cond->SourceGroup, cond->SourceEntry);
    return false;
}
    ...
    for (uint8 i = 0; i < MAX_SPELL_EFFECTS; ++i)
    {
        ...
        if (conditionEffMask & (1 << i))
        {
            ...
            case CONDITION_OBJECT_ENTRY_GUID:
            {
                ...
                {
                    FC_LOG_ERROR("sql.sql", "%s in `condition` table - spell %u EFFECT_%u - "
    ...
    for (uint32 effectMask : sharedMasks)
    {
        ...
        if (uint32 commonMask = effectMask & conditionEffMask)
        {
            ...
            if (sharedList)
            {
                ...
                {
                    FC_LOG_ERROR("sql.sql", "%s in `condition` table, has incorrect SourceGroup %u (spell effectMask) set - "
...
bool ConditionMgr::addToPhases(Condition* cond) const
{
    ...
    FC_LOG_ERROR("sql.sql", "%s Area %u does not have phase %u.", cond->ToString().c_str(), cond->SourceGroup, cond->SourceEntry);
    return false;
}
...
bool ConditionMgr::addToSpellArea(Condition* cond) const
{
    ...
    {
        FC_LOG_ERROR("sql.sql", "%s invalid spell ID %u specified for area/zone ID %u.", cond->ToString().c_str(), cond->SourceGroup, cond->SourceEntry);
        return false;
    }

    FC_LOG_ERROR("sql.sql", "%s SpellID %u does not have a entry for area/zone ID %u.", cond->ToString().c_str(), cond->SourceGroup, cond->SourceEntry);
    return false;
}
...
bool ConditionMgr::isSourceTypeValid(Condition* cond) const
{
    ...
    {
        FC_LOG_ERROR("sql.sql", "%s Invalid ConditionSourceType in `condition` table, ignoring.", cond->ToString().c_str());
        return false;
    }
    ...
    case CONDITION_SOURCE_TYPE_CREATURE_LOOT_TEMPLATE:
    {
        ...
        {
            FC_LOG_ERROR("sql.sql", "%s SourceGroup in `condition` table, does not exist in `creature_loot_template`, ignoring.", cond->ToString().c_str());
            return false;
        }
        ...
        {
            FC_LOG_ERROR("sql.sql", "%s SourceType, SourceEntry in `condition` table, does not exist in `item_template`, ignoring.", cond->ToString().c_str());
            return false;
        }
        break;
    }
    case CONDITION_SOURCE_TYPE_DISENCHANT_LOOT_TEMPLATE:
    {
        ...
        {
            FC_LOG_ERROR("sql.sql", "%s SourceGroup in `condition` table, does not exist in `disenchant_loot_template`, ignoring.", cond->ToString().c_str());
            return false;
        }
    ...
    case CONDITION_SOURCE_TYPE_FISHING_LOOT_TEMPLATE:
    {
        ...
        {
            FC_LOG_ERROR("sql.sql", "%s SourceGroup in `condition` table, does not exist in `fishing_loot_template`, ignoring.", cond->ToString().c_str());
            return false;
        }
    ...
    case CONDITION_SOURCE_TYPE_GAMEOBJECT_LOOT_TEMPLATE:
    {
        ...
        {
            FC_LOG_ERROR("sql.sql", "%s SourceGroup in `condition` table, does not exist in `gameobject_loot_template`, ignoring.", cond->ToString().c_str());
            return false;
        }
    ...
    case CONDITION_SOURCE_TYPE_ITEM_LOOT_TEMPLATE:
    {
        ...
        {
            FC_LOG_ERROR("sql.sql", "%s SourceGroup in `condition` table, does not exist in `item_loot_template`, ignoring.", cond->ToString().c_str());
            return false;
        }
    ...
    case CONDITION_SOURCE_TYPE_MAIL_LOOT_TEMPLATE:
    {
        ...
        {
            FC_LOG_ERROR("sql.sql", "%s SourceGroup in `condition` table, does not exist in `mail_loot_template`, ignoring.", cond->ToString().c_str());
            return false;
        }
    ...
    case CONDITION_SOURCE_TYPE_MILLING_LOOT_TEMPLATE:
    {
        ...
        {
            FC_LOG_ERROR("sql.sql", "%s SourceGroup in `condition` table, does not exist in `milling_loot_template`, ignoring.", cond->ToString().c_str());
            return false;
        }
    ...
    case CONDITION_SOURCE_TYPE_PICKPOCKETING_LOOT_TEMPLATE:
    {
        ...
        {
            FC_LOG_ERROR("sql.sql", "%s SourceGroup in `condition` table, does not exist in `pickpocketing_loot_template`, ignoring.", cond->ToString().c_str());
            return false;
        }
    ...
    case CONDITION_SOURCE_TYPE_PROSPECTING_LOOT_TEMPLATE:
    {
        ...
        {
            FC_LOG_ERROR("sql.sql", "%s SourceGroup in `condition` table, does not exist in `prospecting_loot_template`, ignoring.", cond->ToString().c_str());
            return false;
        }
    ...
    case CONDITION_SOURCE_TYPE_REFERENCE_LOOT_TEMPLATE:
    {
        ...
        {
            FC_LOG_ERROR("sql.sql", "%s SourceGroup in `condition` table, does not exist in `reference_loot_template`, ignoring.", cond->ToString().c_str());
            return false;
        }
    ...
    case CONDITION_SOURCE_TYPE_SKINNING_LOOT_TEMPLATE:
    {
        ...
        {
            FC_LOG_ERROR("sql.sql", "%s SourceGroup in `condition` table, does not exist in `skinning_loot_template`, ignoring.", cond->ToString().c_str());
            return false;
        }
    ...
    case CONDITION_SOURCE_TYPE_SPELL_LOOT_TEMPLATE:
    {
        ...
        {
            FC_LOG_ERROR("sql.sql", "%s SourceGroup in `condition` table, does not exist in `spell_loot_template`, ignoring.", cond->ToString().c_str());
            return false;
        }
    ...
    case CONDITION_SOURCE_TYPE_SPELL_IMPLICIT_TARGET:
    {
        ...
        if (!spellInfo)
        {
            FC_LOG_ERROR("sql.sql", "%s SourceEntry in `condition` table does not exist in `spell.dbc`, ignoring.", cond->ToString().c_str());
            return false;
        }

        if ((cond->SourceGroup > MAX_EFFECT_MASK) || !cond->SourceGroup)
        {
            FC_LOG_ERROR("sql.sql", "%s in `condition` table, has incorrect SourceGroup (spell effectMask) set, ignoring.", cond->ToString().c_str());
            return false;
        }
        ...
        for (uint8 i = 0; i < MAX_SPELL_EFFECTS; ++i)
        {
            ...
            FC_LOG_ERROR("sql.sql", "SourceEntry %u SourceGroup %u in `condition` table - spell %u does not have implicit targets of types: _AREA_, _CONE_, _NEARBY_, __CHAIN__ for effect %u, SourceGroup needs correction, ignoring.", cond->SourceEntry, origGroup, cond->SourceEntry, uint32(i));
    ...
    case CONDITION_SOURCE_TYPE_CREATURE_TEMPLATE_VEHICLE:
    {
        ...
        {
            FC_LOG_ERROR("sql.sql", "%s SourceEntry in `condition` table, does not exist in `creature_template`, ignoring.", cond->ToString().c_str());
            return false;
        }
        break;
    }
    ...
    case CONDITION_SOURCE_TYPE_QUEST_AVAILABLE:
        ...
        {
            FC_LOG_ERROR("sql.sql", "CONDITION_SOURCE_TYPE_QUEST_AVAILABLE specifies non-existing quest (%u), skipped", cond->SourceEntry);
            return false;
        }
        break;
        if (!sObjectMgr->GetQuestTemplate(cond->SourceEntry))
        {
            FC_LOG_ERROR("sql.sql", "%s SourceEntry specifies non-existing quest, skipped.", cond->ToString().c_str());
            return false;
        }
        break;
    case CONDITION_SOURCE_TYPE_VEHICLE_SPELL:
    ...
    case CONDITION_SOURCE_TYPE_NPC_VENDOR:
    {
        ...
        if (!itemTemplate)
        {
            FC_LOG_ERROR("sql.sql", "%s SourceEntry in `condition` table, does not exist in `item_template`, ignoring.", cond->ToString().c_str());
            return false;
        }
        break;
    }
    case CONDITION_SOURCE_TYPE_TERRAIN_SWAP:
    {
        ...
        {
            FC_LOG_ERROR("sql.sql", "%s SourceEntry in `condition` table, does not exist in Map.dbc, ignoring.", cond->ToString().c_str());
            return false;
        }
        break;
    }
    case CONDITION_SOURCE_TYPE_PHASE:
    {
        ...
        {
            FC_LOG_ERROR("sql.sql", "%s SourceEntry in `condition` table, does not exist in AreaTable.dbc, ignoring.", cond->ToString().c_str());
            return false;
        }
        break;
    }
    ...
    case CONDITION_SOURCE_TYPE_GRAVEYARD:
        ...
        {
            FC_LOG_ERROR("sql.sql", "%s SourceEntry in `condition` table, does not exist in WorldSafeLocs, ignoring.", cond->ToString().c_str());
            return false;
        }
        break;
    default:
...
bool ConditionMgr::isConditionTypeValid(Condition* cond) const
{
    ...
    {
        FC_LOG_ERROR("sql.sql", "%s Invalid ConditionType in `condition` table, ignoring.", cond->ToString(true).c_str());
        return false;
    }

    if (cond->ConditionTarget >= cond->GetMaxAvailableConditionTargets())
    {
        FC_LOG_ERROR("sql.sql", "%s in `condition` table, has incorrect ConditionTarget set, ignoring.", cond->ToString(true).c_str());
        return false;
    }
    ...
    case CONDITION_AURA:
    {
        ...
        {
            FC_LOG_ERROR("sql.sql", "%s has non existing spell (Id: %d), skipped.", cond->ToString(true).c_str(), cond->ConditionValue1);
            return false;
        }

        if (cond->ConditionValue2 > EFFECT_2)
        {
            FC_LOG_ERROR("sql.sql", "%s has non existing effect index (%u) (must be 0..2), skipped.", cond->ToString(true).c_str(), cond->ConditionValue2);
            return false;
        }
        break;
    }
    case CONDITION_ITEM:
    {
        ...
        if (!proto)
        {
            FC_LOG_ERROR("sql.sql", "%s Item (%u) does not exist, skipped.", cond->ToString(true).c_str(), cond->ConditionValue1);
            return false;
        }

        if (!cond->ConditionValue2)
        {
            FC_LOG_ERROR("sql.sql", "%s Zero item count in ConditionValue2, skipped.", cond->ToString(true).c_str());
            return false;
        }
        break;
    }
    ...
    case CONDITION_ZONEID:
    {
        ...
        if (!areaEntry)
        {
            FC_LOG_ERROR("sql.sql", "%s Area (%u) does not exist, skipped.", cond->ToString(true).c_str(), cond->ConditionValue1);
            return false;
        }

        if (areaEntry->ParentAreaID != 0)
        {
            FC_LOG_ERROR("sql.sql", "%s requires to be in area (%u) which is a subzone but zone expected, skipped.", cond->ToString(true).c_str(), cond->ConditionValue1);
            return false;
        }
        break;
    }
    case CONDITION_REPUTATION_RANK:
    {
        ...
        if (!factionEntry)
        {
            FC_LOG_ERROR("sql.sql", "%s has non existing faction (%u), skipped.", cond->ToString(true).c_str(), cond->ConditionValue1);
            return false;
        }
        break;
    }
    case CONDITION_TEAM:
    {
        ...
        {
            FC_LOG_ERROR("sql.sql", "%s specifies unknown team (%u), skipped.", cond->ToString(true).c_str(), cond->ConditionValue1);
            return false;
        }
        break;
    }
    case CONDITION_SKILL:
    {
        ...
        if (!pSkill)
        {
            FC_LOG_ERROR("sql.sql", "%s specifies non-existing skill (%u), skipped.", cond->ToString(true).c_str(), cond->ConditionValue1);
            return false;
        }

        if (cond->ConditionValue2 < 1 || cond->ConditionValue2 > sWorld->GetConfigMaxSkillValue())
        {
            FC_LOG_ERROR("sql.sql", "%s specifies skill (%u) with invalid value (%u), skipped.", cond->ToString(true).c_str(), cond->ConditionValue1, cond->ConditionValue2);
            return false;
        }
        break;
    }
    case CONDITION_QUESTSTATE:
        ...
        {
            FC_LOG_ERROR("sql.sql", "%s has invalid state mask (%u), skipped.", cond->ToString(true).c_str(), cond->ConditionValue2);
            return false;
        }
        [[fallthrough]];
    case CONDITION_QUESTREWARDED:
    ...
    case CONDITION_DAILY_QUEST_DONE:
    {
        ...
        {
            FC_LOG_ERROR("sql.sql", "%s points to non-existing quest (%u), skipped.", cond->ToString(true).c_str(), cond->ConditionValue1);
            return false;
        }
        break;
    }
    case CONDITION_ACTIVE_EVENT:
    {
        ...
        {
            FC_LOG_ERROR("sql.sql", "%s has non existing event id (%u), skipped.", cond->ToString(true).c_str(), cond->ConditionValue1);
            return false;
        }
        break;
    }
    case CONDITION_ACHIEVEMENT:
    {
        ...
        if (!achievement)
        {
            FC_LOG_ERROR("sql.sql", "%s has non existing achivement id (%u), skipped.", cond->ToString(true).c_str(), cond->ConditionValue1);
            return false;
        }
        break;
    }
    case CONDITION_CLASS:
    {
        ...
        {
            FC_LOG_ERROR("sql.sql", "%s has non existing classmask (%u), skipped.", cond->ToString(true).c_str(), cond->ConditionValue1 & ~CLASSMASK_ALL_PLAYABLE);
            return false;
        }
        break;
    }
    case CONDITION_RACE:
    {
        ...
        {
            FC_LOG_ERROR("sql.sql", "%s has non existing racemask (%u), skipped.", cond->ToString(true).c_str(), cond->ConditionValue1 & ~RACEMASK_ALL_PLAYABLE);
            return false;
        }
        break;
    }
    case CONDITION_GENDER:
    {
        ...
        {
            FC_LOG_ERROR("sql.sql", "%s has invalid gender (%u), skipped.", cond->ToString(true).c_str(), cond->ConditionValue1);
            return false;
        }
        break;
    }
    case CONDITION_MAPID:
    {
        ...
        if (!me)
        {
            FC_LOG_ERROR("sql.sql", "%s has non existing map (%u), skipped", cond->ToString(true).c_str(), cond->ConditionValue1);
            return false;
        }
        break;
    }
    ...
    case CONDITION_LEVEL:
    {
        ...
        {
            FC_LOG_ERROR("sql.sql", "%s has invalid ComparisionType (%u), skipped.", cond->ToString(true).c_str(), cond->ConditionValue2);
            return false;
        }
        break;
    }
    case CONDITION_DRUNKENSTATE:
    {
        ...
        {
            FC_LOG_ERROR("sql.sql", "%s has invalid state (%u), skipped.", cond->ToString(true).c_str(), cond->ConditionValue1);
            return false;
        }
        break;
    }
    case CONDITION_NEAR_CREATURE:
    {
        ...
        {
            FC_LOG_ERROR("sql.sql", "%s has non existing creature template entry (%u), skipped", cond->ToString(true).c_str(), cond->ConditionValue1);
            return false;
        }
        break;
    }
    case CONDITION_NEAR_GAMEOBJECT:
    {
        ...
        {
            FC_LOG_ERROR("sql.sql", "%s has non existing gameobject template entry (%u), skipped.", cond->ToString().c_str(), cond->ConditionValue1);
            return false;
        }
        break;
    }
    case CONDITION_OBJECT_ENTRY_GUID:
    {
        ...
        case TYPEID_UNIT:
            ...
            {
                FC_LOG_ERROR("sql.sql", "%s has non existing creature template entry (%u), skipped.", cond->ToString(true).c_str(), cond->ConditionValue2);
                return false;
            }
            if (cond->ConditionValue3)
            {
                if (CreatureData const* creatureData = sObjectMgr->GetCreatureData(cond->ConditionValue3))
                {
                    ...
                    {
                        FC_LOG_ERROR("sql.sql", "%s has guid %u set but does not match creature entry (%u), skipped.", cond->ToString(true).c_str(), cond->ConditionValue3, cond->ConditionValue2);
                        return false;
                    }
                }
                else
                {
                    FC_LOG_ERROR("sql.sql", "%s has non existing creature guid (%u), skipped.", cond->ToString(true).c_str(), cond->ConditionValue3);
                    return false;
                }
            }
            break;
        case TYPEID_GAMEOBJECT:
            ...
            {
                FC_LOG_ERROR("sql.sql", "%s has non existing gameobject template entry (%u), skipped.", cond->ToString(true).c_str(), cond->ConditionValue2);
                return false;
            }
            if (cond->ConditionValue3)
            {
                if (GameObjectData const* goData = sObjectMgr->GetGameObjectData(cond->ConditionValue3))
                {
                    ...
                    {
                        FC_LOG_ERROR("sql.sql", "%s has guid %u set but does not match gameobject entry (%u), skipped.", cond->ToString(true).c_str(), cond->ConditionValue3, cond->ConditionValue2);
                        return false;
                    }
                }
                else
                {
                    FC_LOG_ERROR("sql.sql", "%s has non existing gameobject guid (%u), skipped.", cond->ToString(true).c_str(), cond->ConditionValue3);
                    return false;
                }
            }
            break;
        ...
        case TYPEID_CORPSE:
            ...
            break;
        default:
            FC_LOG_ERROR("sql.sql", "%s has wrong typeid set (%u), skipped", cond->ToString(true).c_str(), cond->ConditionValue1);
            return false;
        }
        break;
    }
    case CONDITION_TYPE_MASK:
    {
        ...
        {
            FC_LOG_ERROR("sql.sql", "%s has invalid typemask set (%u), skipped.", cond->ToString(true).c_str(), cond->ConditionValue2);
            return false;
        }
        break;
    }
    case CONDITION_RELATION_TO:
    {
        ...
        {
            FC_LOG_ERROR("sql.sql", "%s has invalid ConditionValue1(ConditionTarget selection) (%u), skipped.", cond->ToString(true).c_str(), cond->ConditionValue1);
            return false;
        }
        if (cond->ConditionValue1 == cond->ConditionTarget)
        {
            FC_LOG_ERROR("sql.sql", "%s has ConditionValue1(ConditionTarget selection) set to self (%u), skipped.", cond->ToString(true).c_str(), cond->ConditionValue1);
            return false;
        }
        if (cond->ConditionValue2 >= RELATION_MAX)
        {
            FC_LOG_ERROR("sql.sql", "%s has invalid ConditionValue2(RelationType) (%u), skipped.", cond->ToString(true).c_str(), cond->ConditionValue2);
            return false;
        }
        break;
    }
    case CONDITION_REACTION_TO:
    {
        ...
        if (!cond->ConditionValue2)
        {
            FC_LOG_ERROR("sql.sql", "%s has invalid ConditionValue2(rankMask) (%u), skipped.", cond->ToString(true).c_str(), cond->ConditionValue2);
            return false;
        }
        break;
    }
    case CONDITION_DISTANCE_TO:
    {
        ...
        {
            FC_LOG_ERROR("sql.sql", "%s has invalid ComparisionType (%u), skipped.", cond->ToString(true).c_str(), cond->ConditionValue3);
            return false;
        }
        break;
    }
    case CONDITION_HP_VAL:
    {
        ...
        if (cond->ConditionValue3)
            FC_LOG_ERROR("sql.sql", "%s has useless data in value3 (%u)!", cond->ToString(true).c_str(), cond->ConditionValue3);
        break;
    }
    case CONDITION_HP_PCT:
    {
        ...
        {
            FC_LOG_ERROR("sql.sql", "%s has too big percent value (%u), skipped.", cond->ToString(true).c_str(), cond->ConditionValue1);
            return false;
        }
        if (cond->ConditionValue2 >= COMP_TYPE_MAX)
        {
        ...
        if (cond->ConditionValue3)
            FC_LOG_ERROR("sql.sql", "%s has useless data in value3 (%u)!", cond->ToString().c_str(), cond->ConditionValue3);
        break;
    }
    case CONDITION_WORLD_STATE:
    {
        ...
        {
            FC_LOG_ERROR("sql.sql", "%s has non existing world state in value1 (%u), skipped.", cond->ToString(true).c_str(), cond->ConditionValue1);
            return false;
        }
        break;
    }
    case CONDITION_PHASEID:
    {
        ...
        {
            FC_LOG_ERROR("sql.sql", "Phase condition has nonexistent phaseid in value1 (%u), skipped", cond->ConditionValue1);
            return false;
        }
        break;
    }
    ...
    {
        if (cond->ConditionValue2)
            FC_LOG_ERROR("sql.sql", "Terrain swap condition has useless data in value2 (%u)!", cond->ConditionValue2);
        if (cond->ConditionValue3)
            FC_LOG_ERROR("sql.sql", "Terrain swap condition has useless data in value3 (%u)!", cond->ConditionValue3);
        break;
    }
    case CONDITION_TITLE:
    {
        ...
        if (!titleEntry)
        {
            FC_LOG_ERROR("sql.sql", "%s has non existing title in value1 (%u), skipped.", cond->ToString(true).c_str(), cond->ConditionValue1);
            return false;
        }
        break;
    }
    case CONDITION_SPAWNMASK:
    {
        ...
        {
            FC_LOG_ERROR("sql.sql", "%s has non existing SpawnMask in value1 (%u), skipped.", cond->ToString(true).c_str(), cond->ConditionValue1);
            return false;
        }
        break;
    }
    case CONDITION_UNIT_STATE:
    {
        ...
        {
            FC_LOG_ERROR("sql.sql", "%s has non existing UnitState in value1 (%u), skipped.", cond->ToString(true).c_str(), cond->ConditionValue1);
            return false;
        }
        break;
    }
    case CONDITION_CREATURE_TYPE:
    {
        ...
        {
            FC_LOG_ERROR("sql.sql", "%s has non existing CreatureType in value1 (%u), skipped.", cond->ToString(true).c_str(), cond->ConditionValue1);
            return false;
        }
        break;
    }
    ...
    case CONDITION_REALM_ACHIEVEMENT:
    {
        ...
        if (!achievement)
        {
            FC_LOG_ERROR("sql.sql", "%s has non existing realm first achivement id (%u), skipped.", cond->ToString(true).c_str(), cond->ConditionValue1);
            return false;
        }
        break;
    }
    case CONDITION_STAND_STATE:
    {
        ...
        if (!valid)
        {
            FC_LOG_ERROR("sql.sql", "%s has non-existing stand state (%u,%u), skipped.", cond->ToString(true).c_str(), cond->ConditionValue1, cond->ConditionValue2);
            return false;
        }
        break;
    }
    case CONDITION_PET_TYPE:
        ...
        {
            FC_LOG_ERROR("sql.sql", "%s has non-existing pet type %u, skipped.", cond->ToString(true).c_str(), cond->ConditionValue1);
            return false;
        }
        break;
    case CONDITION_IN_WATER:
...
{
    FC_LOG_ERROR("sql.sql", "%s has useless data in ConditionValue%u (%u)!", cond->ToString(true).c_str(), index, value);
}
