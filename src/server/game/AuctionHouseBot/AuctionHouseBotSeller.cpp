...
bool AuctionBotSeller::Initialize()
{
    ...
    FC_LOG_DEBUG("ahbot", "AHBot seller filters:");
    ...
    FC_LOG_DEBUG("ahbot", "Forced Inclusion %u items", (uint32)includeItems.size());
    FC_LOG_DEBUG("ahbot", "Forced Exclusion %u items", (uint32)excludeItems.size());

    FC_LOG_DEBUG("ahbot", "Loading npc vendor items for filter..");
    ...
    FC_LOG_DEBUG("ahbot", "Npc vendor filter has %u items", (uint32)npcItems.size());

    FC_LOG_DEBUG("ahbot", "Loading loot items for filter..");
    ...
    FC_LOG_DEBUG("ahbot", "Loot filter has %u items", (uint32)lootItems.size());
    FC_LOG_DEBUG("ahbot", "Sorting and cleaning items for AHBot seller...");
    ...
    if (!itemsAdded)
    {
        FC_LOG_ERROR("ahbot", "AuctionHouseBot seller not have items, disabled.");
    ...
    FC_LOG_DEBUG("ahbot", "AuctionHouseBot seller will use %u items to fill auction house (according your config choices)", itemsAdded);

    LoadConfig();

    FC_LOG_DEBUG("ahbot", "Items loaded \tGray\tWhite\tGreen\tBlue\tPurple\tOrange\tYellow");
    for (uint32 i = 0; i < MAX_ITEM_CLASS; ++i)
        FC_LOG_DEBUG("ahbot", "\t\t%u\t%u\t%u\t%u\t%u\t%u\t%u",
    ...
    FC_LOG_DEBUG("ahbot", "AHBot seller configuration data loaded and initialized");
    return true;
}
...
uint32 AuctionBotSeller::SetStat(SellerConfiguration& config)
{
    ...
    FC_LOG_DEBUG("ahbot", "AHBot: Missed Item       \tGray\tWhite\tGreen\tBlue\tPurple\tOrange\tYellow");
    for (uint32 i = 0; i < MAX_ITEM_CLASS; ++i)
    {
        FC_LOG_DEBUG("ahbot", "AHBot: \t\t%u\t%u\t%u\t%u\t%u\t%u\t%u",
...
void AuctionBotSeller::AddNewAuctions(SellerConfiguration& config)
{
    ...
    if (config.LastMissedItem > sAuctionBotConfig->GetItemPerCycleBoost())
    {
        ...
        FC_LOG_DEBUG("ahbot", "AHBot: Boost value used to fill AH! (if this happens often adjust both ItemsPerCycle in worldserver.conf)");
    }
    else
    ...
    while (GetItemsToSell(config, itemsToSell, allItems) && items > 0)
    {
        ...
        ItemToSell const& sellItem = Firelands::Containers::SelectRandomContainerElement(itemsToSell);

        // Set itemId with random item ID for selected categories and color, from _itemPool table
        uint32 itemId = Firelands::Containers::SelectRandomContainerElement(_itemPool[sellItem.Color][sellItem.Itemclass]);
        ...
        if (!itemId)
        {
            FC_LOG_DEBUG("ahbot", "AHBot: Item entry 0 auction creating attempt.");
            continue;
        }
        ...
        if (!prototype)
        {
            FC_LOG_DEBUG("ahbot", "AHBot: Unknown item %u auction creating attempt.", itemId);
            continue;
        }
        ...
        if (!item)
        {
            FC_LOG_ERROR("ahbot", "AHBot: Item::CreateItem() returned nullptr for item %u (stack: %u)", itemId, stackCount);
            return;
        }
    ...
    FC_LOG_DEBUG("ahbot", "AHBot: Added %u items to auction", count);
}
...
bool AuctionBotSeller::Update(AuctionHouseType houseType)
{
    ...
    {
        FC_LOG_DEBUG("ahbot", "AHBot: %s selling ...", AuctionBotConfig::GetHouseTypeName(houseType));
