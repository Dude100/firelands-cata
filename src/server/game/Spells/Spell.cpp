...
{
    if (!m_targetMask)
        FC_LOG_DEBUG("spells", "No targets");

    FC_LOG_DEBUG("spells", "target mask: %u", m_targetMask);
    if (m_targetMask & (TARGET_FLAG_UNIT_MASK | TARGET_FLAG_CORPSE_MASK | TARGET_FLAG_GAMEOBJECT_MASK))
        FC_LOG_DEBUG("spells", "Object target: %s", m_objectTargetGUID.ToString().c_str());
    if (m_targetMask & TARGET_FLAG_ITEM)
        FC_LOG_DEBUG("spells", "Item target: %s", m_itemTargetGUID.ToString().c_str());
    if (m_targetMask & TARGET_FLAG_TRADE_ITEM)
        FC_LOG_DEBUG("spells", "Trade item target: %s", m_itemTargetGUID.ToString().c_str());
    if (m_targetMask & TARGET_FLAG_SOURCE_LOCATION)
        FC_LOG_DEBUG("spells", "Source location: transport guid:%s trans offset: %s position: %s", m_src._transportGUID.ToString().c_str(), m_src._transportOffset.ToString().c_str(), m_src._position.ToString().c_str());
    if (m_targetMask & TARGET_FLAG_DEST_LOCATION)
        FC_LOG_DEBUG("spells", "Destination location: transport guid:%s trans offset: %s position: %s", m_dst._transportGUID.ToString().c_str(), m_dst._transportOffset.ToString().c_str(), m_dst._position.ToString().c_str());
    if (m_targetMask & TARGET_FLAG_STRING)
        FC_LOG_DEBUG("spells", "String: %s", m_strTarget.c_str());
    FC_LOG_DEBUG("spells", "speed: %f", m_speed);
    FC_LOG_DEBUG("spells", "elevation: %f", m_elevation);
}
...
Spell::~Spell()
{
    ...
    if (m_referencedFromCurrentSpell && m_selfContainer && *m_selfContainer == this)
    {
        ...
        FC_LOG_ERROR("spells", "SPELL: deleting spell for spell ID %u. However, spell still referenced.", m_spellInfo->Id);
        *m_selfContainer = nullptr;
    }
...
void Spell::SelectEffectImplicitTargets(SpellEffIndex effIndex, SpellImplicitTargetInfo const& targetType, uint32& processedEffectMask)
{
    ...
    case TARGET_SELECT_CATEGORY_NYI:
        FC_LOG_DEBUG("spells", "SPELL: target type %u, found in spellID %u, effect %u is not implemented yet!", m_spellInfo->Id, effIndex, targetType.GetTarget());
        break;
    default:
...
void Spell::SelectImplicitChannelTargets(SpellEffIndex effIndex, SpellImplicitTargetInfo const& targetType)
{
    ...
    if (!channeledSpell)
    {
        FC_LOG_DEBUG("spells", "Spell::SelectImplicitChannelTargets: cannot find channel spell for spell ID %u, effect %u", m_spellInfo->Id, effIndex);
        return;
    }
    ...
    case TARGET_UNIT_CHANNEL_TARGET:
    {
        ...
        if (target && target->ToUnit())
            ...
            FC_LOG_DEBUG("spells", "SPELL: cannot find channel spell target for spell ID %u, effect %u", m_spellInfo->Id, effIndex);
        break;
    }
    case TARGET_DEST_CHANNEL_TARGET:
        ...
        else if (WorldObject* target = ObjectAccessor::GetWorldObject(*m_caster, m_originalCaster->GetChannelObjectGuid()))
        {
            ...
            FC_LOG_DEBUG("spells", "SPELL: cannot find channel spell destination for spell ID %u, effect %u", m_spellInfo->Id, effIndex);
        break;
...
void Spell::SelectImplicitNearbyTargets(SpellEffIndex effIndex, SpellImplicitTargetInfo const& targetType, uint32 effMask)
{
    ...
    {
        FC_LOG_DEBUG("spells", "Spell::SelectImplicitNearbyTargets: no conditions entry for target with TARGET_CHECK_ENTRY of spell ID %u, effect %u - selecting default targets", m_spellInfo->Id, effIndex);
        switch (targetType.GetObjectType())
        {
    ...
    if (!target)
    {
        FC_LOG_DEBUG("spells", "Spell::SelectImplicitNearbyTargets: cannot find nearby target for spell ID %u, effect %u", m_spellInfo->Id, effIndex);
        SendCastResult(SPELL_FAILED_BAD_IMPLICIT_TARGETS);
        finish(false);
        return;
    }
    ...
    if (!target)
    {
        FC_LOG_DEBUG("spells", "Spell::SelectImplicitNearbyTargets: OnObjectTargetSelect script hook for spell Id %u set nullptr target, effect %u", m_spellInfo->Id, effIndex);
        SendCastResult(SPELL_FAILED_BAD_IMPLICIT_TARGETS);
        finish(false);
        return;
    }
    ...
    case TARGET_OBJECT_TYPE_UNIT:
        ...
        {
            FC_LOG_DEBUG("spells", "Spell::SelectImplicitNearbyTargets: OnObjectTargetSelect script hook for spell Id %u set object of wrong type, expected unit, got %s, effect %u", m_spellInfo->Id, target->GetGUID().GetTypeName(), effMask);
            SendCastResult(SPELL_FAILED_BAD_IMPLICIT_TARGETS);
            finish(false);
            return;
        }
    ...
    case TARGET_OBJECT_TYPE_GOBJ:
        ...
        {
            FC_LOG_DEBUG("spells", "Spell::SelectImplicitNearbyTargets: OnObjectTargetSelect script hook for spell Id %u set object of wrong type, expected gameobject, got %s, effect %u", m_spellInfo->Id, target->GetGUID().GetTypeName(), effMask);
            SendCastResult(SPELL_FAILED_BAD_IMPLICIT_TARGETS);
            finish(false);
            return;
        }
    ...
    case TARGET_OBJECT_TYPE_CORPSE:
        ...
        {
            FC_LOG_DEBUG("spells", "Spell::SelectImplicitNearbyTargets: OnObjectTargetSelect script hook for spell Id %u set object of wrong type, expected corpse, got %s, effect %u", m_spellInfo->Id, target->GetGUID().GetTypeName(), effMask);
            SendCastResult(SPELL_FAILED_BAD_IMPLICIT_TARGETS);
            finish(false);
            return;
        }
...
void Spell::SelectImplicitConeTargets(SpellEffIndex effIndex, SpellImplicitTargetInfo const& targetType, uint32 effMask)
{
    ...
    {
        Firelands::WorldObjectSpellConeTargetCheck check(coneSrc, DegToRad(coneAngle), radius, m_caster, m_spellInfo, selectionType, condList);
        Firelands::WorldObjectListSearcher<Firelands::WorldObjectSpellConeTargetCheck> searcher(m_caster, targets, check, containerTypeMask);
        SearchTargets<Firelands::WorldObjectListSearcher<Firelands::WorldObjectSpellConeTargetCheck> >(searcher, containerTypeMask, m_caster, m_caster, radius);
        ...
        if (!targets.empty())
        {
            ...
            if (uint32 maxTargets = m_spellValue->MaxAffectedTargets)
                Firelands::Containers::RandomResize(targets, maxTargets);
...
void Spell::SelectImplicitCasterDestTargets(SpellEffIndex effIndex, SpellImplicitTargetInfo const& targetType)
{
    ...
    case TARGET_DEST_DB:
        ...
        {
            FC_LOG_DEBUG("spells", "SPELL: unknown target coordinates for spell ID %u", m_spellInfo->Id);
            if (WorldObject* target = m_targets.GetObjectTarget())
...
void Spell::SelectImplicitTrajTargets(SpellEffIndex effIndex, SpellImplicitTargetInfo const& targetType)
{
    ...
    std::list<WorldObject*> targets;
    Firelands::WorldObjectSpellTrajTargetCheck check(dist2d, &srcPos, m_caster, m_spellInfo, targetType.GetCheckType(), m_spellInfo->Effects[effIndex].ImplicitTargetConditions);
    Firelands::WorldObjectListSearcher<Firelands::WorldObjectSpellTrajTargetCheck> searcher(m_caster, targets, check, GRID_MAP_TYPE_MASK_ALL);
    SearchTargets<Firelands::WorldObjectListSearcher<Firelands::WorldObjectSpellTrajTargetCheck> >(searcher, GRID_MAP_TYPE_MASK_ALL, m_caster, &srcPos, dist2d);
    if (targets.empty())
        return;

    targets.sort(Firelands::ObjectDistanceOrderPred(m_caster));
...
WorldObject* Spell::SearchNearbyTarget(float range, SpellTargetObjectTypes objectType, SpellTargetCheckTypes selectionType, ConditionContainer* condList)
{
    ...
    if (!containerTypeMask)
        return nullptr;
    Firelands::WorldObjectSpellNearbyTargetCheck check(range, m_caster, m_spellInfo, selectionType, condList);
    Firelands::WorldObjectLastSearcher<Firelands::WorldObjectSpellNearbyTargetCheck> searcher(m_caster, target, check, containerTypeMask);
    SearchTargets<Firelands::WorldObjectLastSearcher<Firelands::WorldObjectSpellNearbyTargetCheck> >(searcher, containerTypeMask, m_caster, m_caster, range);
    return target;
}
...
void Spell::SearchAreaTargets(std::list<WorldObject*>& targets, float range, Position const* position, Unit* referer, SpellTargetObjectTypes objectType, SpellTargetCheckTypes selectionType, ConditionContainer* condList)
{
    ...
    if (!containerTypeMask)
        return;
    Firelands::WorldObjectSpellAreaTargetCheck check(range, position, m_caster, referer, m_spellInfo, selectionType, condList);
    Firelands::WorldObjectListSearcher<Firelands::WorldObjectSpellAreaTargetCheck> searcher(m_caster, targets, check, containerTypeMask);
    SearchTargets<Firelands::WorldObjectListSearcher<Firelands::WorldObjectSpellAreaTargetCheck> >(searcher, containerTypeMask, m_caster, position, range);
}
...
GameObject* Spell::SearchSpellFocus()
{
    ...
    Firelands::GameObjectFocusCheck check(m_caster, m_spellInfo->RequiresSpellFocus);
    Firelands::GameObjectSearcher<Firelands::GameObjectFocusCheck> searcher(m_caster, focus, check);
    SearchTargets<Firelands::GameObjectSearcher<Firelands::GameObjectFocusCheck> >(searcher, GRID_MAP_TYPE_MASK_GAMEOBJECT, m_caster, m_caster, m_caster->GetVisibilityRange());
    return focus;
}
    ...
    if (!m_hitTriggerSpells.empty())
    {
        ...
        for (auto i = m_hitTriggerSpells.begin(); i != m_hitTriggerSpells.end(); ++i)
        {
            if (CanExecuteTriggersOnHit(effMask, i->triggeredByAura) && roll_chance_i(i->chance))
            {
                ...
                FC_LOG_DEBUG("spells", "Spell %d triggered spell %d by SPELL_AURA_ADD_TARGET_TRIGGER aura", m_spellInfo->Id, i->triggeredSpell->Id);
...
void Spell::prepare(SpellCastTargets const& targets, AuraEffect const* triggeredByAura)
{
    ...
    FC_LOG_DEBUG("spells", "Spell::prepare: spell id %u source %u caster %d customCastFlags %u mask %u", m_spellInfo->Id, m_caster->GetEntry(), m_originalCaster ? m_originalCaster->GetEntry() : -1, _triggeredCastFlags, m_targets.GetTargetMask());
...
void Spell::cancel(Spell* interruptingSpell /* = nullptr */)
{
    ...
    case SPELL_STATE_CASTING:
        ...
        {
            // Remove if no miss.
            bool removeOwnedAura = (*ihit).missCondition == SPELL_MISS_NONE;
            if (interruptingSpell != nullptr)
            {
                ...
                if (m_spellInfo->Id == interruptingSpell->m_spellInfo->Id && m_spellInfo->IsChanneled())
                {
                    ...
                    removeOwnedAura = (*ihit).targetGUID != interruptingSpell->m_targets.GetUnitTargetGUID();
                }
            }
...
void Spell::update(uint32 difftime)
{
    ...
    {
        FC_LOG_DEBUG("spells", "Spell %u is cancelled due to removal of target.", m_spellInfo->Id);
        cancel();
        return;
    }
    ...
    case SPELL_STATE_CASTING:
    {
        if (m_timer)
        {
            ...
            {
                FC_LOG_DEBUG("spells", "Channeled spell %d is removed due to lack of targets", m_spellInfo->Id);
                m_timer = 0;
...
void Spell::finish(bool ok, bool sendChannelUpdate /* = false */)
{
    ...
    if (m_caster->GetTypeId() == TYPEID_UNIT && m_caster->IsSummon())
    {
        ...
        {
            FC_LOG_DEBUG("spells", "Statue %d is unsummoned in spell %d finish", m_caster->GetGUID().GetCounter(), m_spellInfo->Id);
            ...
            if (m_caster->getDeathState() != JUST_DIED)
...
void Spell::SendSpellStart()
{
    ...
    //FC_LOG_DEBUG("spells", "Sending SMSG_SPELL_START id=%u", m_spellInfo->Id);
...
void Spell::SendSpellGo()
{
    ...
    //FC_LOG_DEBUG("spells", "Sending SMSG_SPELL_GO id=%u", m_spellInfo->Id);
...
void Spell::TakeCastItem()
{
    ...
    if (!proto)
    {
        ...
        FC_LOG_ERROR("spells", "Cast item has no item prototype %s", m_CastItem->GetGUID().ToString().c_str());
        return;
    }
...
void Spell::TakePower()
{
    ...
    {
        FC_LOG_ERROR("spells", "Spell::TakePower: Unknown power type '%d'", powerType);
        return;
    }
...
void Spell::HandleThreatSpells()
{
    ...
    FC_LOG_DEBUG("spells", "Spell %u, added an additional %f threat for %s %u target(s)", m_spellInfo->Id, threat, IsPositive() ? "assisting" : "harming", uint32(m_UniqueTargetInfo.size()));
}
...
void Spell::HandleEffects(Unit* pUnitTarget, Item* pItemTarget, GameObject* pGoTarget, Corpse* pCorpseTarget, uint32 i, SpellEffectHandleMode mode)
{
    ...
    FC_LOG_DEBUG("spells", "Spell: %u Effect : %u", m_spellInfo->Id, eff);
...
SpellCastResult Spell::CheckCast(bool strict, uint32* param1 /*= nullptr*/, uint32* param2 /*= nullptr*/, MountResult* mountResult /*= nullptr*/)
{
    ...
    if (m_caster->GetTypeId() == TYPEID_PLAYER && m_caster->ToPlayer()->isMoving() && !m_caster->HasAuraTypeWithAffectMask(SPELL_AURA_CAST_WHILE_WALKING, m_spellInfo) && (!m_caster->IsCharmed() || !m_caster->GetCharmerGUID().IsCreature()))
    {
    ...
    for (uint8 i = 0; i < MAX_SPELL_EFFECTS; ++i)
    {
        ...
        case SPELL_EFFECT_CHARGE:
        {
            ...
            if (GetSpellInfo()->NeedsExplicitUnitTarget())
            {
                ...
                m_preGeneratedPath = Firelands::make_unique<PathGenerator>(m_caster);
...
SpellCastResult Spell::CheckPower() const
{
    ...
    {
        FC_LOG_ERROR("spells", "Spell::CheckPower: Unknown power type '%d'", m_spellInfo->PowerType);
...
void Spell::Delayed() // only called in DealDamage()
{
    ...
    FC_LOG_DEBUG("spells", "Spell %u partially interrupted for (%d) ms at damage", m_spellInfo->Id, delaytime);
...
void Spell::DelayedChannel()
{
    ...
    FC_LOG_DEBUG("spells", "Spell %u partially interrupted for %i ms, new duration: %u ms", m_spellInfo->Id, delaytime, m_timer);
...
SpellEvent::~SpellEvent()
{
    ...
    {
        FC_LOG_ERROR("spells", "~SpellEvent: %s %u tried to delete non-deletable spell %u. Was not deleted, causes memory leak.",
...
void Spell::LoadScripts()
{
    ...
    for (auto itr = m_loadedScripts.begin(); itr != m_loadedScripts.end(); ++itr)
    {
        FC_LOG_DEBUG("spells", "Spell::LoadScripts: Script `%s` for spell `%u` is loaded now", (*itr)->_GetScriptName()->c_str(), m_spellInfo->Id);
        (*itr)->Register();
    }
}
...
namespace Firelands
{
        ...
        if (unitTarget)
        {
            ...
            case TARGET_CHECK_ENEMY:
                if (unitTarget->IsTotem())
                    return false;
                if (!target->IsCorpse() && !_caster->_IsValidAttackTarget(unitTarget, _spellInfo))
                    return false;
                break;
            case TARGET_CHECK_ALLY:
                if (unitTarget->IsTotem())
    ...
    WorldObjectSpellConeTargetCheck::WorldObjectSpellConeTargetCheck(Position const& coneSrc, float coneAngle, float range, Unit* caster,
