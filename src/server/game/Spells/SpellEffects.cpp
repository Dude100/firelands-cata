...
{
    FC_LOG_DEBUG("spells", "WORLD: Spell Effect DUMMY");
}
...
void Spell::EffectDummy(SpellEffIndex effIndex)
{
    ...
    FC_LOG_DEBUG("spells", "Spell ScriptStart spellid %u in EffectDummy(%u)", m_spellInfo->Id, effIndex);
...
void Spell::EffectTriggerSpell(SpellEffIndex effIndex)
{
    ...
    if (!spellInfo)
    {
        FC_LOG_ERROR("spells.effecttriggerspell", "Spell::EffectTriggerSpell spell %u tried to trigger unknown spell %u", m_spellInfo->Id, triggered_spell_id);
        return;
    }
...
void Spell::EffectTriggerMissileSpell(SpellEffIndex effIndex)
{
    ...
    if (!spellInfo)
    {
        FC_LOG_ERROR("spells.effecttrigermissilespell", "Spell::EffectTriggerMissileSpell spell %u tried to trigger unknown spell %u.", m_spellInfo->Id, triggered_spell_id);
        return;
    }
...
void Spell::EffectForceCast(SpellEffIndex effIndex)
{
    ...
    if (!spellInfo)
    {
        FC_LOG_ERROR("spells", "Spell::EffectForceCast of spell %u: triggering unknown spell id %i.", m_spellInfo->Id, triggered_spell_id);
        return;
    }
...
void Spell::EffectTriggerRitualOfSummoning(SpellEffIndex effIndex)
{
    ...
    if (!spellInfo)
    {
        FC_LOG_ERROR("spells", "EffectTriggerRitualOfSummoning of spell %u: triggering unknown spell id %i.", m_spellInfo->Id, summonSpellId);
        return;
    }
...
void Spell::EffectTeleportUnits(SpellEffIndex /*effIndex*/)
{
    ...
    if (!m_targets.HasDst())
    {
        FC_LOG_ERROR("spells", "Spell::EffectTeleportUnits - does not have a destination for spellId %u.", m_spellInfo->Id);
        return;
    }
    ...
    {
        FC_LOG_ERROR("spells", "Spell::EffectTeleportUnits - spellId %u attempted to teleport creature to a different map.", m_spellInfo->Id);
        return;
    }
...
void Spell::EffectUnlearnSpecialization(SpellEffIndex effIndex)
{
    ...
    FC_LOG_DEBUG("spells", "Spell: Player %u has unlearned spell %u from NpcGUID: %u", player->GetGUID().GetCounter(), spellToUnlearn, m_caster->GetGUID().GetCounter());
}
...
void Spell::EffectSendEvent(SpellEffIndex effIndex)
{
    ...
    FC_LOG_DEBUG("spells", "Spell ScriptStart %u for spellid %u in EffectSendEvent ", m_spellInfo->Effects[effIndex].MiscValue, m_spellInfo->Id);
...
void Spell::EffectHeal(SpellEffIndex effIndex)
{
    ...
    if (unitTarget && unitTarget->IsAlive() && damage >= 0)
    {
        ...
        else if (m_spellInfo->TargetAuraState == AURA_STATE_SWIFTMEND && unitTarget->HasAuraState(AURA_STATE_SWIFTMEND, m_spellInfo, m_caster))
        {
            ...
            if (!targetAura)
            {
                FC_LOG_ERROR("spells", "Target (%s) has the aurastate AURA_STATE_SWIFTMEND, but no matching aura.", unitTarget->GetGUID().ToString().c_str());
                return;
            }
...
void Spell::EffectHealthLeech(SpellEffIndex effIndex)
{
    ...
    FC_LOG_DEBUG("spells", "HealthLeech :%i", damage);
...
void Spell::SendLoot(ObjectGuid guid, LootType loottype)
{
    ...
    if (gameObjTarget)
    {
        ...
        {
            FC_LOG_INFO("entities.player.cheat", "Possible hacking attempt: Player %s [guid: %u] tried to loot a gameobject [entry: %u id: %u] which is on respawn timer without being in GM mode!",
        ...
        case GAMEOBJECT_TYPE_CHEST:
            ...
            {
                FC_LOG_DEBUG("spells", "Chest ScriptStart id %u for GO %u", gameObjTarget->GetGOInfo()->chest.eventId, gameObjTarget->GetSpawnId());
...
void Spell::EffectOpenLock(SpellEffIndex effIndex)
{
    ...
    if (m_caster->GetTypeId() != TYPEID_PLAYER)
    {
        FC_LOG_DEBUG("spells", "WORLD: Open Lock - No Player Caster!");
        return;
    }
    ...
    {
        FC_LOG_DEBUG("spells", "WORLD: Open Lock - No GameObject/Item Target!");
        return;
    }
...
void Spell::EffectSummonType(SpellEffIndex effIndex)
{
    ...
    if (!properties)
    {
        FC_LOG_ERROR("spells", "EffectSummonType: Unhandled summon type %u.", m_spellInfo->Effects[effIndex].MiscValueB);
        return;
    }
...
void Spell::EffectLearnSpell(SpellEffIndex effIndex)
{
    ...
    FC_LOG_DEBUG("spells", "Spell: Player %u has learned spell %u from NpcGUID: %u", player->GetGUID().GetCounter(), spellToLearn, m_caster->GetGUID().GetCounter());
}
...
void Spell::EffectEnchantItemPrismatic(SpellEffIndex effIndex)
{
    ...
    // support only enchantings with add socket in this slot
    {
        ...
        if (!add_socket)
        {
            FC_LOG_ERROR("spells", "Spell::EffectEnchantItemPrismatic: attempt to apply the enchant spell %u with SPELL_EFFECT_ENCHANT_ITEM_PRISMATIC (%u), but without ITEM_ENCHANTMENT_TYPE_PRISMATIC_SOCKET (%u), not supported yet.",
...
void Spell::EffectEnchantItemTmp(SpellEffIndex effIndex)
{
    ...
    if (!enchant_id)
    {
        FC_LOG_ERROR("spells", "Spell %u Effect %u (SPELL_EFFECT_ENCHANT_ITEM_TEMPORARY) has enchanting id 0.", m_spellInfo->Id, effIndex);
        return;
    }
    ...
    if (!pEnchant)
    {
        FC_LOG_ERROR("spells", "Spell %u Effect %u (SPELL_EFFECT_ENCHANT_ITEM_TEMPORARY) has a non-existing enchanting id %u ", m_spellInfo->Id, effIndex, enchant_id);
        return;
    }
...
void Spell::EffectScriptEffect(SpellEffIndex effIndex)
{
    ...
    case SPELLFAMILY_GENERIC:
    {
        ...
        case 60123: // Lightwell
        {
            ...
            default:
                FC_LOG_ERROR("spells", "Unknown Lightwell spell caster %u.", m_caster->GetEntry());
                return;
            }
    ...
    // normal DB scripted effect
    FC_LOG_DEBUG("spells", "Spell ScriptStart spellid %u in EffectScriptEffect(%u)", m_spellInfo->Id, effIndex);
    m_caster->GetMap()->ScriptsStart(sSpellScripts, uint32(m_spellInfo->Id | (effIndex << 24)), m_caster, unitTarget);
}
...
void Spell::EffectStuck(SpellEffIndex /*effIndex*/)
{
    ...
    FC_LOG_DEBUG("spells", "Spell Effect: Stuck");
    FC_LOG_DEBUG("spells", "Player %s (guid %u) used the auto-unstuck feature at map %u (%f, %f, %f).", player->GetName().c_str(), player->GetGUID().GetCounter(), player->GetMapId(), player->GetPositionX(), player->GetPositionY(), player->GetPositionZ());
...
void Spell::EffectActivateObject(SpellEffIndex effIndex)
{
    ...
    case GameObjectActions::UseArtKit4:
    {
        ...
        if (artKitValue == 0)
            FC_LOG_ERROR("sql.sql", "GameObject %d hit by spell %d needs `artkit%d` in `gameobject_template_addon`", gameObjTarget->GetEntry(), m_spellInfo->Id, artKitIndex);
        else
    ...
    case GameObjectActions::GoTo10thFloor:
        ...
        if (gameObjTarget->GetGoType() == GAMEOBJECT_TYPE_TRANSPORT)
            ...
            FC_LOG_ERROR("spell", "Spell %d targeted non-transport gameobject for transport only action \"Go to Floor\" %d in effect %d", m_spellInfo->Id, int32(action), int32(effIndex));
        break;
    case GameObjectActions::None:
        FC_LOG_FATAL("spell", "Spell %d has action type NONE in effect %d", m_spellInfo->Id, int32(effIndex));
        break;
    default:
        FC_LOG_ERROR("spell", "Spell %d has unhandled action %d in effect %d", m_spellInfo->Id, int32(action), int32(effIndex));
        break;
    }
...
void Spell::EffectForceDeselect(SpellEffIndex /*effIndex*/)
{
    ...
    Firelands::MessageDistDelivererToHostile notifierBreak(m_caster, &data, dist);
    Cell::VisitWorldObjects(m_caster, notifierBreak, dist);

    // and selection
    data.Initialize(SMSG_CLEAR_TARGET, 8);
    data << uint64(m_caster->GetGUID());
    Firelands::MessageDistDelivererToHostile notifierClear(m_caster, &data, dist);
    Cell::VisitWorldObjects(m_caster, notifierBreak, dist);
...
void Spell::EffectPullTowards(SpellEffIndex effIndex)
{
    ...
    {
        FC_LOG_ERROR("spells", "Spell %u with SPELL_EFFECT_PULL_TOWARDS called with invalid speedZ.", m_spellInfo->Id);
        return;
    }
...
void Spell::EffectPullTowardsDest(SpellEffIndex effIndex)
{
    ...
    if (!m_targets.HasDst())
    {
        FC_LOG_ERROR("spells", "Spell %u with SPELL_EFFECT_PULL_TOWARDS_DEST has no dest target", m_spellInfo->Id);
        return;
    }
    ...
    if (!std::isfinite(speedZ))
    {
        FC_LOG_ERROR("spells", "Spell %u with SPELL_EFFECT_PULL_TOWARDS_DEST called with invalid speedZ", m_spellInfo->Id);
        return;
    }
...
void Spell::EffectTransmitted(SpellEffIndex effIndex)
{
    ...
    if (!goinfo)
    {
        FC_LOG_ERROR("sql.sql", "Gameobject (Entry: %u) does not exist and is not created by spell (ID: %u) cast.", name_id, m_spellInfo->Id);
        return;
    }
    ...
    FC_LOG_DEBUG("spells", "AddObject at SpellEfects.cpp EffectTransmitted");
    //m_caster->AddGameObject(pGameObj);
...
void Spell::EffectSkill(SpellEffIndex /*effIndex*/)
{
    ...
    FC_LOG_DEBUG("spells", "WORLD: SkillEFFECT");
}
...
void Spell::EffectSkinPlayerCorpse(SpellEffIndex /*effIndex*/)
{
    ...
    FC_LOG_DEBUG("spells", "Effect: SkinPlayerCorpse");
...
void Spell::EffectStealBeneficialBuff(SpellEffIndex effIndex)
{
    ...
    FC_LOG_DEBUG("spells", "Effect: StealBeneficialBuff");
...
void Spell::EffectPlayMusic(SpellEffIndex effIndex)
{
    ...
    {
        FC_LOG_ERROR("spells", "EffectPlayMusic: Sound (Id: %u) does not exist in spell %u.", soundId, m_spellInfo->Id);
        return;
    }
...
void Spell::EffectPlaySound(SpellEffIndex effIndex)
{
    ...
    if (!sSoundEntriesStore.LookupEntry(soundId))
    {
        FC_LOG_ERROR("spells", "EffectPlaySound: Sound (Id: %u) does not exist in spell %u.", soundId, m_spellInfo->Id);
        return;
    }
...
void Spell::EffectBind(SpellEffIndex effIndex)
{
    ...
    FC_LOG_DEBUG("spells", "EffectBind: New homebind X: %f, Y: %f, Z: %f, MapId: %u, AreaId: %u",
...
void Spell::EffectSummonPersonalGameObject(SpellEffIndex effIndex)
{
    ...
    {
        FC_LOG_WARN("spells", "SpellEffect Failed to summon personal gameobject. SpellId %u, effect %u", m_spellInfo->Id, effIndex);
        delete go;
        return;
    }
