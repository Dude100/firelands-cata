...
void LootItemStorage::LoadStorageFromDB()
{
    ...
    if (result)
    {
        ...
        FC_LOG_INFO("server.loading", ">> Loaded %u stored item loots in %u ms", count, GetMSTimeDiffToNow(oldMSTime));
    }
    else
        FC_LOG_INFO("server.loading", ">> Loaded 0 stored item loots");
    ...
    if (result)
    {
        ...
        FC_LOG_INFO("server.loading", ">> Loaded %u stored item money in %u ms", count, GetMSTimeDiffToNow(oldMSTime));
    }
    else
        FC_LOG_INFO("server.loading", ">> Loaded 0 stored item money");
}
...
void LootItemStorage::AddNewStoredLoot(Loot* loot, Player* player)
{
    ...
    // read
    {
        ...
        if (itr != _lootItemStore.end())
        {
            FC_LOG_ERROR("misc", "Trying to store item loot by player: %s for container id: %u that is already in storage!", player->GetGUID().ToString().c_str(), loot->containerID);
            return;
        }
    }
