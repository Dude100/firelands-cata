...
inline void MarkDependentColumn(TableStruct& tableStruct, std::string const& columnName, GuidType dependentType)
{
    ...
    {
        FC_LOG_FATAL("server.loading", "Column `%s` declared in table `%s` marked as dependent but doesn't exist, PlayerDump will not work properly, please update table definitions",
    ...
    {
        FC_LOG_FATAL("server.loading", "Attempt to mark column `%s` in table `%s` as dependent column but already marked! please check your code.",
            columnName.c_str(), tableStruct.TableName.c_str());
        ABORT();
        return;
    }
...
inline void MarkWhereField(TableStruct& tableStruct, std::string const& whereField)
{
    ...
    {
        FC_LOG_FATAL("server.loading", "Column name `%s` set as 'WHERE' column for table `%s` doesn't exist. PlayerDump won't work properly",
...
void PlayerDump::InitializeTables()
{
    ...
    for (DumpTable const& dumpTable : DumpTables)
    {
        ...
        case DTT_EQSET_TABLE:
            ...
            {
                std::string itColumn = Firelands::StringFormat("item%u", j);
        ...
        case DTT_PET_TABLE:
            ...
            break;
        default:
            FC_LOG_FATAL("server.loading", "Wrong dump table type %u, probably added a new table type without updating code", uint32(dumpTable.Type));
            ABORT();
            return;
        }
    ...
    for (TableStruct const& tableStruct : CharacterTables)
    {
        ...
        {
            FC_LOG_FATAL("server.loading", "Table `%s` defined in player dump doesn't have a WHERE query field", tableStruct.TableName.c_str());
            ABORT();
        }
    }
    ...
    FC_LOG_INFO("server.loading", ">> Initialized tables for PlayerDump in %u ms.", GetMSTimeDiffToNow(oldMSTime));
}
...
inline bool ValidateFields(TableStruct const& ts, std::string const& str, size_t lineNumber)
{
    ...
    if (s == std::string::npos)
    {
        FC_LOG_ERROR("misc", "LoadPlayerDump: (line " UI64FMTD ") dump format not recognized.", lineNumber);
        return false;
    }
    s += 4;
    ...
    {
        FC_LOG_ERROR("misc", "LoadPlayerDump: (line " UI64FMTD ") unexpected end of line", lineNumber);
        return false;
    }
    ...
    do
    {
        ...
        if (columnIndex == -1)
        {
            FC_LOG_ERROR("misc", "LoadPlayerDump: (line " UI64FMTD ") unknown column name `%s` for table `%s`, aborting due to incompatible DB structure.", lineNumber, column.c_str(), ts.TableName.c_str());
            return false;
        }
...
DumpReturn PlayerDumpReader::LoadDump(std::string const& file, uint32 account, std::string name, ObjectGuid::LowType guid)
{
    ...
    while (!feof(fin.get()))
    {
        ...
        if (tn.empty())
        {
            FC_LOG_ERROR("misc", "LoadPlayerDump: (line " UI64FMTD ") Can't extract table name!", lineNumber);
            return DUMP_FILE_BROKEN;
        }
        ...
        {
            FC_LOG_ERROR("misc", "LoadPlayerDump: (line " UI64FMTD ") Unknown table: `%s`!", lineNumber, tn.c_str());
            return DUMP_FILE_BROKEN;
        }
