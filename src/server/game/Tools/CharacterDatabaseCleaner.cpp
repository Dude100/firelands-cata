...
void CharacterDatabaseCleaner::CleanDatabase()
{
    ...
    FC_LOG_INFO("misc", "Cleaning character database...");
    ...
    FC_LOG_INFO("server.loading", ">> Cleaned character database in %u ms", GetMSTimeDiffToNow(oldMSTime));
}
...
void CharacterDatabaseCleaner::CheckUnique(char const* column, char const* table, bool (*check)(uint32))
{
    ...
    if (!result)
    {
        FC_LOG_INFO("misc", "Table %s is empty.", table);
        return;
    }
