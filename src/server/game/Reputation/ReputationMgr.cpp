...
bool ReputationMgr::IsAtWar(uint32 faction_id) const
{
    ...
    if (!factionEntry)
    {
        FC_LOG_ERROR("misc", "ReputationMgr::IsAtWar: Can't get AtWar flag of %s for unknown faction (faction id) #%u.", _player->GetName().c_str(), faction_id);
        return 0;
    }
...
int32 ReputationMgr::GetReputation(uint32 faction_id) const
{
    ...
    if (!factionEntry)
    {
        FC_LOG_ERROR("misc", "ReputationMgr::GetReputation: Can't get reputation of %s for unknown faction (faction id) #%u.", _player->GetName().c_str(), faction_id);
        return 0;
    }
    ...
    for (int i = 0; i < 4; i++)
    {
        if ((factionEntry->ReputationRaceMask[i] & raceMask ||
            ...
            (factionEntry->ReputationClassMask[i] & classMask ||
                (factionEntry->ReputationClassMask[i] == 0 &&
                    factionEntry->ReputationBase[i] != 0)))
