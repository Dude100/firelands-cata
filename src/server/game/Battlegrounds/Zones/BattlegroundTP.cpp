...
bool BattlegroundTP::SetupBattleground()
{
    ...
    {
        FC_LOG_ERROR("misc", "BatteGroundTP: Failed to spawn some objects. Battleground not created!");
        return false;
    }
    ...
    for (uint32 i = TP_GRAVEYARD_START_ALLIANCE; i < TP_MAX_GRAVEYARDS; ++i)
    {
        ...
        if (grave)
        {
            ...
            {
                FC_LOG_ERROR("misc", "BatteGroundTP: Failed to spawn spirit guide id: %u. Battleground not created!", grave->ID);
                return false;
            }
        }
        else
        {
            FC_LOG_ERROR("misc", "BatteGroundTP: Failed to find grave %u. Battleground not created!", BG_TP_GraveyardIds[i]);
            return false;
        }
    }
...
void BattlegroundTP::RemovePlayer(Player* player, ObjectGuid guid, uint32 /* team */)
{
    ...
    if (IsAllianceFlagPickedup() && m_FlagKeepers[TEAM_ALLIANCE] == guid)
    {
        if (!player)
        {
            FC_LOG_ERROR("bg.battleground", "BattlegroundTP: Removing offline player who has the FLAG!!");
            SetAllianceFlagPicker(ObjectGuid::Empty);
            RespawnFlag(ALLIANCE, false);
        }
        else
