    ...
    if (m_ValidStartPositionTimer >= CHECK_PLAYER_POSITION_INVERVAL)
    {
        ...
        for (BattlegroundPlayerMap::const_iterator itr = GetPlayers().begin(); itr != GetPlayers().end(); ++itr)
        {
            if (Player* player = ObjectAccessor::FindPlayer(itr->first))
            {
                ...
                {
                    FC_LOG_DEBUG("bg.battleground", "BATTLEGROUND: Sending %s back to start location (map: %u) (possible exploit)", player->GetName().c_str(), GetMapId());
...
inline void Battleground::_ProcessJoin(uint32 diff)
{
    ...
    if (m_CountdownTimer >= 10000)
    {
        ...
        packet.Type = 0;
        packet.TimeLeft = countdownMaxForBGType - (GetElapsedTime() / IN_MILLISECONDS);
        packet.TotalTime = countdownMaxForBGType;
    ...
    if (!(m_Events & BG_STARTING_EVENT_1))
    {
        ...
        if (!FindBgMap())
        {
            FC_LOG_ERROR("bg.battleground", "Battleground::_ProcessJoin: map (map id: %u, instance id: %u) is not created!", m_MapId, m_InstanceID);
            EndNow();
            return;
        }
...
Player* Battleground::_GetPlayer(ObjectGuid guid, bool offlineRemove, char const* context) const
{
    ...
    if (!offlineRemove)
    {
        ...
        if (!player)
            FC_LOG_ERROR("bg.battleground", "Battleground::%s: player (%s) not found for BG (map: %u, instance id: %u)!",
...
void Battleground::SendBroadcastText(uint32 id, ChatMsg msgType, WorldObject const* target)
{
    ...
    {
        FC_LOG_ERROR("bg.battleground", "Battleground::SendBroadcastText: `broadcast_text` (ID: %u) was not found", id);
        return;
    }

    Firelands::BroadcastTextBuilder builder(nullptr, msgType, id, GENDER_MALE, target);
...
void Battleground::EndBattleground(uint32 winner)
{
    ...
    for (BattlegroundPlayerMap::iterator itr = m_Players.begin(); itr != m_Players.end(); ++itr)
    {
        ...
        else //needed cause else in av some creatures will kill the players at the end
            player->CombatStop();
            ...
            if (!guildAwarded)
            {
                ...
                if (uint32 guildId = GetBgMap()->GetOwnerGuildId(player->GetBGTeam()))
                {
                    if (Guild* guild = sGuildMgr->GetGuildById(guildId))
                        guild->UpdateAchievementCriteria(ACHIEVEMENT_CRITERIA_TYPE_WIN_BG, player->GetMapId(), 0, 0, nullptr, player);
                }
            }
        }
        else
...
uint32 Battleground::GetBonusHonorFromKill(uint32 kills) const
{
    ...
    return Firelands::Honor::hk_honor_at_level(maxLevel, float(kills));
}
...
void Battleground::RemovePlayerAtLeave(ObjectGuid guid, bool Transport, bool SendPacket)
{
    ...
    if (player)
    {
        ...
        FC_LOG_DEBUG("bg.battleground", "Removed player %s from Battleground.", player->GetName().c_str());
    }
...
void Battleground::Reset()
{
    ...
    if (m_InvitedAlliance > 0 || m_InvitedHorde > 0)
        FC_LOG_ERROR("bg.battleground", "Battleground::Reset: one of the counters is not 0 (alliance: %u, horde: %u) for BG (map: %u, instance id: %u)!",
...
void Battleground::StartBattleground()
{
    ...
    if (m_IsRated)
        FC_LOG_DEBUG("bg.arena", "Arena match type: %u for Team1Id: %u - Team2Id: %u started.", m_ArenaType, m_ArenaTeamIds[TEAM_ALLIANCE], m_ArenaTeamIds[TEAM_HORDE]);
}
...
bool Battleground::AddObject(uint32 type, uint32 entry, float x, float y, float z, float o, float rotation0, float rotation1, float rotation2, float rotation3, uint32 /*respawnTime*/, GOState goState)
{
    ...
    {
        FC_LOG_DEBUG("bg.battleground", "Battleground::AddObject: gameoobject [entry: %u, object type: %u] for BG (map: %u) has zeroed rotation fields, "
    ...
    {
        FC_LOG_ERROR("bg.battleground", "Battleground::AddObject: cannot create gameobject (entry: %u) for BG (map: %u, instance id: %u)!",
...
void Battleground::DoorClose(uint32 type)
{
    if (GameObject* obj = GetBgMap()->GetGameObject(BgObjects[type]))
    {
        ...
        FC_LOG_ERROR("bg.battleground", "Battleground::DoorClose: door gameobject (type: %u, %s) not found for BG (map: %u, instance id: %u)!",
...
void Battleground::DoorOpen(uint32 type)
{
    if (GameObject* obj = GetBgMap()->GetGameObject(BgObjects[type]))
    {
        ...
        FC_LOG_ERROR("bg.battleground", "Battleground::DoorOpen: door gameobject (type: %u, %s) not found for BG (map: %u, instance id: %u)!",
            type, BgObjects[type].ToString().c_str(), m_MapId, m_InstanceID);
}
...
GameObject* Battleground::GetBGObject(uint32 type, bool logError)
{
    ...
    {
        if (logError)
            FC_LOG_ERROR("bg.battleground", "Battleground::GetBGObject: gameobject (type: %u, %s) not found for BG (map: %u, instance id: %u)!",
                type, BgObjects[type].ToString().c_str(), m_MapId, m_InstanceID);
        else
            FC_LOG_INFO("bg.battleground", "Battleground::GetBGObject: gameobject (type: %u, %s) not found for BG (map: %u, instance id: %u)!",
                type, BgObjects[type].ToString().c_str(), m_MapId, m_InstanceID);
    }
    return obj;
}
...
Creature* Battleground::GetBGCreature(uint32 type, bool logError)
{
    ...
    {
        if (logError)
            FC_LOG_ERROR("bg.battleground", "Battleground::GetBGCreature: creature (type: %u, %s) not found for BG (map: %u, instance id: %u)!",
                type, BgCreatures[type].ToString().c_str(), m_MapId, m_InstanceID);
        else
            FC_LOG_INFO("bg.battleground", "Battleground::GetBGCreature: creature (type: %u, %s) not found for BG (map: %u, instance id: %u)!",
                type, BgCreatures[type].ToString().c_str(), m_MapId, m_InstanceID);
    }
    return creature;
}
...
Creature* Battleground::AddCreature(uint32 entry, uint32 type, float x, float y, float z, float o, TeamId /*teamId = TEAM_NEUTRAL*/, uint32 respawntime /*= 0*/, Transport* transport)
{
    ...
    {
        FC_LOG_ERROR("bg.battleground", "Battleground::AddCreature: cannot create creature (entry: %u) for BG (map: %u, instance id: %u)!",
            entry, m_MapId, m_InstanceID);
        delete creature;
        return nullptr;
    }
    ...
    if (!cinfo)
    {
        FC_LOG_ERROR("bg.battleground", "Battleground::AddCreature: creature template (entry: %u) does not exist for BG (map: %u, instance id: %u)!",
            entry, m_MapId, m_InstanceID);
        delete creature;
        return nullptr;
    }
...
bool Battleground::DelCreature(uint32 type)
{
    ...
    FC_LOG_ERROR("bg.battleground", "Battleground::DelCreature: creature (type: %u, %s) not found for BG (map: %u, instance id: %u)!",
        type, BgCreatures[type].ToString().c_str(), m_MapId, m_InstanceID);
    BgCreatures[type].Clear();
    return false;
}
...
bool Battleground::DelObject(uint32 type)
{
    ...
    FC_LOG_ERROR("bg.battleground", "Battleground::DelObject: gameobject (type: %u, %s) not found for BG (map: %u, instance id: %u)!",
        type, BgObjects[type].ToString().c_str(), m_MapId, m_InstanceID);
    BgObjects[type].Clear();
    return false;
}
...
bool Battleground::AddSpiritGuide(uint32 type, float x, float y, float z, float o, TeamId teamId /*= TEAM_NEUTRAL*/)
{
    ...
    FC_LOG_ERROR("bg.battleground", "Battleground::AddSpiritGuide: cannot create spirit guide (type: %u, entry: %u) for BG (map: %u, instance id: %u)!",
...
void Battleground::SendMessageToAll(uint32 entry, ChatMsg msgType, Player const* source)
{
    ...
    Firelands::FirelandsStringChatBuilder builder(nullptr, msgType, entry, source);
    Firelands::LocalizedPacketDo<Firelands::FirelandsStringChatBuilder> localizer(builder);
    BroadcastWorker(localizer);
}
...
void Battleground::PSendMessageToAll(uint32 entry, ChatMsg msgType, Player const* source, ...)
{
    ...
    Firelands::FirelandsStringChatBuilder builder(nullptr, msgType, entry, source, &ap);
    Firelands::LocalizedPacketDo<Firelands::FirelandsStringChatBuilder> localizer(builder);
    BroadcastWorker(localizer);
...
void Battleground::HandleTriggerBuff(ObjectGuid go_guid)
{
    ...
    if (index < 0)
    {
        FC_LOG_ERROR("bg.battleground", "Battleground::HandleTriggerBuff: cannot find buff gameobject (%s, entry: %u, type: %u) in internal data for BG (map: %u, instance id: %u)!",
...
int32 Battleground::GetObjectType(ObjectGuid guid)
{
    for (uint32 i = 0; i < BgObjects.size(); ++i)
        if (BgObjects[i] == guid)
            return i;
    FC_LOG_ERROR("bg.battleground", "Battleground::GetObjectType: player used gameobject (%s) which is not in internal data for BG (map: %u, instance id: %u), cheating?",
...
uint32 Battleground::GetTeamScore(uint32 teamId) const
{
    ...
    FC_LOG_ERROR("bg.battleground", "GetTeamScore with wrong Team %u for BG %u", teamId, GetTypeID());
    return 0;
}

void Battleground::HandleAreaTrigger(Player* player, uint32 trigger)
{
    FC_LOG_DEBUG("bg.battleground", "Unhandled AreaTrigger %u in Battleground %u. Player coords (x: %f, y: %f, z: %f)",
...
{
    FC_LOG_ERROR("bg.battleground", "Battleground::CheckAchievementCriteriaMeet: No implementation for criteria %u", criteriaId);
    return false;
}
