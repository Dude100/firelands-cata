...
void QuestPoolMgr::LoadFromDB()
{
    ...
    // load template data from world DB
    {
        ...
        if (!result)
        {
            FC_LOG_INFO("server.loading", ">> Loaded 0 quest pools. DB table `quest_pool_members` is empty.");
            return;
        }
        ...
        do
        {
            ...
            if (fields[2].IsNull())
            {
                FC_LOG_ERROR("sql.sql", "Table `quest_pool_members` contains reference to non-existing pool %u. Skipped.", fields[1].GetUInt32());
                continue;
            }
            ...
            if (!quest)
            {
                FC_LOG_ERROR("sql.sql", "Table `quest_pool_members` contains reference to non-existing quest %u. Skipped.", questId);
                continue;
            }
            if (!quest->IsDailyOrWeekly() && !quest->IsMonthly())
            {
                FC_LOG_ERROR("sql.sql", "Table `quest_pool_members` contains reference to quest %u, which is neither daily, weekly nor monthly. Skipped.", questId);
                continue;
            }
    ...
    // load saved spawns from character DB
    {
        ...
        if (result)
        {
            ...
            do
            {
                ...
                {
                    FC_LOG_ERROR("sql.sql", "Table `pool_quest_save` contains reference to non-existant quest pool %u. Deleted.", poolId);
    ...
    for (auto pair : lookup)
    {
        ...
        {
            FC_LOG_ERROR("sql.sql", "Table `quest_pool_template` contains quest pool %u requesting %u spawns, but only has %zu members. Requested spawns reduced.", pool.poolId, pool.numActive, pool.members.size());
        ...
        if (!doRegenerate)
        {
            ...
            for (size_t i = pool.members.size(); (i--);)
            {
                ...
                if (member.empty())
                {
                    FC_LOG_ERROR("sql.sql", "Table `quest_pool_members` contains no entries at index %zu for quest pool %u. Index removed.", i, pool.poolId);
                ...
                for (auto it = member.begin(); (++it) != member.end();)
                {
                    ...
                    if (status != otherStatus)
                        FC_LOG_WARN("sql.sql", "Table `pool_quest_save` %s quest %u (in pool %u, index %zu) saved, but its index is%s active (because quest %u is%s in the table). Set quest %u to %sactive.", (status ? "does not have" : "has"), *it, pool.poolId, i, (status ? "" : " not"), member[0], (status ? "" : " not"), *it, (status ? "" : "in"));
                    if (otherStatus)
            ...
            for (uint32 quest : pool.activeQuests)
                FC_LOG_WARN("sql.sql", "Table `pool_quest_save` has saved quest %u for pool %u, but that quest is not part of the pool. Skipped.", quest, pool.poolId);
            ...
            {
                doRegenerate = true;
                FC_LOG_ERROR("sql.sql", "Table `pool_quest_save` has %u active members saved for pool %u, which requests %u active members. Pool spawns re-generated.", activeCount, pool.poolId, pool.numActive);
            }
        }
        ...
        for (QuestPool::Member const& member : pool.members)
        {
            for (uint32 quest : member)
            {
                ...
                if (ref)
                {
                    FC_LOG_ERROR("sql.sql", "Table `quest_pool_members` lists quest %u as member of pool %u, but it is already a member of pool %u. Skipped.", quest, pool.poolId, ref->poolId);
                    continue;
                }
                ref = &pool;
            }
    ...
    FC_LOG_INFO("server.loading", ">> Loaded %zu daily, %zu weekly and %zu monthly quest pools in %u ms", _dailyPools.size(), _weeklyPools.size(), _monthlyPools.size(), GetMSTimeDiffToNow(oldMSTime));
}
